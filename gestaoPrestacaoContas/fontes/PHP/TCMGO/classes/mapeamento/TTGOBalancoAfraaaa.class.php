...
class TTGOBalancoAfraaaa extends Persistente
{
    /**
    * Método Construtor
    * @access Private
*/
    public function TTGOBalancoAfraaaa()
    {
        parent::Persistente();
        $this->setTabela("tcmgo.balanco_afraaaa");
    ...
    {
        $stSql = "
            SELECT  balanco_afraaaa.cod_plano
                 ,  tipo_lancamento
                 ,  cod_estrutural
                 ,  nom_conta
              FROM  tcmgo.balanco_afraaaa
        INNER JOIN  contabilidade.plano_analitica
                ON  plano_analitica.cod_plano = balanco_afraaaa.cod_plano
               AND  plano_analitica.exercicio = balanco_afraaaa.exercicio
        INNER JOIN  contabilidade.plano_conta
                ON  plano_conta.cod_conta = plano_analitica.cod_conta
               AND  plano_conta.exercicio = plano_analitica.exercicio
             WHERE  balanco_afraaaa.exercicio = '".$this->getDado('exercicio')."' ";
        if ( $this->getDado('tipo_lancamento') ) {
            $stSql.= " AND balanco_afraaaa.tipo_lancamento = ".$this->getDado('tipo_lancamento')." ";
        }
        if ( $this->getDado('cod_plano') ) {
            $stSql.= " AND balanco_afraaaa.cod_plano = ".$this->getDado('cod_plano')." ";
        }
