...
class TTGOBalancoPfdaaaa extends Persistente
{
    /**
    * Método Construtor
    * @access Private
*/
    public function TTGOBalancoPfdaaaa()
    {
        parent::Persistente();
        $this->setTabela("tcmgo.balanco_pfdaaaa");
        ...
        $this->AddCampo( 'desdobramento_tipo' ,'integer' ,true, '' ,false ,false );
    }
    ...
    {
        $stSql = "
            SELECT  balanco_pfdaaaa.cod_plano
                 ,  tipo_lancamento
                 ,  cod_estrutural
                 ,  nom_conta
        ...
        INNER JOIN  contabilidade.plano_conta
             ...
             WHERE  balanco_pfdaaaa.exercicio = '".$this->getDado('exercicio')."' ";
        if ( $this->getDado('tipo_lancamento') ) {
            $stSql.= " AND  balanco_pfdaaaa.tipo_lancamento = ".$this->getDado('tipo_lancamento')." ";
        }
        if ( $this->getDado('desdobramento_tipo') ) {
            $stSql.= " AND  balanco_pfdaaaa.desdobramento_tipo = ".$this->getDado('desdobramento_tipo')." ";
        }
        if ( $this->getDado('cod_plano') ) {
            $stSql.= " AND  balanco_pfdaaaa.cod_plano = ".$this->getDado('cod_plano')." ";
        }
