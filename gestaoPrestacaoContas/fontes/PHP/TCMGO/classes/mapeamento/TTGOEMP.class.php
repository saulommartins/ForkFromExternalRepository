...
Revision 1.7  2007/10/05 18:35:14  hboaventura
Correção dos desdobramentos

Revision 1.6  2007/06/12 20:44:11  hboaventura
inclusão dos casos de uso uc-06.04.00

Revision 1.5  2007/06/12 18:34:05  hboaventura
inclusão dos casos de uso uc-06.04.00

Revision 1.4  2007/06/11 16:06:41  hboaventura
Correção de bugs

Revision 1.3  2007/06/07 15:36:31  hboaventura
Inclusão do filtro por periodicidade

Revision 1.2  2007/05/21 20:53:22  hboaventura
...
*/
    ...
    public function montaRecuperaEmpenho()
    {
        ...
        $stSql.= "'99' as assunto
            ...
            if (Sessao::getExercicio() > '2011') {
             /*   $stSql.= "COALESCE((SELECT cpf FROM sw_cgm_pessoa_fisica WHERE sw_cgm_pessoa_fisica.numcgm =
                                (SELECT numcgm FROM administracao.auditoria WHERE auditoria.objeto =
                                    (SELECT objeto FROM administracao.auditoria WHERE objeto =
                                        (SELECT 'cod_empenho = '||empenho.empenho.cod_empenho||', exercicio = '||empenho.empenho.exercicio||', cod_entidade = '||empenho.empenho.cod_entidade  FROM empenho.empenho AS emp2
                                            WHERE empenho.exercicio    = emp2.exercicio
                                            AND empenho.cod_entidade   = emp2.cod_entidade
                                            AND empenho.cod_empenho    = emp2.cod_empenho limit 1))
                                        AND auditoria.cod_acao = 822
                                        AND substr(timestamp,1,4) = ". Sessao::getExercicio() . ")),'') AS cpf_resp

                            , COALESCE((SELECT nom_cgm FROM sw_cgm WHERE sw_cgm.numcgm =
                                (SELECT numcgm FROM administracao.auditoria WHERE auditoria.objeto =
                                    (SELECT objeto FROM administracao.auditoria WHERE objeto =
                                        (SELECT 'cod_empenho = '||empenho.empenho.cod_empenho||', exercicio = '||empenho.empenho.exercicio||', cod_entidade = '||empenho.empenho.cod_entidade FROM empenho.empenho AS emp2
                                            WHERE empenho.exercicio    = emp2.exercicio
                                            ...
                                            AND empenho.cod_empenho    = emp2.cod_empenho limit 1))
                                        AND auditoria.cod_acao = 822
                                        AND substr(timestamp,1,4) = ". Sessao::getExercicio() . ")), '') AS nom_resp*/
