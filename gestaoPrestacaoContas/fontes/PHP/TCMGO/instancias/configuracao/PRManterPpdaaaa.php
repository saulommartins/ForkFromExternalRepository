...
/**
    ...
    * $Id: PRManterPpdaaaa.php 59612 2014-09-02 12:00:51Z gelson $
        ...
        if ( count( $arExcluidas ) > 0 ) {
            foreach ($arExcluidas as $arAux) {
                foreach ($arAux as $arContas) {
                    $obTTGOBalancoPpdaaaa->setDado('cod_plano',$arContas['cod_plano']);
                    $obTTGOBalancoPpdaaaa->exclusao();
                }
            }
        }
        if ( count( $arContas ) > 0 ) {
            foreach ($arContas as $arAux) {
                if ( count( $arAux ) > 0 ) {
                    foreach ($arAux as $arContas) {
                        $obTTGOBalancoPpdaaaa->setDado( 'cod_plano', $arContas['cod_plano'] );
                        $obTTGOBalancoPpdaaaa->setDado( 'tipo_lancamento', $arContas['tipo_lancamento'] );
                        $obTTGOBalancoPpdaaaa->recuperaRelacionamento( $rsContas );
                        if ( $rsContas->getNumLinhas() <= 0 ) {
                            $obTTGOBalancoPpdaaaa->inclusao();
                        }
                    }
                }
            }
