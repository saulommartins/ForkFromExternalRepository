...
include_once CAM_GPC_TGO_MAPEAMENTO.'TTGOCTB.class.php';
...
if (Sessao::getExercicio() > 2010) {
    ...
    if (Sessao::getExercicio() > '2012') {
     $arRegistro[0]['vlSaldoExercAntVinculado'] =
        ...
        $arRegistro[0]['vlSaldoExercAntVinculado'] =
              number_format(
                           $obTMapeamento->recuperaTotalConta ( " where valor_lancamento.tipo = 'I'
                                                                    and plano_conta.cod_estrutural like '1.1.1.1.4%'
                                                                    and plano_conta.exercicio = '".Sessao::getExercicio()."'
                                                                                         " )
                            +
                           $obTMapeamento->recuperaTotalConta ( " where valor_lancamento.tipo = 'I'
    ...
    if (Sessao::getExercicio() > '2012') {
        ...
        $arRegistro[0]['vlSaldoMesSegVinculado'] =
                number_format($obTMapeamento->recuperaTotalConta ( " where plano_conta.cod_estrutural like '1.1.1.1.3%'
                                                           and plano_conta.exercicio = '".Sessao::getExercicio()."'
                                                           and lote.dt_lote  >= to_date( '01/01/".Sessao::getExercicio()."', 'dd/mm/yyyy' )
                                                           and lote.dt_lote  < to_date( '".$arFiltroRelatorio['stDataFinal']."', 'dd/mm/yyyy' ) + 1
                                                   " )
...
if (Sessao::getExercicio() > 2012) {
    ...
    $obExportador->roUltimoArquivo->roUltimoBloco->roUltimaColuna->setTamanhoFixo(99);
} elseif (Sessao::getExercicio() > '2011') {
    $obExportador->roUltimoArquivo->roUltimoBloco->roUltimaColuna->setTamanhoFixo(84);
} elseif ((Sessao::getExercicio() > '2008') && (Sessao::getExercicio() < '2011')) {
    $obExportador->roUltimoArquivo->roUltimoBloco->roUltimaColuna->setTamanhoFixo(101);
} else { $obExportador->roUltimoArquivo->roUltimoBloco->roUltimaColuna->setTamanhoFixo(82);
}
