...
<?php
/**
    * Classe de mapeamento do Demonstratio das despesas pessoais
...
class FSTNRGFAnexo1 extends Persistente
{
/**
    * Método Construtor
    * @access Private
...
function FSTNRGFAnexo1()
{
    parent::Persistente();
}

function recuperaDadosRelatorioAnexo1Cabecalho(&$rsRecordSet, $boTransacao = "")
{
    $obErro      = new Erro;
    $obConexao   = new Conexao;
    $rsRecordSet = new RecordSet;

    $stSql  = $this->montaRecuperaDadosRelatorioAnexo1Cabecalho();
    $obErro = $obConexao->executaSQL( $rsRecordSet, $stSql, $boTransacao );

    return $obErro;
}

function montaRecuperaDadosRelatorioAnexo1Cabecalho()
{
...
function recuperaDadosRelatorioAnexo1(&$rsRecordSet, $boTransacao = "")
{
    $obErro      = new Erro;
    $obConexao   = new Conexao;
    $rsRecordSet = new RecordSet;

    $stSql  = $this->montaRecuperaDadosRelatorioAnexo1();
    $obErro = $obConexao->executaSQL( $rsRecordSet, $stSql, $boTransacao );

    return $obErro;
}

function montaRecuperaDadosRelatorioAnexo1()
{
    $stSql .= " SELECT *                                                                            \n";
    $stSql .= "      FROM stn.fn_rgf_anexo1( '".$this->getDado("stExercicio"   )."'                 \n";
    $stSql .= "                             ,'".$this->getDado("stEntidade")."'                     \n";
    $stSql .= "                             , ".$this->getDado("stBimestre"    ).")AS tabela        \n";
    $stSql .= "                ( codigo    integer,                                                 \n";
    $stSql .= "                  nivel     integer,                                                 \n";
