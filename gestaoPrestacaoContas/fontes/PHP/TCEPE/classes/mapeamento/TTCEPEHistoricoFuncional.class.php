...
/**
    ...
    * @author Desenvolvedor:  Michel Teixeira
    $Id: TTCEPEHistoricoFuncional.class.php 60535 2014-10-27 18:06:09Z michel $
*/

include_once '../../../../../../gestaoAdministrativa/fontes/PHP/framework/include/valida.inc.php';
include_once CLA_PERSISTENTE;

class TTCEPEHistoricoFuncional extends Persistente
{
    /*
     * Método Construtor
     *
     * @return void
     */
    public function TTCEPEHistoricoFuncional()
    {
        parent::Persistente();
    }

    /**
    * Executa um Select no banco de dados a partir do comando SQL montado no método montaRecuperaHistoricoFuncional.
    * @access Public
    ...
    */
    public function recuperaHistoricoFuncional(&$rsRecordSet, $stCondicao = "" , $stOrdem = "" , $boTransacao = "")
    {
        $obErro      = new Erro;
        ...
        $stSql = $this->montaRecuperaHistoricoFuncional().$stCondicao.$stOrdem;
        $this->setDebug( $stSql );
    ...
    public function montaRecuperaHistoricoFuncional()
    {
        $stSql = "
               --SERVIDOR
                   SELECT cpf
                        , LPAD(CAST(matricula AS varchar),15,'0') AS matricula
                        , cod_cargo
                        , cod_contrato
                        , CASE WHEN tipo_ato IS NULL THEN
                                CASE WHEN tipo_ato_inicial IS NOT NULL THEN
                                    tipo_ato_inicial
                                WHEN tipo_ato_final IS NOT NULL THEN
                                    tipo_ato_final 
                                WHEN to_char(periodo_inicial, 'mmyyyy') = '".$this->getDado('stMes')."' AND cod_motivo IS NOT NULL THEN
                                    99
                                END
                          ELSE
                                tipo_ato
                        ...
                        , CASE WHEN tipo_ato IS NULL THEN
                                CASE WHEN tipo_ato_inicial IS NOT NULL THEN
                                    dt_movimentacao_inicial
                                WHEN tipo_ato_final IS NOT NULL THEN
                                    dt_movimentacao_final 
                                WHEN to_char(periodo_inicial, 'mmyyyy') = '".$this->getDado('stMes')."' AND cod_motivo IS NOT NULL THEN
                                    CAST(to_char(periodo_inicial, 'ddmmyyyy') AS VARCHAR(8))
                                END
                          ELSE
                                dt_movimentacao
                        ...
                        , dt_obito
                        , cod_classe
                        , CASE WHEN tipo_ato IS NULL AND tipo_ato_inicial IS NULL AND tipo_ato_final IS NULL AND to_char(periodo_inicial, 'mmyyyy') = '".$this->getDado('stMes')."' AND cod_motivo IS NOT NULL THEN
                                descricao_assentamento
            ...
            --PENSIONISTA
                 ...
                 ORDER BY matricula, dt_movimentacao              
                ";
        return $stSql;
    }
