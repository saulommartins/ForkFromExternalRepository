...
class TTCEPETransferenciaConcedida extends Persistente
{
     ...
     * @return void
     */
    public function TTCEPETransferenciaConcedida()
    {
        parent::Persistente();
    }
    ...
    */
    ...
    function recuperaTransferenciaConcedida(&$rsRecordSet,$stFiltro="",$stOrder="",$boTransacao="")
    {
        return $this->executaRecupera("montaRecuperaTransferenciaConcedida",$rsRecordSet,$stFiltro,$stOrder,$boTransacao);
    }


    public function montaRecuperaTransferenciaConcedida()
    {
        $stSql = " SELECT 0 AS reservado_tce
                          , retorno.vl_lancamento AS vl_mensal
                          , (SELECT valor FROM administracao.configuracao_entidade WHERE cod_modulo = 63 AND parametro = 'tcepe_codigo_unidade_gestora'
                                AND cod_entidade = tipo_transferencia_concedida.cod_entidade_beneficiada 
                            ) AS unidade_gestora_beneficiada
                          , (SELECT valor FROM administracao.configuracao_entidade WHERE cod_modulo = 63 AND parametro = 'tcepe_codigo_unidade_gestora'
                                AND cod_entidade = tipo_transferencia_concedida.cod_entidade
	       ...
	       LEFT JOIN tcepe.codigo_fonte_recurso
		    ...
		    JOIN tcepe.tipo_transferencia_concedida
                      ON tipo_transferencia_concedida.cod_lote      = transferencia.cod_lote
                     AND tipo_transferencia_concedida.cod_entidade  = transferencia.cod_entidade
                     AND tipo_transferencia_concedida.exercicio     = transferencia.exercicio
                     AND tipo_transferencia_concedida.tipo          = transferencia.tipo
                     
		    JOIN tcepe.tipo_transferencia
                      ON tipo_transferencia.cod_tipo = tipo_transferencia_concedida.cod_tipo_tcepe
