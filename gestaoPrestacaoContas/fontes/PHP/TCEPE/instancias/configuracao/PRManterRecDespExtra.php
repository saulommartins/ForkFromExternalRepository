...
include_once(TTPE."TTPEPlanoAnaliticaTipoRetencao.class.php");
...
$obMapeamento = new TTPEPlanoAnaliticaRelacionamento();
Sessao::getTransacao()->setMapeamento( $obMapeamento );
$stAcao = $_POST["stAcao"] ? $_POST["stAcao"] : $_GET["stAcao"];

$obTTPEPlanoAnaliticaTipoRetencao = new TTPEPlanoAnaliticaTipoRetencao();

switch ($_REQUEST['stAcao']) {
  default:
    $obMapeamento->setDado('exercicio',Sessao::getExercicio());
    $obTTPEPlanoAnaliticaTipoRetencao->setDado('exercicio',Sessao::getExercicio());
    ...
    foreach ($_REQUEST as $stKey => $stValue) {
        ...
        if (strstr($stKey,'inRetencao')){
            ...
            $obTTPEPlanoAnaliticaTipoRetencao->setDado('cod_plano',$arCodigo[1]);
            $obTTPEPlanoAnaliticaTipoRetencao->setDado('cod_tipo',$stValue);
            $obTTPEPlanoAnaliticaTipoRetencao->recuperaPorChave($rsRecordSet);
            if ($stValue != '') {
                if ($rsRecordSet->eof()) {
                    $obTTPEPlanoAnaliticaTipoRetencao->inclusao();
                } else {
                    $obTTPEPlanoAnaliticaTipoRetencao->alteracao();
                }
            } else {
              $obTTPEPlanoAnaliticaTipoRetencao->exclusao();
            }
        }
    }
