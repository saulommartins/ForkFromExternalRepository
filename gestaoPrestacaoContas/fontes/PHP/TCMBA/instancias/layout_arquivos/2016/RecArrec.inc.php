...
*/
    ...
    include_once( CAM_GPC_TCMBA_MAPEAMENTO.Sessao::getExercicio()."/TTBARecArrec.class.php" );

    $obTTBARecArrec = new TTBARecArrec();
    $obTTBARecArrec->setDado('mes'      , $inMes );
    $obTTBARecArrec->setDado('entidades', $stEntidades );
    $obTTBARecArrec->setDado('dt_inicial', $stDataInicial );
    $obTTBARecArrec->setDado('dt_final', $stDataFinal );
    $obTTBARecArrec->setDado('unidade_gestora', $inCodUnidadeGestora );
    $obTTBARecArrec->recuperaDadosTribunal($rsRecordSet);
