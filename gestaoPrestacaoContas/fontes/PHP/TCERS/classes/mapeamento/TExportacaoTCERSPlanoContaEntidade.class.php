...
class TExportacaoTCERSPlanoContaEntidade extends Persistente
    {
    ...
    /**
        ...
        * @access Private
    */
    public function TExportacaoTCERSPlanoContaEntidade()
    {
        parent::Persistente();

        $this->setTabela('tcers.plano_conta_entidade');
    ...
    {
        $stSql = "
        SELECT   pce.cod_entidade
                ,cgm.nom_cgm
                ,plc.cod_estrutural
                ,plc.cod_conta
                ,plc.nom_conta
                ,plc.exercicio
        FROM     tcers.plano_conta_entidade as pce
                ,orcamento.entidade         as ore
                ,sw_cgm                     as cgm
                ,contabilidade.plano_conta  as plc
        WHERE   pce.exercicio       = ore.exercicio
        AND     pce.cod_entidade    = ore.cod_entidade
        AND     ore.numcgm          = cgm.numcgm
        AND     pce.exercicio       = plc.exercicio
        AND     pce.cod_conta       = plc.cod_conta
        ".( $this->getDado('exercicio') ? " AND     plc.exercicio       = '".$this->getDado('exercicio')."'" : '' )."
        ORDER BY pce.cod_entidade, plc.cod_estrutural
        ";
