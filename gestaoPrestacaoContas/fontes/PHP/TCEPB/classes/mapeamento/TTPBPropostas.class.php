...
Revision 1.2  2007/04/23 15:28:54  rodrigo_sr
uc-06.03.00
...
class TTPBPropostas extends Persistente
{
/**
    * Método Construtor
    * @access Private
*/
function TTPBPropostas()
{
    parent::Persistente();
...
{
    $stSql  = "
                select cotacao_licitacao.cod_licitacao || cotacao_licitacao.exercicio_licitacao as num_licitacao
                     ,  tcepb.fn_depara_modalidade_licitacao( cotacao_licitacao.cod_modalidade ) as modalidade_licitacao
                     , ( coalesce (  ( select cpf
                     ...
                     , cotacao_fornecedor_item.vl_cotacao as valor_ofertado
                     ,case when julgamento_item.ordem = 1
                           then 1
                           else 2
                      end as situacao_proposta
    ...
    if ( $this->getDado('exercicio') ) {
        $stSql .= " AND cotacao_licitacao.exercicio_licitacao = '".$this->getDado('exercicio')."'       \n";
    }
