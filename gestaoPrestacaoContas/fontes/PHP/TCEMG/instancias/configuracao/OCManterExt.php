...
/**
    ...
    * @ignore
    ...
    $Id: OCManterExtmmaa.php 56934 2014-01-08 19:46:44Z gelson $
...
switch ($stCtrl) {
    ...
    case 'incluirConta' :
        ...
        $obTTCEMGBalanceteExtmmaa->setDado( 'cod_plano', $_REQUEST['inCodConta'] );
        $obTTCEMGBalanceteExtmmaa->recuperaRelacionamento( $rsContas );

        if ($rsContas->getNumLinhas() > 0) {
            $arTipoLancamento = array();
            $arTipoLancamento[1]  = 'Depósitos e Consignações';
            $arTipoLancamento[2]  = 'Débitos de Tesouraria';
            $arTipoLancamento[3]  = 'Ativo Realizável';
            $arTipoLancamento[4]  = 'Transferências Financeiras';
            $arTipoLancamento[99] = 'Outros';

            $stJs.= "document.getElementById('inCodConta').value = '';";
            $stJs.= "document.getElementById('stConta').innerHTML = '&nbsp;';";

            $stMensagem = 'Esta conta já consta na lista do tipo '.$arTipoLancamento[$rsContas->getCampo('tipo_lancamento')].'!';
        }
...
//            if ( ($_REQUEST['inTipoLancamento'] == 1 OR $_REQUEST['inTipoLancamento'] == 4) AND ($_REQUEST['inSubTipo'] != '') ) {
            if ( !empty($_REQUEST['inTipoLancamento']) AND !empty($_REQUEST['inSubTipo'])) {
                $obTTCEMGBalanceteExtmmaa = new TTCEMGBalanceteExtmmaa();
                $obTTCEMGBalanceteExtmmaa->setDado( 'exercicio', Sessao::getExercicio() );
                $obTTCEMGBalanceteExtmmaa->setDado( 'tipo_lancamento', $_REQUEST['inTipoLancamento'] );
                $obTTCEMGBalanceteExtmmaa->setDado( 'categoria', $_REQUEST['inCategoria'] );
                $obTTCEMGBalanceteExtmmaa->setDado( 'sub_tipo_lancamento', $_REQUEST['inSubTipo'] );
                $obTTCEMGBalanceteExtmmaa->recuperaRelacionamento( $rsContas );
        ...
        if ($_REQUEST['inTipoLancamento'] != '') {
            ...
            switch ($_REQUEST['inTipoLancamento']) {
                ...
                case 3:
                    ...
                    $obCmbSubTipo->setRotulo( 'Subtipo' );
                    $obCmbSubTipo->addOption('','Selecione');
                    $obCmbSubTipo->addOption('1','Salário-família');
                    $obCmbSubTipo->addOption('2','Salário-maternidade');
                    $obCmbSubTipo->addOption('3','Outros benefícios com mais de um favorecido');
                    $obCmbSubTipo->addOption('999','Outro');
                    $obCmbSubTipo->setNull( false );
                    $obCmbSubTipo->obEvento->setOnChange("montaParametrosGET('preencheLista','inTipoLancamento,inCategoria,inSubTipo','true');");
                    $obFormulario = new Formulario();
                ...
                case 4:
                    ...
                    $obCmbSubTipo->setRotulo( 'Subtipo' );
                    $obCmbSubTipo->addOption('','Selecione');
                    $obCmbSubTipo->addOption('1','Repasse à Câmara');
                    $obCmbSubTipo->addOption('2','Devolução de numerário para a prefeitura');
                    $obCmbSubTipo->addOption('3','Aporte de Recursos para Cobertura de Insuficiência Financeira para o RPPS');
                    $obCmbSubTipo->addOption('4','Aporte de Recursos para Formação de Reserva Financeira para o RPPS');
                    $obCmbSubTipo->addOption('5','Outros Aportes Financeiros para o RPPS');
                    $obCmbSubTipo->addOption('6','Aporte de Recursos para Cobertura de Déficit Financeiro para o RPPS');
                    $obCmbSubTipo->addOption('7','Aporte de Recursos para Cobertura ou Amortização de Déficit Atuarial para o RPPS');
                    $obCmbSubTipo->addOption('8','Outros Aportes Previdenciários para o RPPS');
                    $obCmbSubTipo->addOption('999','Outro');
                    $obCmbSubTipo->setNull( false );
                    $obCmbSubTipo->obEvento->setOnChange("montaParametrosGET('preencheLista','inTipoLancamento,inCategoria,inSubTipo','true');");
                    $obFormulario = new Formulario();
                ...
                default :
                    ...
                    $obTTCEMGBalanceteExtmmaa->setDado( 'tipo_lancamento', $_REQUEST['inTipoLancamento'] );
                    $obTTCEMGBalanceteExtmmaa->setDado( 'categoria', $_REQUEST['inCategoria'] );
                    $obTTCEMGBalanceteExtmmaa->setDado( 'sub_tipo_lancamento', 0 );
                    $obTTCEMGBalanceteExtmmaa->recuperaRelacionamento( $rsContas );
