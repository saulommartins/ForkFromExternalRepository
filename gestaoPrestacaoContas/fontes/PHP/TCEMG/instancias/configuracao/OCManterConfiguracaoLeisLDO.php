...
function preencheLeisConsultaLDO()
{
    ...
    $stFiltro  = " AND tipo_configuracao = 'consulta' ";
    ...
    while (!$rsTTCEMGConfiguracaoLeisLDO->eof()) {
        $inCodNorma = $rsTTCEMGConfiguracaoLeisLDO->getCampo("cod_norma");

        $rsTTCEMGConfiguracaoLeisLDO->proximo();
    }

    Sessao::write('inCodNormaSalvo', $inCodNorma);

    $stJs .= "f.inCodLeiLDO.focus();\n";
    $stJs .= "f.inCodLeiLDO.value = '".$inCodNorma."';\n";
    $stJs .= "f.stCodNorma.focus();\n";
...
function excluirNorma()
{
    ...
    foreach ($arNormas as $arNorma) {
        if ($arNorma['inId'] != $_GET['inId']) {
            ...
            $arTempRemovido[] = $arNorma;
        }
    }

    $arNormas = $arTemp;
    $arNormasRemovidos .= $arTempRemovido;

    Sessao::write('arNormasRemovido', $arTempRemovido);
    Sessao::write('arNormas', $arNormas);
