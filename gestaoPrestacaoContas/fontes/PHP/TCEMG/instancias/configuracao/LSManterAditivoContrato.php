...
<?php
/*
    * Lista de Cadastros de Aditivos de Contratos TCEMG
...
if ($stAcao == "alterar") {
    if ($_REQUEST["inNumeroAditivo"]) {
        $stFiltro .= " TCA.nro_aditivo = ".$_REQUEST["inNumeroAditivo"]." \n and ";
    }
    if ($_REQUEST["stExercioAditivo"]) {
        $stFiltro .= " TCA.exercicio = '".$_REQUEST["stExercioAditivo"]."' \n and ";
    }
}

$stFiltro .= " cod_objeto >= 1  \n and ";
$stFiltro .= " cod_objeto <= 3  \n and ";
...
if ($stAcao == 'incluir') {   
   ...
   $obTTCEMGContrato->recuperaTodos($rsContratos, $stFiltro, "contrato.cod_entidade,contrato.exercicio,contrato.nro_contrato");
    ...
    $obTTCEMGContratoAditivo->recuperaContratoAditivo($rsContratos, $stFiltro, "contrato.cod_entidade,contrato.exercicio,contrato.nro_contrato,TCA.nro_aditivo" );
}
...
if ($stAcao == "incluir") {
    ...
    $obLista->addDado();
    $obLista->ultimoDado->setAlinhamento("CENTRO");
    $obLista->ultimoDado->setCampo( "[nro_aditivo]/[exercicio_aditivo]" );
    $obLista->commitDado();
...
if ($stAcao != 'incluir') {
    $obLista->ultimaAcao->addCampo( "&inNumeroAditivo", "nro_aditivo" );
