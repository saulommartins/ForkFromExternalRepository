    ...
    public function montaRecuperaTodos()
    {
        $stSql  = "
            SELECT ".$this->getDado('bimestre')." AS bimestre
                 ...
                 , 'S' AS nada_declarar
                 , 0 AS caixa_rppsas
                 , 0 AS conta_movimento_rppsas
                 , 0 AS contas_vinculadas_rppsas
                 , 0 AS aplicacoes_financeiras_rppsas
                 , 0 AS compromissado_rppsas
                 , 0 AS aplicacoes_financeiras_vinc
                 , 0 AS aplicacoes_financeiras_vinc_rpps
                 , 0 AS aplicacoes_financeiras_vinc_rppsas
              FROM ".$this->getTabela()."('" . $this->getDado('exercicio') . "','" . $this->getDado('cod_entidade') . "'," . $this->getDado('bimestre') . ") AS retorno
                                          ( bimestre                    INTEGER ,
                                            caixa                       NUMERIC ,
