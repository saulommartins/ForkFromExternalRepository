...
function montaRecuperaDadosArquivo()
{
    $stSql  = "
                SELECT ".$this->getDado('periodo')." AS periodo
                    , CASE  WHEN valor_acrescimo < 0.00 THEN 
                                REPLACE((valor_acrescimo*-1)::VARCHAR,'.','') 
                            ELSE 
                                REPLACE((valor_acrescimo)::VARCHAR,'.','') 
                    END AS valor_acrescimo
                    , CASE  WHEN vl_reducao < 0.00 THEN 
                                REPLACE((vl_reducao*-1)::VARCHAR, '.','') 
                            ELSE 
                                REPLACE((vl_reducao)::VARCHAR, '.','') 
                    END AS vl_reducao
                    , cod_tipo 
