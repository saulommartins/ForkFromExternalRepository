...
/**
  ...
  * @ignore
  ...
  $Id: TTCEMGORGAO2015.class.php 62310 2015-04-20 19:54:55Z franver $
  $Date: $
  $Author: $
  $Rev: $
*/
...
class TTCEMGORGAO2015 extends Persistente
{
    /**
        * Método Construtor
        * @access Private
    */
    public function TTCEMGORGAO2015()
    {
        parent::Persistente();
    }
    ...
    public function montaRecuperaOrgao()
    {
        $stSql  = "
          SELECT 10 AS tipoRegistro
               , (
                  SELECT valor::INTEGER
                   ...
                   WHERE exercicio = ACE.exercicio
                     ...
                     AND cod_entidade = ACE.cod_entidade
                 ) AS codOrgao
               ...
               , sem_acentos('".$this->getDado('versao')."')::text AS versao
      ...
      INNER JOIN (
              ...
              INNER JOIN (
                          SELECT sw_cgm.numcgm
                               , CASE WHEN sw_cgm_pessoa_fisica.numcgm = sw_cgm.numcgm
                                      THEN 1
                                      WHEN sw_cgm_pessoa_juridica.numcgm = sw_cgm.numcgm
                                      THEN 2
                                      ELSE 3
    ...
    $stSql = "
      ...
      INNER JOIN sw_uf 
           ...
           WHERE configuracao_entidade.exercicio    = '".$this->getDado('exercicio')."'
             ...
             AND configuracao_orgao.tipo_responsavel NOT IN (5)
