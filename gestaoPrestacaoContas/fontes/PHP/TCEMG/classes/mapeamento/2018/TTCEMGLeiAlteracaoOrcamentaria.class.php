    ...
    */
    ...
    public function montaRecuperaExportacao10()
    {
        $stSql = "SELECT 10 AS tipo_registro,
                  ...
                  FROM orcamento.suplementacao
                       LEFT JOIN orcamento.suplementacao_suplementada ON suplementacao_suplementada.exercicio = suplementacao.exercicio
                            AND suplementacao_suplementada.cod_suplementacao = suplementacao.cod_suplementacao
                       LEFT JOIN orcamento.suplementacao_reducao ON suplementacao_reducao.exercicio = suplementacao.exercicio
                            AND suplementacao_reducao.cod_suplementacao = suplementacao.cod_suplementacao
                       INNER JOIN orcamento.despesa ON despesa.exercicio = suplementacao_suplementada.exercicio
                             AND despesa.cod_despesa = suplementacao_suplementada.cod_despesa
                       INNER JOIN normas.norma ON norma.cod_norma = suplementacao.cod_norma
                       INNER JOIN administracao.configuracao_entidade ON configuracao_entidade.cod_entidade  = despesa.cod_entidade
                       ...
                       INNER JOIN tcemg.norma_detalhe ON norma_detalhe.cod_norma = norma.cod_norma
    ...
    public function montaRecuperaExportacao11()
    {
        $stSql = "SELECT tabela.tipo_registro,
                        ...
                        CASE
                          WHEN tabela.artigo_lei_alteracao IS NOT NULL
                            THEN regexp_replace(remove_acentos(REPLACE((REPLACE(REPLACE(tabela.artigo_lei_alteracao,Chr(39), ''), Chr(10), '')), Chr(13), ' ')),'[º|°]', '', 'gi')
                        END AS artigo_lei_alteracao,
                        CASE
                          WHEN tabela.descricao_artigo IS NOT NULL
                            THEN TRIM(regexp_replace(remove_acentos(REPLACE(REPLACE(REPLACE(REPLACE(tabela.descricao_artigo,Chr(39), ''), Chr(10), ''), Chr(13), ' '), Chr(9), ' ')),'[º|°|%|§]', '', 'gi'))
                        END AS descricao_artigo
                  FROM (SELECT 11 AS tipo_registro,
                               norma.num_norma AS nro_lei_alteracao,
                               tipo_lei_alteracao_orcamentaria.cod_tipo_lei AS tipo_lei_alteracao,
                               CASE
                                 WHEN tipo_transferencia.cod_tipo = 15
                                   THEN LPAD('0,00', 14, '0')
                                 WHEN tipo_transferencia.cod_tipo = 16
                                   THEN LPAD('0,00', 14, '0')
                        ...
                        FROM orcamento.suplementacao
                             INNER JOIN normas.norma ON norma.cod_norma = suplementacao.cod_norma
                             LEFT JOIN tcemg.norma_artigo ON norma_artigo.cod_norma = norma.cod_norma
                             LEFT JOIN (SELECT OSS.exercicio,
                                        ...
                                        ORDER BY OSS.exercicio,
                                                 OSS.cod_suplementacao,
                                                 RECURSO.cod_recurso
                                       ) AS suplementacao_suplementada ON suplementacao.exercicio = suplementacao_suplementada.exercicio
                             ...
                             LEFT JOIN (SELECT OSR.exercicio,
                                        ...
                                        FROM orcamento.suplementacao_reducao AS OSR
                                             INNER JOIN orcamento.despesa AS OD ON OSR.cod_despesa = OD.cod_despesa
                                        ...
                                        ORDER BY OSR.exercicio,
                                                 OSR.cod_suplementacao
                                       ) AS suplementacao_reducao ON suplementacao.exercicio = suplementacao_reducao.exercicio
                                       AND suplementacao.cod_suplementacao = suplementacao_reducao.cod_suplementacao
                             LEFT JOIN contabilidade.tipo_transferencia ON suplementacao.cod_tipo = tipo_transferencia.cod_tipo
                                  AND suplementacao.exercicio = tipo_transferencia.exercicio
                             LEFT JOIN contabilidade.transferencia_despesa ON suplementacao.cod_tipo = transferencia_despesa.cod_tipo
                             ...
                             INNER JOIN tcemg.norma_detalhe ON norma_detalhe.cod_norma = norma.cod_norma
                                    AND norma_detalhe.tipo_lei_origem_decreto = 3
                             LEFT JOIN tcemg.tipo_lei_alteracao_orcamentaria ON tipo_lei_alteracao_orcamentaria.cod_tipo_lei = norma_detalhe.tipo_lei_alteracao_orcamentaria
                        WHERE suplementacao.exercicio = '" . $this->getDado('exercicio') . "'
                              AND (suplementacao_suplementada.cod_entidade IN (" . $this->getDado('entidades') . ")
                                  OR
                                   suplementacao_reducao.cod_entidade IN (" . $this->getDado('entidades') . "))
                              AND suplementacao.dt_suplementacao BETWEEN TO_DATE('01/" . $this->getDado('mes') . "/" . $this->getDado('exercicio') . "', 'dd/mm/yyyy')
                              AND last_day(TO_DATE('" . $this->getDado('exercicio') . "' || '-' || '" . $this->getDado('mes') . "' || '-' || '01', 'yyyy-mm-dd'))
    ...
    {
        $stSql = "SELECT DISTINCT tabela.tipo_registro
             , nro_lei_alter_orcam
