    ...
    */
    ...
    function montaRecuperaDadosMensalDDC10(){
	    $stSql = "SELECT '10' AS tipo_registro,
                ...
                FROM tcemg.configuracao_ddc AS t_ddc
	                   INNER JOIN normas.norma AS t_norma ON t_norma.cod_norma = t_ddc.cod_norma
    ...
    function montaRecuperaDadosMensalDDC30(){
      $stSql = "SELECT '30' AS tipo_registro,
                       ...
                       sub_tipo,
                       CASE
                         WHEN sw_cgm_pessoa_fisica.numcgm IS NOT NULL
                           THEN CASE
                             WHEN sw_cgm_pessoa_fisica.cod_nacionalidade = 1
                               THEN 1
						                 ELSE 3
			    								 END
			    						   ELSE 2
                       ...
                       REPLACE(valor_saldo_atual::VARCHAR, '.', ',') AS vl_saldo_atual
				        FROM tcemg.configuracao_ddc
			               INNER JOIN normas.norma AS t_norma ON t_norma.cod_norma = configuracao_ddc.cod_norma
					           JOIN sw_cgm ON sw_cgm.numcgm = configuracao_ddc.numcgm
 			               LEFT JOIN sw_cgm_pessoa_fisica ON sw_cgm_pessoa_fisica.numcgm = sw_cgm.numcgm
 			               LEFT JOIN sw_cgm_pessoa_juridica ON sw_cgm_pessoa_juridica.numcgm = sw_cgm.numcgm
