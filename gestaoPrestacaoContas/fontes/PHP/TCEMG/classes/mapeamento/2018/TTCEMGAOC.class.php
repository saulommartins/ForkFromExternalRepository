    ...
    */
    ...
    public function montaRecuperaDadosAOC10()
    {
        $stSql = "SELECT tiporegistro,
                  ...
                  FROM (SELECT '10'::text AS tiporegistro,
                               CASE
                                 WHEN EXTRACT(MONTH FROM suplementacao.dt_suplementacao) <> EXTRACT(MONTH FROM norma.dt_assinatura)
                                   THEN TO_CHAR(suplementacao.dt_suplementacao,'ddmmyyyy')
                               ...
                               CASE
                                 WHEN suplementacao_anulada.cod_suplementacao IS NOT NULL
                                   THEN 'anulada'
                                 ELSE 'valida'
                               END as situacao
                        FROM orcamento.suplementacao
                             LEFT JOIN orcamento.suplementacao_anulada ON suplementacao.exercicio = suplementacao_anulada.exercicio
                                  AND suplementacao.cod_suplementacao = suplementacao_anulada.cod_suplementacao
                             JOIN contabilidade.tipo_transferencia ON tipo_transferencia.cod_tipo = suplementacao.cod_tipo
                                  AND tipo_transferencia.exercicio = suplementacao.exercicio
                             JOIN orcamento.suplementacao_suplementada ON suplementacao.cod_suplementacao = suplementacao_suplementada.cod_suplementacao
                                  AND suplementacao.exercicio = suplementacao_suplementada.exercicio
                             JOIN orcamento.despesa ON suplementacao_suplementada.cod_despesa = despesa.cod_despesa
                                  AND suplementacao_suplementada.exercicio = despesa.exercicio
                             JOIN contabilidade.transferencia_despesa ON transferencia_despesa.cod_tipo = suplementacao.cod_tipo
                             ...
                             JOIN normas.norma ON suplementacao.cod_norma = norma.cod_norma
                             LEFT JOIN tcemg.norma_detalhe ON norma_detalhe.cod_norma = norma.cod_norma
        ...
        $stSql = "SELECT '11' AS tiporegistro,
                   ...
                   FROM (SELECT CASE
                         ...
                         FROM orcamento.suplementacao
                              LEFT JOIN orcamento.suplementacao_anulada ON suplementacao.exercicio = suplementacao_anulada.exercicio
                                   AND suplementacao.cod_suplementacao = suplementacao_anulada.cod_suplementacao
                              INNER JOIN contabilidade.tipo_transferencia ON tipo_transferencia.cod_tipo = suplementacao.cod_tipo
                                    AND tipo_transferencia.exercicio = suplementacao.exercicio
                              INNER JOIN orcamento.suplementacao_suplementada ON suplementacao.cod_suplementacao = suplementacao_suplementada.cod_suplementacao
                                    AND suplementacao.exercicio = suplementacao_suplementada.exercicio
                              INNER JOIN orcamento.despesa ON suplementacao_suplementada.cod_despesa = despesa.cod_despesa
                                    AND suplementacao_suplementada.exercicio = despesa.exercicio
                              INNER JOIN contabilidade.transferencia_despesa ON transferencia_despesa.cod_tipo = suplementacao.cod_tipo
                                    AND transferencia_despesa.cod_suplementacao = suplementacao.cod_suplementacao
                                    AND transferencia_despesa.exercicio = suplementacao.exercicio
                                    AND transferencia_despesa.cod_tipo <> 16
                              INNER JOIN normas.norma ON suplementacao.cod_norma = norma.cod_norma
                              LEFT JOIN tcemg.norma_detalhe ON norma_detalhe.cod_norma = norma.cod_norma
                         WHERE suplementacao.exercicio = '" . $this->getDado('exercicio') . "'
                               AND suplementacao.dt_suplementacao >= to_date('" . $this->getDado('dt_inicial') . "', 'dd/mm/yyyy')
                               AND suplementacao.dt_suplementacao <= to_date('" . $this->getDado('dt_final') . "', 'dd/mm/yyyy')
        ...
        $stSql = "SELECT '12' AS tiporegistro,
                        ...
                        (SELECT DISTINCT atributo_norma_valor.valor
                         FROM normas.atributo_norma_valor
                              ...
                              INNER JOIN administracao.atributo_dinamico ON atributo_dinamico.cod_modulo = atributo_tipo_norma.cod_modulo
                    ...
                    FROM (SELECT CASE
                          ...
                          FROM orcamento.suplementacao
                               ...
                               JOIN contabilidade.transferencia_despesa ON transferencia_despesa.cod_tipo = suplementacao.cod_tipo
                                    ...
                                    AND transferencia_despesa.cod_tipo <> 16
                               JOIN normas.norma ON suplementacao.cod_norma = norma.cod_norma
                               JOIN tcemg.norma_detalhe ON norma_detalhe.cod_norma = norma.cod_norma
                               LEFT JOIN tcemg.tipo_lei_origem_decreto ON tipo_lei_origem_decreto.cod_tipo_lei = norma_detalhe.tipo_lei_origem_decreto
                          WHERE suplementacao.exercicio = '" . $this->getDado('exercicio') . "'
                                AND suplementacao.dt_suplementacao >= to_date('" . $this->getDado('dt_inicial') . "', 'dd/mm/yyyy')
                                AND suplementacao.dt_suplementacao <= to_date('" . $this->getDado('dt_final') . "', 'dd/mm/yyyy')
                                AND transferencia_despesa.cod_entidade IN (" . $this->getDado('entidade') . ")
        ...
        $stSql = "SELECT tabela.dt_suplementacao,
                         ...
                         LPAD((SELECT valor
                               ...
                               WHERE exercicio = '" . $this->getDado('exercicio') . "'
                                     AND cod_entidade = tabela.cod_entidade
                                     AND parametro = 'tcemg_codigo_orgao_entidade_sicom'), 2, '0') AS codorgao,
                         ...
                         CASE
                           WHEN SUBSTR(REPLACE(tabela.cod_estrutural, '.', ''), 1, 6) = '339009' OR SUBSTR(REPLACE(tabela.cod_estrutural, '.', ''), 1, 6) = '339005'
                             THEN '319005'
                           ELSE CASE
                                  WHEN SUBSTR(REPLACE(tabela.cod_estrutural, '.', ''), 1, 6) = '339013'
                                    THEN '319013'
                                  ELSE SUBSTR(REPLACE(tabela.cod_estrutural, '.', ''), 1, 6)::VARCHAR
                                END
                  ...
                  FROM (SELECT CASE
                               ...
                               CASE
                                 ...
                                 WHEN suplementacao.cod_tipo = 11
                                   THEN '98'
                                 WHEN suplementacao.cod_tipo = 12
                                   THEN '98'
                                 WHEN suplementacao.cod_tipo = 13
                                   THEN '98'
                                 WHEN suplementacao.cod_tipo = 14
                                   THEN '98'
                                 WHEN suplementacao.cod_tipo = 15
                                   THEN '98'
                               END AS origemrecalteracao,
                               uniorcam.num_orgao,
                        ...
                        FROM orcamento.suplementacao
                             ...
                             INNER JOIN contabilidade.tipo_transferencia ON tipo_transferencia.cod_tipo = suplementacao.cod_tipo
                                  AND tipo_transferencia.exercicio = suplementacao.exercicio
                             INNER JOIN orcamento.suplementacao_suplementada ON suplementacao_suplementada.cod_suplementacao = suplementacao.cod_suplementacao
                                  AND suplementacao_suplementada.exercicio = suplementacao.exercicio
                             INNER JOIN orcamento.despesa ON despesa.cod_despesa = suplementacao_suplementada.cod_despesa
                                  AND despesa.exercicio = suplementacao_suplementada.exercicio
                             INNER JOIN orcamento.conta_despesa ON conta_despesa.exercicio = despesa.exercicio
                             ...
                             INNER JOIN contabilidade.transferencia_despesa ON transferencia_despesa.cod_tipo = suplementacao.cod_tipo
                                  ...
                                  AND transferencia_despesa.cod_tipo <> 16
                             INNER JOIN normas.norma ON suplementacao.cod_norma = norma.cod_norma
                             INNER JOIN orcamento.unidade ON unidade.exercicio = despesa.exercicio
                                  AND unidade.num_unidade = despesa.num_unidade
                                  AND unidade.num_orgao = despesa.num_orgao
                             INNER JOIN tcemg.uniorcam ON uniorcam.num_unidade = unidade.num_unidade
                                  AND uniorcam.num_orgao = unidade.num_orgao
                                  AND uniorcam.exercicio = unidade.exercicio
                             INNER JOIN orcamento.funcao ON funcao.exercicio = despesa.exercicio
                                  AND funcao.cod_funcao = despesa.cod_funcao
                             INNER JOIN orcamento.subfuncao ON subfuncao.exercicio = despesa.exercicio
                                  AND subfuncao.cod_subfuncao = despesa.cod_subfuncao
                             INNER JOIN orcamento.programa ON orcamento.programa.exercicio = despesa.exercicio
                                  AND orcamento.programa.cod_programa = despesa.cod_programa
                             INNER JOIN orcamento.programa_ppa_programa ON programa_ppa_programa.cod_programa = orcamento.programa.cod_programa
                             ...
                             INNER JOIN orcamento.pao ON despesa.num_pao = pao.num_pao
                                  AND despesa.exercicio = pao.exercicio
                             INNER JOIN orcamento.pao_ppa_acao ON pao_ppa_acao.num_pao = pao.num_pao
                                  AND pao_ppa_acao.exercicio = pao.exercicio
                             INNER JOIN ppa.acao ON acao.cod_acao = pao_ppa_acao.cod_acao
                             INNER JOIN orcamento.recurso ON recurso.exercicio = despesa.exercicio
                  ...
                  FROM (SELECT CASE
                        ...
                        FROM orcamento.suplementacao
                             ...
                             INNER JOIN contabilidade.tipo_transferencia ON tipo_transferencia.cod_tipo = suplementacao.cod_tipo
                                   AND tipo_transferencia.exercicio = suplementacao.exercicio
                             INNER JOIN orcamento.suplementacao_reducao ON suplementacao_reducao.cod_suplementacao = suplementacao.cod_suplementacao
                                   AND suplementacao_reducao.exercicio = suplementacao.exercicio
                             INNER JOIN orcamento.despesa ON despesa.cod_despesa = suplementacao_reducao.cod_despesa
                             ...
                             INNER JOIN orcamento.subfuncao ON subfuncao.exercicio = despesa.exercicio
                                   AND subfuncao.cod_subfuncao = despesa.cod_subfuncao
                             INNER JOIN orcamento.programa ON programa.exercicio = despesa.exercicio
                                   AND programa.cod_programa = despesa.cod_programa
                             INNER JOIN orcamento.programa_ppa_programa ON programa_ppa_programa.exercicio = despesa.exercicio
                                   AND programa_ppa_programa.cod_programa = despesa.cod_programa
                             INNER JOIN ppa.programa ON ppa.programa.cod_programa = programa_ppa_programa.cod_programa_ppa
                             INNER JOIN orcamento.despesa_acao ON despesa_acao.exercicio_despesa = despesa.exercicio
