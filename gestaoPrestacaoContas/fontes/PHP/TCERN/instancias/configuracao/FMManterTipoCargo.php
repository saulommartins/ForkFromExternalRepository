...
require_once CAM_GPC_TCERN_MAPEAMENTO.'TTCERNDescricaoSIAI.class.php';
require_once CAM_GPC_TCERN_MAPEAMENTO.'TTCERNSubdivisaoDescricaoSIAI.class.php';
...
$obTTCERNDescricaoSIAI = new TTCERNDescricaoSIAI;
$stOrder = ' ORDER BY cod_siai';
$obTTCERNDescricaoSIAI->recuperaTodos($rsDescricaoSIAI, '', $stOrder);
...
$obTPessoalSubDivisao->recuperaDeParaTipoCargo($rsSubDivisao, '', $stOrder, '', 'pessoal'.Sessao::getEntidade());

$obTTCERNSubdivisaoDescricaoSIAI = new TTCERNSubdivisaoDescricaoSIAI;
$obTTCERNSubdivisaoDescricaoSIAI->recuperaRelacionamento($rsTTCERNSubdivisaoDescricaoSIAI);

$arTTCERNSubdivisaoDescricaoSIAI = $rsTTCERNSubdivisaoDescricaoSIAI->getElementos();
$arSubDivisao = $rsSubDivisao->getElementos();

foreach ($arTTCERNSubdivisaoDescricaoSIAI as $key => $value) {
    $arTTCERNSubdivisaoDescricaoSIAINew[$value['cod_sub_divisao']] = $value['cod_siai'];
}
foreach ($arSubDivisao as $key => $value) {
    $value['cod_siai'] = $arTTCERNSubdivisaoDescricaoSIAINew[$value['cod_sub_divisao']];
    $arSubDivisaoNew[] = $value;
}
$rsSubDivisao->preenche($arSubDivisaoNew);
...
$obCmbTipoCargo->setCampoId   ('[cod_siai]');
$obCmbTipoCargo->setCampoDesc ('[descricao]');
$obCmbTipoCargo->addOption    ('','Selecione');
$obCmbTipoCargo->preencheCombo($rsDescricaoSIAI);
$obCmbTipoCargo->setValue     ('[cod_siai]');
...
$obTable->Head->addCabecalho('Cod Descrição SIAI', 15);
