...
/**
    ...
    * $Id: TTCEALPublicacaoRGF.class.php 65345 2016-05-13 18:07:34Z michel $
...
class TTCEALPublicacaoRGF extends Persistente
{
    /**
        * Método Construtor
        * @access Private
    */
    ...
    {
        parent::Persistente();
        $this->setTabela("tceal.publicacao_rgf");
    ...
    {
        $stSql  = "SELECT publicacao_rgf.exercicio 
                        , publicacao_rgf.cod_entidade
                        , to_char( publicacao_rgf.dt_publicacao, 'dd/mm/yyyy' ) as dt_publicacao
                        , publicacao_rgf.numcgm as num_veiculo
                        , publicacao_rgf.num_publicacao
                        , publicacao_rgf.observacao
                        , sw_cgm.nom_cgm as nom_veiculo
                     FROM tceal.publicacao_rgf
               INNER JOIN sw_cgm
                       ON sw_cgm.numcgm = publicacao_rgf.numcgm
                    WHERE exercicio = '".$this->getDado('exercicio')."'
                      AND cod_entidade =".$this->getDado('cod_entidade');
        return $stSql;
    }
    
    function recuperaExportacaoRGF(&$rsRecordSet, $stFiltro = "", $stOrdem = "", $boTransacao = "")
    {
        $obErro      = new Erro;
        $obConexao   = new Conexao;
        $rsRecordSet = new RecordSet;
        ...
        $stSql = $this->montaRecuperaExportacaoRGF().$stFiltro.$stOrdem;
        $this->setDebug( $stSql );
        $obErro = $obConexao->executaSQL( $rsRecordSet, $stSql, $boTransacao );
    ...
    function montaRecuperaExportacaoRGF()
    {
        $stSql  = "SELECT ( SELECT PJ.cnpj
               ...
               INNER JOIN sw_cgm
                    ...
                    WHERE exercicio = '".$this->getDado('stExercicio')."'
                      ...
                      AND ( publicacao_rgf.dt_publicacao
