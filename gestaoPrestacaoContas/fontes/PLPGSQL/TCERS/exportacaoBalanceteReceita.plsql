...
CREATE OR REPLACE FUNCTION tcers.fn_exportacao_balancete_receita(varchar,varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    SELECT tabela.cod_estrutural
         ...
         , coalesce(tabela.vl_arrecadado   ,0.00) as vl_arrecadado
    ...
    LOOP
        /* Se nao começar com 9 e for menor que zero*/
        --IF (substr(reRegistro.cod_estrutural,1,1) < 9 ) and (reRegistro.vl_arrecadado < 0) THEN
        --IF (substr(reRegistro.cod_estrutural,1,1) < 9 ) THEN
        --     reRegistro.vl_arrecadado := reRegistro.vl_arrecadado * -1;
        --END IF;
        /* Se começar com 9, multiplica por  -1 se for maior que 0 */
        --IF (substr(reRegistro.cod_estrutural,1,1) = 9 ) and (reRegistro.vl_arrecadado > 0) THEN
             reRegistro.vl_arrecadado := reRegistro.vl_arrecadado * -1;
        --END IF;
