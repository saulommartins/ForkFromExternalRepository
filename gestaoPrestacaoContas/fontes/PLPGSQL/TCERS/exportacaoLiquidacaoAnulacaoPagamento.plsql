...
CREATE OR REPLACE FUNCTION tcers.fn_exportacao_liquidacao_anulacao_pagamento(varchar,integer,integer,varchar) RETURNS NUMERIC(14,2)  AS '
DECLARE
...
BEGIN
    ...
    WHERE   enl.exercicio            =  stExercicio
     ...
     -- Nota Liquidacao Paga Anulada
        ...
        AND	to_date(enlpa.timestamp_anulada::varchar,''yyyy-mm-dd'') <= to_date(''31/12/''||to_number(stExercicioAtual,''9999'')-1,''dd/mm/yyyy'' );
    RETURN nuSoma;
END;
' LANGUAGE 'plpgsql';
