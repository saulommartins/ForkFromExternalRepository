...
CREATE OR REPLACE FUNCTION tcers.fn_exportacao_liquidacao_total_anulado(varchar,integer,integer,varchar) RETURNS NUMERIC(14,2)  AS '
DECLARE
...
BEGIN
    ...
    WHERE   enl.exercicio       =   stExercicio
        ...
        AND eli.cod_entidade    =   enl.cod_entidade
         ...
         AND to_date(lia.timestamp::varchar,''yyyy-mm-dd'') <=  to_date(''31/12/''||to_number(stExercicioAtual,''9999'')-1,''dd/mm/yyyy'');
