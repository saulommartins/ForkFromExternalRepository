...
CREATE OR REPLACE FUNCTION tcers.fn_total_valor_credito(VARCHAR,NUMERIC,VARCHAR) RETURNS NUMERIC AS $$
DECLARE
...
BEGIN
   ...
   WHERE
	 suplementacao_suplementada.exercicio = '''||stExercicio||''' 
     AND suplementacao_suplementada.cod_suplementacao = '||inCodSuplementacao||'
