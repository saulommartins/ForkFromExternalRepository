...
CREATE OR REPLACE FUNCTION tcern.fn_exportacao_despesa(varchar,varchar,varchar,varchar) RETURNS SETOF
RECORD AS $$
DECLARE
...
BEGIN
    ...
    stSql := '
        CREATE TEMPORARY TABLE tmp_empenho_implantado AS
                            SELECT  restos_pre_empenho.cod_estrutural
                        ...
                        INNER JOIN  empenho.empenho
                          ...
                          GROUP BY  restos_pre_empenho.cod_estrutural
    ';
    ...
    stSql := '
                  SELECT
                       ...
                       ,  despesa.cod_recurso::integer as fonte_recurso
                       ,  (LPAD(despesa.num_orgao::VARCHAR, 2, ''0'')::varchar||LPAD(despesa.num_unidade::VARCHAR, 2, ''0'')::varchar)::varchar as classificacao_institucional
                       ,  (despesa.cod_funcao::varchar || despesa.cod_subfuncao::varchar)::varchar as classificacao_funcional
                       ,  despesa.cod_programa::integer as classificacao_programa
                       ,  despesa.num_pao::integer as classificacao_projeto
                       ...
                       ,  ( SUM(COALESCE(restos_pagar.valor,0))
                            +
                            (SELECT SUM(tmp_empenho_implantado.valor) FROM tmp_empenho_implantado WHERE SUBSTR(REPLACE(tmp_empenho_implantado.cod_estrutural,''.'',''''),1,6) = SUBSTR(REPLACE(conta_despesa.cod_estrutural,''.'',''''),1,6)
                            )
                            -
                            (SELECT SUM(tmp_empenho_anulado.valor) FROM tmp_empenho_anulado WHERE SUBSTR(REPLACE(tmp_empenho_anulado.cod_estrutural,''.'',''''),1,6) = SUBSTR(REPLACE(conta_despesa.cod_estrutural,''.'',''''),1,6)
                            )
                            -
                            (SELECT SUM(tmp_liquidacao.valor) FROM tmp_liquidacao WHERE SUBSTR(REPLACE(tmp_liquidacao.cod_estrutural,''.'',''''),1,6) = SUBSTR(REPLACE(conta_despesa.cod_estrutural,''.'',''''),1,6)
                            )
                            +
                            (SELECT SUM(tmp_liquidacao_anulado.valor) FROM tmp_liquidacao_anulado WHERE SUBSTR(REPLACE(tmp_liquidacao_anulado.cod_estrutural,''.'',''''),1,6) = SUBSTR(REPLACE(conta_despesa.cod_estrutural,''.'',''''),1,6)
                            )
              ...
              INNER JOIN  orcamento.conta_despesa
               ...
               LEFT JOIN (  SELECT  pre_empenho_despesa.exercicio
                ...
                LEFT JOIN (  SELECT pre_empenho_despesa.exercicio
                                    , pre_empenho_despesa.cod_despesa
                                    , empenho.cod_entidade
                                    , SUM(COALESCE(nota_liquidacao_item.vl_total,00)) - SUM(COALESCE(nota_liquidacao_item_anulado.vl_anulado, 00)) AS valor
                                FROM empenho.pre_empenho_despesa
                           ...
                           LEFT JOIN ( SELECT SUM(nota_liquidacao_item.vl_total) AS vl_total
                                            ...
                                            JOIN empenho.nota_liquidacao_item
                                             ...
                                             AND nota_liquidacao_item.cod_nota = nota_liquidacao.cod_nota
                                             AND nota_liquidacao_item.cod_entidade = nota_liquidacao.cod_entidade
                                            WHERE nota_liquidacao.dt_liquidacao  BETWEEN  TO_DATE('|| quote_literal(stDataInicial) ||',''dd/mm/yyyy'')
                          ...
                          LEFT JOIN ( SELECT SUM(nota_liquidacao_item_anulado.vl_anulado) AS vl_anulado
                                            , nota_liquidacao.exercicio_empenho
                                            , nota_liquidacao.cod_empenho
                                            , nota_liquidacao.cod_entidade
                                        FROM empenho.nota_liquidacao
                                        ...
                                        WHERE COALESCE( TO_DATE( timestamp::VARCHAR, ''yyyy-mm-dd'' ), TO_DATE('|| quote_literal(stDataInicial) ||',''dd/mm/yyyy'') ) 
                                                                                      BETWEEN TO_DATE('|| quote_literal(stDataInicial) ||',''dd/mm/yyyy'')
                                                                                          AND TO_DATE('|| quote_literal(stDataFinal) ||',''dd/mm/yyyy'')
                                        GROUP BY nota_liquidacao.exercicio_empenho
                                                , nota_liquidacao.cod_empenho
                                                , nota_liquidacao.cod_entidade
                                    ) AS nota_liquidacao_item_anulado
                                  ON nota_liquidacao_item_anulado.exercicio_empenho = empenho.exercicio
                                 AND nota_liquidacao_item_anulado.cod_empenho = empenho.cod_empenho
                                 AND nota_liquidacao_item_anulado.cod_entidade = empenho.cod_entidade
                ...
                LEFT JOIN ( SELECT pre_empenho_despesa.exercicio
                           ...
                           LEFT JOIN ( SELECT SUM(nota_liquidacao_item.vl_total) AS vl_total
                                            ...
                                            JOIN empenho.nota_liquidacao_item
                                             ...
                                             AND nota_liquidacao_item.cod_nota = nota_liquidacao.cod_nota
                                             AND nota_liquidacao_item.cod_entidade = nota_liquidacao.cod_entidade
                                            WHERE nota_liquidacao.dt_liquidacao <= TO_DATE('|| quote_literal(stDataFinal) ||',''dd/mm/yyyy'')
                                            GROUP BY nota_liquidacao.exercicio_empenho
                                                    , nota_liquidacao.cod_empenho
                                                    , nota_liquidacao.cod_entidade
                                    ) AS nota_liquidacao_item
                           ...
                           LEFT JOIN ( SELECT SUM(nota_liquidacao_item_anulado.vl_anulado) AS vl_anulado
                                        ...
                                        JOIN empenho.nota_liquidacao_item_anulado
                                         ...
                                         AND nota_liquidacao_item_anulado.cod_nota = nota_liquidacao.cod_nota
                                         AND nota_liquidacao_item_anulado.cod_entidade = nota_liquidacao.cod_entidade
                                        WHERE COALESCE( TO_DATE( timestamp::VARCHAR, ''yyyy-mm-dd'' ) <= TO_DATE('|| quote_literal(stDataFinal) ||',''dd/mm/yyyy''))
                                        GROUP BY nota_liquidacao.exercicio_empenho
                                                , nota_liquidacao.cod_empenho
                                                , nota_liquidacao.cod_entidade
                                    ) AS nota_liquidacao_item_anulado
