...
*/
CREATE OR REPLACE FUNCTION tceal.empenho_pago_estornado_restos(varchar,varchar,varchar,varchar)
...
BEGIN
   ...
   if (stSituacao = '1') then
           stSql := stSql || ', to_char(nlp.timestamp,''dd/mm/yyyy'') as data
                              ...
                              , ptd.cod_tipo_documento AS tipo_pagamento
                              , ptd.num_documento AS num_documento ';
   end if;
           ...
           stSql := stSql || ', to_char(nlpa.timestamp_anulada,''dd/mm/yyyy'') as data
            ...
            stSql := stSql || '
                ...
                if (stSituacao = '1') then
                    ...
                    LEFT JOIN tceal.pagamento_tipo_documento AS ptd
                      ON (ptd.cod_entidade = nlp.cod_entidade
                     AND ptd.cod_nota     = nlp.cod_nota
                     AND ptd.exercicio    = nlp.exercicio
                     AND ptd.timestamp    = nlp.timestamp)
                    , tmp_pago_rp tmp
                    ';
                end if;
            ...
            WHERE
                    e.exercicio         = ' || quote_literal(reReg.exercicio) || '
