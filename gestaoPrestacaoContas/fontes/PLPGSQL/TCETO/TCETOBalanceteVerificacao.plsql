...
* $Id: TCETOBalanceteVerificacao.plsql 60732 2014-11-12 16:00:40Z evandro $
* $Revision: $
* $Name$
* $Author: $
* $Date: $
*
*/
CREATE OR REPLACE FUNCTION tceto.balancete_verificacao(VARCHAR, VARCHAR, VARCHAR, VARCHAR) RETURNS SETOF RECORD AS $$ 
DECLARE
...
BEGIN
    ...
    stSql := 'CREATE TEMPORARY TABLE tmp_debito AS
                ...
                FROM (
                 ...
                 WHERE cod_entidade IN ('||stEntidades||')
                 ';
    ...
    CREATE TEMPORARY TABLE tmp_totaliza_debito AS
        SELECT *
        FROM  tmp_debito
        WHERE dt_lote BETWEEN to_date( '01/01/' || stExercicio || '' , 'dd/mm/yyyy' ) AND   to_date( stDtFinal::varchar , 'dd/mm/yyyy' )
        AND   tipo <> 'I';
    ...
    stSql := ' SELECT
                       ...
                       , CAST(CASE WHEN pa.cod_plano IS NULL
                                   THEN pc.nom_conta
                                   ELSE pa.cod_plano ||''-''|| pc.nom_conta
                       ...
                       , 0.00 as vl_saldo_debitos_bimestre
                       , 0.00 as vl_saldo_creditos
                       , 0.00 as vl_saldo_creditos_bimestre
    ...
    LOOP
        arRetorno := tceto.totaliza_balancete_verificacao( publico.fn_mascarareduzida(reRegistro.cod_estrutural) , stDtInicial, stDtFinal);
        reRegistro.vl_saldo_anterior          := arRetorno[1];
        reRegistro.vl_saldo_debitos           := arRetorno[2];
        reRegistro.vl_saldo_creditos          := arRetorno[3];
        reRegistro.vl_saldo_debitos_bimestre  := arRetorno[6];
        reRegistro.vl_saldo_creditos_bimestre := arRetorno[7];
        reRegistro.vl_saldo_atual             := arRetorno[4];
        reRegistro.cod_entidade               := arRetorno[5];
