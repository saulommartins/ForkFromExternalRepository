/*
    ...
    **********************************************************************************
*/
CREATE OR REPLACE FUNCTION stn.fn_rreo_despesa_empenhada_funcao_subfuncao(stMascRed VARCHAR, stExercicio VARCHAR, stEntidades VARCHAR, stDtIni VARCHAR, stDtFim VARCHAR, stRecurso VARCHAR, stFuncao VARCHAR, stSubFuncao VARCHAR, stDespesa VARCHAR, boIntra BOOLEAN) RETURNS NUMERIC(14,2) AS 
$$
...
BEGIN 
  ...
  INNER JOIN empenho.empenho AS e
	...
	END IF;
    IF (stDespesa <> '') THEN 
		stSql := stSql || ' AND ode.cod_despesa IN ('||stDespesa||')  ';
	END IF;
