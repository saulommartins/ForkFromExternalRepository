...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo1_receitas_novo(varchar, varchar, varchar, varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
CREATE TEMPORARY TABLE tmp_rreo_an1_receita AS (
    ...
    SELECT
        ...
        coalesce(previsao_inicial,0.00)::numeric(14,2) as previsao_inicial,
        coalesce(previsao_inicial,0.00)::numeric(14,2) as previsao_atualizada,
        ...
        FROM (SELECT nivel , 
                    ...
                    FROM(
                        SELECT
                            CASE WHEN ocr.cod_estrutural like ''9.1%'' AND  publico.fn_nivel(ocr.cod_estrutural) =2 THEN publico.fn_nivel(''1.7.0.0.00.00.00.00.00'') 
                                 WHEN ocr.cod_estrutural like ''9.1%'' AND  publico.fn_nivel(ocr.cod_estrutural) =3 THEN  publico.fn_nivel(''1.7.2.0.00.00.00.00.00'')
                                 WHEN ocr.cod_estrutural like ''9.1%'' AND  publico.fn_nivel(ocr.cod_estrutural) =3 THEN  publico.fn_nivel(''1.3.3.0.00.00.00.00.00'')
                        ...
                        WHERE            
                            -- Filtros
                                ...
                                --AND NOT ocr.cod_estrutural LIKE ''1.3.3.3.01.00.00.00.00''
                                
                                OR
                                
                                (( ocr.cod_estrutural LIKE ''1.3.6.0.00%'' -- conta não encontrada
                               OR ocr.cod_estrutural LIKE ''1.5.1.0.00%'' -- conta não encontrada
                               OR ocr.cod_estrutural LIKE ''1.7.7.0.00%'' -- conta não encontrada
                               OR ocr.cod_estrutural LIKE ''1.9.4.0.00%'' -- conta não encontrada
                               OR ocr.cod_estrutural LIKE ''2.4.7.7.00%'' -- conta não encontrada
                               OR ocr.cod_estrutural LIKE ''2.5.2.0.00%'' -- conta não encontrada
...
UNION 
    ...
    FROM(
        ...
        OR 
         ...
         ((ocr.cod_estrutural like ''2.7.6.1.04%'' )
         AND ocr.exercicio = '|| quote_literal(stExercicio) ||')
...
UNION 
    ...
    FROM(
        ...
        FROM
            ...
            orcamento.conta_receita     as ocr
      INNER JOIN orcamento.receita AS ore
              ON ore.cod_conta = ocr.cod_conta
        ...
        WHERE
         ...
         ((ocr.cod_estrutural like ''7.2.1.0.29%'' )
         AND ocr.exercicio = '|| quote_literal(stExercicio) ||')
...
-- ATUALIZA OS TOTAIS PARA GRUPO 1, NIVEL 1
...
-- ATUALIZA OS PERCENTUAIS A PARTIR DOS TOTAIS PARA GRUPO 1, NIVEL 1
    
    UPDATE tmp_rreo_an1_receita 
       SET p_no_bimestre = ( SELECT CASE WHEN coalesce(previsao_inicial,0.00) <> ''0.00'' THEN 
                                     CAST((coalesce(no_bimestre,0.00)*100/coalesce(previsao_inicial,0.00)) as numeric(14,2))
                                    ELSE ''0.00''::numeric(14,2) END as p_no_bimestre 
                               FROM tmp_rreo_an1_receita 
                              WHERE grupo = 1 AND nivel = 1
           ),
           
           p_ate_bimestre = ( SELECT CASE WHEN coalesce(previsao_inicial,0.00) <> ''0.00'' THEN 
                                        CAST((coalesce(ate_bimestre,0.00)*100/coalesce(previsao_inicial,0.00)) as numeric(14,2))
                                     ELSE ''0.00''::numeric(14,2) END as p_ate_bimestre
                                     FROM tmp_rreo_an1_receita 
                                    WHERE grupo = 1 AND nivel = 1
           )
    WHERE cod_estrutural like ''1.0.0.0.00.00.00.00.00'';
