...
*
* $Id: relatorioRGFAnexo2ValoresNaoIntegrantesRPPS.plsql 66641 2016-10-20 16:58:39Z carlos.silva $

* Casos de uso: uc-06.01.02
*/

CREATE OR REPLACE FUNCTION stn.fn_rgf_anexo2_valores_nao_integrantes_rpps(varchar,varchar,integer,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
  ...
  IF stContasConfiguracao <> '' THEN
    arFiltro[1] := '( '||SUBSTR(stContasConfiguracao, 5)||' ) ';
  ...
  arFiltro[3] := '( REPLACE(plano_conta.cod_estrutural,''''.'''','''''''') LIKE ''''21881%'''' ) '; 
  ...
  IF stContasConfiguracao <> '' THEN
    arFiltro[4] := '( '||SUBSTR(stContasConfiguracao, 5)||' ) ';
  ...
  IF stContasConfiguracao <> '' THEN
    arFiltro[5] := '( '||SUBSTR(stContasConfiguracao, 5)||' ) ';
  ...
  stSql := '
    CREATE TEMPORARY TABLE tmp_valores_rpps(
  ...
  FOR i IN 1..5 LOOP
    ...
    IF(arFiltro[i] != '') THEN
      ...
      LOOP
         stSql := ' INSERT INTO tmp_valores_rpps 
  ...
  IF arBoFiltroVazio[1] THEN
      INSERT INTO tmp_valores_rpps VALUES (arDescricao[1],1,0,0,0,0);
  END IF;

  IF arBoFiltroVazio[2] THEN
      INSERT INTO tmp_valores_rpps VALUES (arDescricao[2],2,0,0,0,0);
  END IF;
  ...
  IF arBoFiltroVazio[5] THEN
      INSERT INTO tmp_valores_rpps VALUES (arDescricao[5],5,0,0,0,0);
  END IF;

  --
  -- FAZ OS UPDATES DOS NIVEIS
  -- 
  UPDATE tmp_valores_rpps SET nivel = 0 WHERE ordem IN (1,2,3,4,5);

  stSql := 'SELECT * FROM tmp_valores_rpps ORDER BY ordem';
