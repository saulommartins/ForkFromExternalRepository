...
*/
/**
    * Script de função PLPGSQL - Calcula o valor da DP vinculada via sistema
...
CREATE OR REPLACE FUNCTION stn.fn_calcula_dp_vinculada(varchar, varchar, varchar) RETURNS NUMERIC AS $$
DECLARE
...
BEGIN
    ...
    SELECT SUM(valor) AS valor
     ...
     WHERE exercicio = ''' || stExercicio || '''
       ...
       AND (    TO_DATE(''01/'' || mes || ''/'' || ano, ''dd/mm/yyyy'') < TO_DATE(''01/' || (TO_NUMBER(arData[2],'99') + 1) || '/'||arData[3]||''',''dd/mm/yyyy'')
            AND TO_DATE(''01/'' || mes || ''/'' || ano, ''dd/mm/yyyy'') >= TO_DATE(''01/' || (TO_NUMBER(arData[2],'99') + 1) || '/'||arData[3]||''',''dd/mm/yyyy'') - INTERVAL ''12 MONTHS'') 
    ';
