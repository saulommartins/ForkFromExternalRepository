...
CREATE OR REPLACE FUNCTION stn.fn_anexo14_despesas_liquidada(stMascRed VARCHAR, stExercicio VARCHAR, stEntidades VARCHAR, stCodRecursos VARCHAR, stDtIni VARCHAR, stDtFim VARCHAR, boIntra BOOLEAN) RETURNS NUMERIC(14,2) AS 
$$
...
BEGIN 
	...
	WHERE 
		...
		pedcd.cod_recurso IN (' || stCodRecursos || ') AND
