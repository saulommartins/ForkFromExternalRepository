...
*/
/**
    * Relatório RREO Anexo 9 - Despesas. 
...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo9_despesas(VARCHAR, VARCHAR, VARCHAR, INTEGER) RETURNS SETOF RECORD AS $$
DECLARE 
    ...
    */
    ...
    SELECT grupo
      ...
      FROM 
            (            
                SELECT * 
                  FROM contabilidade.fn_relatorio_balanco_orcamentario_despesa_novo(''' || stExercicio || ''',''' || stDtIni || ''' ,''' || stDtFim || ''', ''' || stEntidades || ''', '''') as
            retorno (                                                                                          
                grupo                   integer ,                                                                  
