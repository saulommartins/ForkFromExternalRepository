...
*/
/*
 * Script de função PLPGSQL - Relatório STN - RREO - Anexo 7 
 *
...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo7(varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
--left para achar o cod_estrutural
  ...
  -- cria a tabela temporaria para o valor liquidado
  StSql := '
...
--consulta para retornar todas os orgaos para nao intra-orcamentarias
  ...
  stSql := '
   ...
   UNION ALL 
    ...
    SELECT ''PODER EXECUTIVO'' AS tipo
         , 2 AS ordem
         , ( SELECT SUM(vl_total)
               FROM tmp_processados_exercicios_anteriores
         ...
         , ( SELECT SUM(vl_total)
           ...
           , ( SELECT SUM(vl_total)
              ...
              WHERE (nom_entidade NOT ILIKE ''%câmara%'' AND nom_entidade NOT ILIKE ''%camara%'')
                AND cod_estrutural <> ''91''
           ) AS valor_nao_processado_liquidado
    UNION ALL
  ...
  GROUP BY tmp_orgao.num_orgao, tmp_orgao.nom_orgao
   ...
   UNION ALL 
    ...
    SELECT ''PODER LEGISLATIVO'' AS tipo
         , 4 AS ordem
         , ( SELECT SUM(vl_total)
               FROM tmp_processados_exercicios_anteriores
  ...
  UNION ALL 
    ...
    SELECT ''PODER JUDICIÁRIO'' AS tipo
         , 7 AS ordem
         ...
         , 0.00 AS valor_nao_processado_liquidado

UNION ALL 
    
    SELECT ''MINISTÉRIO PÚBLICO'' AS tipo
         , 8 AS ordem
         , 0.00 AS valor_processado_exercicios_anteriores
         , 0.00 AS valor_processado_exercicio_anterior
         , 0.00 AS valor_processado_cancelado
