...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo12_despesas_subfuncao_ultimo_bimestre( varchar, varchar, varchar, varchar ) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
  ...
  --left para achar o cod_estrutural
    ...
    INSERT INTO tmp_retorno VALUES (   1
                                     ...
                                     , CASE WHEN (SELECT  SUM( vl_total) FROM tmp_nao_processados_exercicio_anterior WHERE cod_subfuncao = 301) IS NULL THEN 0.00 END 
    ...
    UPDATE tmp_retorno SET porcentagem = ROUND((despesa_liquidada+( SELECT SUM( restos_nao_processados) FROM tmp_retorno ))/( SELECT SUM( dotacao_atualizada) FROM tmp_retorno ));
    ...
    stSql := '
        SELECT grupo
             , subgrupo
             , descricao 
             , COALESCE(dotacao_inicial,0.0) AS dotacao_inicial
             , COALESCE(dotacao_atualizada,0.0) AS dotacao_atualizada
             , restos_nao_processados 
             , COALESCE(despesa_liquidada,0.0) AS despesa_liquidada
             , COALESCE((((despesa_liquidada+ restos_nao_processados)/dotacao_atualizada)*100),0) AS porcentagem
          FROM tmp_retorno
