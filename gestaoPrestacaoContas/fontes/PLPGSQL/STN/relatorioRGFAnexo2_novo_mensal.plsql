...
CREATE OR REPLACE FUNCTION stn.fn_rgf_anexo2_novo_mensal(varchar,varchar,integer,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
  ...
  INSERT INTO tmp_valores VALUES (arDescricao[12]
                               ...
                               , ( (SELECT valor_mes FROM tmp_valores WHERE ordem = 1) - (SELECT valor_mes FROM tmp_valores WHERE ordem = 8) )
                                 );
  ...
  INSERT INTO tmp_valores VALUES( arDescricao[13],13,vlExercicioAnterior,vlPeriodo1 );
  ...
  SELECT CASE WHEN(vlExercicioAnterior <> 0) THEN 
       ...
       , CASE WHEN(vlPeriodo1 <> 0) THEN
         ROUND( ( SELECT valor_mes FROM tmp_valores WHERE ordem = 1 )
           /
           ( SELECT valor_mes FROM tmp_valores WHERE ordem = 13 )
         ,4)*100
         ELSE 0
         END AS valor_mes

    INTO vlExercicioAnterior, vlPeriodo1;

  INSERT INTO tmp_valores VALUES (arDescricao[14],14,vlExercicioAnterior,vlPeriodo1);
  ...
  SELECT CASE WHEN(vlExercicioAnterior <> 0) THEN
         ...
         END AS valor_exercicio_anterior
       , CASE WHEN(vlPeriodo1 <> 0) THEN
         ROUND( ( SELECT valor_mes FROM tmp_valores WHERE ordem = 12 )
           /
           ( SELECT valor_mes FROM tmp_valores WHERE ordem = 13 )
         ,4)*100
         ELSE 0
  ...
  INSERT INTO tmp_valores VALUES (arDescricao[15],15,vlExercicioAnterior,vlPeriodo1);
  ...
  INSERT INTO tmp_valores VALUES (  'LIMITE DEFINIDO POR RESOLUÇÃO SENADO FEDERAL - 120%'
                                  ...
                                  , (SELECT ROUND((valor_mes*120)/100,4) FROM tmp_valores WHERE ordem = 13)
                                 );
  --
  -- 17 LIMITE DE ALERTA                                                                   
  --
  INSERT INTO tmp_valores VALUES (  'LIMITE DE ALERTA (inciso III do § 1º do art. 59 da LRF) - 90%'           
                                  ...
                                  , (SELECT ROUND((valor_mes*90)/100,4) FROM tmp_valores WHERE ordem = 16)
                                 );
