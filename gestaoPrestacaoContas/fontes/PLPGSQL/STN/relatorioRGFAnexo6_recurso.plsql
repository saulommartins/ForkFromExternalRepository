...
CREATE OR REPLACE FUNCTION stn.fn_rgf_anexo6_recurso(varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
            ...
            INNER JOIN empenho.pre_empenho
             ...
             LEFT JOIN (  SELECT nota_liquidacao_item.exercicio
                       ...
                       LEFT JOIN (  SELECT exercicio
                                     ...
                                     WHERE TO_DATE(timestamp::VARCHAR,''dd/mm/yyyy'') <= TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'')
     ...
     LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
              ...
              INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                              ...
                              FROM (  SELECT nota_liquidacao_paga.exercicio
                                       ...
                                       WHERE TO_DATE(nota_liquidacao_paga.timestamp::VARCHAR,''dd/mm/yyyy'') <= TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'')
                         ...
                         LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                       ...
                                       WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada::VARCHAR,''dd/mm/yyyy'') < TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'')
             ...
             LEFT JOIN (  SELECT nota_liquidacao_item.exercicio
                            ...
                            FROM empenho.nota_liquidacao_item
                                 LEFT JOIN (  SELECT exercicio
                                                   ...
                                                   , SUM(COALESCE(vl_anulado,0.00)) AS vl_anulado
                                                FROM empenho.nota_liquidacao_item_anulado
                                               WHERE TO_DATE(timestamp::VARCHAR,''dd/mm/yyyy'') BETWEEN TO_DATE('|| quote_literal('01/01/'|| stExercicioAnterior) ||',''dd/mm/yyyy'') AND TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'')
     ...
     LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
              ...
              INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                              ...
                              FROM (  SELECT nota_liquidacao_paga.exercicio
                                           ...
                                           , SUM(nota_liquidacao_paga.vl_pago) AS vl_total
                                        FROM empenho.nota_liquidacao_paga
                                       WHERE TO_DATE(nota_liquidacao_paga.timestamp::VARCHAR,''dd/mm/yyyy'') BETWEEN TO_DATE('|| quote_literal('01/01/'|| stExercicioAnterior) ||',''dd/mm/yyyy'') AND TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'')
                                         AND nota_liquidacao_paga.cod_entidade IN ('|| stCodEntidade ||')
                                    GROUP BY nota_liquidacao_paga.exercicio
                                           , nota_liquidacao_paga.cod_entidade
                                           , nota_liquidacao_paga.cod_nota
                         ...
                         LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                           ...
                                           , SUM(COALESCE(nota_liquidacao_paga_anulada.vl_anulado,0.00)) AS vl_anulado
                                        FROM empenho.nota_liquidacao_paga_anulada
                                       WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada::VARCHAR,''dd/mm/yyyy'') BETWEEN TO_DATE('|| quote_literal('01/01/'|| stExercicioAnterior) ||',''dd/mm/yyyy'') AND TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'')
                                         AND nota_liquidacao_paga_anulada.cod_entidade IN ('|| stCodEntidade ||')
                                    GROUP BY nota_liquidacao_paga_anulada.exercicio
                                           , nota_liquidacao_paga_anulada.cod_entidade
                                           , nota_liquidacao_paga_anulada.cod_nota
        ...
        FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
            ...
            INNER JOIN empenho.item_pre_empenho
             ...
             LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                           ...
                           WHERE TO_DATE(empenho_anulado_item.timestamp::VARCHAR,''dd/mm/yyyy'') <= TO_DATE('|| quote_literal('31/12/'|| stExercicio) ||',''dd/mm/yyyy'') 
        ...
        FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
            ...
            INNER JOIN empenho.item_pre_empenho
             ...
             LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                               ...
                               , SUM(empenho_anulado_item.vl_anulado) AS vl_anulado
                            FROM empenho.empenho_anulado_item
                           WHERE TO_DATE(empenho_anulado_item.timestamp::VARCHAR,''dd/mm/yyyy'') BETWEEN TO_DATE('|| quote_literal('01/01/'|| stExercicioAnterior) ||',''dd/mm/yyyy'') AND TO_DATE('|| quote_literal('31/12/'||  stExercicio) ||',''dd/mm/yyyy'')
                        GROUP BY empenho_anulado_item.exercicio
                               , empenho_anulado_item.cod_pre_empenho
                               , empenho_anulado_item.num_item
                       ) AS empenho_anulado_item
...
--consulta para retornar todas os orgaos para nao intra-orcamentarias
  ...
  stSql := '
      ...
      CREATE TEMPORARY TABLE tmp_rec_liq_e_nao_liq_do_exer_' || inIdentificador || ' AS
      SELECT *
        FROM stn.fn_rel_rgf6_emp_liq_exercicio_recurso( '|| quote_literal(stCodEntidade) ||', '|| quote_literal(stExercicio) ||', '|| quote_literal(dtInicial) ||', '|| quote_literal(dtFinal) ||', '|| quote_literal(dtFinal) ||' ) as retorno
             ( cod_recurso integer, nom_recurso varchar, liquidados_nao_pagos numeric, empenhados_nao_liquidados numeric)
...
--                           , empenhados_nao_liquidados
--                        FROM tmp_liquidados_e_nao_liquidados_do_exercicio ) as tlnl
--                  ON( tlnl.cod_recurso = tmp_recursos_orgao.cod_recurso )
