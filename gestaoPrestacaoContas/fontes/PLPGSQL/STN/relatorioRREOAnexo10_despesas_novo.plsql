...
/*
	...
	* Exemplo de chamada da Função:
	...
	SELECT 
		...
		pct 
	FROM 
		stn.fn_rreo_anexo10_despesas_novo('2008', '1,2,3,4', 1, 6) AS (
		grupo INTEGER , 
		subgrupo INTEGER , 
...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo10_despesas_novo(stExercicio VARCHAR, stEntidades VARCHAR, inOpcao INTEGER, stDtIni VARCHAR, stDtFim VARCHAR) RETURNS SETOF RECORD AS $$
...
BEGIN 
    ...
    */
    ...
    stSQL := 'SELECT CAST(grupo AS INTEGER) AS grupo, 
                     ...
                     CAST(liq_bim AS NUMERIC(14,2)) AS liq_bim, 
                     CAST(liq_tot AS NUMERIC(14,2)) AS liq_tot, 
                     CAST(pct AS NUMERIC(14,2)) AS pct
                FROM tmp_rreo_retorno ORDER BY grupo, nivel, descricao';
