/*
    ...
    **********************************************************************************
*/
CREATE OR REPLACE FUNCTION stn.sub_consulta_rcl_dedutora (varchar, varchar, integer) RETURNS SETOF RECORD AS $$
DECLARE
    dtData          ALIAS FOR $1;
    ...
    while i >= 1 loop
            ...
            stSql = stSql || '
                     ,coalesce(  cast ( orcamento.fn_somatorio_balancete_receita( 
                                                 publico.fn_mascarareduzida(  substr( plano_conta.cod_estrutural, 3,15))
                                                ,'||quote_literal(stDataIni) ||',
                                                 '||quote_literal(stDataFim) ||') * -1  as numeric(14,2) ), 0 )  as mes_'||i ;
            i := i - 1;
    end loop;
...
--                and plano_conta.exercicio = '|| quote_literal(inExercicio) ||'
                                                        ';
