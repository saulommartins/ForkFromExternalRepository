...
*/
/**
    * PL que busca os valores constantes do Demonstrativo III do AMF
...
CREATE OR REPLACE FUNCTION stn.fn_amf_demonstrativo3_constantes(VARCHAR, VARCHAR, INTEGER, INTEGER) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    -------------------------------------------------------
    stSql := '
    CREATE TEMPORARY TABLE tmp_demonstrativo3_constantes_'||inIdentificador||' (
          ordem             INTEGER
    ...
    SELECT indice
     ...
     WHERE cod_tipo_indicador = inCodInflacao
       AND exercicio = stExercicioAnterior1;
    ...
    FOR reRegistroRealizado IN EXECUTE stSql LOOP
        ...
        END IF;
        vlExercicioAnterior1[inCount] := reRegistroRealizado.vl_realizado / vlDeflacao[inCount];
    ...
    SELECT indice
      ...
      FROM ldo.indicadores
     WHERE cod_tipo_indicador = inCodInflacao
       AND exercicio = stExercicioAnterior2;
    ...
    FOR reRegistroRealizado IN EXECUTE stSql LOOP
        ...
        ELSE
            vlDeflacao[inCount] := vlDeflacao[inCount-1] * (1 + (vlTMP / 100));
        END IF;
        vlExercicioAnterior2[inCount] := reRegistroRealizado.vl_realizado / vlDeflacao[inCount];
    ...
    SELECT indice
      ...
      FROM ldo.indicadores
     WHERE cod_tipo_indicador = inCodInflacao
       AND exercicio = stExercicioAnterior3;
    ...
    FOR reRegistroRealizado IN EXECUTE stSql LOOP
        ...
        ELSE
            vlDeflacao[inCount] := vlDeflacao[inCount-1] * (1 + (vlTMP / 100));
        END IF;
        vlExercicioAnterior3[inCount] := reRegistroRealizado.vl_realizado / vlDeflacao[inCount];
    ...
    stSql := '
        SELECT * 
          FROM stn.fn_amf_demonstrativo1('''||stExercicio||''', '||inCodPPA||', 0, '||inCodInflacao||') AS (
                  ordem             INTEGER
                , especificacao     VARCHAR
                , vl_corrente_1     DECIMAL(14,2)
                , vl_corrente_2     DECIMAL(14,2)
    ...
    FOR reRegistroPrevisao IN EXECUTE stSql LOOP
        vlExercicioPrevisto1[inCount] := reRegistroPrevisao.vl_constante_1;
        vlExercicioPrevisto2[inCount] := reRegistroPrevisao.vl_constante_2;
        vlExercicioPrevisto3[inCount] := reRegistroPrevisao.vl_constante_3;
        inCount := inCount + 1;
    END LOOP;
    ...
    FOR inCount IN 1..8 LOOP
        stSql := '
        INSERT INTO tmp_demonstrativo3_constantes_'||inIdentificador||' ( ordem
                                                                       , especificacao
                                                                       , vl_corrente_1
                                                                       , vl_corrente_2
                                                                       , vl_corrente_3
    ...
    stSql := '
        SELECT * 
          FROM tmp_demonstrativo3_constantes_'||inIdentificador||'
      ORDER BY ordem
    ';
    ...
    EXECUTE 'DROP TABLE tmp_demonstrativo3_constantes_'||inIdentificador;
