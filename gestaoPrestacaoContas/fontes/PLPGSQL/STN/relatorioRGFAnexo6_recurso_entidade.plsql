...
*/
/*
 * PL do Relatório RGF Anexo 6 com entidades(utilizado no restos a pagar com insuficiencia financeira)
...
CREATE OR REPLACE FUNCTION stn.fn_rgf_anexo6_recurso_entidade(varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
  ...
  stSql := '
    CREATE TEMPORARY TABLE tmp_recursos_processados_exercicios_anteriores AS
      ...
      SELECT busca_recurso.cod_recurso
	   ...
	   , liquidado.exercicio		
            ,CAST(CPA.cod_plano as varchar)          AS cod_plano_debito
            ...
            INNER JOIN empenho.pre_empenho
             ...
             LEFT JOIN (  SELECT nota_liquidacao_item.exercicio
                       ...
                       LEFT JOIN (  SELECT exercicio
                                     ...
                                     WHERE TO_DATE(timestamp,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'')
     ...
     LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
              ...
              INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                              ...
                              FROM (  SELECT nota_liquidacao_paga.exercicio
                                       ...
                                       WHERE TO_DATE(nota_liquidacao_paga.timestamp,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'')
                         ...
                         LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                       ...
                                       WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada,''yyyy-mm-dd'') < TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'')
             ...
             LEFT JOIN (  SELECT nota_liquidacao_item.exercicio
                            ...
                            FROM empenho.nota_liquidacao_item
                                 LEFT JOIN (  SELECT exercicio
                                                   ...
                                                   , SUM(COALESCE(vl_anulado,0.00)) AS vl_anulado
                                                FROM empenho.nota_liquidacao_item_anulado
                                               WHERE TO_DATE(timestamp,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'')
     ...
     LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
              ...
              INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                              ...
                              FROM (  SELECT nota_liquidacao_paga.exercicio
                                           ...
                                           , SUM(nota_liquidacao_paga.vl_pago) AS vl_total
                                        FROM empenho.nota_liquidacao_paga
                                       WHERE TO_DATE(nota_liquidacao_paga.timestamp,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'')
                                         AND nota_liquidacao_paga.cod_entidade IN ('||stCodEntidade||')
                                    GROUP BY nota_liquidacao_paga.exercicio
                                           , nota_liquidacao_paga.cod_entidade
                                           , nota_liquidacao_paga.cod_nota
                         ...
                         LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                           ...
                                           , SUM(COALESCE(nota_liquidacao_paga_anulada.vl_anulado,0.00)) AS vl_anulado
                                        FROM empenho.nota_liquidacao_paga_anulada
                                       WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'')
                                         AND nota_liquidacao_paga_anulada.cod_entidade IN ('||stCodEntidade||')
                                    GROUP BY nota_liquidacao_paga_anulada.exercicio
                                           , nota_liquidacao_paga_anulada.cod_entidade
                                           , nota_liquidacao_paga_anulada.cod_nota
        ...
        FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
            ...
            INNER JOIN empenho.item_pre_empenho
             ...
             LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                           ...
                           WHERE TO_DATE(empenho_anulado_item.timestamp,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicio||''',''dd/mm/yyyy'') 
        ...
        FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
            ...
            INNER JOIN empenho.item_pre_empenho
             ...
             LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                               ...
                               , SUM(empenho_anulado_item.vl_anulado) AS vl_anulado
                            FROM empenho.empenho_anulado_item
                           WHERE TO_DATE(empenho_anulado_item.timestamp,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'|| stExercicio||''',''dd/mm/yyyy'')
                        GROUP BY empenho_anulado_item.exercicio
                               , empenho_anulado_item.cod_pre_empenho
                               , empenho_anulado_item.num_item
                       ) AS empenho_anulado_item
...
--consulta para retornar todas os orgaos para nao intra-orcamentarias
  stSql := '
    ...
    GROUP BY cod_recurso, nom_recurso, cod_entidade, exercicio, cod_plano_debito
  ';
...
SELECT 
	...
	tbl.cod_entidade,
	tbl.exercicio,
	tbl.cod_plano_debito,
	tbl.cod_plano_credito,
	tbl.tmp_recursos_processados_exercicios_anteriores,
	tbl.tmp_recursos_processados_exercicio_anterior,
	tbl.tmp_recursos_nao_processados_exercicios_anteriores,
	tbl.tmp_recursos_nao_processados_exercicio_anterior,
	tbl.liquidados_nao_pagos,
	tbl.empenhados_nao_liquidados,
	PC_credito.cod_estrutural as estrutural_credito,
	PC_debito.cod_estrutural as estrutural_debito	
  FROM(
    ...
    SELECT
	 ...
	 , tlnl.cod_entidade as cod_entidade
	 , tlnl.exercicio as exercicio
         ,CAST(tlnl.cod_plano_debito as varchar)          AS cod_plano_debito
         , cast((SELECT contabilidade.fn_busca_conta_lancamento_rp(tlnl.exercicio,tlnl.cod_entidade,0)) as varchar) AS cod_plano_credito
  ...
  GROUP BY tlnl.cod_recurso
	 ...
	 , tlnl.cod_entidade
	 , tlnl.exercicio
	 , tlnl.cod_plano_debito
	 , cod_plano_credito

    UNION ALL

    SELECT tmp_recursos_orgao.cod_recurso::integer AS cod_recurso
         , tmp_recursos_orgao.nom_recurso AS tipo
	 , tmp_recursos_orgao.cod_entidade::integer
	 , tmp_recursos_orgao.exercicio as exercicio
         , CAST(tmp_recursos_orgao.cod_plano_debito as varchar)          AS cod_plano_debito
         , cast((SELECT contabilidade.fn_busca_conta_lancamento_rp(tmp_recursos_orgao.exercicio,tmp_recursos_orgao.cod_entidade,0)) as varchar) AS cod_plano_credito
      ...
      FROM tmp_recursos_orgao
           ...
           LEFT JOIN( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
			   ...
			   , cod_entidade
                        FROM tmp_recursos_processados_exercicios_anteriores
                    GROUP BY cod_recurso, cod_entidade ) as trpeas
           ...
           LEFT JOIN( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
	 		   ...
	 		   , cod_entidade
                        FROM tmp_recursos_processados_exercicio_anterior
                    GROUP BY cod_recurso, cod_entidade )  as trpea
           ...
           LEFT JOIN( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
			   ...
			   , cod_entidade
                        FROM tmp_recursos_nao_processados_exercicios_anteriores
                    GROUP BY cod_recurso, cod_entidade )  as trnpeas
           ...
           LEFT JOIN( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
			   ...
			   , cod_entidade
                        FROM tmp_recursos_nao_processados_exercicio_anterior
                    GROUP BY cod_recurso, cod_entidade )  as trnpea
                  ON(  trnpea.cod_recurso = tmp_recursos_orgao.cod_recurso
 		   AND trnpea.cod_entidade = tmp_recursos_orgao.cod_entidade )


  GROUP BY tmp_recursos_orgao.cod_recurso, tmp_recursos_orgao.nom_recurso, tmp_recursos_orgao.cod_entidade, tmp_recursos_orgao.exercicio, tmp_recursos_orgao.cod_plano_debito
) as tbl

    LEFT JOIN contabilidade.plano_analitica as PA_credito ON(
        tbl.cod_plano_credito = PA_credito.cod_plano AND
        tbl.exercicio = PA_credito.exercicio
    )
    LEFT JOIN contabilidade.plano_conta as PC_credito ON(
        PA_credito.cod_conta    = PC_credito.cod_conta AND
        PA_credito.exercicio    = PC_credito.exercicio
    )
    LEFT JOIN contabilidade.plano_analitica as PA_debito ON(
        tbl.cod_plano_debito  = PA_debito.cod_plano AND
        tbl.exercicio = PA_debito.exercicio
    )
    LEFT JOIN contabilidade.plano_conta as PC_debito ON(
        PA_debito.cod_conta    = PC_debito.cod_conta AND
        PA_debito.exercicio    = PC_debito.exercicio
    )
