/*
    ...
    **********************************************************************************
*/
CREATE OR REPLACE FUNCTION stn.fn_rreo_despesa_liquidada_anexo2(inCodDespesa INTEGER, stExercicio VARCHAR, stEntidades VARCHAR, stDtIni VARCHAR, stDtFim VARCHAR, boIntra BOOLEAN) RETURNS NUMERIC(14,2) AS 
$$
...
BEGIN 
	...
	FROM 
		...
		WHERE 
          ...
          AND ped.cod_despesa = ' || inCodDespesa || '  
	...
	WHERE 
	    ...
	    SUBSTRING(pedcd.cod_estrutural, 5, 3) <> ''9.1''                         	  ';
