...
CREATE OR REPLACE FUNCTION stn.fn_anexo2_intraorcamentaria(varchar,varchar,varchar,varchar) RETURNS SETOF anexo2_intra_orcamentaria AS $$
DECLARE
...
BEGIN
    ...
    stSql := '
    CREATE TEMPORARY TABLE tmp_intra AS
    SELECT
           d.cod_funcao
         , d.cod_subfuncao
      ...
      FROM
           ...
           -- EMPENHADO BIMESTRE
           LEFT JOIN( SELECT
                             sum(coalesce(item_empenho_empenhado.vl_empenhado, 0.00)) AS vl_total
                           , ped.cod_despesa
                        FROM
                   ...
                   INNER JOIN ( 
                          ...
                          INNER JOIN empenho.item_pre_empenho as eipe
                               ...
                               WHERE empenho.cod_entidade IN ('||stCodEntidades||')
                                 AND empenho.dt_empenho BETWEEN to_date( '''||dtInicial||''', ''dd/mm/yyyy'')
  ...
  GROUP BY cod_funcao
    ...
    stSql :='
    INSERT INTO tmp_intra
    SELECT 0                                  AS cod_funcao
         , 0                                  AS cod_subfuncao
         , ''DESPESAS (INTRA-ORÇAMENTÁRIAS)'' AS nom_funcao
         , ''DESPESAS (INTRA-ORÇAMENTÁRIAS)'' AS nom_subfuncao
         , sum(vl_original)                   AS vl_original
         , sum(vl_suplementacoes)             AS vl_suplementacoes
         , sum(vl_empenhado_bimestre)         AS vl_empenhado_bimestre
         , sum(vl_empenhado_ate_bimestre)     AS vl_empenhado_ate_bimestre
