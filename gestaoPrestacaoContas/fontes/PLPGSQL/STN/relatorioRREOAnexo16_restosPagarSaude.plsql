...
CREATE OR REPLACE FUNCTION stn.fn_rreo_anexo16_restos_pagar_saude( varchar, integer,varchar ) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
              ...
              INNER JOIN empenho.pre_empenho
               ...
               LEFT JOIN (  SELECT nota_liquidacao_item.exercicio
                         ...
                         LEFT JOIN (  SELECT exercicio
                                       ...
                                       WHERE TO_DATE(timestamp::VARCHAR,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'') 
                                    GROUP BY exercicio
                                           , cod_nota
                                           , num_item
                                           , exercicio_item
       ...
       LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
                ...
                INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                                ...
                                FROM (  SELECT nota_liquidacao_paga.exercicio
                                         ...
                                         WHERE TO_DATE(nota_liquidacao_paga.timestamp::VARCHAR,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'')
                           ...
                           LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                         ...
                                         WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada::VARCHAR,''yyyy-mm-dd'') < TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'')
              ...
              INNER JOIN empenho.pre_empenho
               ...
               LEFT JOIN (  SELECT nota_liquidacao_item.exercicio
                         ...
                         LEFT JOIN (  SELECT exercicio
                                           ...
                                           , SUM(COALESCE(vl_anulado,0.00)) AS vl_anulado
                                        FROM empenho.nota_liquidacao_item_anulado
                                       WHERE TO_DATE(timestamp::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'')
                                    GROUP BY exercicio
                                           , cod_nota
                                           , num_item
                                           , exercicio_item
       ...
       LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
                ...
                INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                                ...
                                FROM (  SELECT nota_liquidacao_paga.exercicio
                                             ...
                                             , SUM(nota_liquidacao_paga.vl_pago) AS vl_total
                                          FROM empenho.nota_liquidacao_paga
                                         WHERE TO_DATE(nota_liquidacao_paga.timestamp::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'')
                                      GROUP BY nota_liquidacao_paga.exercicio
                                             , nota_liquidacao_paga.cod_entidade
                                             , nota_liquidacao_paga.cod_nota
                                     ) AS nota_liquidacao_paga
                           ...
                           LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                             ...
                                             , SUM(COALESCE(nota_liquidacao_paga_anulada.vl_anulado,0.00)) AS vl_anulado
                                          FROM empenho.nota_liquidacao_paga_anulada
                                         WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'')
                                      GROUP BY nota_liquidacao_paga_anulada.exercicio
                                             , nota_liquidacao_paga_anulada.cod_entidade
                                             , nota_liquidacao_paga_anulada.cod_nota
                                     ) AS nota_liquidacao_paga_anulada
  ...
  --left para achar o cod_estrutural
    ...
    INNER JOIN (  SELECT empenho_anulado_item.exercicio
                   ...
                   WHERE TO_DATE(empenho_anulado_item.timestamp::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicio||''',''dd/mm/yyyy'') AND TO_DATE('''||dtFinal||''',''dd/mm/yyyy'')
  ...
  --left para achar o cod_estrutural
    ...
    INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                    ...
                    FROM (  SELECT nota_liquidacao_paga.exercicio
                                 ...
                                 , SUM(nota_liquidacao_paga.vl_pago) AS vl_total
                              FROM empenho.nota_liquidacao_paga
                             WHERE TO_DATE(nota_liquidacao_paga.timestamp::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicio||''',''dd/mm/yyyy'') AND TO_DATE('''||dtFinal||''',''dd/mm/yyyy'')
                          GROUP BY nota_liquidacao_paga.exercicio
                                 , nota_liquidacao_paga.cod_entidade
                                 , nota_liquidacao_paga.cod_nota
                         ) AS nota_liquidacao_paga
               ...
               LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                 ...
                                 , SUM(COALESCE(nota_liquidacao_paga_anulada.vl_anulado,0.00)) AS vl_anulado
                              FROM empenho.nota_liquidacao_paga_anulada
                             WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicio||''',''dd/mm/yyyy'') AND TO_DATE('''||dtFinal||''',''dd/mm/yyyy'')
                          GROUP BY nota_liquidacao_paga_anulada.exercicio
                                 , nota_liquidacao_paga_anulada.cod_entidade
                                 , nota_liquidacao_paga_anulada.cod_nota
                         ) AS nota_liquidacao_paga_anulada
          ...
          FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
              ...
              INNER JOIN empenho.item_pre_empenho
               ...
               LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                              ...
                              FROM empenho.empenho_anulado_item
                             WHERE TO_DATE(empenho_anulado_item.timestamp::VARCHAR,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'') 
                          GROUP BY empenho_anulado_item.exercicio
                                 , empenho_anulado_item.cod_pre_empenho
                                 , empenho_anulado_item.num_item
                         ) AS empenho_anulado_item
          ...
          FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
              ...
              INNER JOIN empenho.item_pre_empenho
               ...
               LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                                 ...
                                 , SUM(empenho_anulado_item.vl_anulado) AS vl_anulado
                              FROM empenho.empenho_anulado_item
                             WHERE TO_DATE(empenho_anulado_item.timestamp::VARCHAR,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE(''31/12/'|| stExercicioAnterior||''',''dd/mm/yyyy'')
                          GROUP BY empenho_anulado_item.exercicio
                                 , empenho_anulado_item.cod_pre_empenho
                                 , empenho_anulado_item.num_item
                         ) AS empenho_anulado_item
