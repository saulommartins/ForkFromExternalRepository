/*
    ...
    **********************************************************************************
*/
CREATE OR REPLACE FUNCTION stn.fn_rreo_despesa_empenhada_anexo_12(stMascRed VARCHAR, stExercicio VARCHAR, stEntidades VARCHAR, stDtIni VARCHAR, stDtFim VARCHAR, boIntra BOOLEAN) RETURNS NUMERIC(14,2) AS 
$$
...
BEGIN 
	...
	WHERE e.exercicio = ''' || stExercicio || ''' 
            ...
            AND to_date( to_char( ea.timestamp, ''dd/mm/yyyy''), ''dd/mm/yyyy'' ) BETWEEN to_date(''' || stDtIni || ''', ''dd/mm/yyyy'') AND to_date(''' || stDtFim || ''', ''dd/mm/yyyy'') 
