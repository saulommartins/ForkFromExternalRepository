 ...
 * 
 * $Id: FTCMGORestosPagar.plsql 61567 2015-02-06 11:47:54Z michel $
 */
...
CREATE OR REPLACE FUNCTION tcmgo.fn_restos_pagar(varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
    ...
    CREATE TEMPORARY TABLE tmp_processados_exercicio_anterior AS
      ...
      SELECT liquidado.cod_empenho
           ...
           , CASE WHEN (orgao_implantado.num_orgao IS NULL) AND (orgao.num_orgao IS NULL)
                  ...
                  THEN orgao.num_orgao
             END AS num_orgao  
           , ( SUM(COALESCE(liquidado.vl_liquidado,0.00))) AS vl_total
        FROM (  SELECT pre_empenho.exercicio
                     , pre_empenho.cod_pre_empenho
                     , empenho.cod_entidade
                     , empenho.cod_empenho
...
--left para achar o cod_estrutural
     ...
     LEFT JOIN orcamento.orgao AS orgao_implantado
      ...
      GROUP BY liquidado.cod_empenho
             ...
             , orgao_implantado.num_orgao
             , despesa.dt_criacao
        HAVING ( SUM(COALESCE(liquidado.vl_liquidado,0.00))) > 0
  ';
  ...
  INNER JOIN (  SELECT empenho_anulado_item.exercicio
                 ...
                 WHERE TO_DATE(empenho_anulado_item.timestamp::TEXT,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'') -- AND TO_DATE('''||dtFinal||''',''dd/mm/yyyy'')
  ...
  INNER JOIN (  SELECT nota_liquidacao.cod_empenho
                  ...
                  FROM empenho.nota_liquidacao
                 WHERE nota_liquidacao.dt_liquidacao <= TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'') -- AND TO_DATE('''||dtFinal||''',''dd/mm/yyyy'')
...
--left para achar o cod_estrutural
   ...
   LEFT JOIN orcamento.orgao AS orgao_implantado
       ...
       WHERE empenho.exercicio <= '''||stExercicio||'''
         AND empenho.dt_empenho <= TO_DATE(''31/12/'||stExercicioAnterior||''',''dd/mm/yyyy'')
         AND nota_liquidacao.dt_liquidacao BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||''',''dd/mm/yyyy'') AND TO_DATE('''||dtFinal||''',''dd/mm/yyyy'')
         AND empenho.cod_entidade IN ('||stCodEntidade||')
  ...
  stSql := '
            SELECT cod_empenho
                  , cod_entidade
                  , exercicio
                  , SUM(valor_processado_exercicios_anteriores) AS valor_processado_exercicios_anteriores
                  , SUM(valor_processado_exercicio_anterior) AS valor_processado_exercicio_anterior
                  , SUM(valor_processado_cancelado) AS valor_processado_cancelado
                  , SUM(valor_processado_pago) AS valor_processado_pago
                  , SUM(valor_nao_processado_exercicios_anteriores) AS valor_nao_processado_exercicios_anteriores
                  , SUM(valor_nao_processado_exercicio_anterior) AS valor_nao_processado_exercicio_anterior
                  , SUM(valor_nao_processado_cancelado) AS valor_nao_processado_cancelado
