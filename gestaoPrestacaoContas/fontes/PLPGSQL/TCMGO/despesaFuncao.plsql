...
CREATE OR REPLACE FUNCTION tcmgo.despesaFuncao (VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR ) RETURNS SETOF RECORD AS $$
DECLARE
      ...
      FROM orcamento.funcao                
         ...
         , empenho.nota_liquidacao         
           LEFT JOIN ( SELECT cod_entidade 
                        ...
                        WHERE COALESCE( TO_DATE( timestamp::text, ''yyyy-mm-dd'' ), TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' ) )
                                                                    BETWEEN TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' )  
                                                                        AND TO_DATE( ''' || stDataFinal || ''', ''dd/mm/yyyy'' )  
                        GROUP BY exercicio                                                                        
                               , cod_entidade                                                                     
           ...
           LEFT JOIN ( SELECT exercicio                                                                           
                        ...
                        WHERE coalesce( TO_DATE( timestamp_anulada::text, ''yyyy-mm-dd'' ), TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' ) )
                                                                            BETWEEN TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' )
                                                                                AND TO_DATE( ''' || stDataFinal || ''', ''dd/mm/yyyy'' )
                        GROUP BY exercicio                                                                                         
                               , cod_entidade                                                                                      
