...
--
   ...
   BEGIN
       FOR reRegistro IN SELECT  'DROP FUNCTION '         ||
                           ...
                           WHERE pg_proc.prorettype     <> 'pg_catalog.cstring'::pg_catalog.regtype
                             ...
                             AND pg_namespace.nspname   = 'tcmgo'
                             ORDER BY 1
       LOOP
   ...
   --
   CREATE OR REPLACE FUNCTION TCMGO.numero_anulacao_liquidacao( varExercicio         VARCHAR
                                                              , varExercicioEmpenho  VARCHAR
                                                              , intCodEntidade       INTEGER
                                                              , intCodNota           INTEGER
                                                              , varTimeStamp         TIMESTAMP ) RETURNS INTEGER AS $$
   DECLARE
      recRegistro       RECORD;
      intRetorno        INTEGER;
      intCodPreEmpenho  INTEGER;
   BEGIN
      ALTER SEQUENCE tcmgo.seqLiquidacaoAnulacao RESTART WITH 1;
      ...
      FOR recRegistro
       IN SELECT timestamp
           ...
           ORDER BY  timestamp
      LOOP
         intRetorno := nextval('tcmgo.seqLiquidacaoAnulacao');
