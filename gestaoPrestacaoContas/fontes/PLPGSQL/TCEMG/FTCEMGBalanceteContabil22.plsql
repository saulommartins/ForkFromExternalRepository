...
*
* $Id: FTCEMGBalanceteContabil22.plsql 62872 2015-07-01 20:16:55Z franver $
* $Revision: 62872 $
* $Author: franver $
...
*/
CREATE OR REPLACE FUNCTION tcemg.fn_balancete_contabil_22(VARCHAR, VARCHAR, VARCHAR, VARCHAR) RETURNS SETOF balancete_contabil_registro_22 AS $$ 
DECLARE
...
BEGIN
   ...
   CREATE TEMPORARY TABLE tmp_credito AS
    ...
    CREATE UNIQUE INDEX unq_debito  ON tmp_debito  (cod_estrutural varchar_pattern_ops, cod_ctb, oid_temp);
    CREATE UNIQUE INDEX unq_credito ON tmp_credito (cod_estrutural varchar_pattern_ops, cod_ctb, oid_temp);
    ...
    CREATE UNIQUE INDEX unq_totaliza_credito ON tmp_totaliza_credito (cod_estrutural varchar_pattern_ops, cod_ctb, oid_temp);
    CREATE UNIQUE INDEX unq_totaliza_debito  ON tmp_totaliza_debito  (cod_estrutural varchar_pattern_ops, cod_ctb, oid_temp);
   ...
   CREATE TEMPORARY TABLE tmp_totaliza AS
    ...
    CREATE UNIQUE INDEX unq_totaliza ON tmp_totaliza (cod_estrutural varchar_pattern_ops, cod_ctb, oid_temp);

    stSql := '
          SELECT 22::INTEGER AS tipo_registro
               , plano_conta_pcasp.cod_estrutural_contabil
                , CASE plano_conta.indicador_superavit
                       WHEN ''permanente'' THEN ''P''
                       WHEN ''financeiro'' THEN ''F''
                       ELSE plano_conta.indicador_superavit
    ...
    LOOP
        arRetorno := tcemg.fn_balancete_contabil_totaliza_codigoCTB( publico.fn_mascarareduzida(reRegistro.cod_estrutural_contabil) , reRegistro.cod_ctb);
