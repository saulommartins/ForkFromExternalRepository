...
*
* $Id: FTCEMGBalanceteContabil17.plsql 62872 2015-07-01 20:16:55Z franver $
* $Revision: 62872 $
* $Author: franver $
...
*/
CREATE OR REPLACE FUNCTION tcemg.fn_balancete_contabil_17(VARCHAR, VARCHAR, VARCHAR, VARCHAR) RETURNS SETOF balancete_contabil_registro_17 AS $$ 
DECLARE
...
BEGIN
             ...
             SELECT *
               FROM (SELECT pc.cod_estrutural
                  ...
                  LEFT JOIN tcemg.conta_bancaria
                         ON conta_bancaria.exercicio = pc.exercicio
                        AND conta_bancaria.cod_conta = pc.cod_conta
   ...
   CREATE TEMPORARY TABLE tmp_credito AS
    ...
    CREATE UNIQUE INDEX unq_debito  ON tmp_debito  (cod_estrutural varchar_pattern_ops, cod_ctb, cod_recurso, oid_temp);
    CREATE UNIQUE INDEX unq_credito ON tmp_credito (cod_estrutural varchar_pattern_ops, cod_ctb, cod_recurso, oid_temp);
    ...
    CREATE UNIQUE INDEX unq_totaliza_credito ON tmp_totaliza_credito (cod_estrutural varchar_pattern_ops, cod_ctb, cod_recurso, oid_temp);
    CREATE UNIQUE INDEX unq_totaliza_debito  ON tmp_totaliza_debito  (cod_estrutural varchar_pattern_ops, cod_ctb, cod_recurso, oid_temp);
   ...
   CREATE TEMPORARY TABLE tmp_totaliza AS
    ...
    CREATE UNIQUE INDEX unq_totaliza ON tmp_totaliza (cod_estrutural varchar_pattern_ops, cod_ctb, cod_recurso, oid_temp);
    ...
    stSql := '
          SELECT 17::INTEGER AS tipo_registro
               ...
               , CASE WHEN cod_ctb_anterior IS NULL
                      ...
                      ELSE cod_ctb_anterior
                  END AS cod_ctb
               , plano_recurso.cod_recurso AS cod_recurso_ctb
       ...
       LEFT JOIN tcemg.conta_bancaria
              ON conta_bancaria.exercicio = plano_conta.exercicio
             AND conta_bancaria.cod_conta = plano_conta.cod_conta
       LEFT JOIN (SELECT publico.fn_mascarareduzida(plano_conta.cod_estrutural)||''%'' AS cod_estrutural_reduzido
                   ...
                   WHERE plano_conta.escrituracao_pcasp = ''S''
                     AND plano_conta.atributo_tcemg = 17
    ...
    LOOP
        arRetorno := tcemg.fn_balancete_contabil_totaliza_codigoCTB_recurso( publico.fn_mascarareduzida(reRegistro.cod_estrutural_contabil) , reRegistro.cod_ctb, reRegistro.cod_recurso_ctb);
