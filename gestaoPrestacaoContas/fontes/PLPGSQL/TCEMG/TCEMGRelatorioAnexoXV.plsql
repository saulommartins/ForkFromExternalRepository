...
*/
/*
    * PL do TCEMG AnexoXIV - Arquivo TCEMG da GPC 
    ...
    *
*/
CREATE OR REPLACE  FUNCTION tcemg.relatorio_anexoXV(character varying, character varying, character varying, character varying, character varying, boolean, character varying, character varying, character varying, character varying, character varying, character varying, varchar, varchar, varchar ) RETURNS SETOF record
AS $$
DECLARE
    ...
    stEntidades                   ALIAS FOR $7; 
    stCodEstruturalInicial        ALIAS FOR $8;
    stCodEstruturalFinal          ALIAS FOR $9;
    stCodReduzidoInicial          ALIAS FOR $10;
    stCodReduzidoFinal            ALIAS FOR $11;
    stControleDetalhado           ALIAS FOR $12;
    inNumOrgao                    ALIAS FOR $13;
    inNumUnidade                  ALIAS FOR $14;
    stVerificaCreateDropTables    ALIAS FOR $15;
...
BEGIN
   ...
   --CRIA TABELA TEMPORÁRIA COM TODOa AS DESPESAS DA DESPESA, SETA ELAS COMO MÃE
    CREATE TEMPORARY TABLE tmp_pre_empenho_despesa AS
     ...
     --ESTE SELECT PREVEM DA TABELA PRE_EMPENHO_DESPESA ONDE TODOS OS REGISTROS SÃO SETADOS COMO FILHAS
        ...
        SELECT valor INTO stNomePrefeitura
          FROM administracao.configuracao
         WHERE exercicio = '' || stExercicio || ''
...
stSql := 'CREATE TEMPORARY TABLE tmp_retorno AS (
    ...
    IF ( stTipoSituacao = 'empenhado' ) THEN
        stSql := stSql || ' coalesce(tcemg.fn_despesa_empenhado_mes_anexo_ii(''' || stExercicio || ''',''' || stDataInicial || ''', ''' || stDataFinal || ''', tr.cod_conta, od.num_orgao, od.num_unidade, od.cod_funcao, od.cod_subfuncao, acao.num_acao, programa.num_programa, od.cod_entidade, od.cod_recurso, od.cod_despesa ),0.00) as vl_tipo_situacao_per, ';
    END IF;    

    IF ( stTipoSituacao = 'pago' ) THEN                
        stSql := stSql || ' coalesce(tcemg.fn_despesa_paga_mes_anexo_ii(''' || stExercicio || ''',''' || stDataInicial || ''', ''' || stDataFinal || ''', tr.cod_conta, od.num_orgao, od.num_unidade, od.cod_funcao, od.cod_subfuncao, acao.num_acao, programa.num_programa, od.cod_entidade, od.cod_recurso, od.cod_despesa ),0.00) as vl_tipo_situacao_per, ';
    END IF;
    
    IF ( stTipoSituacao = 'liquidado' ) THEN
        stSql := stSql || ' coalesce(tcemg.fn_despesa_liquidado_mes_anexo_ii(''' || stExercicio || ''',''' || stDataInicial || ''', ''' || stDataFinal || ''', tr.cod_conta, od.num_orgao, od.num_unidade, od.cod_funcao, od.cod_subfuncao, acao.num_acao, programa.num_programa, od.cod_entidade, od.cod_recurso, od.cod_despesa ),0.00) as vl_tipo_situacao_per, ';
    END IF;
...
stSql := '
    INSERT INTO tmp_retorno
         SELECT 
                ...
                nom_orgao,
                nom_unidade,
                ''SAÚDE E SANEAMENTO'' AS nom_funcao,
                ''SAÚDE E SANEAMENTO'' AS nom_subfuncao,
                ''SAÚDE E SANEAMENTO'' AS nom_programa,
                nom_pao,
                0.00 AS vl_tipo_situacao_per,                
                0.00 AS vl_sub_total,
				...
				END AS nom_programa,
			     ...
			     AND programa.exercicio    = restos_pre_empenho.exercicio
			      ...
			      AND subfuncao.exercicio     = restos_pre_empenho.exercicio
                            ...
                            WHERE restos_pre_empenho.cod_funcao = 10
                              AND restos_pre_empenho.cod_subfuncao IN ( 122,272,301,302,303,304,305 ) 
                              AND restos_pre_empenho.recurso = 102
   ...
   IF (stTipoSituacao = 'pago' AND boRestosPagar = TRUE) THEN 
    ...
    stSql := '
       ...
       GROUP BY cod_funcao, cod_subfuncao, cod_programa, nom_programa, vl_sub_total
       ORDER BY cod_funcao, cod_subfuncao, cod_programa ';

    --Atualiza valor sub_total por cod_funcao e sub_funcao
    SELECT sum(tab.vl_sub_total) INTO nuValorSubTotal
      FROM ( SELECT sum(vl_tipo_situacao_per) AS vl_sub_total
               FROM tmp_retorno
              WHERE cod_funcao = 10
