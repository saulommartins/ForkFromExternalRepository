...
*
* $Id: FTCEMGBalanceteContabil15.plsql 62872 2015-07-01 20:16:55Z franver $
* $Revision: 62872 $
* $Author: franver $
...
*/
CREATE OR REPLACE FUNCTION tcemg.fn_balancete_contabil_15(VARCHAR, VARCHAR, VARCHAR, VARCHAR) RETURNS SETOF balancete_contabil_registro_15 AS $$ 
DECLARE
...
BEGIN
   ...
   CREATE TEMPORARY TABLE tmp_totaliza AS
    ...
    stSql := '
        ...
        LEFT JOIN (SELECT publico.fn_mascarareduzida(plano_conta.cod_estrutural)||''%'' AS cod_estrutural_reduzido
                    ...
                    WHERE plano_conta.escrituracao_pcasp = ''S''
                      AND plano_conta.atributo_tcemg = 15
    ...
    LOOP
        arRetorno := contabilidade.fn_totaliza_balancete_verificacao( publico.fn_mascarareduzida(reRegistro.cod_estrutural_contabil) , stDtInicial, stDtFinal);
