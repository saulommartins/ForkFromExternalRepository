...
--left para achar o cod_estrutural
  ...
  INNER JOIN (
              SELECT nota_liquidacao.exercicio_empenho
          ...
          INNER JOIN empenho.nota_liquidacao
               ...
               WHERE nota_liquidacao_item_anulado.exercicio < '''||stExercicio||'''
                 AND nota_liquidacao_item_anulado.cod_entidade IN ('||stCodEntidade||')
                 AND nota_liquidacao_item_anulado.timestamp::date BETWEEN TO_DATE(''2016'||'-'||inMes||'-'||'01'', ''yyyy-mm-dd'') AND TO_DATE('''||dtFinal||''', ''dd/mm/yyyy'')
          ...
          ON empenho_anulado_item.cod_empenho       = empenho.cod_empenho
         AND empenho_anulado_item.exercicio_empenho = empenho.exercicio
...
--left para achar o cod_estrutural
  ...
  INNER JOIN (
              SELECT empenho_anulado.exercicio
                   ...
                   , COALESCE(SUM(COALESCE(empenho_anulado_item.vl_anulado,0.00)),0.00) AS vl_anulado
          ...
          INNER JOIN empenho.empenho_anulado
               ...
               WHERE empenho_anulado.exercicio < '''||stExercicio||'''
                 AND empenho_anulado.cod_entidade IN ('||stCodEntidade||')
                 AND empenho_anulado.timestamp::date BETWEEN TO_DATE(''2016'||'-'||inMes||'-'||'01'', ''yyyy-mm-dd'') AND TO_DATE('''||dtFinal||''', ''dd/mm/yyyy'')
            GROUP BY empenho_anulado.exercicio
                   , empenho_anulado.cod_empenho
                   , empenho_anulado.cod_entidade
                   , empenho_anulado.timestamp
                   , empenho_anulado.motivo
            ORDER BY empenho_anulado.cod_empenho
             ) AS empenho_anulado_item
          ON empenho_anulado_item.cod_empenho = empenho.cod_empenho
