...
CREATE OR REPLACE FUNCTION tcems.fn_rgf_anexo1_despesas ( varchar,varchar,varchar,varchar ) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    stSql := stSql || '
        ...
        SELECT
            ...
            ';
    IF (dtFinal = '31/12/'||stExercicio) THEN
        stSql := stSql || 'quadrimestre3 as liquidado ,';
    ELSEIF (dtFinal = '31/08/'||stExercicio) THEN
	stSql := stSql || 'quadrimestre2 as liquidado ,';
    ELSE
	stSql := stSql || 'quadrimestre1 as liquidado ,';
    END IF;
    stSql := stSql || '
            0.00 as restos
        FROM
            tcems.despesas_nao_computadas
        WHERE
