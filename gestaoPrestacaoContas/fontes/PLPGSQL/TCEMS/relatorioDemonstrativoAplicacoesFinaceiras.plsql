...
CREATE OR REPLACE FUNCTION tcems.fn_relatorio_aplicacoes_financeiras(VARCHAR, VARCHAR, VARCHAR, VARCHAR, CHAR) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    stSql := ' CREATE TEMPORARY TABLE tmp_debito AS
                ...
                FROM (
                 ...
                 WHERE cod_entidade IN ('|| quote_literal(stFiltro) ||') AND substr(cod_estrutural,1,1)::integer in (1, 2, 3, 4, 5, 6, 9) AND cod_estrutural BETWEEN ''1.1.1.1.2.00.00.00.00.00'' AND ''1.1.1.1.4.99.99.99.99.99'' ';
    EXECUTE stSql;
    ...
    ELSE
        stSqlComplemento := 'dt_lote BETWEEN to_date( ''01/01/'' || substr(to_char(to_date('|| quote_literal(stDtInicial) ||',''dd/mm/yyyy'') - 1,''dd/mm/yyyy''),7),''dd/mm/yyyy'') AND to_date( '|| quote_literal(stDtInicial) || ',' || quote_literal('dd/mm/yyyy') || ')-1 ';
    END IF;
