...
<?php
/**
    * Classe de mapeamento da tabela ima.configuracao_bb_conta
...
class TIMAConfiguracaoBbConta extends Persistente
{
/**
    * Método Construtor
    * @access Private
*/
function TIMAConfiguracaoBbConta()
{
    parent::Persistente();
    $this->setTabela("ima".sessao::getentidade().".configuracao_bb_conta");

    $this->setCampoCod('');
    $this->setComplementoChave('cod_convenio,cod_banco,cod_agencia,cod_conta_corrente,timestamp');

    $this->AddCampo('cod_convenio'      ,'integer'		 ,true  ,''    ,true  ,'TIMAConfiguracaoConvenioBb');
    $this->AddCampo('cod_banco'         ,'integer'		 ,true  ,''    ,true  ,'TIMAConfiguracaoConvenioBb');
...
{
    $stSql  = "    SELECT configuracao_bb_conta.*                                                          \n";
    ...
    $stSql .= "      FROM ima.configuracao_bb_conta                                                        \n";
    $stSql .= "INNER JOIN monetario.conta_corrente                                                         \n";
    $stSql .= "        ON configuracao_bb_conta.cod_conta_corrente = conta_corrente.cod_conta_corrente     \n";
    $stSql .= "       AND configuracao_bb_conta.cod_agencia = conta_corrente.cod_agencia                   \n";
    $stSql .= "       AND configuracao_bb_conta.cod_banco   = conta_corrente.cod_banco                     \n";
    $stSql .= "INNER JOIN ( SELECT configuracao_bb_conta.cod_convenio									   \n";
    $stSql .= "		   			 , max(configuracao_bb_conta.timestamp) as timestamp                       \n";
    $stSql .= "		  	      FROM ima.configuracao_bb_conta                                               \n";
    $stSql .= "		  		 WHERE configuracao_bb_conta.vigencia = to_date('".$this->getDado("vigencia")."','dd/mm/yyyy') \n";
    $stSql .= "			  GROUP BY configuracao_bb_conta.cod_convenio                                      \n";
    $stSql .= "			  ) as max_configuracao_bb_conta                                                   \n";
    $stSql .= "		   ON configuracao_bb_conta.cod_convenio = max_configuracao_bb_conta.cod_convenio      \n";
    $stSql .= "		  AND configuracao_bb_conta.timestamp = max_configuracao_bb_conta.timestamp 		   \n";
