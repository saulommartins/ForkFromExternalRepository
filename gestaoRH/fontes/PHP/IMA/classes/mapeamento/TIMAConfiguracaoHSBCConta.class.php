...
class TIMAConfiguracaoHSBCConta extends Persistente
{
    /**
        * Método Construtor
        * @access Private
    */
    public function TIMAConfiguracaoHSBCConta()
    {
        parent::Persistente();
        $this->setTabela("ima.configuracao_hsbc_conta");

        $this->setCampoCod('');
        $this->setComplementoChave('cod_convenio,cod_banco,cod_agencia,cod_conta_corrente,timestamp');

        $this->AddCampo('cod_convenio'      ,'integer'       ,true  ,''    ,true  ,'TIMAConfiguracaoConvenioHSBC');
        $this->AddCampo('cod_banco'         ,'integer'       ,true  ,''    ,true  ,'TIMAConfiguracaoConvenioHSBC');
    ...
    {
        $stSql  = "    SELECT configuracao_hsbc_conta.*                                                                                \n";
        ...
        $stSql .= "         , configuracao_convenio_hsbc.cod_convenio_banco                                                            \n";
        $stSql .= "      FROM ima.configuracao_hsbc_conta                                                                              \n";
        $stSql .= "INNER JOIN ima.configuracao_convenio_hsbc                                                                           \n";
        $stSql .= "        ON configuracao_convenio_hsbc.cod_convenio = configuracao_hsbc_conta.cod_convenio                           \n";
        $stSql .= "       AND configuracao_convenio_hsbc.cod_banco    = configuracao_hsbc_conta.cod_banco                              \n";
        $stSql .= "INNER JOIN monetario.conta_corrente                                                                                 \n";
        $stSql .= "        ON configuracao_hsbc_conta.cod_conta_corrente = conta_corrente.cod_conta_corrente                           \n";
        $stSql .= "       AND configuracao_hsbc_conta.cod_agencia = conta_corrente.cod_agencia                                         \n";
        $stSql .= "       AND configuracao_hsbc_conta.cod_banco   = conta_corrente.cod_banco                                           \n";
        $stSql .= "INNER JOIN ( SELECT configuracao_hsbc_conta.cod_convenio                                                            \n";
        $stSql .= "                        , max(configuracao_hsbc_conta.timestamp) as timestamp                                       \n";
        $stSql .= "                    FROM ima.configuracao_hsbc_conta                                                                \n";
        $stSql .= "                   WHERE configuracao_hsbc_conta.vigencia = to_date('".$this->getDado("vigencia")."','dd/mm/yyyy')  \n";
        $stSql .= "              GROUP BY configuracao_hsbc_conta.cod_convenio                                                         \n";
        $stSql .= "              ) as max_configuracao_hsbc_conta                                                                      \n";
        $stSql .= "           ON configuracao_hsbc_conta.cod_convenio = max_configuracao_hsbc_conta.cod_convenio                       \n";
        $stSql .= "          AND configuracao_hsbc_conta.timestamp = max_configuracao_hsbc_conta.timestamp                             \n";
