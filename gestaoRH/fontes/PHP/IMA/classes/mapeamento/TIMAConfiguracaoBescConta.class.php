...
<?php
/**
    * Classe de mapeamento da tabela ima.configuracao_besc_conta
...
class TIMAConfiguracaoBescConta extends Persistente
{
    /**
        * Método Construtor
        * @access Private
    */
    public function TIMAConfiguracaoBescConta()
    {
        parent::Persistente();
        $this->setTabela("ima.configuracao_besc_conta");

        $this->setCampoCod('');
        $this->setComplementoChave('cod_convenio,cod_banco,cod_agencia,cod_conta_corrente,timestamp');

        $this->AddCampo('cod_convenio'      ,'integer'       ,true  ,''    ,true  ,'TIMAConfiguracaoConvenioBesc');
        $this->AddCampo('cod_banco'         ,'integer'       ,true  ,''    ,true  ,'TIMAConfiguracaoConvenioBesc');
...
{
    $stSql  = "    SELECT configuracao_besc_conta.*                                                          	\n";
    ...
    $stSql .= "      FROM ima.configuracao_besc_conta                                                        	\n";
    $stSql .= "INNER JOIN monetario.conta_corrente                                                         		\n";
    $stSql .= "        ON configuracao_besc_conta.cod_conta_corrente = conta_corrente.cod_conta_corrente     	\n";
    $stSql .= "       AND configuracao_besc_conta.cod_agencia = conta_corrente.cod_agencia                   	\n";
    $stSql .= "       AND configuracao_besc_conta.cod_banco   = conta_corrente.cod_banco                     	\n";
    $stSql .= "INNER JOIN ( SELECT configuracao_besc_conta.cod_convenio									   		\n";
    $stSql .= "		   			 , max(configuracao_besc_conta.timestamp) as timestamp                       	\n";
    $stSql .= "		  	      FROM ima.configuracao_besc_conta                                               	\n";
    $stSql .= "		  		 WHERE configuracao_besc_conta.vigencia = to_date('".$this->getDado("vigencia")."','dd/mm/yyyy') \n";
    $stSql .= "			  GROUP BY configuracao_besc_conta.cod_convenio                                      	\n";
    $stSql .= "			  ) as max_configuracao_besc_conta                                                   	\n";
    $stSql .= "		   ON configuracao_besc_conta.cod_convenio = max_configuracao_besc_conta.cod_convenio      	\n";
    $stSql .= "		  AND configuracao_besc_conta.timestamp = max_configuracao_besc_conta.timestamp 		   	\n";
...
function recuperaVigencias(&$rsRecordSet, $stFiltro="", $stOrdem="")
{
...
function montaRecuperaVigencias()
{
