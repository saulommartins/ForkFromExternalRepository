...
switch ($stAcao) {
    case 'incluir':
        ...
        $stFiltro = "\nORDER BY cod_convenio desc";
        $obTIMAConfiguracaoConvenioHSBC->recuperaTodos($rsConfiguracao, $stFiltro);
    ...
    case 'alterar':
        ...
        $obTIMAConfiguracaoHSBCConta->setDado('vigencia', $dtVigencia);
        $obTIMAConfiguracaoHSBCConta->recuperaRelacionamento($rsConfiguracao);
        $stCodConvenio = $rsConfiguracao->getCampo("cod_convenio_banco");
        break;
}
...
$obIMontaAgenciaConta->obIMontaAgencia->obITextBoxSelectBanco->setTitle("Código do Banco HSBC (399). Necessário que este banco esteja previamente cadastrado em Gestão Tributátia :: Cadastro Monetário :: Banco :: Incluir Banco.");
...
$stFiltro = "WHERE num_banco != '399'";
$obISelectMultiploBanco = new ISelectMultiploBanco(false, $stFiltro);
$obISelectMultiploLotacao->setNullBarra(false);
$obISelectMultiploBanco->setRotulo("Outros bancos");
$obISelectMultiploBanco->setTitle("Selecione outros bancos.");
$arComponentes[] = $obISelectMultiploBanco;
$stIds .= $obISelectMultiploBanco->getNomeLista2();
...
if ( $rsPeriodoMovimentacao->getNumLinhas()>0 ) {
    ...
    $obFormulario->incluirAlterar("ConfHSBC", $arComponentes, false, false, $stIds,'post');
