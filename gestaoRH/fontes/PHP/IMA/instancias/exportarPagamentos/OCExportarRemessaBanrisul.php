...
/**
    ...
    * @ignore
    ...
    $Id: OCExportarRemessaBanrisul.php 65860 2016-06-22 18:08:07Z michel $
...
function gerarSpanGeral($stSituacao)
{
    ...
    $obIFiltroComponentes->obCmbTipoFiltro->obEvento->setOnChange("montaParametrosGET('atualizaCompetencia','inAno,inCodMes');");
...
{
    $stJs .= limparSpans();
    $stJs .= atualizaCompetencia(false, false);
...
function gerarSpanComplementar($boGerar = true)
{
    if ($boGerar) {
        ...
        $obIFiltroCompetencia->obSeletorAno->obTxtAno->obEvento->setOnChange("montaParametrosGET('atualizarGrupoConta','inAno,inCodMes');");
...
function limparForm()
{
    $stJs .= "montaParametrosGET('gerarSpan','stSituacao,inAno,inCodMes');";
...
function atualizarGrupoConta(Request $request)
{
    ...
    $obTIMAConfiguracaoBanrisulConta->recuperaVigencias($rsVigencia,$stFiltro,$stOrdem);

    $obTIMAConfiguracaoBanrisulConta->setDado("vigencia",$rsVigencia->getCampo("vigencia"));
    $obTIMAConfiguracaoBanrisulConta->recuperaRelacionamento($rsConfiguracaoBanrisulConta);
    Sessao::write("rsContas",$rsConfiguracaoBanrisulConta);

    $obLista = new Lista;
    $obLista->setTitulo("Grupos de Contas do Convênio");
    $obLista->setRecordSet( $rsConfiguracaoBanrisulConta );
    ...
    $obChkGrupo = new CheckBox;
    $obChkGrupo->setName ( "boGrupo[id]");
    $obChkGrupo->setValue( 'true' );
    $obChkGrupo->setChecked(true);

    $obLista->addDadoComponente( $obChkGrupo );
    $obLista->ultimoDado->setCampo( "[checkbox]" );
...
function atualizaCompetencia($inAno, $inMes){
    if($inAno!=''&&$inMes!=''){
        $arDados['inAno']  = $inAno;
        $arDados['inCodMes'] = $inMes;
        
        Sessao::write('Competencia', $arDados);
    }else{
        Sessao::remove('Competencia');    
    }
}

switch ($request->get('stCtrl')) {
    case "gerarSpan":
        $stJs .= atualizaCompetencia($request->get('inAno'), $request->get('inCodMes'));        
