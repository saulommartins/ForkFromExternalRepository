...
<?php
/**
    * Página de Oculto do Exportação Remessa BESC
...
function atualizarGrupoConta()
{
    ...
    $obTIMAConfiguracaoBescConta->recuperaVigencias($rsVigencia,$stFiltro,$stOrdem);

    $obTIMAConfiguracaoBescConta->setDado("vigencia",$rsVigencia->getCampo("vigencia"));
    $obTIMAConfiguracaoBescConta->recuperaRelacionamento($rsConfiguracaoBescConta);
    Sessao::write("rsContas",$rsConfiguracaoBescConta);

    $obLista = new Lista;
    $obLista->setTitulo("Grupos de Contas do Convênio");
    $obLista->setRecordSet( $rsConfiguracaoBescConta );
