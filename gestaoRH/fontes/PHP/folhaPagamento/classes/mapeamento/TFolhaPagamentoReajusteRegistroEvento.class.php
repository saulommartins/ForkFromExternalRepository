...
<?php
/**
    * Classe de mapeamento da tabela folhapagamento.reajuste_registro_evento
...
class TFolhaPagamentoReajusteRegistroEvento extends Persistente
{
    /**
    * Método Construtor
    * @access Private
    */
    public function TFolhaPagamentoReajusteRegistroEvento()
    {
        parent::Persistente();
        $this->setTabela("folhapagamento.reajuste_registro_evento");

        $this->setCampoCod('');
        $this->setComplementoChave('cod_reajuste,cod_registro,timestamp,cod_evento');

        $this->AddCampo('cod_reajuste','integer'  ,true  ,'',true,'TFolhaPagamentoReajuste');
        $this->AddCampo('cod_registro','integer'  ,true  ,'',true,'TFolhaPagamentoRegistroEvento');
        $this->AddCampo('timestamp'   ,'timestamp',true  ,'',true,'TFolhaPagamentoRegistroEvento');
        $this->AddCampo('cod_evento'  ,'integer'  ,true  ,'',true,'TFolhaPagamentoRegistroEvento');
    }

    public function recuperaReajusteSalario(&$rsRecordSet,$stFiltro="",$stOrder="",$boTransacao="")
    {
        return $this->executaRecupera("montaRecuperaReajusteSalario",$rsRecordSet,$stFiltro,$stOrder,$boTransacao);
    }

    public function recuperaReajustePensionistaSalario(&$rsRecordSet,$stFiltro="",$stOrder="",$boTransacao="")
    {
        return $this->executaRecupera("montaRecuperaReajustePensionistaSalario",$rsRecordSet,$stFiltro,$stOrder,$boTransacao);
    }

    public function montaRecuperaReajustePensionistaSalario()
    {
        $stSql  = "     SELECT registro_evento_periodo.cod_contrato                                                                                                                                                                 \n";
        $stSql .= "          , registro_evento.cod_evento                                                                                                                                                                           \n";
        $stSql .= "          , registro_evento.timestamp                                                                                                                                                                            \n";
        $stSql .= "          , to_real(registro_evento.valor) as valor                                                                                                                                                              \n";
        ...
        $stSql .= " INNER JOIN folhapagamento.reajuste_registro_evento                                                                                                                                     \n";
        $stSql .= "         ON reajuste.cod_reajuste = reajuste_registro_evento.cod_reajuste                                                                                                                                        \n";
        $stSql .= " INNER JOIN folhapagamento.registro_evento_periodo                                                                                                                                      \n";
        $stSql .= "         ON reajuste_registro_evento.cod_registro = registro_evento_periodo.cod_registro                                                                                                                         \n";
        $stSql .= " INNER JOIN folhapagamento.registro_evento                                                                                                                                              \n";
        $stSql .= "         ON reajuste_registro_evento.cod_registro     = registro_evento.cod_registro                                                                                                                             \n";
        $stSql .= "        AND reajuste_registro_evento.timestamp        = registro_evento.timestamp                                                                                                                                \n";
        $stSql .= "        AND reajuste_registro_evento.cod_evento       = registro_evento.cod_evento                                                                                                                               \n";
        $stSql .= " INNER JOIN folhapagamento.evento                                                                                                                                                       \n";
        $stSql .= "         ON registro_evento.cod_evento = evento.cod_evento                                                                                                                                                       \n";
        ...
        $stSql .= "           AND registro_evento_periodo.cod_periodo_movimentacao = ".$this->getDado("inCodPeriodoMovimentacao")."  \n";
        $stSql .= "           AND reajuste.cod_reajuste = ".$this->getDado("inCodReajuste")."                                        \n";

        return $stSql;
    }

    public function montaRecuperaReajusteSalario()
    {
        $stSql  = "        SELECT registro_evento_periodo.cod_contrato                                                                                                                                   \n";
        $stSql .= "             , registro_evento.cod_evento                                                                                                                                             \n";
        $stSql .= "             , registro_evento.timestamp                                                                                                                                              \n";
        ...
        $stSql .= "            ON registro_evento_periodo.cod_contrato = servidor_contrato_servidor.cod_contrato                                                                                     \n";
