...
CREATE OR REPLACE FUNCTION  calculaFolhaFerias(INTEGER,BOOLEAN,VARCHAR,VARCHAR) RETURNS BOOLEAN as $$
DECLARE
...
BEGIN
    ...
    inCodConfiguracao           := criarBufferInteiro(  'inCodConfiguracao', 2 );
    ...
    stPagar13Ferias             := pegaPagamentoUmTercoFerias(inCodContrato,inCodPeriodoMovimentacao);
    stPagar13Ferias             := criarBufferTexto('stPagar13Ferias',stPagar13Ferias);
    ...
    boRetorno := processarEventosAutomaticosFerias();
    boRetorno := criarTemporariaRegistrosFixos();
    --IF   boErro  =  FALSE THEN
        boRetorno := calculaEventoFeriasPorContrato(); 
    --ELSE
    --    boRetorno := calculaEventoFeriasPorContratoErro(); 
    --END IF;
    RETURN boRetorno;
END;
$$ LANGUAGE 'plpgsql'; 
