...
CREATE OR REPLACE FUNCTION pega0MontaBaseValorFeriasMesAnterior(varchar) RETURNS numeric as $$
...
BEGIN
    ...
    LOOP
        ...
        IF stNaturezaEvento = 'D' THEN
            nuValorBase := COALESCE(nuValorBase,0.00) - COALESCE(pegaValorCalculadoFeriasMesAnterior( inCodEvento, stDesdobramento ),0.00);
         ELSE
            nuValorBase := COALESCE(nuValorBase,0.00) + COALESCE(pegaValorCalculadoFeriasMesAnterior( inCodEvento, stDesdobramento ),0.00);
         END IF;    
