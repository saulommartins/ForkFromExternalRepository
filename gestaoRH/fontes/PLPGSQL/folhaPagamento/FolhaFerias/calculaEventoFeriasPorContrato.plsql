...
CREATE OR REPLACE FUNCTION calculaEventoFeriasPorContrato() RETURNS BOOLEAN as $$
...
EXCEPTION    
    WHEN others THEN 
        ...
        stSql := 'INSERT INTO folhapagamento'||stEntidade||'.log_erro_calculo_ferias (cod_evento,cod_registro,timestamp,desdobramento,erro) VALUES ('||inCodEvento||','||inCodRegistro||', '||quote_literal(stTimestamp)||','||quote_literal(stDesdobramento)||','||quote_literal(stMensagem)||')';
        EXECUTE stSql;
        RETURN FALSE;
END;
$$ LANGUAGE 'plpgsql';
