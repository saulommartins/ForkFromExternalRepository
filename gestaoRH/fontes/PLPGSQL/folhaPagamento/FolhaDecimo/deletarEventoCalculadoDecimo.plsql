...
CREATE OR REPLACE FUNCTION deletarEventoCalculadoDecimo(INTEGER,INTEGER) RETURNS BOOLEAN as '
...
BEGIN
    stSql := ''SELECT evento_decimo_calculado.*
                ...
                WHERE evento_decimo_calculado.cod_registro    = ultimo_registro_evento_decimo.cod_registro
    ...
    LOOP
        stSql := ''DELETE FROM folhapagamento''||stEntidade||''.evento_decimo_calculado_dependente WHERE cod_registro = ''||Registro.cod_registro||''
