...
CREATE OR REPLACE FUNCTION pega0MontaBaseValorDecimo(varchar) RETURNS numeric as $$
    ...
    LOOP
        ...
        IF stNaturezaEvento = 'D' THEN
            IF (stDesdobramento = '') THEN
                ...
                inControle := criarBufferInteiro('inControle',(inControle-1));
            ELSE
                nuValorBase := nuValorBase - pegaValorCalculadoDecimo( stEventoFormatado, stDesdobramento, inCodConfiguracao  );
            END IF;
        ELSE
            IF (stDesdobramento = '') THEN
                inControle := recuperarBufferInteiro('inControle');
