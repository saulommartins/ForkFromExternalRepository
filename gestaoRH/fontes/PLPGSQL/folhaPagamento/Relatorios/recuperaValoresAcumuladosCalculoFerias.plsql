...
*/
-- /**
--     * PLpgsql recuperaValoresAcumuladosCalculoFerias
...
CREATE OR REPLACE FUNCTION recuperaValoresAcumuladosCalculoFerias(integer,integer,integer,varchar,varchar) RETURNS SETOF colunasValoresAcumuladosFerias AS $$
DECLARE
    ...
    rwValoresAcumulados                 colunasValoresAcumuladosFerias%ROWTYPE;
    stFolhas                            VARCHAR:='';
    stSql                               VARCHAR;
...
BEGIN   
    ...
    FOR reEventos IN EXECUTE stSql LOOP
        ...
        IF stNatureza = 'D' THEN
            stSqlAux := stSqlAux     ||' AND registro_evento_ferias.cod_contrato != '|| inCodContrato;    
        END IF;                 
        nuAux := selectIntoNumeric(stSqlAux);
        IF nuAux is not null THEN
            nuValorEvento := nuValorEvento + nuAux;
