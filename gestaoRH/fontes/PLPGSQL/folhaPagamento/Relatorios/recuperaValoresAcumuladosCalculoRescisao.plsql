...
CREATE OR REPLACE FUNCTION recuperaValoresAcumuladosCalculoRescisao(integer,integer,integer,varchar,varchar) RETURNS SETOF colunasValoresAcumuladosRescisao AS $$
DECLARE
    ...
    reContratos                         RECORD;
    reEvento                            RECORD;
    nuValorEventoSaldoAviso             NUMERIC:=0;
...
BEGIN   
    ...
    FOR reEventos IN EXECUTE stSql LOOP
        ...
        IF nuAux is not null THEN
            nuValorEventoSaldoAviso := nuValorEventoSaldoAviso + nuAux;
        END IF;       
