...
CREATE OR REPLACE FUNCTION pega0MontaBaseQuantidadeRescisao(varchar) RETURNS numeric as $$
    ...
    LOOP
        ...
        IF stNaturezaEvento = 'D' THEN
            IF (stDesdobramento = '') THEN
                ...
                inControle := criarBufferInteiro('inControle',(inControle-1));
            ELSE       
                nuQuantidadeBase := nuQuantidadeBase - pegaQuantidadeCalculadoRescisao( stEventoFormatado, stDesdobramento, inCodConfiguracao  );
            END IF;
        ELSE
            IF (stDesdobramento = '') THEN
                inControle :=  recuperarBufferInteiro('inControle');
