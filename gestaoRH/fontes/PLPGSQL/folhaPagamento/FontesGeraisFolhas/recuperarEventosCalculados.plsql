...
CREATE OR REPLACE FUNCTION recuperarEventosCalculados(INTEGER,INTEGER,INTEGER,INTEGER,VARCHAR,VARCHAR) RETURNS SETOF colunasEventosCalculados AS $$
DECLARE
    ...
    stOrdem                     ALIAS FOR $6;
    rwEventosCalculados         colunasEventosCalculados%ROWTYPE;
    stSql                       VARCHAR;
...
BEGIN
    ...
    IF inCodConfiguracao = 0 THEN
        arDesdobramentos  := '{'''','||quote_literal('F')||','||quote_literal('D')||','||quote_literal('A')||','||quote_literal('I')||'}';
        stSqlComplementar := '
                       SELECT registro_evento_complementar_parcela.parcela as quantidade_parc
                            ...
                            , evento_complementar_calculado.cod_configuracao
                            , evento_complementar_calculado.valor
                            , evento_complementar_calculado.quantidade
                            , evento_complementar_calculado.timestamp                            
        ...
        IF inCodComplementar <> 0 THEN
            stSqlComplementar :=  stSqlComplementar ||'AND registro_evento_complementar.cod_complementar = '||inCodComplementar;
        END IF;
        ...
        stSqlComplementar := stSqlComplementar ||' 
                  INNER JOIN folhapagamento'||stEntidade||'.evento
                   ...
                   LEFT JOIN folhapagamento.registro_evento_complementar_parcela
                          ON evento.apresenta_parcela = TRUE
                         AND registro_evento_complementar_parcela.cod_evento = evento_complementar_calculado.cod_evento
    ...
    IF inCodConfiguracao = 1 THEN
        ...
        stSqlSalario := '
                      SELECT registro_evento_parcela.parcela as quantidade_parc
        ...
        stSqlSalario := stSqlSalario ||'                 
                  INNER JOIN folhapagamento'||stEntidade||'.evento
                   ...
                   LEFT JOIN folhapagamento.registro_evento_parcela
                          ON evento.apresenta_parcela = TRUE
                         AND registro_evento_parcela.cod_evento = evento_calculado.cod_evento
    ...
    IF inCodConfiguracao = 2 THEN
        ...
        stSqlFerias := '    
                      SELECT registro_evento_ferias_parcela.parcela as quantidade_parc
        ...
        stSqlFerias := stSqlFerias ||'                 
                  INNER JOIN folhapagamento'||stEntidade||'.evento
                   ...
                   LEFT JOIN folhapagamento.registro_evento_ferias_parcela
                          ON evento.apresenta_parcela = TRUE
                         AND registro_evento_ferias_parcela.cod_evento = evento_ferias_calculado.cod_evento
    ...
    IF inCodConfiguracao = 3 THEN
        ...
        stSqlDecimo := '
                      SELECT registro_evento_decimo_parcela.parcela as quantidade_parc
        ...
        stSqlDecimo := stSqlDecimo ||'                 
                  INNER JOIN folhapagamento'||stEntidade||'.evento
                   ...
                   LEFT JOIN folhapagamento.registro_evento_decimo_parcela
                          ON evento.apresenta_parcela = TRUE
                         AND registro_evento_decimo_parcela.cod_evento = evento_decimo_calculado.cod_evento
    ...
    IF inCodConfiguracao = 4 THEN
        ...
        stSqlRescisao := '    
                      SELECT registro_evento_rescisao_parcela.parcela as quantidade_parc
        ...
        stSqlRescisao := stSqlRescisao ||'                 
                  INNER JOIN folhapagamento'||stEntidade||'.evento
                   ...
                   LEFT JOIN folhapagamento.registro_evento_rescisao_parcela
                          ON evento.apresenta_parcela = TRUE
                         AND registro_evento_rescisao_parcela.cod_evento = evento_rescisao_calculado.cod_evento
    ...
    WHILE arDesdobramentos[inIndex] IS NOT NULL LOOP
        ...
        IF inCodConfiguracao = 0 THEN
             IF arDesdobramentos[inIndex] != quote_literal('') THEN 
        ...
        FOR reRegistro IN EXECUTE stSql LOOP
            ...
            rwEventosCalculados.quantidade_total_parcela     := reRegistro.quantidade_parc;       
            rwEventosCalculados.desdobramento       := reRegistro.desdobramento;
            rwEventosCalculados.desdobramento_texto := (CASE WHEN inCodConfiguracao != 0 THEN getDesdobramentoFolha(inCodConfiguracao,reRegistro.desdobramento,stEntidade)
	                                                     WHEN inCodConfiguracao  = 0 AND reRegistro.cod_configuracao=2 AND reRegistro.desdobramento = 'F' THEN 'Férias'
	                                                     WHEN inCodConfiguracao  = 0 AND reRegistro.cod_configuracao=2 AND reRegistro.desdobramento = 'D' THEN 'Adiant.Férias'
	                                                     WHEN inCodConfiguracao  = 0 AND reRegistro.cod_configuracao=2 AND reRegistro.desdobramento = 'A' THEN 'Abono Férias'
                                                             WHEN inCodConfiguracao  = 0 AND reRegistro.cod_configuracao=3 AND reRegistro.desdobramento = 'D' THEN 'Décimo'
                                                             WHEN inCodConfiguracao  = 0 AND reRegistro.cod_configuracao=3 AND reRegistro.desdobramento = 'A' THEN 'Adiant.Décimo'                                                             
                                                        END);
