...
CREATE OR REPLACE FUNCTION deletarReajusteRegistroEvento(INTEGER,INTEGER,VARCHAR,VARCHAR) RETURNS BOOLEAN as $$
DECLARE
...
BEGIN
    ...
    IF stTipoFolha = 'S' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.reajuste_registro_evento 
                   WHERE cod_registro = '|| inCodRegistro;
    END IF;
    IF stTipoFolha = 'C' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.reajuste_registro_evento_complementar
                   WHERE cod_registro = '|| inCodRegistro ||'
    ...
    IF stTipoFolha = 'F' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.reajuste_registro_evento_ferias
                   WHERE cod_registro = '|| inCodRegistro ||'
                     AND cod_evento = '|| inCodEvento ||'                
    ...
    IF stTipoFolha = 'R' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.reajuste_registro_evento_rescisao
                   WHERE cod_registro = '|| inCodRegistro ||'
                     AND cod_evento = '|| inCodEvento ||'                
                     AND desdobramento = '|| quote_literal(stDesdobramento) ||'
                     AND timestamp = '|| quote_literal(stTimestamp) ||' ';
    ...
    IF stTipoFolha = 'D' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.reajuste_registro_evento_decimo
                   WHERE cod_registro = '|| inCodRegistro ||'
                     AND cod_evento = '|| inCodEvento ||'                
                     AND desdobramento = '|| quote_literal(stDesdobramento) ||'
                     AND timestamp = '|| quote_literal(stTimestamp) ||' ';
