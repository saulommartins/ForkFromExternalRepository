...
CREATE OR REPLACE FUNCTION deletarLogErroCalculo(INTEGER,INTEGER,VARCHAR,VARCHAR) RETURNS BOOLEAN as $$
DECLARE
...
BEGIN
    ...
    IF stTipoFolha = 'R' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.log_erro_calculo_rescisao
                   WHERE cod_registro = '|| inCodRegistro ||'
                     AND cod_evento = '|| inCodEvento ||'                
                     AND desdobramento = '|| quote_literal(stDesdobramento) ||'
                     AND timestamp = '|| quote_literal(stTimestamp) ||' ';
