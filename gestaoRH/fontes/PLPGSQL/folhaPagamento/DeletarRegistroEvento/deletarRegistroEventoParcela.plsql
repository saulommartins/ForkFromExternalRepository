...
CREATE OR REPLACE FUNCTION deletarRegistroEventoParcela(INTEGER,INTEGER,VARCHAR,VARCHAR) RETURNS BOOLEAN as $$
DECLARE
...
BEGIN
    ...
    IF stTipoFolha = 'C' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.registro_evento_complementar_parcela
                   WHERE cod_registro = '|| inCodRegistro ||'
    ...
    IF stTipoFolha = 'R' THEN
        stSql := 'DELETE FROM folhapagamento'|| stEntidade ||'.registro_evento_rescisao_parcela
                   WHERE cod_registro = '|| inCodRegistro ||'
                     AND cod_evento = '|| inCodEvento ||'                
                     AND desdobramento = '|| quote_literal(stDesdobramento) ||'
                     AND timestamp = '|| quote_literal(stTimestamp) ||' ';
