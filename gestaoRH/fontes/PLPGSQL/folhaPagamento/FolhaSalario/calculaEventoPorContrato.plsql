/*
    ...
    **********************************************************************************
*/
CREATE OR REPLACE FUNCTION calculaEventoPorContrato(INTEGER) RETURNS BOOLEAN as $$
    ...
    stSql := 'INSERT INTO folhapagamento'||stEntidade||'.registro_evento_ordenado (
                 SELECT ultimo_registro_evento.cod_evento
                  ...
                  WHERE registro_evento_periodo.cod_periodo_movimentacao = '||inCodPeriodoMovimentacao||'
                    ...
                    AND COALESCE((SELECT mes_carencia FROM folhapagamento'||stEntidade||'.registro_evento_parcela
                          WHERE ultimo_registro_evento.cod_registro = registro_evento_parcela.cod_registro
                            AND ultimo_registro_evento.cod_evento = registro_evento_parcela.cod_evento
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        stMensagem := SQLSTATE||' - '||SQLERRM;
        stSql := 'INSERT INTO folhapagamento'||stEntidade||'.log_erro_calculo (cod_evento,cod_registro,timestamp,erro) VALUES ('||inCodEvento||','||inCodRegistro||','||quote_literal(stTimestamp)||', '||quote_literal(stMensagem)||')';
        --RAISE NOTICE 'Debug: %', stSql;
        EXECUTE stSql;
        RETURN FALSE;
END;
