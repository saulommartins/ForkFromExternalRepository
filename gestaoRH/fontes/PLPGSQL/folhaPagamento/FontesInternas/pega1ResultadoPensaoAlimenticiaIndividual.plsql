...
CREATE OR REPLACE FUNCTION pega1ResultadoPensaoAlimenticiaIndividual(INTEGER) RETURNS numeric as $$

DECLARE
    inCodDependente             ALIAS FOR $1;
 ...
 BEGIN
    ...
    WHERE sd.cod_servidor = '||inCodServidor||'
      AND sd.cod_dependente = '||inCodDependente||'
