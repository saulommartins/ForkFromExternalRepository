...
CREATE OR REPLACE FUNCTION pega0SituacaoDaFolhaSalario() RETURNS VARCHAR as $$
DECLARE
...
BEGIN
    ...
    stSql := 'SELECT situacao
                FROM folhapagamento'||stEntidade||'.folha_situacao
            ORDER BY timestamp DESC
               LIMIT 1';
               
    stSituacaoDaFolha := selectIntoVarchar(stSql);
    RETURN stSituacaoDaFolha;
END;
$$ LANGUAGE 'plpgsql';
