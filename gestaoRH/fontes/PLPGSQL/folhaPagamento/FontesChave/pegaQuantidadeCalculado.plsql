...
CREATE OR REPLACE FUNCTION pegaQuantidadeCalculado(VARCHAR,INTEGER) RETURNS NUMERIC AS $$
DECLARE
...
BEGIN
    inCountControle := recuperarBufferInteiro('inControle');
    
    IF inCountControle > 1 THEN
      nuQuantidadeEvento      := pegaQuantidadeCalculadoFixo(stCodigoEvento,inCodConfiguracao);
    ELSE    
        nuQuantidadeEvento := recuperarBufferNumerico(stCodigoEvento||inCodConfiguracao||'Quantidade');    
    END IF;
