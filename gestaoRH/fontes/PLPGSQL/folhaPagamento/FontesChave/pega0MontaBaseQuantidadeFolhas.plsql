...
--
--
CREATE OR REPLACE FUNCTION pega0MontaBaseQuantidadeFolhas(varchar,varchar) RETURNS numeric as $$
...
BEGIN
    ...
    IF stTipoFolha = 'S' OR stTipoFolha = 'C' THEN
        nuQuantidadeBase := pega0MontaBaseQuantidade(stLista);
        IF stTipoFolha = 'C' THEN
             stNomeBase := stNomeBase || recuperarBufferInteiro('inCodConfiguracao');
        END IF;
    ELSE
        IF stTipoFolha = 'F' THEN
            nuQuantidadeBase := pega0MontaBaseQuantidadeFerias(stLista);
        END IF;

        IF stTipoFolha = 'D' THEN
            nuQuantidadeBase := pega0MontaBaseQuantidadeDecimo(stLista);
        END IF;

        IF stTipoFolha = 'R' THEN
            nuQuantidadeBase := pega0MontaBaseQuantidadeRescisao(stLista);
        END IF;
