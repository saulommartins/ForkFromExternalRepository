...
CREATE OR REPLACE FUNCTION pega0MontaBaseQuantidadeFixo(varchar) RETURNS numeric as '
...
BEGIN
  ...
  LOOP
    ...
    IF stNaturezaEvento = ''D'' THEN 
        nuQuantidadeBase := nuQuantidadeBase - pegaQuantidadeCalculadoFixo( stEventoFormatado, inCodConfiguracao  );
    ELSE
        nuQuantidadeBase := nuQuantidadeBase + pegaQuantidadeCalculadoFixo( stEventoFormatado, inCodConfiguracao  );
    END IF;    
