...
CREATE OR REPLACE FUNCTION pega0MontaBaseValorFixo(varchar) RETURNS numeric as '
...
BEGIN
  ...
  LOOP
    ...
    IF stNaturezaEvento = ''D'' THEN 
        nuValorBase := nuValorBase - pegaValorCalculadoFixo( stEventoFormatado, inCodConfiguracao  );
    ELSE
        nuValorBase := nuValorBase + pegaValorCalculadoFixo( stEventoFormatado, inCodConfiguracao  );
    END IF;   
