...
*/
/*
 * PL para retorno os servidores cadastrados para a sequencia REGIME/SUBDIVISÃO/CARGO
 ...
 * @ignore # só use se FOR paginas que o cliente visualiza, se FOR mapeamento ou classe de negocio não se usa
 ...
 $Id:$
 */
CREATE OR REPLACE FUNCTION consultarServidoresPorCargo(integer,integer,integer,varchar) RETURNS SETOF colunasConsultarServidoresPorCargo AS $$
DECLARE
...
BEGIN
    stSql := 'SELECT (SELECT registro FROM pessoal'||stEntidade||'.contrato WHERE cod_contrato = contrato_servidor.cod_contrato) as registro
                ...
                JOIN pessoal'||stEntidade||'.contrato_servidor_funcao
               ...
               WHERE ((    contrato_servidor.cod_cargo = '||inCodCargo||'
                  ...
                  AND recuperarSituacaoDoContrato(contrato_servidor.cod_contrato, 0, '''||stEntidade||''') not IN (''R'',''P'')
             ORDER BY nom_cgm';
