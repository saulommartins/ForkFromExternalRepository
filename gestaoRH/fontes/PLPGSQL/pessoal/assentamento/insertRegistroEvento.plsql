...
CREATE OR REPLACE FUNCTION insertRegistroEvento(INTEGER,INTEGER,INTEGER,NUMERIC,VARCHAR,VARCHAR) RETURNS BOOLEAN as $$
...
BEGIN
    ...
    stSql := 'INSERT INTO folhapagamento'|| stEntidade ||'.registro_evento (cod_registro,timestamp,cod_evento,automatico,valor,quantidade,proporcional)
                    VALUES ('|| inCodRegistro ||',TO_TIMESTAMP('|| quote_literal(stTimestamp) ||',''yyyy-mm-dd hh24:mi:ss.us''),'|| inCodEvento ||',false,'|| nuValor ||','|| nuQuantidade ||','|| quote_literal(stProporcional) ||');';
    EXECUTE stSql;
