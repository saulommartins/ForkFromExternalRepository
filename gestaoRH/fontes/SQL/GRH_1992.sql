...
SELECT atualizarbanco('CREATE TABLE ima.configuracao_hsbc_bancos (
    ...
    cod_banco_outros    INTEGER     NOT NULL,
    CONSTRAINT pk_configuracao_hsbc_bancos      PRIMARY KEY (cod_convenio, cod_banco, cod_agencia, cod_conta_corrente, timestamp, cod_banco_outros),
    CONSTRAINT fk_configuracao_hsbc_bancos_1    FOREIGN KEY                             (cod_convenio, cod_banco, cod_agencia, cod_conta_corrente, timestamp)
                                                REFERENCES  ima.configuracao_hsbc_conta (cod_convenio, cod_banco, cod_agencia, cod_conta_corrente, timestamp),
    CONSTRAINT fk_configuracao_hsbc_bancos_2    FOREIGN KEY                             (cod_banco_outros)
                                                REFERENCES  monetario.banco             (cod_banco)
);
');

SELECT atualizarbanco('GRANT ALL ON ima.configuracao_hsbc_bancos TO GROUP urbem;');


---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- ADICIONANDO FUNCOES pegaValorCalculadoDecimoCompetenciasAnteriores, gravarBufferEventoFolhas E recuperaOcorrenciadoEventonaListadaBase COMO FUNCOES INTERNAS
---------------------------------------------------------------------------------------------------------------------------------------------------------------
   --
   -- Insere a função.
   --
   ...
   BEGIN
    ...
    --Inclusão de função interna arrecadacao/fn_acrescimo_indice.plsql
      ...
      intCodFuncao   := public.manutencao_funcao   (  27, 1, 'pegaValorCalculadoDecimoCompetenciasAnteriores', 4);
                                                 --( intCodmodulo , intCodBiblioteca , varNomeFunc , intCodTiporetorno )

      intCodVariavel := public.manutencao_variavel (  27, 1, intCodFuncao, 'inCodEvento', 1 );
      PERFORM           public.manutencao_parametro(  27, 1, intCodFuncao, intCodVariavel );


      intCodFuncao   := public.manutencao_funcao   (  27, 1, 'gravarBufferEventoFolhas', 3);
                                                 --( intCodmodulo , intCodBiblioteca , varNomeFunc , intCodTiporetorno )

      intCodVariavel := public.manutencao_variavel (  27, 1, intCodFuncao, 'nuValor', 4 );
      PERFORM           public.manutencao_parametro(  27, 1, intCodFuncao, intCodVariavel );
      intCodVariavel := public.manutencao_variavel (  27, 1, intCodFuncao, 'nuQuantidade', 4 );
      PERFORM           public.manutencao_parametro(  27, 1, intCodFuncao, intCodVariavel );


      intCodFuncao   := public.manutencao_funcao   (  27, 1, 'recuperaOcorrenciadoEventonaListadaBase', 3);
                                                 --( intCodmodulo , intCodBiblioteca , varNomeFunc , intCodTiporetorno )

      intCodVariavel := public.manutencao_variavel (  27, 1, intCodFuncao, 'stNomBase', 2 );
      PERFORM           public.manutencao_parametro(  27, 1, intCodFuncao, intCodVariavel );
      intCodVariavel := public.manutencao_variavel (  27, 1, intCodFuncao, 'stCodigoEvento', 2 );
      PERFORM           public.manutencao_parametro(  27, 1, intCodFuncao, intCodVariavel );
