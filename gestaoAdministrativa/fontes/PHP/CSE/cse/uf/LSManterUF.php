...
if ($_REQUEST['inCodPais']!="") {$stFiltro.=" AND sw_uf.cod_pais = ".$_REQUEST['inCodPais']."     \n";}
if ($_REQUEST['stNomeUf']!="") {$stFiltro.=" AND sw_uf.nom_uf ILIKE '%".$_REQUEST['stNomeUf']."%'\n";}

$obTUf->recuperaEstado($rsLista, $stFiltro);
...
while (!$rsLista->eof()) {
        ...
        $arLista[$inCount]['sigla_uf'] = $rsLista->getCampo('sigla_uf');
...
$obLista->ultimoDado->setCampo( "cod_uf" );
$obLista->commitDado();
...
$obLista->ultimaAcao->addCampo("&stNome"         ,"nom_uf"             );
$obLista->ultimaAcao->addCampo("&stNomeSigla"    ,"sigla_uf"           );
$obLista->ultimaAcao->addCampo("&stNacionalidade","nacionalidade"      );
$obLista->ultimaAcao->addCampo("&stDescQuestao"  ,"[cod_uf] - [nom_uf]");

if ($stAcao == "excluir") {
    $stCaminho = CAM_CSE."cse/uf";
