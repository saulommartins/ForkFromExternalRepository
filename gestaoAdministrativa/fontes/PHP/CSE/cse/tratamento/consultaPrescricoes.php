...
?>
            <tr>
                ...
                <td class='show_dados'><?=$codTratamento;?></td>
...
<tr>
    ...
    <td class='field' width='70%'>
        ...
        <input type='hidden' name="codCidadao" value="<?=$codCidadao;?>">
        <?=$codCidadao;?>&nbsp;
        <input type='hidden' name="nomCidadao" value="<?=$nomCidadao;?>">
        <?=$nomCidadao;?>
    </td>
</tr>
<tr>
...
<tr>
    ...
    <td class='field'>
        ...
        <input type='hidden' name="dataTermino" value="<?=$dataTermino;?>">
        <?=$dataInicio."&nbsp;a&nbsp;".$dataTermino;?>
    </td>
</tr>
<tr>
...
<tr>
    ...
    <td class='field'>
        ...
        <?=$codClassificacao;?>&nbsp;
        <?php
            echo pegaDado("nom_classificacao","cse.classificacao_tratamento", ' Where cod_classificacao = '.$codClassificacao );
        ?>
    </td>
</tr>
<tr>
    ...
    <td class='field'>
        <input type='hidden' name="codTxtTipo" value="<?=$codTipo;?>">
        <?=$codTipo;?>&nbsp;
    <?php
        echo pegaDado("nom_tratamento","cse.tipo_tratamento", ' Where cod_tratamento = '.$codTipo );
    ?>
    </td>
</tr>
</table>
...
<tr>
    ...
    if (count($sessao->transf[exame]) > 0) {
        $aux = geraResumoExame($sessao->transf[exame], true);
        echo $aux[1];
    }
     ?>
    </td>
...
<tr>
    ...
    if (count($sessao->transf[internacao]) > 0) {
        $aux = geraResumoInternacao($sessao->transf[internacao], true);
        echo $aux[1];
    }
     ?>
    </td>
...
/*** Se não houver restrições faz a inclusão dos dados ***/
    if (!$ok) {
        ...
        if ($obj->alterarTratamento($var) ) {
            ...
            sistemaLegado::alertaAviso($PHP_SELF,$objeto,"alterar","aviso");
        } else {
