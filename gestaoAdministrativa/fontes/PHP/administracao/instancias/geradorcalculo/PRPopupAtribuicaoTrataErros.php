...
if (( $_REQUEST['stTipoAtribuicao']) == 'Funcao' ) {
     ...
     while ( !$rsParametros->eof() ) {
             ...
             $stValor        = 'stValor_'.trim($stNomVariavel);
             $$stValor       = $_POST[ $stValor ];
             $stVariavel     = 'stVariavel_'.trim($stNomVariavel);
     ...
     $stFuncao .= ' ); ';
     $stTrataErro = $stFuncao;
     $arPosicao = explode("-",$_REQUEST['stPosicao']); //Primeira posição: Indice numérico - Segunda posição: Nível

} else {
 $stTrataErro = $_POST['stVariavelInicial']." <- TRATA_ERROS( \\'\\'".$_POST['stFuncao']."\\'\\', \\'\\'".$_POST["stTipoVariavel"]."\\'\\'";
//, ARRAY[";
     $stTrataErroParams = '';
     while ( !$rsParametros->eof() ) {
             $rsVariavel     = new RecordSet;
             ...
             $stTrataErroParams .= ' ';
             if (( $stTipoVariavel == 'TEXTO') && (trim($$stValor) != "")) {
                  $stTrataErroParams .= "\\'\\'".$stParametro."\\'\\'";
             } else {
                  $stTrataErroParams .= $stParametro."::varchar";
             }
             $stTrataErroParams .= " ,\\'\\'".$stTipoVariavel."\\'\\' ,";
             $rsParametros->proximo();
     }

     if ( substr($stTrataErroParams ,strlen($stTrataErroParams)-1,strlen($stTrataErroParams)) == ',') {
         $stTrataErroParams = substr($stTrataErroParams,0,strlen($stTrataErroParams)-1);
     }

     if ($stTrataErroParams != '') {
        $stTrataErro .= ", ARRAY[".$stTrataErroParams."]";
     }
     $stTrataErro .= ');';
     $stFuncao = $stTrataErro;
     $arPosicao = explode("-",$_REQUEST['stPosicao']); //Primeira posição: Indice numérico - Segunda posição: Nível
}
...
//
  ...
  $stCorpoPL = str_replace("\\\'","\'",$stCorpoPL);
    ...
    case "alterar":
        if ( trim($_REQUEST['stTipoAtribuicao']) == 'Funcao' ) {
