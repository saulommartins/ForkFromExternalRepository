...
switch ($variavel) {
    case 'codClassifAssunto':
        ...
        if ($codClassificacaof == $codClassificacao) {
            ...
            while (!$dbAss->eof()) {
                ...
                $nomAssuntoW  = trim( addslashes($dbAss->pegaCampo("nom_assunto")));
                $dbAss->vaiProximo();
    ...
    case 'codClassificacao':
        if ($valor == "xxx") {
            ...
            while (!$dbAss->eof()) {
                ...
                $nomAssuntof  = trim( addslashes($dbAss->pegaCampo("nom_assunto")));
                $dbAss->vaiProximo();
    ...
    case 'codMasSetor':
        ...
        if ($_REQUEST['valor'] == "") {
            ...
            $js .= "f.codOrgao.options[0] = new Option('Selecione Orgão','xxx');\n";
            $js .= "f.codOrgao.options[0].selected = true;\n";

            $js .= "limpaSelect(f.codUnidade,0); \n";
            $js .= "f.codUnidade.options[0] = new Option('Selecione Unidade','xxx');\n";

            $js .= "limpaSelect(f.codDepartamento,0); \n";
            $js .= "f.codDepartamento.options[0] = new Option('Selecione Departamento','xxx');\n";

            $js .= "limpaSelect(f.codSetor,0); \n";
            $js .= "f.codSetor.options[0] = new Option('Selecione Setor','xxx');\n";
        } else {
            ...
            if (!$boRegistrosOrg) {
                ...
                if (!$boRegistroUni) {
                    ...
                    while (!$dbUni->eof()) {
                        ...
                        $js .= "f.codUnidade.options[$contUni] = new Option('".$nomUnidadeC."','".$codUnidadeC."-".$codOrgaoF."-".$anoExercicio."'".$selected."); \n";
                        $contUni++;
                        $dbUni->vaiProximo();
                    }
                    ...
                    if ($codUnidadeF) {
                        ...
                        while (!$dbDep->eof()) {
                            ...
                            $js .= "f.codDepartamento.options[$contDep] = new Option('".$nomDepartamentoC."','".$codDepartamentoC."-".$codUnidadeF."-".$codOrgaoF."-".$anoExercicio."'".$selected."); \n";
                            $contDep++;
                            $dbDep->vaiProximo();
                        }
                        ...
                        if ($codSetorF) {
                            ...
                            while (!$dbSet->eof()) {
                                ...
                                $js .= "f.codSetor.options[$contSet] = new Option('".$nomSetorC."','".$codSetorC."-".$codDepartamentoF."-".$codUnidadeF."-".$codOrgaoF."-".$anoExercicio."'".$selected."); \n";
                                $contSet++;
                                $dbSet->vaiProximo();
                            }
                            $js .=  "f.codSetor.focus();\n";
    ...
    case 'codOrgao':
        ...
        if ($codOrgao != "xxx") {
            ...
            $sSQL = "SELECT cod_unidade, nom_unidade FROM administracao.unidade WHERE cod_orgao = ".$codOrgaom."
             and ano_exercicio = '".$anoExercicio."' ORDER by nom_unidade";
            //echo  $sSQL;
 ...
 $codMasSetor = validaMascaraDinamica($mascaraSetor,$codOrgaoS."-".$codUnidadeS."-".$codDepartamentoS."-".$codSetorS."/".$anoExercicio);
    ...
    case 'codUnidade':
        ...
        if ($codUnidade != "xxx") {
            ...
            $cont = 1;
            $js = "";
            $codMasSetor = validaMascaraDinamica($mascaraSetor, $codOrgaod."-".$codUnidaded."-".$codDepartamentoS."-".$codSetorS."/".$anoExercicio);
            $js .= "f.codMasSetor.value = \"".$codMasSetor[1]."\";\n";
            $js .= "limpaSelect(f.codDepartamento,0); \n";
            $js .= "f.codDepartamento.options[0] = new Option('Selecione','xxx'".$default."); \n";
            while (!$dbEmp->eof()) {
    ...
    case 'codSetor':
        ...
        if ($codSetor == "xxx") {
            ...
            $variaveis = preg_split( "/[^a-zA-Z0-9]/",$codSetor);
        }
...
//		$codMasSetor = validaMascaraDinamica($mascaraSetor,$codOrgaoS."-".$codUnidadeS."-".$codDepartamentoS."-".$codSetorS."/".$anoExercicio);
        ...
        $sSQL .= " administracao.setor.ano_exercicio = '".$anoExercicio."' and ";
