...
function MontaDotacaoOrcamentaria()
{
    ...
    $this->setMascara( $this->obRConfiguracaoOrcamento->getMascDespesa() );
}
...
*/
function geraFormulario(&$obFormulario,$stAcao='',$nuCountDespesaExercicio='',$arDespesa='')
{
        ...
        $obTxtMascDotacaoOrcamentaria->setNull     ( false );
        ...
        $obCmbOrgao->setCampoId   ( "[cod_orgao]-[num_orgao]-[ano_exercicio]"   );
        ...
        $this->obRDespesa->obROrcamentoPrograma->listar( $rsPrograma, "ORDER BY cod_programa" );
        ...
        $obCmbPrograma->setNull      ( false                  );
        $obCmbPrograma->setCampoId   ( "cod_programa"         );
        $obCmbPrograma->setCampoDesc ( "[cod_programa] - [descricao]" );
        ...
        $this->obRDespesa->obROrcamentoProjetoAtividade->listar( $rsPAO, "ORDER BY num_pao" );
        $obCmbPAO = new Select;
        $obCmbPAO->setName      ( 'inCodPAO'                        );
        $obCmbPAO->setValue     ( ''            );
        $obCmbPAO->setRotulo    ( 'Projeto, Atividade ou Operações' );
        $obCmbPAO->setStyle     ( "width: 400px"                    );
        $obCmbPAO->setNull      ( false                             );
        $obCmbPAO->setCampoId   ( "num_pao"                         );
        $obCmbPAO->setCampoDesc ( "[num_pao] - [nom_pao]"            );
        ...
        while (!$rsUnidade->eof()) {
            if ($rsUnidade->getCampo('cod_unidade') == $arDespesa[0]['num_unidade']) {
                $obLblUnidade->setValue ( $rsUnidade->getCampo('cod_unidade').' - '.$rsUnidade->getCampo('nom_unidade') );
                $obHdnUnidade->setValue ( $rsUnidade->getCampo('cod_unidade') );
            }
            $rsUnidade->proximo();
        }
        ...
        while (!$rsPrograma->eof()) {
            if ($rsPrograma->getCampo('cod_programa') == $arDespesa[0]['cod_programa']) {
                $obLblPrograma->setValue ( $rsPrograma->getCampo('cod_programa').' - '.$rsPrograma->getCampo('descricao') );
        ...
        while (!$rsPAO->eof()) {
            if ($rsPAO->getCampo('num_pao') == $arDespesa[0]['num_pao']) {
                $obLblPAO->setValue ( $rsPAO->getCampo('num_pao').' - '.$rsPAO->getCampo('nom_pao') );
   ...
   if ($stAcao == 'alterar' && $nuCountDespesaExercicio != 0) {
        ...
        $obFormulario->addRotulo( "Informe a dotação orçamentária.", "*Dotação Orçamentária", 1 );
        $obFormulario->addCampo( $obTxtMascDotacaoOrcamentaria );
        $obFormulario->fechaLinha();

        $obFormulario->abreLinha();
        $obFormulario->addRotulo( "Selecione o orgão orçamentário.", "*Orgão", 1 );
        $obFormulario->addCampo( $obCmbOrgao );
        $obFormulario->fechaLinha();

        $obFormulario->abreLinha();
        $obFormulario->addRotulo( "Selecione a unidade orçamentária.", "*Unidade", 1 );
        $obFormulario->addCampo( $obCmbUnidade );
        $obFormulario->fechaLinha();

        $obFormulario->abreLinha();
        $obFormulario->addRotulo( "Selecione a função.", "*Função", 1 );
        $obFormulario->addCampo( $obCmbFuncao );
        $obFormulario->fechaLinha();

        $obFormulario->abreLinha();
        $obFormulario->addRotulo( "Selecione a subfunção.", "*Subfunção", 1 );
        $obFormulario->addCampo ( $obCmbSubFuncao );
        $obFormulario->fechaLinha();

        $obFormulario->abreLinha();
        $obFormulario->addRotulo( "Selecione o programa.", "*Programa", 1 );
        $obFormulario->addCampo( $obCmbPrograma );
        $obFormulario->fechaLinha();

        $obFormulario->abreLinha();
        $obFormulario->addRotulo( "Selecione o PAO.", "*Projeto, Atividade ou Operações", 1 );
        $obFormulario->addCampo( $obCmbPAO );
        $obFormulario->fechaLinha();
    }
...
function preencheUnidade()
{
    ...
    $this->obRDespesa->obROrcamentoUnidadeOrcamentaria->obROrcamentoOrgaoOrcamentario->listar( $rsOrgao, "ORDER BY ano_exercicio, cod_orgao, num_orgao" );
