...
<?php
/**
    * Classe de mapeamento da tabela PPA.PPA_RECEITA
...
class TPPAPPAReceita extends Persistente
{
    /**
        * Método Construtor
        * @access Private
    */
    public function TPPAPPAReceita()
    {
        parent::Persistente();
        $this->setTabela('ppa.ppa_receita');

        $this->setCampoCod('');
        $this->setComplementoChave('cod_configuracao, cod_receita, exercicio');

        $this->AddCampo('cod_configuracao', 'integer', true, '', true, 'TPPAConfiguracao');
        $this->AddCampo('cod_receita', 'integer', true, '', true, 'TOrcamentoReceita');
        $this->AddCampo('exercicio', 'char', true, '4', true, 'TOrcamentoReceita');
