...
<?php
/**
* Arquivo instância para popup de Produto
...
switch ($_REQUEST["stCtrl"]) {
    case "buscaProduto":
        ...
        $obTPPAProduto->setDado('cod_produto', $_REQUEST['inCodigo']);

        $obTPPAProduto->recuperaPorChave($rsProduto);

        if ($rsProduto->inNumLinhas > 0) {
            $stJs = "document.getElementById('".$stCampoDesc."').innerHTML = '".addslashes($rsProduto->getCampo('descricao'))."';";
        } else {
            $stJs  = "document.getElementById('".$stCampoCod."').value = '';";
            $stJs .= "document.getElementById('".$stCampoDesc."').innerHTML = '&nbsp;';";
            $stJs .= "alertaAviso('@Código do Produto (". $inCodigo .") não encontrado.', 'form','erro','".Sessao::getId()."');";
        }
        echo $stJs;
    break;
}
