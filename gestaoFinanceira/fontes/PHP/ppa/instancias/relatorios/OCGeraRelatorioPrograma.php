...
$preview = new PreviewBirt(2, 43, 4);
$preview->setVersaoBirt('2.5.0');
$preview->setExportaExcel(true);
$preview->addParametro('cod_ppa', $_POST['inCodPPA']);
$preview->addParametro('ano_inicial', $arPPA[0]['ano_inicio']);
if ($_REQUEST['inNumProgramaIni'] != '') {
    $preview->addParametro('num_programa_ini',$_REQUEST['inNumProgramaIni']);
} else {
    $preview->addParametro('num_programa_ini','');
}
if ($_REQUEST['inNumProgramaFim'] != '') {
    $preview->addParametro('num_programa_fim',$_REQUEST['inNumProgramaFim']);
} else {
    $preview->addParametro('num_programa_fim','');
}
if ($_REQUEST['inCodTipoPrograma'] != '') {
    $preview->addParametro('cod_tipo_programa',$_REQUEST['inCodTipoPrograma']);
} else {
    $preview->addParametro('cod_tipo_programa','');
}
if ($_REQUEST['stNatureza'] != '') {
    $preview->addParametro('continuo',$_REQUEST['stNatureza']);
} else {
    $preview->addParametro('continuo','');
}
$preview->preview();
