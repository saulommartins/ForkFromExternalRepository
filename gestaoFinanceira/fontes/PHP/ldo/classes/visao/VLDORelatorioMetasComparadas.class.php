...
<?php
/**
 * Classe Visao do 02.10.09 - Emitir Metas Comparadas
 ...
 * @subpackage LDO
 * @uc 02.10.09 - Emitir Metas Comparadas
 */

include_once CAM_GF_LDO_VISAO . 'VLDORelatorio.class.php';

class VLDORelatorioMetasComparadas extends VLDORelatorio implements IVLDORelatorio
{
    ...
    public function emitir(array $arParametros)
    {
        ...
        $pgProg = "PRRelatorioMetasComparadas.php?stAcao=gerar";
    ...
    {
        $obPreview = new PreviewBirt(2, 44, 2);
        $obPreview->setTitulo('Relatório do Birt');
        $obPreview->setVersaoBirt('2.5.0');
        $obPreview->addParametro("ano_ldo", $arParametros['inAnoLDO']);
        $obPreview->addParametro("ano_ldo_1", $arParametros['inAnoLDO']-3);
        $obPreview->addParametro("ano_ldo_2", $arParametros['inAnoLDO']-2);
        $obPreview->addParametro("ano_ldo_3", $arParametros['inAnoLDO']-1);
        $obPreview->addParametro("ano_ldo_4", $arParametros['inAnoLDO']+1);
        $obPreview->addParametro("ano_ldo_5", $arParametros['inAnoLDO']+2);
        $obPreview->addParametro("ano_ldo_6", $arParametros['inAnoLDO']+3);
