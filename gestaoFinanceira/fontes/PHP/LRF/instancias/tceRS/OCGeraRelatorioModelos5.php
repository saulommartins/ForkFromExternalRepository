...
<?php
 /**
    ...
    * Casos de uso :uc-02.05.07, uc-02.01.35
*/
...
if (Sessao::read('modulo') != 8)
    $obPDF->addCabecalho("Modelo 5 - Demonstrativo das Garantias e Contragarantias de Valores", 100, 10);

$obPDF->addRecordSet($rsVazio);
$obPDF->setQuebraPaginaLista( false );
$obPDF->setAlinhamento ( "C" );
if (Sessao::read('modulo') != 8)
    $obPDF->addCabecalho("LC Federal nº 101/2000, ART. 54 e alínea \"C\" do Inciso I do Art. 55", 100, 9);

if (!empty($sessao->transf5[1])) {
    $obPDF->addRecordSet( $rsVazio );
    $obPDF->setQuebraPaginaLista( false );
    $obPDF->setAlinhamento ( "C" );
    $obPDF->addCabecalho("GARANTIAS", 100, 10);
    ...
    $obPDF->addCabecalho("Saldo Exercicio Atual", 27,8);
...
if (!empty($sessao->transf5[2])) {
    ...
    $obPDF->setQuebraPaginaLista( false );
    $obPDF->setAlinhamento ( "C" );
    $obPDF->addCabecalho("CONTRAGARANTIAS", 100, 10);
