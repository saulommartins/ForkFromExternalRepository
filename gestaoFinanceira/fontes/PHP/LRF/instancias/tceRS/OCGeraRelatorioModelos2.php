...
<?php
/**
* Classe de oculto para modelo 2 do modulo LRF
...
* Casos de uso: uc-02.05.04, uc-02.01.35
*/
...
include_once( CAM_GF_LRF_NEGOCIO."RLRFRelatorioModelos2.class.php"  );

$obRegra      = new RLRFRelatorioModelos2;
$obPDF        = new ListaPDF();
...
if (Sessao::read('modulo') != 8 )
    $obPDF->addCabecalho("Modelo 2 - Demonstrativo da Despesa com Pessoal", 100, 12);

$obPDF->addRecordSet($rsVazio);
$obPDF->setQuebraPaginaLista( false );
$obPDF->setAlinhamento ( "C" );
if (Sessao::read('modulo') != 8 )
    $obPDF->addCabecalho("LC Federal nº 101/2000, ARTS. 54 e alínea \"A\" do Inciso I do ART. 55", 100, 9);
...
if (!empty($sessao->transf5[1])) {
    ...
    $obPDF->addCabecalho("Despesa ".$stTipoDespesa." - Em R$", 27,10);
    ...
    $obPDF->addCabecalho("Ajustado", 8, 8);

    $obPDF->addRecordSet( $sessao->transf5[1] );
    $obPDF->setQuebraPaginaLista( false );
    $obPDF->setAlinhamento ( "L" );
    $obPDF->addCabecalho("", 58, 1);
    $obPDF->addCabecalho("", 15, 1);
    $obPDF->addCabecalho("", 9, 1);
    $obPDF->addCabecalho("", 9, 1);
    $obPDF->addCabecalho("", 9, 1);
