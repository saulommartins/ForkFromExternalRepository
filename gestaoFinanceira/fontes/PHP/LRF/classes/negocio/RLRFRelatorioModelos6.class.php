...
<?php
 /**
    * Classe de Regra do Relatório de Modelos Executivo 6
...
class RLRFRelatorioModelos6 extends PersistenteRelatorio
{
/**
    * @var Object
    * @access Private
...
/**
     ...
     * @return Object
*/
function RLRFRelatorioModelos6()
{
...
/**
    ...
    * @access Public
*/
function geraRecordSet(&$rsRecordSet ,&$rsRecordSet1, &$rsRecordSetTotal,&$rsRecordSetTotal1, $stOrder = "")
{
    $stFiltro = "";
...
//    $this->obFLRFModelosExecutivo->debug();
    ...
    while ( !$rsRecordSet->eof()) {
        if ($rsRecordSet->getCorrente() < 5) {
           ...
           $arNomHistoricoOLD = explode( chr(13), $stNomContaTemp );    //maiores ou menores
           $inCount2 = $inCount;
           if ( $rsRecordSet->getCampo('redutora') == 't' ) {
               foreach ($arNomHistoricoOLD as $stNomContaTemp) {
                   if ($inCount == $inCount2) {
           ...
           $stNomContaTemp = str_replace( chr(10), "", 'TOTAL DAS OPERAÇÕES DE CRÉDITO INTERNAS E EXTERNAS');
           $stNomContaTemp = wordwrap( $stNomContaTemp,95,chr(13) );    // NOTA, o valor 66 eh o q deve ser mudado pra
           $arNomHistoricoOLD = explode( chr(13), $stNomContaTemp );    //maiores ou menores
           $inCountAux = 0;
           $inCountAux2 = $inCountAux;
           foreach ($arNomHistoricoOLD as $stNomContaTemp) {
              $arRecordTotal[$inCountAux2]['coluna1']    = $stNomContaTemp;
              $inCountAux2++;
           }
           $arRecordTotal[$inCountAux]['coluna2']    =  number_format($flVlContabil, 2, ',', '.' );
           $arRecordTotal[$inCountAux]['coluna3']    =  number_format($flVlAjuste, 2, ',', '.' );
           $arRecordTotal[$inCountAux]['coluna4']    =  number_format($flVlAjustado, 2, ',', '.' );
           $inCount++;
           ...
           $stNomContaTemp = str_replace( chr(10), "", "TOTAL DAS OPERAÇÕES DE CRÉDITO POR ANTECIPAÇÃO DA RECEITA ORÇAMENTÁRIA - ARO");
           $stNomContaTemp = wordwrap( $stNomContaTemp, 95,chr(13) );    // NOTA, o valor 66 eh o q deve ser mudado pra
           $arNomHistoricoOLD = explode( chr(13), $stNomContaTemp );    //maiores ou menores
           ...
           $inCountAux2 = $inCountAux;
           foreach ($arNomHistoricoOLD as $stNomContaTemp) {
              $arRecordTotal1[$inCountAux2]['coluna1']    = $stNomContaTemp;
              $inCountAux2++;
           }
           $arRecordTotal1[$inCountAux]['coluna2']    =  number_format($flVlContabil, 2, ',', '.' );
           $arRecordTotal1[$inCountAux]['coluna3']    =  number_format($flVlAjuste, 2, ',', '.' );
           $arRecordTotal1[$inCountAux]['coluna4']    =  number_format($flVlAjustado, 2, ',', '.' );
           $inCount3++;
