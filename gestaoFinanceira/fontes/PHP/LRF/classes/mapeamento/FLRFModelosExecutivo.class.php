...
class FLRFModelosExecutivo extends Persistente
{
/**
    * Método Construtor
    * @access Private
*/
function FLRFModelosExecutivo()
{
    parent::Persistente();

    $this->setTabela('tcers.fn_rel_modelos_executivo');
...
function montaRecuperaTodos()
{
    ...
    if ( ($this->getDado("inCodModelo") <> 1) AND ($this->getDado("inCodModelo") <> 2) AND ($this->getDado("inCodModelo") <> 9) ) {
    ...
    $stSql .= "  )                                                                                    ";
    if( $this->getDado('inCodQuadro') )
        $stSql .= " WHERE cod_quadro = " . $this->getDado('inCodQuadro');
