...
<?php
/**
    * Página de Formulário para efetuar Aplicações
...
$stHdnValor = "
    ...
    if (document.frm.inCodPlanoDebito.value == '') {
         erro = true;
         mensagem += '@Deve ser informada uma conta para aplicação!';
    }
    var stValor;
    ...
    if (erro == false) {
        if ( parseFloat(stVlTransf) > parseFloat(document.frm.nuSaldoContaAnalitica.value) ) { ;
            if ( confirm( 'O saldo da conta informada não é suficiente para realizar a aplicação.\\n (Saldo da conta: R$ '+document.frm.nuSaldoContaAnaliticaBR.value+')\\n Se efetuar esta aplicação, o saldo da conta ficará negativo. Deseja continuar?')) {
                erro = false
            } else erro = true;
        }
    } ";
...
$obTxtValor->setTitle    ( "Informe o Valor da aplicação." );
...
$obTxtObs->setTitle  ( "Informe as observações." );
...
$obFormulario->addTitulo    ( "Dados para Aplicações"             );
