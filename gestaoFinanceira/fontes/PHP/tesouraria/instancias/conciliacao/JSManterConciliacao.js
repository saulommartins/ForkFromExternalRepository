...
/**
    ...
    * $Id: JSManterConciliacao.js 66388 2016-08-22 14:22:27Z evandro $
...
function incluirMovimentacao()
{
    ...
    if(parent.frames['telaPrincipal'].document.frm.nuSaldoExtrato.value != ''){
        ...
        if(parent.frames['telaPrincipal'].document.frm.inCodTipoConciliacao ) {
            if(parent.frames['telaPrincipal'].document.frm.inCodTipoConciliacao.value == '') {
                mensagem += '@Campo Tipo de conciliação inválido!()';
            }
        }
        ...
        if( mensagem )
            ...
            valorContabilConciliado = parent.frames['telaPrincipal'].document.frm.nuSaldoContabilConciliado.value.replace(expRg,'');
            ...
            //retorna Tipo Conciliação
            if( parent.frames['telaPrincipal'].document.frm.inCodTipoConciliacao ) {
                parent.frames['telaPrincipal'].document.frm.inCodTipoConciliacao.value = '';
            }
        }
    }else{
...
function calculaSaldo()
{
    ...
    saldoConciliado = (jq('#nuSaldoContabilConciliado').val()).replace(expRg,'');
    ...
    var decimais;
    if( arSaldoConciliado[1] )
        decimais = arSaldoConciliado[1];
    else
        decimais = '00';
...
function ajustaSaldo(valor)
{
    ...
    expRg = new RegExp( "\\.", "g" );
    saldoContabilConciliado = parent.frames['telaPrincipal'].document.frm.nuSaldoContabilConciliado.value.replace(expRg,'');
    ...
    valorAdicionado = jq('#H'+valor3).html();
    valorAdicionado = valorAdicionado.replace(expRg,'');
    valorAdicionado = valorAdicionado.replace(',','.');

    if( ( $(valor).name.indexOf('boManual') > -1 ) || ( $(valor).name.indexOf('boPendencia') > -1 ))
        valorAdicionado = valorAdicionado*(-1);
    ...
    saldoConciliadoFinal = saldoConciliadoFinal.toString().replace('.',',');

    parent.frames['telaPrincipal'].document.frm.nuSaldoContabilConciliado.value = saldoConciliadoFinal;
    formataMoeda(parent.frames['telaPrincipal'].document.frm.nuSaldoContabilConciliado,2,'',true);
...
{  
    ...
    valorAdicionado = parent.frames['telaPrincipal'].document.getElementById('H'+valor3).innerHTML.replace(expRg,'');
    valorAdicionado = valorAdicionado.replace(',','.');
