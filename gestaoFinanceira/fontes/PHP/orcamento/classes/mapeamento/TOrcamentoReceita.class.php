  ...
  */
  ...
  function montaRecuperaReceitaExportacaoPlanejamento10() {
    $stSql = "SELECT tipo_registro,
                     ...
                     CASE
                       WHEN SUBSTR(natureza_receita::text, 1, 1) = '9'
                     ...
                     END AS vl_previsto
              FROM (SELECT 10::integer AS tipo_registro,
                           CASE
                             WHEN SUBSTR(conta_receita.cod_estrutural, 1, 1) = '9'
                               THEN SUBSTR(REPLACE(conta_receita.cod_estrutural, '.', ''), 1, 9)::integer
                             ELSE CASE
                                    WHEN SUBSTR(REPLACE(conta_receita.cod_estrutural, '.', ''), 1, 8)::INTEGER = 17240101 OR SUBSTR(REPLACE(conta_receita.cod_estrutural, '.', ''), 1, 8)::INTEGER = 17240102
                           ...
                           END AS vl_previsto
                    FROM orcamento.receita
                         LEFT JOIN orcamento.recurso('" . Sessao::getExercicio() . "') as rec ON rec.cod_recurso = receita.cod_recurso
                              AND rec.exercicio = receita.exercicio
                         JOIN orcamento.conta_receita ON conta_receita.cod_conta = receita.cod_conta
                              AND conta_receita.exercicio = receita.exercicio
                         JOIN administracao.configuracao_entidade ON configuracao_entidade.cod_entidade = receita.cod_entidade
                         ...
                         JOIN tcemg.fn_detalhamento_receitas('" . Sessao::getExercicio() . "', '', '" . $this->getDado('dt_inicial') . "', '" . $this->getDado('dt_final') . "', '" . $this->getDado('entidades') . "', '', '', '', '', '', '', '')
                              AS detalhamento_receitas (cod_estrutural varchar,
                                                        receita integer,
                                                        recurso varchar,
                                                        descricao varchar,
    ...
    $stSql.= "LEFT JOIN tcemg.receita_indentificadores_peculiar_receita ON receita_indentificadores_peculiar_receita.exercicio = receita.exercicio
                   AND receita_indentificadores_peculiar_receita.cod_receita = receita.cod_receita
              LEFT JOIN tcemg.valores_identificadores ON valores_identificadores.cod_identificador = receita_indentificadores_peculiar_receita.cod_identificador
    ...
    if (Sessao::getExercicio() == '2014') {
      $stSql.= "UNION
                ...
                FROM orcamento.receita
                     LEFT JOIN orcamento.recurso('" . Sessao::getExercicio() . "') as rec ON rec.cod_recurso = receita.cod_recurso
                          AND rec.exercicio = receita.exercicio
                     JOIN tesouraria.arrecadacao_receita_dedutora ON arrecadacao_receita_dedutora.cod_receita_dedutora = receita.cod_receita
                     ...
                     JOIN orcamento.conta_receita ON conta_receita.cod_conta = receita.cod_conta
                          AND conta_receita.exercicio = receita.exercicio
                    LEFT JOIN tcemg.receita_indentificadores_peculiar_receita ON receita_indentificadores_peculiar_receita.exercicio = receita.exercicio
                         AND receita_indentificadores_peculiar_receita.cod_receita = receita.cod_receita
                    LEFT JOIN tcemg.valores_identificadores ON valores_identificadores.cod_identificador = receita_indentificadores_peculiar_receita.cod_identificador
                WHERE receita.exercicio = '" . Sessao::getExercicio() . "'
                      AND receita.cod_entidade IN (" . $this->getDado('entidades') . ")
  ...
  function montaRecuperaReceitaExportacaoPlanejamento11() {
    $stSql = "SELECT 11 AS tipo_registro,
              ...
              FROM orcamento.receita
                   ...
                   JOIN (SELECT cod_estrutural,
                         ...
                         FROM (SELECT *
                               FROM tcemg.fn_detalhamento_receitas('" . Sessao::getExercicio() . "', '', '" . $this->getDado('dt_inicial') . "', '" . $this->getDado('dt_final') . "', '" . $this->getDado('entidades') . "', '', '', '', '', '', '', '')
                                    AS detalhamento_receitas(cod_estrutural varchar,
                                                             receita integer,
                                                             recurso varchar,
                                                             descricao varchar,
    ...
    $stSql.= ") AS detalhamento
              GROUP BY cod_estrutural,
                       receita,
                       recurso,
                       descricao) AS detalhamento_receitas ON detalhamento_receitas.cod_estrutural = conta_receita.cod_estrutural
              WHERE receita.exercicio = '" . Sessao::getExercicio() . "'
                    AND receita.cod_entidade IN (" . $this->getDado('entidades') . ")
                    AND receita.vl_original <> 0.00
