...
class FOrcamentoAnexo1Receita extends Persistente
{
/**
    * Método Construtor
    * @access Private
*/
function FOrcamentoAnexo1Receita()
{
    parent::Persistente();
    $this->setTabela('orcamento.fn_orcamento_somatorio_receita');
...
function montaRecuperaTodos()
{
    ...
    $stSql .= "     fnorcamentoanexo2receita (nivel) as coluna                                      \n";
    $stSql .= " FROM                                                                                \n";
    $stSql .= "               ".$this->getTabela()."('".$this->getDado("exercicio")."','".$this->getDado("stFiltro")."','".$this->getDado("inCodEntidade")."')                \n";
...
{
    $this->setTabela('orcamento.fn_orcamento_somatorio_receita_balanco');
...
function montaRecuperaTodosBalanco()
{
    ...
    $stSql .= "     fnorcamentoanexo2receita (nivel) as coluna                                      \n";
    $stSql .= " FROM                                                                                \n";
    $stSql .= "               ".$this->getTabela()."('".$this->getDado("exercicio")."','".$this->getDado("stFiltro")."','".$this->getDado("inCodEntidade")."','".$this->getDado("stDataInicial")."','".$this->getDado("stDataFinal")."')                \n";
    $stSql .= "     as retorno(cod_conta integer, nivel integer, exercicio char(4), descricao varchar,                 \n";
    $stSql .= "               classificacao varchar,classificacao_reduzida varchar,                 \n";
    $stSql .= "               valor numeric)                                                        \n";
