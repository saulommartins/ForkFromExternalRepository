...
<?php
/**
    * Página de Formulario de Oculto de Previsão Receita
    ...
    * @ignore
    ...
    $Name$
    $Autor: $
    $Date: 2007-06-04 12:33:40 -0300 (Seg, 04 Jun 2007) $
    ...
    case 'mudaValor':
        ...
        if ($vlTotal != 0.00) {
            ...
            for ($x = 1; $x <= $_GET['inNumCampos']; $x++) {
                ...
                $vlAuxSomatorio = bcadd($vlAuxSomatorio,number_format($vlPeriodo,2,'.',''),6);
            ...
            $vlAuxTotalFinal = $vlAuxSomatorio - $vlSomatorioPeriodo;
            $vlAuxPeriodo = $vlPeriodo + ($vlAuxTotalFinal*-1);
            
            $js .= 'f.vlValor_'.($x-1).'.value = "'.number_format($vlAuxPeriodo, 2, ",", ".").'";';
            $js .= 'f.TotalValor.value = "'.number_format($vlSomatorioPeriodo,2, ",", ".").'";';
            $js .= 'f.TotalPorcento.value = "'.number_format($vlSomatorioPercentual, 2, ",", ".").'";';
        } else {
            for ($x = 1; $x <= $_GET['inNumCampos']; $x++) {
        ...
        echo $js;
    break;
    case 'montaContaCreditoTributario':
        ...
        $obCmbEmpenhoCredito->setName       ('inCodContaCreditoTributario');
        $obCmbEmpenhoCredito->setId         ('inCodContaCreditoTributario');
        $obCmbEmpenhoCredito->setRotulo     ('Conta Crédito Tributário');
        ...
        $obCmbEmpenhoCredito->setValue      ($request->get('inCodContaCreditoTributario'));
        $obCmbEmpenhoCredito->setStyle      ('width: 520');

        $obFormulario->addComponente($obCmbEmpenhoCredito);
        $obFormulario->montaInnerHTML();

        echo "document.getElementById('spnContaCreditoTributario').innerHTML = '".$obFormulario->getHTML()."';";
    break;
    case 'desmontaContaCreditoTributario':
        echo "document.getElementById('spnContaCreditoTributario').innerHTML = '';";
    break;
