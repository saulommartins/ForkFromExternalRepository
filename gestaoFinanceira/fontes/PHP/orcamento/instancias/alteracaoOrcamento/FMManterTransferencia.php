...
<?php
/**
    * Página de Formulário de Transferencias
...
if ($rsUltimoMesEncerrado->getCampo('mes') >= $mesAtual AND $boUtilizarEncerramentoMes == 'true') {
    ...
    // TIPO DE CRÉDITO
    if ($stAcao == 'Remaneja') {
        $stTituloForm = "Dados para Transferência de Recursos por Remanejamentos";
        $inCodTipo = 12;
    } elseif ($stAcao == 'Transpos') {
        $stTituloForm = "Dados para Transferência de Recursos por Transposição";
        $inCodTipo = 13;
    } elseif ($stAcao == 'Transfere') {
        $stTituloForm = "Dados para Transferência de Recursos por Transferência";
        $inCodTipo = 14;
    }
    ...
    $obBscHistorico->setRotulo ( "Histórico Contábil" );
    ...
    $obBscHistorico->obCampoCod->obEvento->setOnChange("buscaDado('buscaHistorico');");
