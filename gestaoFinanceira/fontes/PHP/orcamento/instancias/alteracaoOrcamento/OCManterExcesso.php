...
<?php
 /**
    * Paginae Oculta de Credito Especial/Suplementar por Excesso de arrecadação
...
switch ($stCtrl) {
    ...
    case 'buscaDespesa':
    if ($_REQUEST["inCodDespesaSuplementar"] != "" and $_REQUEST["inCodEntidade"] != "") {
        ...
        if (!$stNomDespesa) {
            ...
            $js .= "alertaAviso('@Valor inválido.(".$_REQUEST["inCodDespesaSuplementar"].")','form','erro','".Sessao::getId()."');";
        } else {
            $js = 'd.getElementById("stNomDespesaSuplementar").innerHTML ="'.$stNomDespesa.'";';
        }
    } else {
        $js = 'd.getElementById("stNomDotacaoSuplementar").innerHTML ="&nbsp;";';
    }
    ...
    case 'incluirDespesaSuplementar':
        ...
        $nuVlSomatoria   = (float) $nuVlSuplementar;
        ...
        $nuVlSomatoria = number_format($nuVlSomatoria,2,'.','');
        $nuVlTotal = number_format($nuVlTotal,2,'.','');

        if ( ($nuVlTotal - $nuVlSomatoria) >= 0 ) {
