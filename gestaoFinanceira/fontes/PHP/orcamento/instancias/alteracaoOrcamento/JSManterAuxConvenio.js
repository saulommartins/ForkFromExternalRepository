...
/**
    ...
    * @ignore
    ...
    $Id: JSManterAuxConvenio.js 62838 2015-06-26 13:02:49Z diogo.zarpelon $
...
function incluirDespesaSuplementar() {
    ...
    var nuVlTotal = jq('#nuVlTotal').val();
    nuVlTotal = nuVlTotal.replace(new RegExp("[.]","g"),'');
    nuVlTotal = nuVlTotal.replace(",",".");
    if(nuVlDotacaoSuplementar == 0){
        mensagem += '@Campo Valor inválido!( o valor deve ser maior que 0 (zero) )';
    }
    if(nuVlTotal == ''){
        mensagem += '@Campo Valor Total inválido!( o valor deve ser maior que 0 (zero) )';   
    }
    if( mensagem != '' ) {
