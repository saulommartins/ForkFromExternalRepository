...
<?php
/**
    * Página de Formulário de Cred Especial/Suplementar por Operação de Credito
...
if ($rsUltimoMesEncerrado->getCampo('mes') >= $mesAtual AND $boUtilizarEncerramentoMes == 'true') {
    ...
    // Define Objeto Data  para Data de Lancamento
    ...
    if ($stAcao == 'Especial') {
        $obBscDespesaSuplementar->obCampoCod->obEvento->setOnBlur("buscaDado('buscaDespesaEspecial');");
        ...
        $obBscDespesaSuplementar->setFuncaoBusca("abrePopUp('".CAM_GF_ORC_POPUPS."despesa/LSDespesa.php','frm','inCodDespesaSuplementar','stNomDespesaSuplementar','&inCodEntidade='+document.frm.inCodEntidade.value,'".Sessao::getId()."','800','550');");
    }
    ...
    $obTxtVlSuplementar->setValue     ( $nuVlSuplementar  );
    ...
    if ($stAcao == 'Suplementa') {
        $obFormulario->addTitulo            ( "Dados para Crédito Suplementar por Operação de Crédito" );
    } else {
        $obFormulario->addTitulo            ( "Dados para Crédito Especial por Operação de Crédito" );
    }
