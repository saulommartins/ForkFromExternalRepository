...
?>
    <?php
/**
  * Pagina Oculta de Configurar Conta contabil Restos a Pagar Não Processados
...
switch ($_REQUEST['stCtrl']) {
    ...
    case "carregaListaPlanoContaEntidade" :
        $obTContaEntidade = new TContabiliadadeContaContabilRpNp();
    ...
    case "inserirPlanoContaEntidade" :
        ...
        if (!empty($_GET['inCodEntidade']) && !empty($_GET['stCodReduzido'])) {
            ...
            $obTEntidade->setDado('cod_entidade' , $_GET['inCodEntidade']);
            $obTEntidade->setDado('exercicio', Sessao::getExercicio());
            $obTEntidade->recuperaRelacionamentoNomes($rsEntidade);
            ...
            if ($obDuplicado)
                ...
                $arValores[$inCount][ 'nom_cgm'        ] = $rsEntidade->getCampo( 'entidade' );
