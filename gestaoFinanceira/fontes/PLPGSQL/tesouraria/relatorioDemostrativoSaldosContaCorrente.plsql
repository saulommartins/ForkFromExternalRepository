...
*/
CREATE OR REPLACE FUNCTION tesouraria.fn_relatorio_demostrativo_saldos_conta_corrente(varchar, integer, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
stSql := '
    ...
    FROM (
        SELECT
            ...
            ,CPC.cod_estrutural
            ,contas_ctb.desc_conta_bancaria
            ,CPC.nom_conta
            ,coalesce( 
                tesouraria.fn_saldo_conta_tesouraria( 
                    ...
                    IF stDtInicial = stDataAno THEN 
                        stSql := stSql || '''' || stDtInicial || ''', ';
                    ELSE 
                        stSql := stSql || ' TO_CHAR(TO_DATE( ''' || stDtInicial || ''',''dd/mm/yyyy'') -1, ''dd/mm/yyyy''), ';
                    END IF;
        ...
        FROM 
            ...
            INNER JOIN contabilidade.plano_banco AS CPB
                ON CPA.exercicio    = CPB.exercicio                       
                AND CPA.cod_plano    = CPB.cod_plano                       
            
            INNER JOIN contabilidade.plano_recurso   AS CPR
            ...
            INNER JOIN (    SELECT * 
                            FROM tcemg.contas_ctb_conta_corrente('''|| stExercicio ||''', '''|| inCodEntidade ||''') 
                            as conta_bancaria
                                           (  cod_conta                         INTEGER
        ...
        WHERE 
            ...
            AND CPB.cod_entidade = ' || inCodEntidade || ' 
    ...
    IF inCodRecurso is not null AND inCodRecurso != '' THEN
        stSql := stSql || ' AND CPR.cod_recurso = ' || inCodRecurso;
    END IF;
    ...
    IF inCodEntidade = 3 THEN
        stSql := stSql || ' OR tbl.cod_estrutural like ''1.1.%'' ';
    END IF;
