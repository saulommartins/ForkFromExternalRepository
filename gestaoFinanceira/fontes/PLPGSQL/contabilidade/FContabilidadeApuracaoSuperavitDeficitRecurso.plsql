...
* Casos de uso: uc-02.02.22
                uc-02.08.07
*/
CREATE OR REPLACE FUNCTION contabilidade.fn_rl_apuracao_superavit_defict_recurso(VARCHAR, VARCHAR, VARCHAR, VARCHAR, CHAR) RETURNS SETOF RECORD AS $$ 
DECLARE
...
BEGIN
    ...
    ELSE
        stSql := ' SELECT
                        COALESCE(plano_recurso.cod_recurso, ''100'')::INTEGER AS recurso
                        ,pc.indicador_superavit
                        ,0.00 as vl_saldo_anterior
                        ,0.00 as vl_saldo_debitos
                        ,0.00 as vl_saldo_creditos
                    ...
                    WHERE   pc.exercicio   = ' || quote_literal(stExercicio) || '
                      AND   pc.exercicio   = sc.exercicio
                      AND   pc.cod_sistema = sc.cod_sistema
                      AND pc.indicador_superavit = ''financeiro''
                    GROUP BY recurso
                           , pc.indicador_superavit
                    ORDER BY recurso ';
    END IF;
