...
*/
/*
 * PL do Relatório  de Insuficiencia por Destinacao de Recurso
...
CREATE OR REPLACE FUNCTION contabilidade.relatorio_insuficiencia_destinacao_recurso(varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
  ...
  stSql := '
    CREATE TEMPORARY TABLE tmp_recursos_processados_exercicios_anteriores AS
      ...
      SELECT busca_recurso.num_recurso
...
--left para achar o cod_estrutural
     ...
     LEFT JOIN ( SELECT despesa.exercicio
                      , despesa.cod_despesa
                      , recurso.masc_recurso_red AS num_recurso
                      , recurso.cod_recurso
                      , recurso.nom_recurso
              ...
              LEFT JOIN orcamento.recurso(''' || stExercicio || ''') AS recurso
                  ...
                  WHERE despesa.cod_entidade IN ('||stCodEntidade||')      
             ) as busca_recurso
     ...
     LEFT JOIN empenho.restos_pre_empenho
      ...
      GROUP BY busca_recurso.num_recurso
             , busca_recurso.cod_recurso
             , liquidado.cod_empenho
             , liquidado.cod_entidade
             , busca_recurso.nom_recurso
     ...
     LEFT JOIN (  SELECT ( SUM(liquidacao_paga.vl_total) ) AS vl_pago
              ...
              INNER JOIN (  SELECT nota_liquidacao_paga.exercicio
                         ...
                         LEFT JOIN (  SELECT nota_liquidacao_paga_anulada.exercicio
                                           ...
                                           , SUM(COALESCE(nota_liquidacao_paga_anulada.vl_anulado,0.00)) AS vl_anulado
                                        FROM empenho.nota_liquidacao_paga_anulada
                                       WHERE TO_DATE(nota_liquidacao_paga_anulada.timestamp_anulada,''yyyy-mm-dd'') BETWEEN TO_DATE(''01/01/'||stExercicioAnterior||'''::varchar,''dd/mm/yyyy'') AND TO_DATE(''31/12/'||stExercicio||'''::varchar,''dd/mm/yyyy'')
                                         AND nota_liquidacao_paga_anulada.cod_entidade IN ('||stCodEntidade||')
                                    GROUP BY nota_liquidacao_paga_anulada.exercicio
                                           , nota_liquidacao_paga_anulada.cod_entidade
                                           , nota_liquidacao_paga_anulada.cod_nota
        ...
        FROM (  SELECT (  SUM(COALESCE(item_pre_empenho.vl_total,0.00))
            ...
            INNER JOIN empenho.item_pre_empenho
             ...
             LEFT JOIN (  SELECT empenho_anulado_item.exercicio
                           ...
                           WHERE TO_DATE(empenho_anulado_item.timestamp,''yyyy-mm-dd'') <= TO_DATE(''31/12/'||stExercicio||'''::varchar,''dd/mm/yyyy'') 
...
UPDATE tmp_recursos_processados_exercicios_anteriores     SET num_recurso = '0.0.00', nom_recurso = 'Não Informado' WHERE num_recurso is null;
UPDATE tmp_recursos_processados_exercicio_anterior        SET num_recurso = '0.0.00', nom_recurso = 'Não Informado' WHERE num_recurso is null;
UPDATE tmp_recursos_nao_processados_exercicios_anteriores SET num_recurso = '0.0.00', nom_recurso = 'Não Informado' WHERE num_recurso is null;
UPDATE tmp_recursos_nao_processados_exercicio_anterior    SET num_recurso = '0.0.00', nom_recurso = 'Não Informado' WHERE num_recurso is null;
...
--TODO
       ...
       UNION
      SELECT recurso.masc_recurso_red AS num_recurso
           , recurso.cod_recurso
           , recurso.nom_recurso
  ...
  INNER JOIN orcamento.recurso(''' || stExercicio || ''') AS recurso
    ...
    GROUP BY num_recurso, cod_recurso, nom_recurso, cod_entidade
  ';
  ...
  stSql := '
      ...
      SELECT *
        FROM contabilidade.totaliza_destinacao_recurso_liquidado( '''||stCodEntidade||''', '''||stExercicio||''', '''||dtInicial||''', '''||dtFinal||''', '''||dtFinal||''' ) as retorno
             ( num_recurso varchar, cod_recurso integer, nom_recurso varchar, cod_entidade integer, exercicio varchar, cod_plano_debito varchar, liquidados_nao_pagos numeric, empenhados_nao_liquidados numeric)
  ...
  GROUP BY tlnl.num_recurso
    ...
    SELECT tmp_recursos_orgao.num_recurso
         , tmp_recursos_orgao.cod_recurso
 ...
 LEFT JOIN ( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
                  ...
                  , cod_entidade
               FROM tmp_recursos_processados_exercicios_anteriores
           GROUP BY num_recurso, cod_recurso, cod_entidade
           ) as trpeas
        ON trpeas.num_recurso  = tmp_recursos_orgao.num_recurso
 ...
 LEFT JOIN ( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
        ...
        ON trpea.num_recurso  = tmp_recursos_orgao.num_recurso
 ...
 LEFT JOIN ( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
        ...
        ON trnpeas.num_recurso  = tmp_recursos_orgao.num_recurso
 ...
 LEFT JOIN ( SELECT SUM(coalesce(vl_total,0.00)) as vl_total
        ...
        ON trnpea.num_recurso  = tmp_recursos_orgao.num_recurso
       AND trnpea.cod_entidade = tmp_recursos_orgao.cod_entidade

   GROUP BY tmp_recursos_orgao.num_recurso, tmp_recursos_orgao.cod_recurso, tmp_recursos_orgao.nom_recurso,tmp_recursos_orgao.cod_entidade
