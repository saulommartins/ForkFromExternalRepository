...
* $Name$
* $Author: tonismar $
* $Date: 2007-09-21 17:46:17 -0300 (Sex, 21 Set 2007) $
*
* Casos de uso: uc-02.02.15, uc-02.02.08
*/

--DROP FUNCTION contabilidade.fn_rl_demonstrativo_despesa(varchar, varchar, varchar, varchar, varchar);
CREATE OR REPLACE FUNCTION contabilidade.fn_rl_demonstrativo_despesa (varchar, varchar, varchar, varchar, varchar ) RETURNS SETOF RECORD AS
'
DECLARE
...
AND  to_date( '' || quote_literal(stDtFinal)   || '' , '' || quote_literal(''dd/mm/yyyy'') || ''   ) '';
    ...
    stSql := stSql || '' 
        FROM
                ...
                ON (    D.exercicio = ocd.exercicio
                    AND D.cod_conta = ocd.cod_conta
                )
...
-- COMENTADO NO TICKET 10047            
    
--	            JOIN (	SELECT 	 ipe.exercicio
--								,ipe.cod_pre_empenho
...
--                             ) as anu
--                        ON
--                                anu.exercicio = ipe.exercicio
--                            and anu.cod_pre_empenho = ipe.cod_pre_empenho
--                        GROUP BY ipe.exercicio, ipe.cod_pre_empenho, anu.vl_anulado
--	            ) as ipe ON (
--	            	pe.cod_pre_empenho  = ipe.cod_pre_empenho
--	            	AND pe.exercicio	= ipe.exercicio
--	            )
--	            WHERE e.dt_empenho  BETWEEN to_date('''''' || stDtInicial || '''''',''''dd/mm/yyyy'''') 
--									AND     to_date('''''' || stDtFinal || '''''',''''dd/mm/yyyy'''')
...
--	            ORDER BY ocd.cod_estrutural    
        ...
        (
            ...
            FROM
                    ...
                    WHERE
                        e.dt_empenho BETWEEN to_date('''''' || stDtInicial || '''''',''''dd/mm/yyyy'''')
                                     AND     to_date('''''' || stDtFinal || '''''',''''dd/mm/yyyy'''')
	    ...
	    IF stSituacao = ''Liquidado'' THEN
		    stSql := stSql || ''    ---liquidações do periodo
                               LEFT JOIN (  select nota_liquidacao.exercicio_empenho
                                            ...
                                            where nota_liquidacao.dt_liquidacao BETWEEN to_date('''''' || stDtInicial || '''''',''''dd/mm/yyyy'''')
                                                                                AND     to_date('''''' || stDtFinal   || '''''',''''dd/mm/yyyy'''')
