...
CREATE OR REPLACE FUNCTION contabilidade.fn_busca_historico_liquidacao(INTEGER, INTEGER, VARCHAR, VARCHAR) RETURNS varchar AS $$
DECLARE
...
BEGIN
    ...
    stSql := '
        SELECT descricao || credor
          FROM (
                SELECT CASE WHEN REPLACE(TRIM(COALESCE(nl.observacao,'''')),''\r\n'','''') = '''' 
                            THEN ''''
                            ELSE REPLACE(TRIM(COALESCE(nl.observacao,'''')),''\r\n'','''') || '' - '' 
                       END AS descricao
