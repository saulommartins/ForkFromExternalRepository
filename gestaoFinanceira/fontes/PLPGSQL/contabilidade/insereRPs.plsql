 ...
 *  $Id: gerarRestosPagar.plsql 46711 2012-05-29 14:33:35Z tonismar $
*/

CREATE OR REPLACE FUNCTION contabilidade.fn_insere_rps(varchar,varchar,varchar) RETURNS BOOLEAN AS $$
DECLARE
...
--------------------------------------------------------------
  ...
  stSql := '
    ...
    SELECT * FROM
    (
        SELECT    trpeas.cod_empenho
                , trpeas.cod_entidade
                , 1 AS tipo
        FROM tmp_recursos_orgao
        
        JOIN ( SELECT   cod_empenho
                      , cod_recurso
                      , cod_entidade
                FROM tmp_recursos_processados_exercicios_anteriores
            GROUP BY cod_recurso, cod_entidade, cod_empenho
            ) as trpeas
        ...
        SELECT    trnpeas.cod_empenho
                , trnpeas.cod_entidade
                , 2 AS tipo
        FROM tmp_recursos_orgao
   ...
   GROUP BY resultado.tipo, resultado.cod_empenho,resultado.cod_entidade
  ...
  LOOP
        IF reRegistro.tipo = 1 THEN
            UPDATE empenho.empenho SET restos_pagar = 'Processado' WHERE empenho.cod_empenho = reRegistro.cod_empenho AND empenho.cod_entidade = reRegistro.cod_entidade
                                                                     AND empenho.exercicio BETWEEN quote_literal(stExercicioAnterior) AND quote_literal(stExercicio);
            GET DIAGNOSTICS inErro := ROW_COUNT;
            inCount := inCount + inErro;
        ELSE IF reRegistro.tipo = 2 THEN
                UPDATE empenho.empenho SET restos_pagar = 'Não Processados a Liquidar' WHERE empenho.cod_empenho = reRegistro.cod_empenho AND empenho.cod_entidade = reRegistro.cod_entidade
                                                                                        AND empenho.exercicio BETWEEN quote_literal(stExercicioAnterior) AND quote_literal(stExercicio);
                GET DIAGNOSTICS inErro := ROW_COUNT;
                inCount := inCount + inErro;
            END IF;
