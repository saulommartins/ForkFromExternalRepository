...
CREATE OR REPLACE FUNCTION contabilidade.balanco_patrimonial_compensacao_ativo (varchar, varchar, varchar, varchar) RETURNS SETOF RECORD AS $$
...
BEGIN
    ...
    stSql := '
        SELECT CAST(cod_estrutural AS VARCHAR) as cod_estrutural
          ...
          FROM (
                --CONTA SALDOS DOS ATOS POTENCIAIS ATIVOS
                SELECT ''8.1.0.0.0.00.00'' as cod_estrutural
                     , ARRAY[0.00,0.00,0.00,0.00] as valores
                     , 1 as nivel
                     , ''Saldos dos Atos Potenciais Ativos'' as nom_conta
                     , 1 as  multiplicador

            UNION ALL

                --CONTA EXECUÇÃO DOS ATOS POTENCIAIS ATIVOS
                SELECT '||quote_literal('8.1.1.0.0.00.00')||' as cod_estrutural
                     , contabilidade.totaliza_balanco_patrimonial( publico.fn_mascarareduzida('||quote_literal('8.1.1.0.0.00.00')||') ) as valores
                     , publico.fn_nivel('||quote_literal('8.1.1.0.0.00.00')||') as nivel
                     , ''Execução dos Atos Potenciais Ativos'' as nom_conta
                     , 1 as  multiplicador

            UNION ALL

                --CONTA EXECUÇÃO DE GARANTIAS E CONTRAGARANTIAS RECEBIDAS A EXECUTAR
                SELECT '||quote_literal('8.1.1.1.1.00.00')||' as cod_estrutural
                     , contabilidade.totaliza_balanco_patrimonial( publico.fn_mascarareduzida('||quote_literal('8.1.1.1.1.00.00')||') ) as valores
                     , publico.fn_nivel('||quote_literal('8.1.1.0.0.00.00')||') as nivel
                     , ''Execução De Garantias e Contragarantias Recebidas a executar'' as nom_conta
                     , 1 as  multiplicador

            UNION ALL

                --CONTA EXECUÇÃO DE DIREITOS CONVENIADOS E OUTROS INSTRUMENTOS CONGÊNERES A RECEBER
                SELECT '||quote_literal('8.1.1.2.1.00.00')||' as cod_estrutural
                     , contabilidade.totaliza_balanco_patrimonial( publico.fn_mascarareduzida('||quote_literal('8.1.1.2.1.00.00')||') ) as valores
                     , publico.fn_nivel('||quote_literal('8.1.1.0.0.00.00')||') as nivel
                     , ''Execução De Direitos Conveniados E Outros Instrumentos Congêneres a receber'' as nom_conta
                     , 1 as  multiplicador

            UNION ALL

                --CONTA EXECUÇÃO DE DIREITOS CONTRATUAIS EM EXECUÇÃO
                SELECT '||quote_literal('8.1.1.3.1.00.00')||' as cod_estrutural
                     , contabilidade.totaliza_balanco_patrimonial( publico.fn_mascarareduzida('||quote_literal('8.1.1.3.1.00.00')||') ) as valores
                     , publico.fn_nivel('||quote_literal('8.1.1.0.0.00.00')||') as nivel
                     , ''Execução De Direitos Contratuais em execução'' as nom_conta
                     , 1 as  multiplicador

            UNION ALL

                --CONTA EXECUÇÃO DE OUTROS ATOS POTENCIAIS ATIVOS A EXECUTAR
                SELECT '||quote_literal('8.1.1.9.1.00.00')||' as cod_estrutural
                     , contabilidade.totaliza_balanco_patrimonial( publico.fn_mascarareduzida('||quote_literal('8.1.1.9.1.00.00')||') ) as valores
                     , publico.fn_nivel('||quote_literal('8.1.1.0.0.00.00')||') as nivel
                     , ''Execução De Outros Atos Potenciais Ativos a executar'' as nom_conta
                     , 1 as  multiplicador
            ) as tabela
