...
CREATE OR REPLACE FUNCTION contabilidade.restos_pagar_processados(varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
--  LAÇO PARA CALCULO DOS VALORES
	...
	FOR indice IN 1..array_upper(arTipo,1) LOOP 
      ...
      stSql := 'SELECT
                    ...
                    -- tmp_processados_exercicios_anteriores
                    ...
                    (  SELECT COALESCE(SUM(vl_total), 0.00)::numeric(14,2)
                         FROM tmp_restos
                        WHERE tipo = ''tmp_processados_exercicio_anterior''
                          AND cod_estrutural LIKE '''||arTipo[indice]||''') AS exercicio_anterior,
                    
                    -- tmp_processados_exercicio_anterior
                    (   SELECT COALESCE(SUM(vl_total), 0.00)::numeric(14,2)
                          FROM tmp_restos
                         WHERE tipo = ''tmp_processados_cancelado''   
                           AND cod_estrutural LIKE '''||arTipo[indice]||''') AS cancelados,
                    
                    -- tmp_processados_exercicio_anterior
                    (  SELECT COALESCE(SUM(vl_total), 0.00)::numeric(14,2)
                        FROM tmp_restos
                       WHERE tipo = ''tmp_processados_pago''
