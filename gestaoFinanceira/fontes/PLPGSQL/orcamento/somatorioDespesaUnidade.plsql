...
CREATE OR REPLACE FUNCTION orcamento.fn_somatorio_despesa_unidade(varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    EXECUTE stSql;
        ...
        FOR reRegistro IN
            SELECT   DISTINCT on (num_orgao,num_unidade) *
            FROM     tmp_despesa
            ORDER BY num_orgao
        LOOP
            FOR reRegistro2 IN
            ...
            LOOP
                ...
                IF nuSoma <> 0.00 THEN
                    ...
                    INSERT INTO tmp_relatorio (num_orgao, num_unidade, cod_conta, classificacao, classificacao_reduzida, nivel, exercicio, descricao, vl_original) VALUES (reRegistro.num_orgao, reRegistro.num_unidade, reRegistro2.cod_conta, reRegistro2.classificacao, reRegistro2.classificacao_reduzida, reRegistro2.nivel, reRegistro2.exercicio, reRegistro2.descricao, nuSoma);
                END IF;
            END LOOP;
        END LOOP;
