...
CREATE OR REPLACE FUNCTION orcamento.fn_totaliza_despesa_unidade(varchar,integer,integer) RETURNS numeric(14,2) AS '
DECLARE
    stMascaraReduzida   ALIAS FOR $1;
    nuOrgao             ALIAS FOR $2;
    nuUnidade           ALIAS FOR $3;
    stSql               VARCHAR   := '''';
    nuSoma              NUMERIC   := 0;
    crCursor            REFCURSOR;
...
BEGIN
    stSql := ''SELECT
                ...
                AND     num_orgao   = ''||nuOrgao||''
                AND     num_unidade = ''||nuUnidade||''
                '';
