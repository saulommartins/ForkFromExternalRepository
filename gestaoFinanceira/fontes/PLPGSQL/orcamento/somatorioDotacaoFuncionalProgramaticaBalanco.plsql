...
/*
$Log$
Revision 1.4  2006/07/05 20:38:05  cleisson
Adicionada tag Log aos arquivos

*/

CREATE OR REPLACE FUNCTION orcamento.fn_somatorio_dotacao_funcional_programatica_balanco(varchar,varchar,varchar,varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
IF ( stSituacao = 'empenhados' ) THEN
    stSql := 'CREATE TEMPORARY TABLE tmp_empenhado AS (
       SELECT
            ...
            orcamento.fn_consulta_funcional_programatica(d.exercicio,d.cod_despesa)  as dotacao
        FROM
        ...
        stSql := 'CREATE TEMPORARY TABLE tmp_anulado AS (
            ...
            orcamento.fn_consulta_funcional_programatica(od.exercicio,od.cod_despesa)  as dotacao
               ...
               Where
                 ...
                 AND to_date(to_char(EEAI.timestamp,''dd/mm/yyyy''),''dd/mm/yyyy'') BETWEEN to_date('''||stDataInicial||'''::varchar,''dd/mm/yyyy'') AND to_date('''||stDataFinal||'''::varchar,''dd/mm/yyyy'')';
 ...
 IF ( stSituacao = 'pagos' ) THEN
        ...
        WHERE
            ...
            AND to_date(to_char(ENLP.timestamp,''dd/mm/yyyy''),''dd/mm/yyyy'') BETWEEN to_date('''||stDataInicial||'''::varchar,''dd/mm/yyyy'') AND to_date('''||stDataFinal||'''::varchar,''dd/mm/yyyy'')';
...
stSql := stSql || ')';
        ...
        And ENL.exercicio            = ENLP.exercicio
            ...
            AND to_date(to_char(ENLPA.timestamp_anulada,''dd/mm/yyyy''),''dd/mm/yyyy'') BETWEEN to_date('''||stDataInicial||'''::varchar,''dd/mm/yyyy'') AND to_date('''||stDataFinal||'''::varchar,''dd/mm/yyyy'')';
        stSql := stSql || ')';
        EXECUTE stSql;
  END IF;
  ...
  IF ( stSituacao = 'liquidados' ) THEN
    ...
    stSql := 'CREATE TEMPORARY TABLE tmp_liquidado_estornado AS (
        ...
        stSql := stSql || '
          ...
          And EPED.exercicio           = EPE.exercicio
          And EPED.cod_pre_empenho     = EPE.cod_pre_empenho 
          AND to_date(to_char(ENLIA.timestamp,''dd/mm/yyyy''),''dd/mm/yyyy'') BETWEEN to_date('''||stDataInicial||'''::varchar,''dd/mm/yyyy'') AND to_date('''||stDataFinal||'''::varchar,''dd/mm/yyyy'')';
        stSql := stSql || ')';
        EXECUTE stSql;
  END IF;
        ...
        stSql := 'CREATE TEMPORARY TABLE tmp_despesa AS
                SELECT
                     *
                     ,orcamento.fn_consulta_funcional_programatica(exercicio, cod_despesa)  as dotacao
                     ,orcamento.fn_consulta_funcional_programatica(exercicio, cod_despesa)  as dotacao_sem_ponto
    ...
    LOOP
IF ( stSituacao = 'empenhados' ) THEN
            reRegistro.vl_projeto   := orcamento.fn_totaliza_dotacao_funcional_empenhado(arMascDespesa[6],stDigitoProjeto,reRegistro.dotacao,inPosicao) -  orcamento.fn_totaliza_dotacao_funcional_anulado(arMascDespesa[6],stDigitoProjeto,reRegistro.dotacao,inPosicao);
            reRegistro.vl_atividade := orcamento.fn_totaliza_dotacao_funcional_empenhado(arMascDespesa[6],stDigitoAtividade,reRegistro.dotacao,inPosicao)- orcamento.fn_totaliza_dotacao_funcional_anulado(arMascDespesa[6],stDigitoAtividade,reRegistro.dotacao,inPosicao);
            reRegistro.vl_operacao  := orcamento.fn_totaliza_dotacao_funcional_empenhado(arMascDespesa[6],stDigitoOperacao,reRegistro.dotacao,inPosicao) - orcamento.fn_totaliza_dotacao_funcional_anulado(arMascDespesa[6],stDigitoOperacao,reRegistro.dotacao,inPosicao);
        END IF;
        IF ( stSituacao = 'liquidados' ) THEN
            reRegistro.vl_projeto   := orcamento.fn_totaliza_dotacao_funcional_liquidado(arMascDespesa[6],stDigitoProjeto,reRegistro.dotacao,inPosicao) -  orcamento.fn_totaliza_dotacao_funcional_liquidado_estornado(arMascDespesa[6],stDigitoProjeto,reRegistro.dotacao,inPosicao);
            reRegistro.vl_atividade := orcamento.fn_totaliza_dotacao_funcional_liquidado(arMascDespesa[6],stDigitoAtividade,reRegistro.dotacao,inPosicao)- orcamento.fn_totaliza_dotacao_funcional_liquidado_estornado(arMascDespesa[6],stDigitoAtividade,reRegistro.dotacao,inPosicao);
            reRegistro.vl_operacao  := orcamento.fn_totaliza_dotacao_funcional_liquidado(arMascDespesa[6],stDigitoOperacao,reRegistro.dotacao,inPosicao) - orcamento.fn_totaliza_dotacao_funcional_liquidado_estornado(arMascDespesa[6],stDigitoOperacao,reRegistro.dotacao,inPosicao);
        END IF;
        IF ( stSituacao = 'pagos' ) THEN
            reRegistro.vl_projeto   := orcamento.fn_totaliza_dotacao_funcional_pago(arMascDespesa[6],stDigitoProjeto,reRegistro.dotacao,inPosicao) -  orcamento.fn_totaliza_dotacao_funcional_estornado(arMascDespesa[6],stDigitoProjeto,reRegistro.dotacao,inPosicao);
            reRegistro.vl_atividade := orcamento.fn_totaliza_dotacao_funcional_pago(arMascDespesa[6],stDigitoAtividade,reRegistro.dotacao,inPosicao)- orcamento.fn_totaliza_dotacao_funcional_estornado(arMascDespesa[6],stDigitoAtividade,reRegistro.dotacao,inPosicao);
            reRegistro.vl_operacao  := orcamento.fn_totaliza_dotacao_funcional_pago(arMascDespesa[6],stDigitoOperacao,reRegistro.dotacao,inPosicao) - orcamento.fn_totaliza_dotacao_funcional_estornado(arMascDespesa[6],stDigitoOperacao,reRegistro.dotacao,inPosicao);
        END IF;
