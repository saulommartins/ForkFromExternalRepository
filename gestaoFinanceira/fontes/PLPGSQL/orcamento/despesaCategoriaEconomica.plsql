...
CREATE OR REPLACE FUNCTION orcamento.despesaCategoriaEconomica (VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR ) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    stSql := stSql || '
            ...
            LEFT JOIN( SELECT ENLP.cod_entidade             
                       ...
                       WHERE coalesce( TO_DATE( ENLP.timestamp, ''yyyy-mm-dd'' ), TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' ) )
                                                                        BETWEEN TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' )  
                                                                            AND TO_DATE( ''' || stDataFinal || ''', ''dd/mm/yyyy'' )  
            ...
            LEFT JOIN( SELECT ENLPA.exercicio                   
                       ...
                       WHERE coalesce( TO_DATE( ENLPA.timestamp_anulada, ''yyyy-mm-dd'' ),   
    ...
    from (
            ...
            LEFT JOIN ( SELECT sum( nota_liquidacao_item_anulado.vl_anulado ) AS vl_total_anulado  
                        ...
                        WHERE nota_liquidacao.exercicio    = nota_liquidacao_item_anulado.exercicio
                          ...
                          AND coalesce( TO_DATE( timestamp, ''yyyy-mm-dd'' ), TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' ) ) 
                                                                    BETWEEN TO_DATE( ''' || stDataInicial || ''', ''dd/mm/yyyy'' )
                                                                        AND TO_DATE( ''' || stDataFinal || ''', ''dd/mm/yyyy'' )
                        GROUP BY nota_liquidacao.exercicio_empenho 
                                ,nota_liquidacao.cod_empenho 
