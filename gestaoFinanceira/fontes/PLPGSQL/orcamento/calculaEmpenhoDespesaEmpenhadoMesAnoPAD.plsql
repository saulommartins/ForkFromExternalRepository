...
CREATE OR REPLACE  FUNCTION empenho.fn_despesa_empenhado_mes_ano_pad(character varying, character varying, character varying, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer) RETURNS numeric[]
    AS '
DECLARE
...
BEGIN
    ...
    stSql := ''SELECT
               Sum( Case When TE.dt_empenho >= to_date(''''''|| dtInicioAno ||'''''',''''dd/mm/yyyy'''') And TE.dt_empenho <= to_date(''''''|| stDtFinal ||'''''',''''dd/mm/yyyy'''') Then
                             TE.vl_total
                         Else 0.00 End ) as empenhado_ano,
               Sum( Case When TE.dt_empenho >= to_date(''''''|| stDtInicial ||'''''',''''dd/mm/yyyy'''') And TE.dt_empenho <= to_date(''''''|| stDtFinal ||'''''',''''dd/mm/yyyy'''')  Then
                             TE.vl_total
