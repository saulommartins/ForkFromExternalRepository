...
CREATE OR REPLACE FUNCTION orcamento.fn_totaliza_dotacao_funcional_liquidado_estornado(varchar,varchar,varchar,integer) RETURNS numeric(14,2) AS $$
DECLARE
...
BEGIN
    stSql := 'SELECT
                ...
                AND     substr(sw_fn_mascara_dinamica( CAST('||stMascara||' as varchar) ,CAST(num_pao as varchar) ) , '||inPosicao||',1)::INTEGER IN ('||stDigito||')
                ';
