...
CREATE OR REPLACE FUNCTION orcamento.fn_consolidado_liquidado(VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER)  RETURNS numeric(14,2) AS $$
DECLARE
...
dtFinal     :=  replace(dtFinal,'''','');
     stSql := '
        ...
        FROM
                 tmp_liquidado
        WHERE
                cod_estrutural like ' || quote_literal(stCodEstrutural || '%') ||  '
                AND dataConsulta BETWEEN to_date(' || quote_literal(dtInicial) || ', ''dd/mm/yyyy'') AND to_date('|| quote_literal(dtFinal) ||',''dd/mm/yyyy'')
        ';
                if (inOrgao <> '0') then
