...
CREATE OR REPLACE FUNCTION orcamento.fn_consolidado_estornado(VARCHAR, VARCHAR, VARCHAR, INTEGER, INTEGER)  RETURNS numeric(14,2) AS '
DECLARE
...
BEGIN
     stSql := ''
        SELECT   sum(valor) as valor_estornado
        FROM
                 tmp_estornado
        WHERE
                cod_estrutural like '''''' || stCodEstrutural || ''%'''' AND
                dataConsulta BETWEEN to_date(''''''||dtInicial||'''''',''''dd/mm/yyyy'''') AND to_date(''''''||dtFinal||'''''',''''dd/mm/yyyy'''') '';
                if (inOrgao <> ''0'') then
