...
CREATE OR REPLACE FUNCTION empenho.fn_despesa_paga_mes_ano_pad(character varying, character varying, character varying, integer, integer, integer, integer, integer, integer, integer, integer, integer, integer) RETURNS numeric[]
    AS '
DECLARE
...
BEGIN
    ...
    stSql := ''SELECT (TPaga.pago_ano - (coalesce(TAnulada.anulado_ano,0.00))) as pago_ano,
               ...
               FROM
               (SELECT
               Sum( Case When to_date(to_char(TNLP.timestamp,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') >= to_date(''''''|| dtInicioAno ||'''''',''''dd/mm/yyyy'''') And to_date(to_char(TNLP.timestamp,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') <= to_date(''''''|| stDtFinal ||'''''',''''dd/mm/yyyy'''') Then
                            TNLP.vl_pago
                         Else 0.00 End)  as pago_ano,
               Sum( Case When to_date(to_char(TNLP.timestamp,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') >= to_date(''''''|| stDtInicial ||'''''',''''dd/mm/yyyy'''') And to_date(to_char(TNLP.timestamp,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') <= to_date(''''''|| stDtFinal ||'''''',''''dd/mm/yyyy'''') Then
                                  TNLP.vl_pago
               ...
               (SELECT
               Sum( Case When to_date(to_char(TNLPA.timestamp_anulada,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') >= to_date(''''''|| dtInicioAno ||'''''',''''dd/mm/yyyy'''') And to_date(to_char(TNLPA.timestamp_anulada,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') <= to_date(''''''|| stDtFinal ||'''''',''''dd/mm/yyyy'''') Then
                                  TNLPA.vl_anulado
                         Else 0.00 End)    as anulado_ano,
               Sum( Case When to_date(to_char(TNLPA.timestamp_anulada,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') >= to_date(''''''|| stDtInicial ||'''''',''''dd/mm/yyyy'''') And to_date(to_char(TNLPA.timestamp_anulada,''''dd/mm/yyyy''''),''''dd/mm/yyyy'''') <= to_date(''''''|| stDtFinal ||'''''',''''dd/mm/yyyy'''') Then
                                  TNLPA.vl_anulado
