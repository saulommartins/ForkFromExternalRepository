...
CREATE OR REPLACE FUNCTION orcamento.fn_somatorio_programa_trabalho_balanco(varchar,varchar,varchar,varchar,varchar,varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
IF ( stSituacao = 'empenhados' ) THEN
    stSql := 'CREATE TEMPORARY TABLE tmp_empenhado AS (
       SELECT
            ...
            ,string_to_array( orcamento.fn_consulta_despesa(d.exercicio, d.cod_despesa, true) ,''.'') as dotacao_array
        FROM
        ...
        stSql := 'CREATE TEMPORARY TABLE tmp_anulado AS (
            ...
            ,string_to_array( orcamento.fn_consulta_despesa(od.exercicio, od.cod_despesa, true) ,''.'') as dotacao_array
...
stSql := stSql || ')';
  ...
  END IF;
        ...
        CREATE TEMPORARY TABLE tmp_relatorio(
            ...
            ,vl_corrente    NUMERIC(14,2)
            ,vl_capital     NUMERIC(14,2)
        ...
        stSql := 'CREATE TEMPORARY TABLE tmp_despesa AS
                SELECT
                     *
                     ...
                     ,string_to_array( orcamento.fn_consulta_despesa(exercicio, cod_despesa, true) ,''.'') as dotacao_array
    ...
    LOOP
        IF ( stSituacao = 'empenhados' ) THEN
            reRegistro.vl_corrente  := orcamento.fn_totaliza_programa_trabalho_empenhado(reRegistro.dotacao,3) - orcamento.fn_totaliza_programa_trabalho_anulado(reRegistro.dotacao,3);
            reRegistro.vl_capital   := orcamento.fn_totaliza_programa_trabalho_empenhado(reRegistro.dotacao,4) - orcamento.fn_totaliza_programa_trabalho_anulado(reRegistro.dotacao,4);
            nuValorReserva          := orcamento.fn_totaliza_programa_trabalho_empenhado(reRegistro.dotacao,9) - orcamento.fn_totaliza_programa_trabalho_anulado(reRegistro.dotacao,9);
        END IF;
        IF ( stSituacao = 'liquidados' ) THEN
            reRegistro.vl_corrente  := orcamento.fn_totaliza_programa_trabalho_liquidado(reRegistro.dotacao,3) - orcamento.fn_totaliza_programa_trabalho_liquidado_estornado(reRegistro.dotacao,3);
            reRegistro.vl_capital   := orcamento.fn_totaliza_programa_trabalho_liquidado(reRegistro.dotacao,4) - orcamento.fn_totaliza_programa_trabalho_liquidado_estornado(reRegistro.dotacao,4);
            nuValorReserva          := orcamento.fn_totaliza_programa_trabalho_liquidado(reRegistro.dotacao,9) - orcamento.fn_totaliza_programa_trabalho_liquidado_estornado(reRegistro.dotacao,9);
        END IF;
        IF ( stSituacao = 'pagos' ) THEN
            reRegistro.vl_corrente  := orcamento.fn_totaliza_programa_trabalho_pago(reRegistro.dotacao,3) - orcamento.fn_totaliza_programa_trabalho_estornado(reRegistro.dotacao,3);
            reRegistro.vl_capital   := orcamento.fn_totaliza_programa_trabalho_pago(reRegistro.dotacao,4) - orcamento.fn_totaliza_programa_trabalho_estornado(reRegistro.dotacao,4);
            nuValorReserva          := orcamento.fn_totaliza_programa_trabalho_pago(reRegistro.dotacao,9) - orcamento.fn_totaliza_programa_trabalho_estornado(reRegistro.dotacao,9);
        END IF;
