...
CREATE OR REPLACE FUNCTION orcamento.fn_somatorio_despesa_orgao_balanco(varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar) RETURNS SETOF RECORD AS
$$
DECLARE
...
stSql := ' SELECT   cod_conta
        ...
        IF ( stSituacao = 'empenhados' ) THEN
           stSql := stSql || ',(coalesce(orcamento.fn_consolidado_empenhado(' || quote_literal(stDataInicial) || '
                                                                            ...
                                                                            , publico.fn_mascarareduzida(classificacao),num_orgao,0),0.00) - coalesce(orcamento.fn_consolidado_anulado(' || quote_literal(stDataInicial) || '
                                                                                                                                                                                        , ' || quote_literal(stDataFinal) || '
                                                                                                                                                                                        , publico.fn_mascarareduzida(classificacao),num_orgao,0),0.00)
                                ) as valor
                                ';
        END IF;
        IF ( stSituacao = 'pagos' ) THEN
            stSql := stSql || ',(coalesce(orcamento.fn_consolidado_pago(' || quote_literal(stDataInicial) || '
                                                                        , ' || quote_literal(stDataFinal) || '
                                                                        , publico.fn_mascarareduzida(classificacao),num_orgao,0),0.00) - coalesce(orcamento.fn_consolidado_estornado(' || quote_literal(stDataInicial) || '
                                                                                                                                                                                    , ' || quote_literal(stDataFinal) || '
                                                                                                                                                                                    , publico.fn_mascarareduzida(classificacao),num_orgao,0),0.00)
                                ) as valor
                                ';
        ...
        IF ( stSituacao = 'liquidados' )  THEN
            stSql := stSql || ',(coalesce(orcamento.fn_consolidado_liquidado(' || quote_literal(stDataInicial) || '
                                                                            , ' || quote_literal(stDataFinal) || '
                                                                            , publico.fn_mascarareduzida(classificacao),num_orgao,0),0.00) - coalesce(orcamento.fn_consolidado_liquidado_estornado(' || quote_literal(stDataInicial) || '
                                                                                                                                                                                                    , ' || quote_literal(stDataFinal) || '
                                                                                                                                                                                                    , publico.fn_mascarareduzida(classificacao),num_orgao,0),0.00)
                                ) as valor
                                ';
        ...
        stSql := stSql || ' FROM
                 tmp_relatorio
        GROUP BY num_orgao, descricao, classificacao, classificacao_reduzida, nivel, cod_conta
