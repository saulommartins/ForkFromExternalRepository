...
CREATE OR REPLACE FUNCTION orcamento.fn_consolidado_elem_despesa(varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,integer,integer) RETURNS SETOF RECORD AS $$
DECLARE
...
stSql := stSql || '
    ...
    --AND ('SELECT nom_prefeitura FROM administracao.configuracao WHERE exercicio = ' || stExercicio || '') = 'Tribunal de Contas Estado de Mato Grosso do Sul') THEN
       -- stSql := stSql || '
        --        AND oss.exercicio =''' || stExercicio || '''
                --AND osr.exercicio =''' || stExercicio || '''
      --          ';
   -- END IF;
...
--      coalesce(orcamento.fn_consolidado_empenhado(''' || dataInicio || ''', ''' || dataFim || ''', publico.fn_mascarareduzida(tbl.classificacao),tbl.num_orgao,tbl.num_unidade),0.00) as empenhado_ano,
...
--      coalesce(orcamento.fn_consolidado_anulado(''' || dataInicio || ''', ''' || dataFim || ''', publico.fn_mascarareduzida(tbl.classificacao),tbl.num_orgao,tbl.num_unidade),0.00) as anulado_ano,
...
--      (coalesce(orcamento.fn_consolidado_pago(''' || dataInicio || ''', ''' || dataFim || ''',publico.fn_mascarareduzida(tbl.classificacao),tbl.num_orgao,tbl.num_unidade),0.00) - coalesce(orcamento.fn_consolidado_estornado(''' || dataInicio || ''', ''' || dataFim || ''', publico.fn_mascarareduzida(tbl.classificacao),tbl.num_orgao,tbl.num_unidade),0.00)) as pago_ano,
...
--      (coalesce(orcamento.fn_consolidado_liquidado(''' || dataInicio || ''', ''' || dataFim || ''',publico.fn_mascarareduzida(tbl.classificacao),tbl.num_orgao,tbl.num_unidade),0.00) - coalesce(orcamento.fn_consolidado_liquidado_estornado(''' || dataInicio || ''', ''' || dataFim || ''', publico.fn_mascarareduzida(tbl.classificacao),tbl.num_orgao,tbl.num_unidade),0.00)) as liquidado_ano,
