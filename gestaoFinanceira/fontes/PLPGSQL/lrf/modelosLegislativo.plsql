...
*
* Casos de uso: uc-02.05.11,uc-02.05.12
*/

/*
$Log$
Revision 1.8  2006/07/27 20:22:31  jose.eduardo
Bug #6642#
...
CREATE OR REPLACE FUNCTION tcers.fn_rel_modelos_legislativo(varchar,varchar,varchar,varchar,varchar,varchar,varchar) RETURNS SETOF RECORD AS '
DECLARE
...
/*    stSql := ''
 ...
 --        AND e.exercicio         = '' || stExercicio || ''
    ...
    stSql := ''
        CREATE TEMPORARY TABLE tmp_credito_receita AS
            SELECT
  ...
  --            AND l.exercicio         = '' || stExercicio || ''
    ...
    stSql := ''
        CREATE TEMPORARY TABLE tmp_debito_receita AS
            SELECT
                pc.cod_estrutural   as cod_estrutural,
                l.tipo              as tipo,
                l.exercicio         as exercicio,
...
--      m.exercicio,
        ...
        IF ((substr(stDtFinal,7,4) > 2004) AND (substr(stDtInicial,7,4) < 2005)) THEN
            stSql := stSql || ''
                CASE WHEN substr(pc.cod_estrutural,1,1) = 4 THEN
                    coalesce(tcers.fn_somatorio_contabil_receita(publico.fn_mascarareduzida(pc.cod_estrutural),''''2'''',''''''''),0.00) - coalesce(tcers.fn_somatorio_contabil_siam(substr(publico.fn_mascarareduzida(pc.cod_estrutural),3),''''1'''',''''2'''','''''' || stTipoValorDespesa || '''''',cast(substr('''''' || stDtInicial || '''''',4,2) as integer)),0.00)
                ELSE
                    CASE WHEN substr(pc.cod_estrutural,1,1) = 3 THEN
                        coalesce(tcers.fn_somatorio_contabil(publico.fn_mascarareduzida(pc.cod_estrutural),''''3'''','''''' || stTipoValorDespesa || '''''','''''' || stDtInicial || '''''', '''''' || stDtFinal || ''''''),0.00) + coalesce(tcers.fn_somatorio_contabil_siam(substr(publico.fn_mascarareduzida(pc.cod_estrutural),3),''''2'''',''''2'''','''''' || stTipoValorDespesa || '''''',cast(substr('''''' || stDtInicial || '''''',4,2) as integer)),0.00)
                    ELSE
        ...
        ELSE
            IF (substr(stDtFinal,7,4) > 2004) THEN
                stSql := stSql || ''
                    CASE WHEN substr(pc.cod_estrutural,1,1) = 4 THEN
                        coalesce(tcers.fn_somatorio_contabil_receita(publico.fn_mascarareduzida(pc.cod_estrutural),''''2'''',''''''''),0.00)
                    ELSE
                        CASE WHEN substr(pc.cod_estrutural,1,1) = 3 THEN
            ...
            ELSE
                stSql := stSql || ''
                    CASE WHEN substr(pc.cod_estrutural,1,1) = 4 THEN
                        coalesce(tcers.fn_somatorio_contabil_siam_receita(substr(publico.fn_mascarareduzida(pc.cod_estrutural),3),''''1'''',''''2'''','''''' || stTipoValorDespesa || '''''',cast(substr('''''' || stDtInicial || '''''',4,2) as integer)),0.00)
                    ELSE
                        CASE WHEN substr(pc.cod_estrutural,1,1) = 3 THEN
                            coalesce(tcers.fn_somatorio_contabil_siam(substr(publico.fn_mascarareduzida(pc.cod_estrutural),3),''''2'''',''''2'''','''''' || stTipoValorDespesa || '''''',cast(substr('''''' || stDtInicial || '''''',4,2) as integer)),0.00)
                        END
                    END as valor_contabil,'';
            END IF;
        END IF;
...
--      m.exercicio         = '' || stExercicio || '' AND
  ...
  --    m.exercicio,
    ...
    INSERT INTO tmp (ordem)
        SELECT 29 WHERE ''3.3.1.4.0.13.40.01.00.00'' NOT IN ( SELECT cod_estrutural_completo FROM tmp ) AND stCodModelo = 10
    ;
    INSERT INTO tmp (ordem)
        SELECT 66 WHERE ''3.3.1.4.0.41.39.01.00.00'' NOT IN ( SELECT cod_estrutural_completo FROM tmp ) AND stCodModelo = 13
    ;
    INSERT INTO tmp (ordem)
        SELECT 14 WHERE ''4.1.9.1.2.34.01.01.01.00'' NOT IN ( SELECT cod_estrutural_completo FROM tmp ) AND stCodModelo = 13
    ;
    INSERT INTO tmp (ordem)
        SELECT 16 WHERE ''4.1.9.1.4.01.00.00.00.00'' NOT IN ( SELECT cod_estrutural_completo FROM tmp ) AND stCodModelo = 13
    ;
    INSERT INTO tmp (ordem)
        SELECT 18 WHERE ''4.1.9.3.3.01.01.00.00.00'' NOT IN ( SELECT cod_estrutural_completo FROM tmp ) AND stCodModelo = 13
    ;
...
--        DROP TABLE tmp_conta_despesa;
      ...
      DROP TABLE tmp_credito_receita;
      DROP TABLE tmp_debito_receita;
