...
CREATE OR REPLACE FUNCTION empenho.fn_consultar_valor_nota_liquidacao_paga_anulada(VARCHAR,INTEGER,INTEGER) RETURNS NUMERIC AS '
...
BEGIN
    ...
    SELECT
        coalesce(sum(nla.vl_anulado),0.00) as vl_anulado
        INTO    nuValorAnulado
    FROM
