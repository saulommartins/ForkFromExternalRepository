/*
    ...
    **********************************************************************************
*/
CREATE OR REPLACE FUNCTION empenhoAnulacaoLiquidacao(character varying, numeric, character varying, integer, character varying, integer, integer, character varying, integer, integer, integer, character varying) RETURNS INTEGER AS $$ 
...
DECLARE
    ...
    CODDESPESA              ALIAS FOR $11;
    CODCLASSDESPESA         ALIAS FOR $12;
...
BEGIN
    ...
    LOOP
        ...
        SEQUENCIA := FAZERLANCAMENTO(  REREGISTROSCONTAFIXA.estrutural_debito , REREGISTROSCONTAFIXA.estrutural_credito , 905 , EXERCICIO , VALOR , COMPLEMENTO , CODLOTE , TIPOLOTE , CODENTIDADE, REREGISTROSCONTAFIXA.plano_debito, REREGISTROSCONTAFIXA.plano_credito );
    END LOOP;
        ...
        SQLCONFIGURACAO := '
            ...
            INNER JOIN orcamento.conta_despesa
                 ...
                 WHERE configuracao_lancamento_credito.estorno = ''true''
                   ...
                   AND publico.fn_mascarareduzida(conta_despesa.cod_estrutural) = publico.fn_mascarareduzida('''||CLASDESPESA||''')
        ';
    
        FOR REREGISTROSCONFIGURACAO IN EXECUTE SQLCONFIGURACAO
        LOOP
            SEQUENCIA := FAZERLANCAMENTO(  REREGISTROSCONFIGURACAO.estrutural_debito , REREGISTROSCONFIGURACAO.estrutural_credito , 905 , EXERCICIO , VALOR , COMPLEMENTO , CODLOTE , TIPOLOTE , CODENTIDADE , REREGISTROSCONFIGURACAO.conta_debito, REREGISTROSCONFIGURACAO.conta_credito );
    ...
    SQLVINCULORECURSO := '
                SELECT tabela.conta_debito
                     ...
                     , tabela.cod_plano
                  FROM (
                          SELECT CASE WHEN plano_conta.cod_estrutural like ''8.2.1.1.3%'' THEN
      ...
      LOOP
        ...
        IF STCODESTRUTURALVINCULOCREDITO <> '' AND STCODESTRUTURALVINCULODEBITO <> '' THEN
            SEQUENCIA := FAZERLANCAMENTO( STCODESTRUTURALVINCULODEBITO , STCODESTRUTURALVINCULOCREDITO , 905 , EXERCICIO , VALOR , COMPLEMENTO , CODLOTE , TIPOLOTE , CODENTIDADE , INCODCONTAVINCULODEBITO, INCODCONTAVINCULOCREDITO );
   ...
   RESTOSPAGAR := 'SELECT restos_pagar FROM empenho.empenho WHERE exercicio = '||quote_literal(EXERCICIO)||' AND valor= '||quote_literal(VALOR)||' entidade= '||quote_literal(CODENTIDADE)||' ';
    ...
    --    END LOOP;
    --
    --    IF RESTOSPAGAR = 'NP a Liquidar' THEN
    --         SQLCONTAFIXA := '
    --        SELECT debito.cod_estrutural AS estrutural_debito
    --             , credito.cod_estrutural AS estrutural_credito
    --             , debito.cod_plano AS plano_debito
    ...
    --
    --    END IF; 
    --       IF RESTOSPAGAR = 'NP Liquidado' THEN
    --         SQLCONTAFIXA := '
    --        SELECT debito.cod_estrutural AS estrutural_debito
    --             , credito.cod_estrutural AS estrutural_credito
    --             , debito.cod_plano AS plano_debito
    ...
    --                     ON plano_conta.cod_conta = plano_analitica.cod_conta
    --                    AND plano_conta.exercicio = plano_analitica.exercicio
    --                  WHERE REPLACE(plano_conta.cod_estrutural, ''.'', '''') LIKE ''4640100%''
    --               ) AS credito
    --    INNER JOIN (
    --                 SELECT plano_conta.cod_estrutural
    --                      , plano_analitica.cod_plano
    ...
    --                     ON plano_conta.cod_conta = plano_analitica.cod_conta
    --                    AND plano_conta.exercicio = plano_analitica.exercicio
    --                  WHERE REPLACE(plano_conta.cod_estrutural, ''.'', '''') LIKE ''2131102%''
    --               ) AS debito
    --             ON debito.exercicio = credito.exercicio
    --          WHERE debito.exercicio = '''||EXERCICIO||'''
    --    ';
