...
CREATE OR REPLACE FUNCTION empenho.fn_consultar_valor_pago_anulado_recurso_rp(VARCHAR,VARCHAR,INTEGER,VARCHAR,VARCHAR) RETURNS NUMERIC AS '
...
BEGIN
    ...
    SELECT empenho.fn_consultar_valor_pago_anulado_recurso(stExercicio,stCodEntidade,inCodRecurso,stDtInicial,stDtFinal,''R'') INTO nuValor;

    stSql := ''SELECT
                   coalesce(sum(NLPA.vl_anulado),0.00)
               ...
               WHERE   NLPA.exercicio       = NLP.exercicio
