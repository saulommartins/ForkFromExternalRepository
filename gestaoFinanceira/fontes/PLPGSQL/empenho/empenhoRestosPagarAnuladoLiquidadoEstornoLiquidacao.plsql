...
CREATE OR REPLACE FUNCTION empenho.fn_empenho_restos_pagar_anulado_liquidado_estornoliquidacao(varchar,varchar,varchar,varchar,varchar,varchar, varchar, varchar,varchar,varchar, varchar, varchar, varchar, varchar) RETURNS SETOF RECORD AS $$
DECLARE
...
BEGIN
    ...
    IF stExercicio = '' THEN
        stSqlExercicio := 'SELECT DISTINCT exercicio FROM empenho.empenho WHERE empenho.exercicio < ''' || to_char(to_date(stDtInicial, 'dd/mm/yyyy'), 'yyyy')||''' ORDER BY exercicio';
    ELSE
    ...
    stSql := ' INSERT INTO tmp_empenhos
        ...
        IF (stSituacao = '1') THEN
            stSql := stSql || ', to_char(ea.timestamp,''dd/mm/yyyy'') as data 
        ...
        IF (stSituacao = '3') THEN
                stSql := stSql || ', to_char(nlia.timestamp,''dd/mm/yyyy'') as data 
  ...
  LEFT OUTER JOIN ( SELECT ped.exercicio
            ...
            IF (stSituacao = '1') THEN
                stSql := stSql || '
                    AND to_date(to_char(ea.timestamp,''dd/mm/yyyy''),''dd/mm/yyyy'') BETWEEN to_date(' || quote_literal(stDtInicial) || ',''dd/mm/yyyy'') AND to_date(' ||quote_literal(stDtFinal) || ',''dd/mm/yyyy'')
            ...
            IF (stSituacao = '2') THEN
                stSql := stSql || '
                    ...
                    AND nl.dt_liquidacao BETWEEN to_date(' || quote_literal(stDtInicial) || ',''dd/mm/yyyy'') AND to_date(' || quote_literal(stDtFinal) || ',''dd/mm/yyyy'')
            ...
            if (stSituacao = '3') then
                stSql := stSql || '
                    ...
                    --Ligação NOTA LIQUIDAÇÃO : NOTA LIQUIDAÇÃO ITEM
                    AND to_date(to_char(nlia.timestamp,''dd/mm/yyyy''),''dd/mm/yyyy'') BETWEEN to_date(' || quote_literal(stDtInicial) || ',''dd/mm/yyyy'') AND to_date(' || quote_literal(stDtFinal) || ',''dd/mm/yyyy'')
