...
CREATE OR REPLACE FUNCTION pegaempenhoempenhomodalidade(varchar,integer) RETURNS VARCHAR  AS $$
DECLARE
    ...
    inCodCadastro       INTEGER := 1;
    inCodAtributo       INTEGER := 101;

BEGIN

stSqlGroupBy := ' ,VALOR.cod_pre_empenho, VALOR.exercicio  ';
...
--        END AS valor_desc,
    ...
    WHERE
        ...
        AND VALOR.cod_pre_empenho = '|| inCodPreEmpenho ||'
        ...
        AND PADRAO.cod_valor::varchar   = VALOR.valor
        AND VALOR.cod_atributo::varchar||''-''||VALOR.timestamp::varchar =  (
            SELECT
                VALOR.cod_atributo::varchar||''-''||max(VALOR.timestamp)::varchar
            FROM
                administracao.atributo_dinamico             AS AD,
                empenho.atributo_empenho_valor   AS VALOR
            WHERE
