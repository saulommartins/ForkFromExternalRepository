...
/**
    ...
    * $Id: MontaLocalizacaoLoteamento.class.php 59612 2014-09-02 12:00:51Z gelson $
...
/*
$Log$
Revision 1.6  2006/09/18 09:12:58  fabio
...
function geraFormulario(&$obFormulario, $boObrigatorio = true)
{
    ...
    $obBscChaveLocalizacao = new BuscaInner;
    $obBscChaveLocalizacao->setId     ( "stNomeChaveLocalizacaoLoteamento" );
    $obBscChaveLocalizacao->setRotulo ( "Localização" );
    if ($this->inNivelCorte) {
        $arMascara = explode(".",$this->stMascara);
        $i = 0;
        $stMascara = "";
        while ( $i <= count($this->stMascara - 2 ) ) {
            $this->stMascara .= $arMascara[$i].".";
            $i++;
        }
        $this->stMascara = substr($this->stMascara,0,strlen($this->stMascara -1));
    }
    $obBscChaveLocalizacao->obCampoCod->setMinLength ( strlen($this->stMascara) );
    $obBscChaveLocalizacao->obCampoCod->setMaxLength ( strlen($this->stMascara) );
    $obBscChaveLocalizacao->obCampoCod->setSize      ( strlen($this->stMascara) + 2 );

    $obBscChaveLocalizacao->obCampoCod->setName ( "stChaveLocalizacaoLoteamento" );
    $obBscChaveLocalizacao->obCampoCod->setAlign ("center");
    $obBscChaveLocalizacao->obCampoCod->obEvento->setOnBlur("buscaDado('buscaLocalizacao');");
    $obBscChaveLocalizacao->obCampoCod->obEvento->setOnKeyUp("mascaraDinamico('".$this->stMascara."', this, event);");
    $stTmpAcao = "nomLocalizacao";
    if ($this->inNivelCorte) {
        $stTmpAcao = "buscaReduzido";
    }
    $obBscChaveLocalizacao->setValoresBusca(CAM_GT_CIM_POPUPS."localizacao/OCBuscaLocalizacao.php?".Sessao::getId(),$obFormulario->obForm->getName(),$stTmpAcao);
    $obBscChaveLocalizacao->setFuncaoBusca("abrePopUp('".CAM_GT_CIM_POPUPS."localizacao/FLBuscaLocalizacao.php','frm','stChaveLocalizacaoLoteamento','stNomeChaveLocalizacaoLoteamento','buscaLocalizacaoInner','".Sessao::getId()."','800','550');");

    if ( $this->getObrigatorio() ) {
        $obBscChaveLocalizacao->setNull( false );
    } else {
        $obBscChaveLocalizacao->setNull( true  );
    }
    $obFormulario->addComponente ( $obBscChaveLocalizacao );
}
...
function preencheProxCombo($inPosCombo, $inNumCombos)
{
    ...
    if ($this->stValorReduzido) {
        ...
        if ($inPosCombo != $inNumCombos) {
            ...
            while ( !$rsListaLocalizacao->eof() ) {
                $stChaveLocalizacaoLoteamento  = $rsListaLocalizacao->getCampo( "cod_nivel" )."-";
                $stChaveLocalizacaoLoteamento .= $rsListaLocalizacao->getCampo( "cod_localizacao")."-";
                $stChaveLocalizacaoLoteamento .= $rsListaLocalizacao->getCampo( "valor")."-";
                $stChaveLocalizacaoLoteamento .= $rsListaLocalizacao->getCampo( "valor_reduzido");
                $stNomeLocalizacao   = $rsListaLocalizacao->getCampo( "nom_localizacao" );
                $js .= "f.inCodLocalizacao_".$inPosCombo.".options[$inContador] = ";
                $js .= "new Option('".$stNomeLocalizacao."','".$stChaveLocalizacaoLoteamento."',''); \n";
                $inContador++;
                $rsListaLocalizacao->proximo();
            }
        }
    ...
    while ( !$rsListaNivel->eof() and key( $arValorReduzido ) < count( $arValorReduzido ) ) {
         ...
         if ($boMontaCombos) {
             ...
             while ( !$rsListaLocalizacao->eof() ) {
                 ...
                 $js .= "new Option('".$stNomeLocalizacao."','".$stChaveLocalizacaoLoteamento."','".$stSelected."'); \n";
                 $inContador++;
                 $rsListaLocalizacao->proximo();
             }
         }
