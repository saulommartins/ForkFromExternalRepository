...
<?php
/**
     * Classe de regra de negócio para lote rural
    ...
    * $Id: RCIMLoteRural.class.php 59612 2014-09-02 12:00:51Z gelson $
...
include_once ( CAM_GT_CIM_MAPEAMENTO."TCIMLoteRural.class.php" );
...
class RCIMLoteRural extends RCIMLote
{
/**
    * @access Private
    * @var Object
...
/**
     ...
     * @access Private
*/
function RCIMLoteRural()
{
    parent::RCIMLote();
    $this->obTCIMLoteRural = new TCIMLoteRural;
    $this->obRCadastroDinamico = new RCadastroDinamico;
    $this->obRCadastroDinamico->setPersistenteValores   ( new TCIMAtributoLoteRuralValor );
...
function incluirLote($boTransacao = "")
{
    ...
    if ( !$obErro->ocorreu() ) {
        ...
        if ( !$obErro->ocorreu() ) {
            $this->obTCIMLoteRural->setDado( "cod_lote", $this->inCodigoLote );
            $obErro = $this->obTCIMLoteRural->inclusao( $boTransacao );
            $arChaveAtributoLoteRural =  array( "cod_lote" => $this->inCodigoLote );
            $this->obRCadastroDinamico->setChavePersistenteValores( $arChaveAtributoLoteRural );
...
function excluirLote($boTransacao = "")
{
    ...
    $obErro = $this->obTransacao->abreTransacao( $boFlagTransacao, $boTransacao );
        ...
        if ( !$obErro->ocorreu() ) {
            $this->obTCIMLoteRural->setDado( "cod_lote", $this->inCodigoLote );
            $obErro = $this->obTCIMLoteRural->exclusao( $boTransacao );
            if ( !$obErro->ocorreu() ) {
...
function listarLotes(&$rsRecordSet ,$boTransacao = "")
{
    ...
    $obErro = $this->obTCIMLoteRural->recuperaRelacionamento( $rsRecordSet, $stFiltro, $stOrdem, $boTransacao );
...
function listarLotesDesmembramento(&$rsRecordSet ,$boTransacao = "")
{
    ...
    $obErro = $this->obTCIMLoteRural->recuperaRelacionamentoLoteValidado( $rsRecordSet, $stFiltro, $stOrdem, $boTransacao );
...
{
    $stFiltro = 'AND lote_rural.cod_lote is not null ';
...
function verificaBaixaLote(&$rsBaixaImovel, $boTransacao = "")
{
    ...
    $this->obTCIMLoteRural->recuperaRelacionamentoLoteBaixado( $rsBaixaImovel, $stFiltro, "", $boTransacao );
                ...
                if ($rsLotes->getCampo('validado') == 'f' ) {
                    ...
                    if (!$obErro->ocorreu()) {
                        if ( $rsLotes->getCampo('cod_lote') != $this->getCodigoLote() ) {
                            $obRCIMLoteRural = new RCIMLoteRural;
                            $obRCIMLoteRural->setCodigoLote($rsLotes->getCampo('cod_lote'));
                            $obRCIMLoteRural->obRCIMLocalizacao->setCodigoLocalizacao($this->obRCIMLocalizacao->getCodigoLocalizacao() );
                            $obErro=$obRCIMLoteRural->excluirLote($boTransacao);
                         }
                    }
                } else {
