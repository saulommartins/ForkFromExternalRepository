...
//MONTAGEM DO FILTRO
$stFiltro = "";
if (!empty($_REQUEST['inLivro']) ) {
    $stFiltro .= " \n num_livro  = '".$_REQUEST['inLivro']."' AND ";
}

if (!empty($_REQUEST['stExercicio']) ) {
    $stFiltro .= " \n exercicio_livro = '".$_REQUEST['stExercicio']."' AND ";
}

if (!empty($_REQUEST['inPagina']) ) {
...
$stOrdem = " GROUP BY num_livro, exercicio_livro ORDER BY num_livro,  exercicio_livro ";
$obTDATDividaAtiva = new TDATDividaAtiva;
$obTDATDividaAtiva->recuperaLivroPopUP( $rsLivro, $stFiltro, $stOrdem );
...
*/
$obLista->addDado();
$obLista->ultimoDado->setCampo( "[exercicio_livro]" );
$obLista->commitDado();
...
$obLista->ultimaAcao->addCampo("2","exercicio_livro");
$obLista->commitAcao();
$obLista->show();
