 ...
 * $Id: FMManterEmissaoCombo.php 66419 2016-08-26 12:59:45Z evandro $
...
$pgList     = "LS".$stPrograma."Combo.php?".Sessao::getId().$stLink;
...
foreach ($request->getAll() as $valor => $key) {
    if ( preg_match("/^[a-z]boEmitir_[0-9]/",$valor) ) {
        $arKey = explode('&',$key); //numcgm,cod_inscricao
        $boJaNaLista = false;
        //aqui devo alterar esta funcao para quando for documentos do tipo remissao DA, agrupar por inscricoes IMOB/ECO
        if ($request->get("cmbModelo") == "Certidão de Remissão DA") {
            $obTDATDividaRemissao->recuperaInscricaoTipoRemissao( $rsRemissao, $arKey[6] );
            for ($inX=0; $inX<$inTamanhoLista; $inX++) {
                if ( ( $arLista[$inX][3] == $arKey[3] ) && ( in_array( $arLista[$inX][0], explode( "$$", $arKey[0] ) ) ) ) {
                    $stLinha = $rsRemissao->getCampo( "tipo" ).'_'.$rsRemissao->getCampo( "inscricao" ).'_'.$rsRemissao->getCampo( "dt_remissao" );
                    if ($arRemissao[$stLinha]["num_parcelamentos"]) {
                        $arTMP = explode( ",", $arRemissao[$stLinha]["num_parcelamentos"] );
                        if ( in_array( $arKey[6], $arTMP ) ) {
                            $boJaNaLista = true;
                            break;
                        }
                    }
            ...
            if (!$boJaNaLista) {
                $arRemissao[$stLinha]["num_parcelamentos"] = $rsRemissao->getCampo( "num_parcelamentos" );
            }
        } else {
            for ($inX=0; $inX<$inTamanhoLista; $inX++) {
                if ( ( $arLista[$inX][3] == $arKey[3] ) && ( in_array( $arLista[$inX][0], explode( "$$", $arKey[0] ) ) ) ) {
                    if ( $arKey[10] || ( $request->get("cmbModelo") == "Termo Inscrição DA" ) || ( $request->get("cmbModelo") == "Notificação DA" ) || ( $request->get("cmbModelo") == "Certidão DA" ) ) {
                        //quando for estes documentos o sistema vai tratar eles separados
                        break;
                    } else {
        ...
        if (!$boJaNaLista) {
            $arTMP1 = explode( "$$", $arKey[2] );
            for ( $inX=0; $inX<count($arTMP1); $inX++ ) {
                if ($arTMP1[$inX] == $request->get("cmbModelo")) {
                    $boJaNaLista = true;
                    break;
                }
            }
            ...
            if (!$boJaNaLista) {
                //validacao para incluir os documentos apos ser feita a alteracao da modalidade de acordo como o ticket #24031                
                ...
                $arTMP2 = explode( "$$", $arKey[12] );
                foreach ($arTMP2 as $inCodModalidade) {
                    $stFiltroModalidade  = " WHERE modelo_documento.nome_documento = '".$request->get("cmbModelo")."' ";
                    $stFiltroModalidade .= " AND modalidade_documento.cod_modalidade = ".$inCodModalidade." ";                    
                    $obTDATModalidadeDocumento->recuperaListaDocumentoModalidade($rsModelos,$stFiltroModalidade,$stOrder);
                    
                    if ( $rsModelos->getNumLinhas() > 0) {
                        $stFiltroDocumento  = " WHERE num_parcelamento = ".$arKey[6]." ";
                        $stFiltroDocumento .= " AND cod_tipo_documento = ".$rsModelos->getCampo('cod_tipo_documento')." ";
                        $stFiltroDocumento .= " AND cod_documento = ".$rsModelos->getCampo('cod_documento')." ";
                        $obTDATDividaDocumento->recuperaTodos($rsDocumento,$stFiltroDocumento,"");                                                
                        //Incluir o documento com o tipo para o parcelamento para executar a emissao              
                        if ( $rsDocumento->getNumLinhas() < 0) {                            
                            $obTDATDividaDocumento->setDado('num_parcelamento'  ,$arKey[6]);
                            $obTDATDividaDocumento->setDado('cod_tipo_documento', $rsModelos->getCampo('cod_tipo_documento') );
                            $obTDATDividaDocumento->setDado('cod_documento',$rsModelos->getCampo('cod_documento') );
                            $obTDATDividaDocumento->inclusao();
                            
                        }
                        $arKey[0] .= '$$'.$rsModelos->getCampo('nome_arquivo_agt').'$$';
                        $arKey[1] .= '$$'.$rsModelos->getCampo('nome_arquivo_swx').'$$';
                        $arKey[2] .= '$$'.$rsModelos->getCampo('nome_documento').'$$';
                        $arKey[7] .= '$$'.$rsModelos->getCampo('cod_documento').'$$';
                        $arKey[9] .= '$$'.$rsModelos->getCampo('cod_tipo_documento').'$$';
                    }                                
                }

                if ($arTMP2 == '') {
                    SistemaLegado::alertaAviso($pgList.$stLink, "Documento não disponível para o registro selecionado! Inscrição: ".$arKey[11], "n_imprimir", "erro", Sessao::getId(), "../" );
                    exit;                    
                }
            ...
            $arTMP1 = explode( "$$", $arKey[0] );
            $arLista[$inTamanhoLista][0] = $arTMP1[$inX];

            $arTMP1 = explode( "$$", $arKey[1] );
            $arLista[$inTamanhoLista][1] = $arTMP1[$inX];

            $arTMP1 = explode( "$$", $arKey[2] );
            $arLista[$inTamanhoLista][2] = $arTMP1[$inX];

            $arTMP1 = explode( "$$", $arKey[7] );
            $arLista[$inTamanhoLista][7] = $arTMP1[$inX];

            $arTMP1 = explode( "$$", $arKey[8] );
            $arLista[$inTamanhoLista][8] = $arTMP1[$inX];

            $arTMP1 = explode( "$$", $arKey[9] );
            $arLista[$inTamanhoLista][9] = $arTMP1[$inX];

            $arTMP1 = explode( "$$", $arKey[10] );
            if ($arTMP1[$inX]) { //se tem numero de documento deve buscar as numeracoes envolvidas
                $obTDATDividaDocumento = new TDATDividaDocumento;
                $obTDATDividaDocumento->recuperaListaNumParcelamentoPorDocumento( $rsListaNumParcelamentos, $arLista[$inTamanhoLista][10], $arLista[$inTamanhoLista][7], $arLista[$inTamanhoLista][9] );
                $arLista[$inTamanhoLista][6] = $rsListaNumParcelamentos->getCampo( "numparcelamento" );
            }

            $arLista[$inTamanhoLista][10] = $arTMP1[$inX];
...
foreach ($arLista as $valor => $arKey) {
        //registrando numeracao dos documentos        
            ...
            for ($inX=0; $inX<count($arParcelamentos); $inX++) {
                ...
                if ($arDadosSessao[$inArquivo]["num_documento"]) {
        ...
        //$arKey[0] = "termoAssuncao.agt";
        //$arKey[1] = "termoAssuncaoMata.odt";
        ...
        if ($arKey[0] == "certidaoDARemissaoMata.agt") {
            //certidao de divida ativa remissao MATA!
            $obTDATDividaRemissao->recuperaDadosCertidaoRemissao( $rsDados, $arKey[6] );
        }else
        ...
        if ($arKey[0] == "memorialCalculoDAUrbem.agt") {
            ...
            $arValorOrigemDataAtual[] = array (
                "origem" => $arValorOrigem[count($arValorOrigem)-1]["imposto"],
                "valor_origem" => number_format( $flValorOrigem, 2, ',', '.' ),
                "correcao" => number_format( $flCorrecao, 2, ',', '.' ),
                "juros" => number_format( $flJuros, 2, ',', '.' ),
                "multa" => number_format( $flMulta, 2, ',', '.' ),
        ...
        if ($arKey[0] == "termoParcelamento.agt") {
            ...
            $stFiltro = " WHERE ddp.num_parcelamento in ( ".$arKey[6]." ) ";
            $obTDATDividaAtiva = new TDATDividaAtiva;
            $obTDATDividaAtiva->setDado('num_parcelamento', $arKey[6]);
            $obTDATDividaAtiva->recuperaConsultaTermoParcelamento( $rsDados, $stFiltro );
        ...
        if ($arKey[0] == "notificacaoDivida.agt") { //era o dado que vem do campo agt
            ...
            foreach ($rsDados->arElementos as $key => $value) {
                $stFiltroNorma[$key] = $rsDados->arElementos[$key]['num_parcelamento'];
            }

            if ($stFiltroNorma != "") {
                 $stFiltroNorma = implode(',', $stFiltroNorma);
                $obTDATDividaAtiva->recuperaConsultaNotificacaoDividaNormas( $rsDados2, $stFiltroNorma );
            }

           $inDados=0;

            foreach ($rsDados->arElementos as $dados) {
                $arDados[$inDados]["regiao"]                   =  $dados["regiao"];
                $arDados[$inDados]["distrito"]                 =  $dados["distrito"];
                $arDados[$inDados]["cod_logradouro"]           =  $dados["cod_logradouro"];
                $arDados[$inDados]["cidade_estado"]            =  $dados["cidade_estado"];
                $arDados[$inDados]["bairro_notificado"]        =  $dados["bairro_notificado"];
                $arDados[$inDados]["rg_insc_estadual"]         =  $dados["rg_insc_estadual"];
                $arDados[$inDados]["cep_notificado"]           =  $dados["cep_notificado"];
                $arDados[$inDados]["telefone_notificado"]      =  $dados["telefone_notificado"];
                $arDados[$inDados]["cidade_estado_notificado"] =  $dados["cidade_estado_notificado"];
                $arDados[$inDados]["endereco_notificado"]      =  $dados["endereco_notificado"];
                $arDados[$inDados]["cpf_cnpj_notificado"]      =  $dados["cpf_cnpj_notificado"];
                $arDados[$inDados]["numero_quadra"]            =  $dados["numero_quadra"];
                $arDados[$inDados]["numero_lote"]              =  $dados["numero_lote"];
                $arDados[$inDados]["tipo_inscricao"]           =  $dados["tipo_inscricao"];
                $arDados[$inDados]["inscricao_municipal"]      =  $dados["inscricao_municipal"];
                $arDados[$inDados]["inscricao_economica"]      =  $dados["inscricao_economica"];
                $arDados[$inDados]["cod_inscricao"]            =  $dados["cod_inscricao"];
                $arDados[$inDados]["exercicio_divida"]         =  $dados["exercicio_divida"];
                $arDados[$inDados]["inscricao"]                =  $dados["inscricao"];
                $arDados[$inDados]["imposto"]                  =  $dados["imposto"];
                $arDados[$inDados]["nome_notificado"]          =  $dados["nome_notificado"];
                $arDados[$inDados]["domicilio_fiscal"]         =  $dados["domicilio_fiscal"];
                $arDados[$inDados]["num_parcelamento"]         =  $dados["num_parcelamento"];
                $arDados[$inDados]["numero_parcelamento"]      =  $dados["numero_parcelamento"];
                $arDados[$inDados]["nr_acordo_administrativo"] =  $dados["nr_acordo_administrativo"];
                $arDados[$inDados]["exercicio_cobranca"]       =  $dados["exercicio_cobranca"];
                $arDados[$inDados]["total_pagar"]              =  $dados["total_pagar"];
                $arDados[$inDados]["tpsd"]                     =  $dados["tpsd"];
                $arDados[$inDados]["valor_parcela"]            =  $dados["valor_parcela"];
                $arDados[$inDados]["dt_vencimento_parcela"]    =  $dados["dt_vencimento_parcela"];
                $arDados[$inDados]["parcelas"]                 =  $dados["parcelas"];
                $arDados[$inDados]["parcelas_menos"]           =  $dados["parcelas_menos"];
                $arDados[$inDados]["percentual"]               =  $dados["percentual"];
                $arDados[$inDados]["nom_logradouro"]           =  $dados["nom_logradouro"];
                $inDados++;
            }
        ...
        } elseif ($arKey[0] == "termoInscricao.agt" || $arKey[0] == "certidaoDivida.agt" && $arKey[1] != "certidaoDividaMariana.odt" ) { //era o dado que vem do campo agt
        ...
        if ($arKey[0] == "certidaoDARemissaoMata.agt") {
            $arTMP = $rsDados->getElementos();
            if ( strripos( $arTMP[0]["origem"], "iptu" ) === false ) {
                $arTMP[0]["artigo"] = 14;
            } else {
                $arTMP[0]["artigo"] = 13;
            }

            if ($arTMP[0]["tipo_inscricao"] == "imóvel") {
                $arTMP[0]["texto1"] = "imobiliário em relação ao imóvel de Inscrição Imobiliária Municipal ";
                $arTMP[0]["texto2"] = "de propriedade, domínio útil ou posse do (a) Sr. (a) ";
                $arTMP[0]["texto3"] = "DESTE IMÓVEL";
                $arTMP[0]["texto4"] = "imóvel relacionado";

            } else {
                $arTMP[0]["texto1"] = "econômico de Inscrição Econômica Municipal ";
                $arTMP[0]["texto2"] = "de propriedade privada, do regime jurídico da empresa ";
                $arTMP[0]["texto3"] = "DESTA EMPRESA";
                $arTMP[0]["texto4"] = "lançamento da inscrição relacionada";
            }

            $OOParser->MergeBlock( 'Dat', $arTMP );
            ...
            for ( $inX=0; $inX<count( $arValorCreditos ); $inX++ ) {
                ...
                if ($arValorCreditos[$inX]["cod_natureza"] == 1) { //tributario
                    $arValorCreditosTributario[] = array (
                        ...
                        "exercicio" => $arValorCreditos[$inX]['exercicio'],
        ...
        if ( ( $arKey[0] == "termoAssuncao.agt" ) || ( $arKey[0] == "reqBenDev.agt" ) || ( $arKey[0] == "reqBenTerc.agt" ) || ( $arKey[0] == "termoConfissao.agt" ) ) {
            ...
            $arDataInscricao = explode( "/", date("d/m/Y") );
            $arMes = array( "01" => "Janeiro", "02" => "Fevereiro", "03" => "Março", "04" => "Abril", "05" => "Maio", "06" => "Junho", "07" => "Julho", "08" => "Agosto", "09" => "Setembro", 10 => "Outubro", 11 => "Novembro", 12 => "Dezembro" );
            $arTemp[0]["data_atual"] = $arDataInscricao[0]." de ".$arMes[$arDataInscricao[1]]." de ".$arDataInscricao[2]." ".date("G:i");            $arTemp[0]["matricula"] = $rsDado2->getCampo("registro");
            $OOParser->MergeBlock( 'Dat2',   $arTemp );
        }else
        if ($arKey[0] == "termoParcelamento.agt") {
        ...
        } elseif ( ( $arKey[0] == "termoParcelamentoDAUrbem.agt" ) || ( $arKey[0] == "termoConsolidacaoDAUrbem.agt" ) ) {
            //modelo generico de termo de parcelamento
            //modelo generico de termo de consolidacao
        ...
        if ($arKey[0] == "notificacaoAcordo.agt") { //era o dado que vem do campo agt
            ...
            if (count($rsDados->arElementos) == 0) {
                SistemaLegado::alertaAviso($pgFilt, "Não existem inscrições em dívida sem cobrança administrativa em aberto para a inscrição selecionada","n_erro","aviso",Sessao::getId(), "../");
            } else {
        ...
        } elseif ($arKey[0] == "termoInscricao.agt" || $arKey[0] == "certidaoDivida.agt" && $arKey[1] != "certidaoDividaMariana.odt" ) {
            //esta consulta serve para o termo de inscricao da divida
            //esta consulta serva para certidao de divida ativa do municipio
            $arTMP1 = $rsDados->arElementos;
        ...
        } elseif ($arKey[1] == "certidaoDividaMariana.odt") {
            ...
            $stFiltro = " WHERE dpar.num_parcelamento in ( ".$arKey[6]." ) ";
            $obTDATDividaAtiva->recuperaConsultaTermoInscricaoDividaGenericoValorOrigem( $rsDadosValorCreditos, $stFiltro );
            $obTDATDividaAtiva->recuperaConsultaTermoInscricaoDividaGenericoConfiguracaoUsuario( $rsDadosConfiguracaoUsuario, 1 );
            ...
            for ( $inX=0; $inX<count( $arValorCreditos ); $inX++ ) {
                ...
                $flValorTotal += $arValorCreditos[$inX]["valor_origem"] + $flJurosT + $flMultaT + $flCorrecaoT;
            }
            ...
            $arValorCreditos[0]["total_reducao"] = number_format( $arValorCreditos[0]["total_reducao"], 2, ',', '.' );
            $arValorCreditos[0]["valor_origem"] = number_format( $arValorCreditos[0]["valor_origem"], 2, ',', '.' );
            if ($flValorTotalT <= 0) {
                $arValorCreditosTributario[] = array (
                    "credito_origem" => "",
                    "valor_origem" => "",
            ...
            //Moficações do cabecalho para certidao da divida em mariana já que é especifico
            $stEmissaoDocumento = $arValorCreditos[0]["num_documento"]."/".$inExercicio;
            
            $rsDadosConfiguracaoUsuario->setCampo('nom_prefeitura',"MUNICÍPIO DE MARIANA PIMENTEL");
            $rsDadosConfiguracaoUsuario->setCampo('estado',"ESTADO DO RIO GRANDE DO SUL");
            $rsDadosConfiguracaoUsuario->setCampo('emissao_documento',$stEmissaoDocumento );
        ...
        $arTemp = explode('/', $arKey[11]);
        $stTemp = substr($arTemp[1], 0, 4);
        $inTotal++;
}
...
$obButtonVoltar->obEvento->setOnClick( "Cancelar('".$pgList.$stLink."'); BloqueiaFrames(true, false);" );
