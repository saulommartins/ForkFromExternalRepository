...
<?php
/**
  * Página de Processamento para Remissao
  * Data de criação : 05/05/2010

  * @author Analista: Fábio Bertoldi
  * @author Programador: Eduardo Paculski Schitz
...
if ($inTotalNaListaCreditoSessao <= 0) {
    ...
    SistemaLegado::exibeAviso('Lista de créditos está vazia!', 'n_incluir', 'erro');
    exit;
}
...
} elseif ($_REQUEST['inCodImovelFinal']) {
...
for ($inX=0; $inX < $inTotalNaListaCreditoSessao; $inX++) {
    $arDados = explode('.', $arListaCreditoSessao[$inX]['stCodCredito']);
    $stExercicioTMP = $arListaCreditoSessao[$inX]['stExercicio'];

    $stFiltroCredito .= ' (   calculo.cod_credito  = '.$arDados[0];
    $stFiltroCredito .= ' AND calculo.cod_especie  = '.$arDados[1];
    $stFiltroCredito .= ' AND calculo.cod_genero   = '.$arDados[2];
    $stFiltroCredito .= ' AND calculo.cod_natureza = '.$arDados[3];
    $stFiltroCredito .= ' AND calculo.exercicio    = '.$stExercicioTMP;
    $stFiltroCredito .= ' ) OR ';
}

$stFiltroCredito = substr($stFiltroCredito, 0, strlen($stFiltroCredito)-3);

$stFiltro .= ' AND ('.$stFiltroCredito.') ';
...
$obTDATDividaRemissao = new TDATDividaRemissao;
$obTDATDividaRemissao->ListaLancamentosPraRemissaoCredito($rsListaDados, $stFiltro, $stFuncaoValidacao, str_replace (',', '.', str_replace ('.', '', $_REQUEST['stLimiteCredito'])), $stFiltroLancamentoAtivo);
$obTDATDividaRemissao->debug();
...
for ($inX = 0; $inX < count($arDadosRemissaoTMP); $inX++) {
    ...
    if ($stValidacaoRemissao == 'todos' && $arDadosExcluidos[$stFiltro] == 1) {
        $boIncluir = false;
    }
    ...
    if ($_REQUEST['stLimiteCredito'] && $boIncluir) {
        if (($arDadosRemissaoTMP[$inX]['valor'] > str_replace(',', '.', str_replace('.', '', $_REQUEST['stLimiteCredito']))) && $boIncluir) {
            if ($stValidacaoRemissao == 'todos') {
                $arDadosExcluidos[$stFiltro] = 1;
                unset($arDadosTMP[$stFiltro]); //podia ter alguma coisa anterior deste cara
            }
    ...
    if ($boIncluir) {
        if ($arDadosTMP[$stFiltro]) {
            ...
            $arDadosTMP[$stFiltro]['exercicio_da']     = $arDadosRemissaoTMP[$inX]['exercicio_da'];
            $arDadosTMP[$stFiltro]['cod_inscricao_da']     = $arDadosRemissaoTMP[$inX]['cod_inscricao_da'];
...
if (count($arDadosTMP2)<= 0) {
    SistemaLegado::LiberaFrames();
    SistemaLegado::exibeAviso('Nenhum registro retornado para os crédito selecionados!', 'n_incluir', 'erro');
    exit;
}
...
while ($inInscricoesRemir < $inTotalRemir) {
    ...
    $inCodInscricaoDA = $arDadosTMP2[$inInscricoesRemir]['cod_inscricao_da'];
    $inExercicioDA = $arDadosTMP2[$inInscricoesRemir]['exercicio_da'];
    ...
    for ($inLancamentos = 0; $inLancamentos < count($arLancamentos); $inLancamentos++) {
        ...
        if ($stInscricaoAutomatica == 'nao') {
            if ($stTipoInscricao != 'd') { //carnes devem ser cancelados com o motivo 14
                ...
                $stSql = '
                   ...
                   INNER JOIN ( SELECT min(parcela_origem.num_parcelamento) AS num_parcelamento
                            ...
                            INNER JOIN arrecadacao.parcela
                                 ...
                                 WHERE divida_parcelamento.cod_inscricao = '.$inCodInscricaoDA.'
                                   AND divida_parcelamento.exercicio = '.$inExercicioDA.'
                ...
                //alterar esta consulta pra funfar pelo lancamento
            ...
            if ($stTipoInscricao != 'd') { //inscreve primeiro na divida ativa
                ...
                } elseif ($rsModalidade->getCampo('cod_forma_inscricao') == 3) { //Parcelas Individuais
                    $arParcelasDivida = $rsParcelasDivida->getElementos();
                    $arTMP = array();
                    $inTotalDeParcelas = 0;
                    for ($inW = 0; $inW < count($arParcelasDivida); $inW++) {
                ...
                } elseif ($rsModalidade->getCampo('cod_forma_inscricao') == 2) { //Valor Total Por Crédito
                    $arParcelasDivida = $rsParcelasDivida->getElementos();
                    $arTMP = array();
                    $inTotalDeCreditos = 0;
                    for ($inW = 0; $inW < count($arParcelasDivida); $inW++) {
                ...
                } elseif ($rsModalidade->getCampo('cod_forma_inscricao') == 1) { //Valor Total
                    $obTDATDividaAtiva->recuperaCodigoInscricaoComponente($rsListaPosicao, $boTransacao);
                    $inCodInscricao = $rsListaPosicao->getCampo('max_inscricao')>0?$rsListaPosicao->getCampo('max_inscricao')+1:1;
                    $arInscricoesDA[] = $inCodInscricao;
