...
/**
    ...
    * $Id: RARRLayoutCNAB.class.php 46946 2012-06-29 16:30:55Z tonismar $
...
/*
$Log$
Revision 1.1  2007/09/03 13:16:46  cercato
Ticket#10057#
...
class RARRLayoutCaixaCNAB extends RARRPagamento
{
/**
    * @var Object
    * @access Private
    ...
    if ($boAuto) {
        ...
        foreach ($arDados AS $key => $stValorLinha) {
            ...
            if ( strtoupper($stPrimeiroElemento) == "T" ) {
                ...
                $stNumeracao = substr( $stValorLinha, 46, 10 );
    ...
    */
    ...
    foreach ($arDadosArquivo AS $key => $stValorLinha) {
        ...
        if ($inPosicao <= 1) { //header
            ...
            if ($nuBanco == '104') {
                $nuAgencia = substr( $stValorLinha, 53, 4 );
            } else {
                $nuAgencia = substr( $stValorLinha, 52+(6-$inTamanhoAgencia), $inTamanhoAgencia-1 );
            }
            //$nuAgencia .= "-".substr( $stValorLinha, 57, 1 ); comentado por enquanto pois na base não possui digito verificador
        ...
        if ( strtoupper( $stPrimeiroElemento ) == "U" ) { //CONTEUDO 2
            ...
            $stDataPagamento    = substr( $stValorLinha, 137, 8 ); //data da ocorrência
