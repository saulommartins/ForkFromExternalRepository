...
class RCarneDiversosPetropolis extends RProtocolo
{
    ...
    public function drawCarne($x, $y)
    {
        ...
        if ($this->ImagemCarne) {
            ...
            $this->Image( $this->ImagemCarne, $x, $y+5, 16, 9, $stExt );
        }
        ...
        $this->setFont ( 'Arial', 'B', 7 );
        $this->Text ( $x+38, $y+(9*$this->tamY), "MATA DE SÃO JOÃO" );
        $this->Text ( $x+38, $y+(11*$this->tamY)+0.5, "Sec. de Adm. e Fin." );
        $this->Text ( $x+38, $y+(14*$this->tamY), "EXERCÍCIO 2011" );
        ...
        if ($this->stParcela == 'ÚNICA') {
            ...
            $this->Text ( $x, $y+(60*$this->tamY), "Observação:" );
            $this->Text ( $x, $y+(63*$this->tamY), "RECEBER ATÉ 31/12/2011." );
        }
        ...
        if ($this->stParcela == 'ÚNICA') {
            ...
            $this->Text ( $x+27+$inAlteracaoMata, $y+(36*$this->tamY), "NÃO RECEBER APÓS VENCIMENTO." );
        } else {
            $this->Text ( $x+27+$inAlteracaoMata, $y+(34*$this->tamY), "Instruções" );
            $this->Text ( $x+27+$inAlteracaoMata, $y+(36*$this->tamY), "RECEBER ATÉ 31/12/2011." );
        }
...
O contribuinte que não efetuar o recolhimento da taxa em quota única poderá fazê-lo em até 03 (três) parcelas mensais, na forma e prazos estabelecidos por Ato do Poder Executivo, fixado o vencimento da primeira parcela em 17 de janeiro de 2011.\n
...
Necessitando alterar os dados cadastrais compareça, munido de documentos comprobatórios, à Coordenadoria Fazendária do Município, órgão da Secretaria Municipal de Administração e Finanças, sito na Rua Antônio Luiz Garcez, s/nº - Centro Administrativo - Centro - CEP: 48.280-000 - Mata de São João/BA.\n";
        ...
        $this->setFont( 'Arial','B',10 );
        $this->Text( $x+122 , $y+(88*$inTamY) , 'EXERCÍCIO 2011');
        ...
        $valorDesconto = $parcelaUnica-($parcelaUnica*($percentualDesconto/100));

        $this->Text( $x+55 , $y+(136*$inTamY) , number_format($valorDesconto, 2, ',', '.') );
...
class RCarneTFFMataSaoJoao2011
{
/*
    * @var Integer
    * @access Private
...
/*
    ...
    * @access Private
*/
function RCarneTFFMataSaoJoao2011($arEmissao, $horizontal = 7, $vertical = 95)
{
        ...
        foreach ($chave as $parcela) { // impressao das parcelas selecionadas para cada codigo de lancamento
            ...
            if ($diffBaixa) {
                ...
                if ( $rsParcela->getCampo( 'nr_parcela' ) == 0 ) {
                    ...
                    $this->obRCarnePetropolis->setObservacaoL3 ( 'Receber até 31/12/2011.' );
                    $this->obRCarnePetropolis->setParcela ( 'ÚNICA' );
                    $this->obRCarnePetropolis->stObsVencimento = "Receber até 31/12/2011.";
