...
class RCarneDiversosPetropolis extends RProtocolo
{
    ...
    public function posicionaVariaveis($x, $y)
    {
        ...
        $this->Text   ( ($x+164) , ($y+(42.25*$this->tamY)) , $this->flValor );
        $this->Text   ( ($x+164) , ($y+(62.25*$this->tamY)), number_format($valorJurosMulta,2,',','.'));
        $this->Text   ( ($x+164) , ($y+(68.95*$this->tamY)) , $this->flValorOutros );
        $this->Text   ( ($x+164) , ($y+(75.65*$this->tamY)) , $this->flValorTotal );

        $this->Text   ( ($x+9) , ($y+(50*$this->tamY)) , 'REFERENTE AO IPTU DO EXERCÍCIO DE 2011' );
    ...
    public function posicionaVariaveisNotificacao($x, $y)
    {
        ...
        $this->Text   ( ($x+9) , ($y+(110*$this->tamY)) , 'REFERENTE AO IPTU DO EXERCÍCIO DE 2011' );
...
/*
    ...
    * @access Private
*/
function RCarneIPTUItau($arEmissao, $horizontal = 7, $vertical = 95)
{
...
function imprimirCarne($diffBaixa = FALSE)
{
    ...
    // $stFiltro = " WHERE exercicio = ".Sessao::getExercicio()." AND parametro = 'logotipo_iptu' ";
    // $obTAdministracaoConfiguracao->recuperaTodos( $rsListaImagens, $stFiltro );
    // $stNomeImagemNotificacao = $rsListaImagens->getCampo("valor");
    $stNomeImagemNotificacao = 'logotipo_iptu.png';

    // $rsListaImagens = '';
    // $stFiltro = " WHERE exercicio = ".Sessao::getExercicio()." AND parametro = 'logotipo_iptu_boleto' ";
    // $obTAdministracaoConfiguracao->recuperaTodos( $rsListaImagens, $stFiltro );
    ...
    foreach ($this->arEmissao as $valor => $chave) {
        ...
        // $inCountParcelas = 0;
        // while (!$rsParcela->eof()) {
        //     if ($rsParcela->getCampo('cod_parcela') == $chave[0]['cod_parcela']) {
        //         $arTemp = array(
    ...
    if ( $this->getConsolidacao() ) {
            ...
            foreach ($chave as $parcela) { // impressao das parcelas selecionadas para cada codigo de lancamento
                ...
                $this->obRCarneMata->setValor( number_format($nuValorTotal,2,',','.') );
                $this->obRCarneMata->setInfoParcelas($arInfoParcelas);
