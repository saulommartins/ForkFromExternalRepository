...
<?php
/**
    * Carne Escrituracao de Mata de Sao Joao
    ...
    * $Id: RCarneEscrituracaoMataSaoJoao.class.php 59612 2014-09-02 12:00:51Z gelson $
...
class RCarneMataSaoJoao extends fpdf
{
    /**
     * @access private
    ...
    /**
     * @access private
     * @var Array CGM prestador
     */
    public $arCgmPrestador;
    ...
    public $doTotal;
    public $doAtMonet;
    public $stObservacoes;

    /**
     * Construtor
     */
    public function RCarneMataSaoJoao()
    {
        parent::FPDF();
        /**
    ...
    public function desenhaCarne($x = 0 , $y = 0)
    {
        ...
        $this->Rect( $x+(123*$tamX) , $y + (8*$tamY) , 25*$tamX, 49*$tamY); // via banco
        $this->Rect( $x+(123*$tamX) , ($y*$tamY) + (78*$tamY) , 25*$tamX, 49*$tamY); // via contrib

        /* Retangulo da Inscrica, Exercicio, etc  ... */
        $this->Rect( $x+(5*$tamX) , $y + (10*$tamY) , 118*$tamX, 8*$tamY); // via banco
        $this->Rect( $x+(5*$tamX) , ($y*$tamY) + (80*$tamY) , 118*$tamX, 8*$tamY); // via contrib

        /* Retangulo da Razao, Atividade, Endereco .... */
        $this->Rect( $x+(5*$tamX) , $y + (18*$tamY) , 118*$tamX, 9*$tamY); // via banco
        $this->Rect( $x+(5*$tamX) , ($y*$tamY) + (88*$tamY) , 118*$tamX, 9*$tamY); // via contrib

        /* Retangulo do Nome Fantasia ... */
        $this->Rect( $x+(5*$tamX) , $y + (27*$tamY) , 118*$tamX, 16*$tamY); // via banco
        $this->Rect( $x+(5*$tamX) , ($y*$tamY) + (97*$tamY) , 118*$tamX, 16*$tamY); // via contrib

        $this->Rect( $x+(5*$tamX) , ($y*$tamY) + (113*$tamY) , 118*$tamX, 8*$tamY); // via contrib
        ...
        $this->Line( $x+(28*$tamX) , ($y*$tamY)+(80*$tamY) , $x+(28*$tamX) , ($y*$tamY)+(88*$tamY) ); // via contrib

        /* entre exercicio e aviso */
        $this->Line( $x+(50*$tamX) , $y+(10*$tamY) , $x+(50*$tamX) , $y+(18*$tamY)); // via banco
        $this->Line( $x+(50*$tamX) , ($y*$tamY)+(80*$tamY) , $x+(50*$tamX) , ($y*$tamY)+(88*$tamY)); // via contrib

        /* entre aviso e receita */
        $this->Line( $x+(75*$tamX) , $y+(10*$tamY) , $x+(75*$tamX) , $y+(18*$tamY)); // via banco
        $this->Line( $x+(75*$tamX) , ($y*$tamY)+(80*$tamY) , $x+(75*$tamX) , ($y*$tamY)+(88*$tamY)); // via contrib

        /**
         * Linhas Horizontais
         */
        $this->Line( $x+(5*$tamX) , $y+(24*$tamY) , 131*$tamX , $y+ (24*$tamY) ); // via banco
        $this->Line( $x+(5*$tamX) , ($y*$tamY)+(94*$tamY) , 131*$tamX , ($y*$tamY)+(94*$tamY) ); // via banco

         /* entre parcela e vencimento */
        $this->Line( $x+(123*$tamX) , $y+(8*$tamY) , $x+(148*$tamX) , $y+ (8*$tamY) ); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+(78*$tamY) , $x+(148*$tamX) , ($y*$tamY)+(78*$tamY) ); // via contrib

        /* entre vencimento e valor */
        $this->Line( $x+(123*$tamX) , $y+(15*$tamY) , $x+(148*$tamX) , $y+(15*$tamY) ); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+(85*$tamY) , $x+(148*$tamX) , ($y*$tamY)+(85*$tamY) ); // via contrib

        /* entre valor e multa */
        $this->Line( $x+(123*$tamX) , $y+ (22*$tamY) , $x+(148*$tamX) , $y+ (22*$tamY)); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+ (92*$tamY) , $x+(148*$tamX) , ($y*$tamY)+ (92*$tamY)); // via contrib

        /* entre multa e juros */
        $this->Line( $x+(123*$tamX) , $y+ (30*$tamY) , $x+(148*$tamX) , $y+ (30*$tamY) ); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+(100*$tamY) , $x+(148*$tamX) , ($y*$tamY)+(100*$tamY) ); // via contrib

        /* entre jutos e correcao */
        $this->Line( $x+(123*$tamX) , $y+ (37*$tamY) , $x+(148*$tamX) , $y+ (37*$tamY)); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+(107*$tamY) , $x+(148*$tamX) , ($y*$tamY)+(107*$tamY)); // via contrib

        /* entre correcao e total */
        $this->Line( $x+(123*$tamX) , $y+ (50*$tamY) , $x+(148*$tamX) , $y+ (50*$tamY)); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+(120*$tamY) , $x+(148*$tamX) , ($y*$tamY)+(120*$tamY)); // via contrib

        // entre o total e o numeracao
        $this->Line( $x+(123*$tamX) , $y+ (44*$tamY) , $x+(148*$tamX) , $y+ (44*$tamY) ); // via banco
        $this->Line( $x+(123*$tamX) , ($y*$tamY)+(114*$tamY) , $x+(148*$tamX) , ($y*$tamY)+(114*$tamY) ); // via contrib

        // linha entre numero da nota e cgm
        $this->Line( $x+(5*$tamX) , ($y + (27*$tamY))+(9 * $tamY) , $x + (5*$tamX) + (118*$tamX), ($y + (27*$tamY))+(9 * $tamY) ); // linha entre numero da nota e cgm
        $this->Line( $x+(5*$tamX) , (($y*$tamY) + (97*$tamY))+(9 * $tamY) , $x + (5*$tamX) + (118*$tamX), (($y*$tamY) + (97*$tamY))+(9 * $tamY) ); //linha entre numero da nota e cgm
        ...
        $this->Text   ( ($x+(37*$tamX) ) , (($y*$tamY)+(75*$tamY) ) , $this->stNomePrefeitura ); // via contrib

        /* subtitulo*/
        $this->setFont('Arial','',7);
        $this->Text   ( ($x+(45*$tamX) ) , ($y+(6*$tamY))  , $this->stSubTitulo ); // via banco
        $this->Text   ( ($x+(45*$tamX) ) , (($y*$tamY)+(78*$tamY)) , $this->stSubTitulo ); // via contrib

        $this->setFont('Arial','',5);

        $this->Text   ( ($x+(6*$tamX) ) , ($y+(12*$tamY) ) , 'INSCRIÇÃO MUNICIPAL' ); // via banco
        $this->Text   ( ($x+(6*$tamX) ) , (($y*$tamY)+(82*$tamY) ) , 'INSCRIÇÃO MUNICIPAL' ); // via contrib

        $this->Text   ( ($x+(30*$tamX)) , ($y+(12*$tamY)) , 'EXERCICIO'); // via banco
        $this->Text   ( ($x+(30*$tamX)) , (($y*$tamY)+(82*$tamY)) , 'EXERCICIO'); // via contrib

        $this->Text   ( ($x+(52*$tamX)) , ($y+(12*$tamY)) , 'No DE AVISO'); // via banco
        $this->Text   ( ($x+(52*$tamX)) , (($y*$tamY)+(82*$tamY)) , 'No DE AVISO'); // via contrib

        $this->Text   ( ($x+(77*$tamX)) , ($y+(12*$tamY)) , 'RECEITA'); // via banco
        $this->Text   ( ($x+(77*$tamX)) , (($y*$tamY)+(82*$tamY)) , 'RECEITA'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (10*$tamY)) , 'COMPETÊNCIA'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (80*$tamY)) , 'COMPETÊNCIA'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (17*$tamY))  , 'VENCIMENTO'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (87*$tamY)) , 'VENCIMENTO'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (24*$tamY))  , 'VALOR em R$'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (94*$tamY)) , 'VALOR em R$'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (32*$tamY))  , 'MULTA'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (102*$tamY)) , 'MULTA'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (39*$tamY))  , 'JUROS'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (109*$tamY)) , 'JUROS'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (46*$tamY))  , 'AT. MONET.'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (116*$tamY)) , 'AT. MONET.'); // via contrib

        $this->Text   ( ($x+(124*$tamX)) , ($y + (52*$tamY))  , 'TOTAL'); // via banco
        $this->Text   ( ($x+(124*$tamX)) , (($y*$tamY) + (122*$tamY)) , 'TOTAL'); // via contrib

        $this->Text   ( $x+(7*$tamX) , ($y*$tamY) + (115*$tamY) , 'Observações:'); // via contrib

        /* nome fantasia */
        $this->Text   ( ($x+(6*$tamX)) , ($y+(26*$tamY))  , 'NOME FANTASIA'); // via banco
        $this->Text   ( ($x+(6*$tamX)) , (($y*$tamY)+(96*$tamY)) , 'NOME FANTASIA'); // via contrib

        /* numero da nota */
        $this->Text   ( ($x+(6*$tamX)) , ($y+(29*$tamY))  , 'NÚMERO DA NOTA' ); // via banco
        $this->Text   ( ($x+(6*$tamX)) , (($y*$tamY)+(99*$tamY)) , 'NÚMERO DA NOTA' ); // via contrib

        /* cgm do prestador */
        $this->Text   ( ($x+(6*$tamX)) , ($y+(38*$tamY))  , 'PRESTADOR' ); // via banco
        $this->Text   ( ($x+(6*$tamX)) , (($y*$tamY)+(108*$tamY)) , 'PRESTADOR' ); // via contrib

        /* instruções */
        $this->Text   ( ($x+(5*$tamX)) , ($y+(45*$tamY))  , 'INSTRUÇÕES'); // via banco
        $this->Text   ( ($x+(5*$tamX)) , (($y*$tamY)+(125*$tamY)) , 'INSTRUÇÕES'); // via contrib
        ...
        $this->Text   ( ($x+(10*$tamX)) , (($y*$tamY)+(86*$tamY)) , $this->stInscricaoMunicipal ); // via contrib

        /* exercicio */
        $this->Text   ( ($x+(34*$tamX)) , ($y+(16*$tamY)) , $this->stExercicio ); // via banco
        $this->Text   ( ($x+(34*$tamX)) , (($y*$tamY)+(86*$tamY)) , $this->stExercicio ); // via contrib

        /* aviso */
        $this->Text   ( ($x+(53*$tamX)) , ($y+(16*$tamY)) , $this->stNumAviso ); // via banco
        $this->Text   ( ($x+(53*$tamX)) , (($y*$tamY)+(86*$tamY)) , $this->stNumAviso ); // via contrib

        /* receita */
        $this->Text   ( ($x+(82*$tamX)) , ($y+(16*$tamY)) , $this->stReceita ); // via banco
        $this->Text   ( ($x+(82*$tamX)) , (($y*$tamY)+(86*$tamY)) , $this->stReceita ); // via contrib

        /* competencia*/
        $this->Text   ( ($x+(127*$tamX)) , ($y+(13*$tamY)) , $this->stCompetencia ); // via banco
        $this->Text   ( ($x+(127*$tamX)) , (($y*$tamY)+(83*$tamY)) , $this->stCompetencia ); // via contrib

        /* vencimento */
        $this->setFont('Arial','B',7);
        $this->Text   ( ($x+(128*$tamX)) , ($y+(20*$tamY))  , $this->dtVencimento ); // via banco
        $this->Text   ( ($x+(128*$tamX)) , (($y*$tamY)+(90*$tamY)) , $this->dtVencimento ); // via contrib
        $this->setFont('Arial','B',6);

        /* valor */
        $this->Text   ( ($x+(128*$tamX)) , ($y+(28*$tamY))  , $this->doValor ); // via banco
        $this->Text   ( ($x+(128*$tamX)) , (($y*$tamY)+(98*$tamY)) , $this->doValor ); // via contrib

        /* multa */
        $this->Text   ( ($x+(128*$tamX)) , ($y+(35*$tamY))  , $this->doMulta ); // via banco
        $this->Text   ( ($x+(128*$tamX)) , (($y*$tamY)+(105*$tamY)) , $this->doMulta ); // via contrib

        /* juros */
        $this->Text   ( ($x+(128*$tamX)) , ($y+(42*$tamY))  , $this->doJuros ); // via banco
        $this->Text   ( ($x+(128*$tamX)) , (($y*$tamY)+(112*$tamY)) , $this->doJuros ); // via contrib

        /* at monet */
        $this->Text   ( ($x+(128*$tamX)) , ($y+(49*$tamY))  , $this->doAtMonet ); // via banco
        $this->Text   ( ($x+(128*$tamX)) , (($y*$tamY)+(119*$tamY)) , $this->doAtMonet ); // via contrib

        /* total */
        $this->Text   ( ($x+(128*$tamX)) , ($y+(55*$tamY))  , $this->doTotal ); // via banco
        $this->Text   ( ($x+(128*$tamX)) , (($y*$tamY)+(125*$tamY)) , $this->doTotal ); // via contrib

        /* razao social */
        $this->Text   ( ($x+(6*$tamX)) , ($y+(21*$tamY))  , $this->stRazaoSocial ); // via banco
        $this->Text   ( ($x+(6*$tamX)) , (($y*$tamY)+(91*$tamY)) , $this->stRazaoSocial ); // via contrib

        /* endereco */
        $this->Text   ( ($x+(6*$tamX)) , ($y+(23*$tamY))  , $this->stEndereco ); // via banco
        $this->Text   ( ($x+(6*$tamX)) , (($y*$tamY)+(93*$tamY)) , $this->stEndereco ); // via contrib

        /* nome fantasia */
        $this->Text   ( ($x+(25*$tamX)) , ($y+(26*$tamY))  , $this->stNomeFantasia ); // via banco
        $this->Text   ( ($x+(25*$tamX)) , (($y*$tamY)+(96*$tamY)) , $this->stNomeFantasia ); // via contrib
        ...
        $this->Text   ( ($x+(25*$tamX)) , (($y*$tamY)+(99*$tamY)) , substr( $this->stNumeroNota, 0, 104) ); // via contrib
        $this->Text   ( ($x+(10*$tamX)) , (($y*$tamY)+(101*$tamY)) , substr( $this->stNumeroNota, 80, 120) ); // via contrib
        $this->Text   ( ($x+(10*$tamX)) , (($y*$tamY)+(103*$tamY)) , substr( $this->stNumeroNota, 160, 120) ); // via contrib
        $this->Text   ( ($x+(10*$tamX)) , (($y*$tamY)+(105*$tamY)) , substr( $this->stNumeroNota, 160, 120) ); // via contrib

        //observacoes
        $this->setFont('Arial','', 5);
        $this->Text   ( $x+(7*$tamX) , ($y*$tamY) + (117*$tamY) , substr( $this->stObservacoes, 0, 170) ); // via contrib
        $this->Text   ( $x+(7*$tamX) , ($y*$tamY) + (119*$tamY) , substr( $this->stObservacoes, 170, 170) ); // via contrib

        $this->setFont('Arial','B',6);
        /*cgm prestador*/
        $this->Text   ( ($x+(17*$tamX)) , ($y+(38*$tamY)) , $this->arCgmPrestador[0] ); // via banco
        $this->Text   ( ($x+(17*$tamX)) , ($y+(40*$tamY)) , $this->arCgmPrestador[1] ); // via banco
        $this->Text   ( ($x+(17*$tamX)) , ($y+(42*$tamY)) , $this->arCgmPrestador[2] ); // via banco

        $this->Text   ( ($x+(17*$tamX)) , (($y*$tamY)+(108*$tamY)) , $this->arCgmPrestador[0] ); // via contrib
        $this->Text   ( ($x+(17*$tamX)) , (($y*$tamY)+(110*$tamY)) , $this->arCgmPrestador[1] ); // via contrib
        $this->Text   ( ($x+(17*$tamX)) , (($y*$tamY)+(112*$tamY)) , $this->arCgmPrestador[2] ); // via contrib

        /* instruções */
        $this->setFont('Arial','',5);
        $this->Text   ( ($x+(20*$tamX)) , ($y+(45*$tamY))  , $this->arObservacoes[0]); // via banco
        $this->Text   ( ($x+(20*$tamX)) , ($y+(47*$tamY))  , $this->arObservacoes[1]); // via banco
        $this->Text   ( ($x+(20*$tamX)) , ($y+(49*$tamY))  , $this->arObservacoes[2]); // via banco
        $this->Text   ( ($x+(20*$tamX)) , ($y+(51*$tamY))  , $this->arObservacoes[3]); // via banco

        $this->Text   ( ($x+(20*$tamX)) , (($y*$tamY)+(125*$tamY))  , $this->arObservacoes[0]); // via contrib
        $this->Text   ( ($x+(20*$tamX)) , (($y*$tamY)+(127*$tamY))  , $this->arObservacoes[1]); // via contrib
        $this->Text   ( ($x+(20*$tamX)) , (($y*$tamY)+(129*$tamY))  , $this->arObservacoes[2]); // via contrib
        $this->Text   ( ($x+(20*$tamX)) , (($y*$tamY)+(131*$tamY))  , $this->arObservacoes[3]); // via contrib
        ...
        /* autenticação */
        $this->Text   ( ($x+(70*$tamX)) , (($y*$tamY)+(134*$tamY)) , ' - AUTENTICAÇÃO DO BANCO - ');

        $this->setFont('Arial','',4);
        $this->Text   ( ($x+(2*$tamX)) , (($y*$tamY)+(143*$tamY)) , 'Data de emissão: '.date("d/m/Y h:i:s"));
...
class RCarneEscrituracaoMataSaoJoao
{
/*
    * @var Array
    * @access Private
...
function RCarneEscrituracaoMataSaoJoao($arEmissao)
{
...
function imprimirCarne()
{
   //para gerar carne escrituracao
    ...
    $obCarne = new RCarneMataSaoJoao();

    /* consulta dados para o carne */
    require_once( CAM_GT_ARR_MAPEAMENTO . 'TARRParcela.class.php');
    ...
    $inPaginaAtual = count( $this->arEmissao );
    foreach ($this->arEmissao as $valor => $chave) {
        foreach ($chave as $parcela) {
            $obTARRParcela->setDado( "cod_parcela", $parcela["cod_parcela"] );
            $obTARRParcela->recuperaPorChave( $rsDadosParcela );

            $arTemp = explode ( "/", $rsDadosParcela->getCampo( "vencimento" ) );
            $stFiltro  = " ece.inscricao_economica = ".$parcela["inscricao"]." and ";
            $stFiltro .= " acne.numeracao = '".$parcela["numeracao"]."'";

            $obTARRCadastroEconomicoCalculo->recuperaConsultaReqReceita( $rsDadosCarne , $stFiltro, "", "", $arTemp[2]."-".$arTemp[1]."-".$arTemp[0] );
            $stNumeracaoCarne = "";
            $inX = 0;
            $arPrestador = array();
            while ( !$rsDadosCarne->Eof() ) {
                $arPrestador[$inX] = $rsDadosCarne->getCampo("prestador");
                if ( $inX > 0 )
            ...
            $rsDadosCarne->setprimeiroElemento();
            ...
            $obBarra = new RCodigoBarraFebrabanCompensacaoBB_Anexo5;
            $arBarra = array();
            
            $this->arBarra['valor_documento'] = $rsDadosCarne->getCampo('valor_total');
            $this->arBarra['fator_vencimento'] = (string) $rsDadosCarne->getCampo( 'fator_vencimento' );
            $this->arBarra['nosso_numero'] = (string) $rsDadosCarne->getCampo( 'numeracao' );
            $this->arBarra['convenio'] = 960663;
            $this->arBarra['tipo_moeda'] = 9;

            /* gera codigio de barras e linha digitavel */
            $arCodigoBarra = $obBarra->geraFebraban( $this->arBarra );

            $obCarne->arCgmPrestador = $arPrestador;
            $obCarne->stNumeroNota = $stNumeracaoCarne;
            $obCarne->stObservacoes = $rsDadosCarne->getCampo('observacao');
            ...
            $obCarne->doAtMonet = number_format($rsDadosCarne->getCampo('valor_correcao'),2,',','.');
