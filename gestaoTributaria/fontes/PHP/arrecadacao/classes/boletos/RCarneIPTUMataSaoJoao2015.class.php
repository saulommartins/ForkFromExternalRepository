...
//            $this->Text   ( 70    , 73   , $stObsL7 );
    ...
    */
    public function drawComplemento($x,$y)
    {
        ...
        $this->Text   ( ($x+40)  , ($y+7)   , strtoupper('IPTU-2015') );
...
class RCarneDiversosPetropolis extends RProtocolo
{
    ...
    public function drawCarne($x, $y)
    {
        ...
        $this->Text ( $x+38, $y+(16*$this->tamY), "EXERCÍCIO 2015" );
...
class RCarneDadosCadastraisMataSaoJoao extends RCarneDiversosPetropolis
{
    ...
    public function desenhaCarne($x , $y)
    {
        ...
        $this->Text( $x+62, $y+(10*$inTamY), 'IPTU/TLP/CIP 2015' );
        ...
        $this->setFont( 'Arial', 'B', 8 );
        $this->SetFillColor(240, 240, 240);
        $this->Rect( $x+2, $y+(78*$inTamY), 90, 4*$inTamY, 'DF');
        $this->Text( $x+14, $y+(81*$inTamY), 'Hospital Municipal Dr. Eurico Goulart de Freitas' );
...
//        $this->Line( $x+94, $y+(73*$inTamY), $x+184, $y+(73*$inTamY) );
        ...
        $this->setFont( 'Arial', '', 10 );
        $this->Text( $x+8, $y+(135*$inTamY), 'Prezado(a) Contribuinte,' );
        $this->Text( $x+8, $y+(139*$inTamY), 'O pagamento deste  carnê não  quita  débitos  anteriores. As  parcelas  vencidas  estão  sujeitas a multa e juros' );
        $this->Text( $x+8, $y+(142*$inTamY), 'moratórios. O prazo para  reclamação do lançamento será de até 30  (trinta)  dias,  contados a partir  da data  de' );
        $this->Text( $x+8, $y+(145*$inTamY), 'entrega do carnê. Os valores  do  demonstrativo estão expressos em  Reais  (R$).  Caso  o  vencimento  ocorra' );
        $this->Text( $x+8, $y+(148*$inTamY), 'em dia não útil, o pagamento deverá ser realizado no primeiro dia útil imediato. O  IPTU  não  implica  em  reco-' );
        $this->Text( $x+8, $y+(151*$inTamY), 'nhecimento de  direitos sobre o  imóvel  ou  na  sua  regularização.  Este carnê não poderá  ser  utilizado,  para' );
        $this->Text( $x+8, $y+(154*$inTamY), 'pagamento, após o dia 31 de dezembro de 2015.' );

        $this->Rect( $x+8, $y+(160*$inTamY), 171, 10*$inTamY ); //lado b

        $this->setFont( 'Arial', 'B', 8 );
        $this->Text( $x+12, $y+(165*$inTamY), 'A legislação municipal pertinente aos tributos pode ser encontrada no site da Prefeitura Municipal de Mata de São João,' );
        $this->Text( $x+22, $y+(168*$inTamY), 'no endereço www.pmsj.ba.gov.br .' );

        $this->setFont( 'Arial', 'B', 10 );
        $this->Text( $x+26, $y+(174*$inTamY), 'PAGÁVEL EM QUALQUER AGÊNCIA BANCÁRIA ATÉ O VENCIMENTO.' );

        $this->setFont( 'Arial', 'B', 8 );
        $this->Text( $x+8, $y+(178*$inTamY), 'Qualquer dúvida, consulte o Setor de Tributos e Fiscalização, no Centro Administrativo situado à Rua Luiz Antônio Garcez,' );
        $this->Text( $x+8, $y+(181*$inTamY), '140 - Centro - CEP: 48.280-000 - Mata de São João/BA. Tel: (71) 3635-1669 / (71) 3635-1310 / (71) 9957-6813.' );
        $this->addPage();
        ...
        $this->Text( $x+9, $y+(30*$inTamY), '(estes  valores   estão  definidos   na  Planta Genérica de Valores do Município, Lei nº 538/2013 alterada pela Lei nº 564/2014.' );
        ...
        $this->Text( $x+9, $y+(63*$inTamY), 'Fiscal/2015)' );
...
//fim do novo texto
        ...
        $this->Text( $x+122 , $y+(88*$inTamY) , 'EXERCÍCIO DE 2015');
...
class RCarneIPTUMataSaoJoao2015
{
/*
    * @var Integer
    * @access Private
...
/*
    ...
    * @access Private
*/
function RCarneIPTUMataSaoJoao2015($arEmissao, $horizontal = 7, $vertical = 95)
{
