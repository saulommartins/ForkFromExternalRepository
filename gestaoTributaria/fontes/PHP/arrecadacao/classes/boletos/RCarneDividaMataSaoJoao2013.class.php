...
class RCarneDividaMataSaoJoao2013
{
/*
    * @var Integer
    * @access Private
...
/*
    ...
    * @access Private
*/
function RCarneDividaMataSaoJoao2013($arEmissao, $horizontal = 7, $vertical = 95)
{
...
function imprimirCarne($diffBaixa = FALSE)
{
    ...
    foreach ($this->arEmissao as $valor => $chave) {
        ...
        $stFiltroCapa = " ddc.numcgm = ".$chave[0]['numcgm']." AND ap.cod_parcela = ".$chave[0]['cod_parcela']." \n";
        $obTDATDividaAtiva = new TDATDividaAtiva;

            $obTDATDividaAtiva->RecuperaCapaCarneDividaMataSaoJoao( $rsListaCarneCapa, $stFiltroCapa );
            $rsListaCarneCapa->ordena( "exercicio_original" );
            $arDadosParcelas = array();
            $inTotalParcelas = 0;
            if ( !$rsListaCarneCapa->Eof() ) {
                $rsListaCarneCapa->setPrimeiroElemento();
                ...
                while ( !$rsListaCarneCapa->Eof() ) {
                    $boIncluirInscricao = true;
                    for ($inD=0; $inD<$inTotalParcelas; $inD++) {
                        if ( $arParcelasNaLista[$inD] == $rsListaCarneCapa->getCampo("num_parcela") ) {
                    ...
                    if ($boIncluirInscricao) {
                        $this->obRCarnePetropolis->arDemonstrativoParcelas[] = $rsListaCarneCapa->getCampo("vlr_parcela");
                        $this->obRCarnePetropolis->arVencimentosDemonstrativos[] = $rsListaCarneCapa->getCampo("dt_vencimento_parcela");
                        $arParcelasNaLista[$inTotalParcelas] = $rsListaCarneCapa->getCampo("num_parcela");
                        $inTotalParcelas++;
                    }

                    $boIncluirInscricao = true;
                    for ($inD=0; $inD<$inTotalInscricoes; $inD++) {
                        if ( $arInscricoesNaLista[$inD]["inscricao"] == $rsListaCarneCapa->getCampo("cod_inscricao")."/".$rsListaCarneCapa->getCampo("exercicio_da") ) {
                            $boIncluirInscricao = false;
                            break;
                        }
                    }
                    ...
                    if ($boIncluirInscricao) {
                        $stFiltro = " dda.cod_inscricao = ".$rsListaCarneCapa->getCampo("cod_inscricao")." AND dda.exercicio = '".$rsListaCarneCapa->getCampo("exercicio_da")."' \n";
                        ...
                        $arInscricoesNaLista[$inTotalInscricoes]["inscricao"] = $rsListaCarneCapa->getCampo("cod_inscricao")."/".$rsListaCarneCapa->getCampo("exercicio_da");
                        $this->obRCarnePetropolis->arDetalhamentoInscricao[$inTotalInscricoes]["inscricao"] = $rsListaCarneCapa->getCampo("cod_inscricao")."/".$rsListaCarneCapa->getCampo("exercicio_da");
                        $this->obRCarnePetropolis->arDetalhamentoInscricao[$inTotalInscricoes]["ano devido"] = $rsListaCarneCapa->getCampo("exercicio_original");
                        if ($stImposto) {
                            $stImposto .= ", ".$rsListaCarneCapa->getCampo("exercicio_original");
                        }else
                            $stImposto = $rsListaCarneCapa->getCampo("exercicio_original");
                        ...
                        if ($rsListaCarne2->getCampo("juros2_mora") != "") {
                            $this->obRCarnePetropolis->arDetalhamentoInscricao[$inTotalInscricoes]["juros2"] = number_format($rsListaCarne2->getCampo("juros2_mora"), 2, ',', '.' );
                        } else {
                              $this->obRCarnePetropolis->arDetalhamentoInscricao[$inTotalInscricoes]["juros2"] = number_format(0.00, 2, ',', '.' );
                        }
                         if ($rsListaCarne2->getCampo("atualizacao2_mora") != "") {
                            $this->obRCarnePetropolis->arDetalhamentoInscricao[$inTotalInscricoes]["honorario"] = number_format($rsListaCarne2->getCampo("atualizacao2_mora"), 2, ',', '.' );
                              ...
                              $this->obRCarnePetropolis->arDetalhamentoInscricao[$inTotalInscricoes]["honorario"] = number_format(0.00, 2, ',', '.' );
                         }
                    ...
                    $rsListaCarneCapa->proximo();
                }
            }

        $rsListaCarneCapa->setPrimeiroElemento();
        $this->obRCarnePetropolis->stObservacoesComplementares  = (string) $rsListaCarneCapa->getCampo("observacao");
        ...
        $this->obRCarnePetropolis->stInscricaoImobiliaria       = (string) $rsListaCarneCapa->getCampo("inscricao_municipal");
        $this->obRCarnePetropolis->stInscricaoEconomica         = (string) $rsListaCarneCapa->getCampo("inscricao_economica");
        $this->obRCarnePetropolis->stImpostoTaxa                = $stImpostos;
        $this->obRCarnePetropolis->stNroCobrancaDA              = (string) $rsListaCarneCapa->getCampo("numero_parcelamento")."/".$rsListaCarneCapa->getCampo("exercicio_cobranca");
        $this->obRCarnePetropolis->stDataAcordo                 = (string) $rsListaCarneCapa->getCampo("dt_acordo");
        $this->obRCarnePetropolis->stReducao                    = $rsListaCarne2->getCampo("total_reducao");
        $this->obRCarnePetropolis->setInscricaoDivida   ( $rsListaCarneCapa->getCampo("cod_inscricao")."/".$rsListaCarneCapa->getCampo("exercicio_da") );
        //a capa termina aqui
        ...
        foreach ($arExercicioImposto AS $stTributoExerc => $arImposto) {
            foreach ($arImposto as $stimposto) {
                if (strlen($stTributoExercicio) + strlen($stimposto) > $inContTamanho * 75 ) {
                    //ajuste para montar sem quebras no boleto
                    $stTributoExercicio = str_pad($stTributoExercicio,$inContTamanho * 75,' ');
                    $inContTamanho++;
                }
                $stTributoExercicio .= $stimposto.",";
            }
        }
        ...
        while ( !$rsGeraCarneCabecalho->eof() ) {
            ...
            if ( preg_match('/LIMPEZA.*/',$rsGeraCarneCabecalho->getCampo( 'descricao_credito' ) ) ) {
        ...
        $this->obRCarnePetropolis->setImpostoAnualReal  ( number_format($this->obRCarnePetropolis->getImpostoAnualReal(),2,',','.') );
         if ($this->obRCarnePetropolis->getValorTributoReal() != "") {
            ...
            $this->obRCarnePetropolis->setValorTributoReal(number_format(0.00, 2, ',', '.' ));
         }
            ...
            foreach ($chave as $parcela) { // impressao das parcelas selecionadas para cada codigo de lancamento
                ...
                if ($diffBaixa) {
                    ...
                    if ( $rsParcela->getCampo( 'nr_parcela' ) == 0 ) {
                        ...
                        $this->obRCarnePetropolis->stObsVencimento = "Não receber após 90(noventa) dias do vencimento.";
