...
class RCarneDiversosPetropolis extends RProtocolo
{
    ...
    public $stZoneamento;
    public $stExercicio;
    public $stExercicio_selecionado;
    public $inInscricao;
    public $inCodDivida;
    public $stTributo;
    ...
    public $arObservacaoLayout;
    public $arLayoutInfo;
    public $stLayoutObsDebt;
    public $arLayoutVariaveis;
    public $stNomPref;
    public $stEndPref;
    public $stFonePref;
    public $stCNPJPref;
...
//eh aakiiiiii o carnets
        ...
        for ( $inVar=0; $inVar<count( $this->arLayoutInfo ); $inVar++ ) {
            if ($inPosicaoAtual != $this->arLayoutInfo[$inVar]["ordem"]) {
                $inPosicaoAtual = $this->arLayoutInfo[$inVar]["ordem"];
                $this->Line( $x, ($y+120)+($inTotalLinhas*7), (197+$x), ($y+120)+($inTotalLinhas*7) );
                $inTotalLinhas++;
                $inPosicaoX = 0;
            }

            $this->setFont ( 'Arial', '', 8 );
            $this->Text ( $x+$inPosicaoX+2, ($y+116)+($inTotalLinhas*7), $this->arLayoutInfo[$inVar]["descricao"].":" ); //titulo da variavel
            $this->setFont( 'Arial', '', 7 );
            $this->Text ( $x+$inPosicaoX+2, ($y+119)+($inTotalLinhas*7), $this->arLayoutInfo[$inVar]["valor"] ); //valor da variavel

            $inPosicaoX += $this->arLayoutInfo[$inVar]["largura"];

            $this->Line ( $x+$inPosicaoX, ($y+113)+($inTotalLinhas*7), $x+$inPosicaoX, ($y+120)+($inTotalLinhas*7) );
        }

        $this->Line( $x, ($y+120)+($inTotalLinhas*7), (197+$x), ($y+120)+($inTotalLinhas*7) );
        $inPosicaoAtual = 0;
        for ( $inVar=0; $inVar<count( $this->arLayoutVariaveis ); $inVar++ ) {
            if ($inPosicaoAtual != $this->arLayoutVariaveis[$inVar]["ordem"]) {
                $inPosicaoAtual = $this->arLayoutVariaveis[$inVar]["ordem"];
                $this->Line( $x, ($y+120)+($inTotalLinhas*7), (197+$x), ($y+120)+($inTotalLinhas*7) );
                $inTotalLinhas++;
                $inPosicaoX = 0;
            }
            ...
            $this->setFont ( 'Arial', '', 8 );
            $this->Text ( $x+$inPosicaoX+2, ($y+116)+($inTotalLinhas*7), $this->arLayoutVariaveis[$inVar]["nom_atributo"].":" ); //titulo da variavel
            $this->setFont( 'Arial', '', 7 );
            $this->Text ( $x+$inPosicaoX+2, ($y+119)+($inTotalLinhas*7), $this->arLayoutVariaveis[$inVar]["valor"] ); //valor da variavel

            $inPosicaoX += $this->arLayoutVariaveis[$inVar]["largura"];
...
//esta linha vai subir um pouco
    ...
    public function posicionaVariaveis($x, $y)
    {
        ...
        $this->Text   ( $x+40, $y+212, $this->stExercicio_selecionado ); //exercicios selecionados lado B //item novo/alterado 11_03_2008

        $stObs = str_replace( "\n\r", " ", $this->stObservacao );
        $this->Text   ( $x+40, $y+218, substr( $stObs, 0, 120 ) ); //item novo/alterado 11_03_2008

        $stObs = str_replace( "\n\r", " ", $this->arObservacaoLayout[1] );
        $this->Text   ( $x+40, $y+221, substr( $stObs, 0, 120 ) ); //item novo/alterado 11_03_2008
        $this->Text   ( $x+40, $y+224, substr( $stObs, 120, 120 ) ); //item novo/alterado 11_03_2008
        $this->Text   ( $x+40, $y+227, substr( $stObs, 240, 120 ) ); //item novo/alterado 11_03_2008
        $this->Text   ( $x+40, $y+230, substr( $stObs, 360, 120 ) ); //item novo/alterado 11_03_2008
        $this->Text   ( $x+40, $y+233, substr( $stObs, 480, 120 ) ); //item novo/alterado 11_03_2008
        $this->Text   ( $x+40, $y+236, substr( $stObs, 600, 120 ) ); //item novo/alterado 11_03_2008
    ...
    public function posicionaVariaveisCapa($x, $y)
    {
        ...
        $this->Text   ( $x+150, $y+119, $this->stValorDesconto ); //desconto lado A

        $this->Text   ( $x+2, $y+84, substr( $this->arObservacaoLayout[0], 0, 140 ) );
        $this->Text   ( $x+2, $y+88, substr( $this->arObservacaoLayout[0], 140, 140 ) );
        $this->Text   ( $x+2, $y+92, substr( $this->arObservacaoLayout[0], 280, 140 ) );
        $this->Text   ( $x+2, $y+96, substr( $this->arObservacaoLayout[0], 320, 140 ) );
        $this->Text   ( $x+2, $y+100, substr( $this->arObservacaoLayout[0], 460, 140 ) );
        $this->Text   ( $x+2, $y+104, substr( $this->arObservacaoLayout[0], 600, 140 ) );
        $this->Text   ( $x+2, $y+108, substr( $this->arObservacaoLayout[0], 740, 140 ) );

        $this->Text   ( $x+2, $y+112, substr( $this->stLayoutObsDebt, 0, 120 ) ); //item novo/alterado 11_03_2008

        for ( $inX=0; $inX<count($this->arDados); $inX++ ) {
            $this->Text   ( $x+2, $y+60+($inX*3), substr($this->arDados[$inX]["tributo"], 0, 48) );
...
class RCarneDiversosLayoutUrbem
{
/*
    * @var Integer
    * @access Private
...
/*
    ...
    * @access Private
*/
function RCarneDiversosLayoutUrbem($arEmissao, $horizontal = 7, $vertical = 95)
{
...
function imprimirCarne($diffBaixa = FALSE)
{
    ...
    foreach ($this->arEmissao as $valor => $chave) {
        ...
        $obTARRCarne->recuperaDadosPrefeituraIPTUGenerico( $rsListaCarnePrefeitura, Sessao::getExercicio() );
         ...
         //mudancas 03_04_08
        $inCodModelo = $chave[0]["cod_modelo"];

        $stFiltroLayout = " AND cod_modelo = ".$inCodModelo;
        $obTARRModeloCarne = new TARRModeloCarne;
        $obTARRModeloCarne->recuperaListaModeloCarneLayout( $rsModelos, $stFiltroLayout );
        $boCapaPrimeiraFolha = $rsModelos->getCampo("capa_primeira_folha");

        $stFiltroLayout = " WHERE cod_modelo = ".$inCodModelo;
        $obTARRObservacaoLayoutCarne = new TARRObservacaoLayoutCarne;
        $obTARRObservacaoLayoutCarne->recuperaTodos( $rsObservacoes, $stFiltroLayout );

        $obTARRObservacaoDebitoLayoutCarne = new TARRObservacaoDebitoLayoutCarne;
        $obTARRObservacaoDebitoLayoutCarne->recuperaTodos( $rsObservacoesDebito, $stFiltroLayout );
        if ( !$rsObservacoesDebito->Eof() ) {
            $obTARRCarne->recuperaSituacaoArrecadacaoCGM( $rsSituacaoDebito, $chave[0]["numcgm"] );
            if ( $rsSituacaoDebito->Eof() )
                $this->obRCarneMata->stLayoutObsDebt = $rsObservacoesDebito->getCampo("observacao_nao_devedor");
            else
                $this->obRCarneMata->stLayoutObsDebt = $rsObservacoesDebito->getCampo("observacao_devedor");
        }

        $obTARRInformacaoAdicionalLayoutCarne = new TARRInformacaoAdicionalLayoutCarne;
        $obTARRInformacaoAdicionalLayoutCarne->recuperaInfoLayout( $rsInfo, $stFiltroLayout, " ORDER BY ordem, posicao_inicial ASC " ); //utilizando mesmo filtro das observações

        $arLayoutInfo = array();

        while ( !$rsInfo->Eof() ) {
            $stSql = " SELECT ".$rsInfo->getCampo("nom_funcao")."('".$chave[0]["inscricao"]."') AS resultado";
            $obConexao->executaSQL( $rsResultadoFuncao, $stSql, $boTransacao );
            $stValorVariavel = $rsResultadoFuncao->getCampo( "resultado" );
            $arLayoutInfo[] = array (
                "largura" => $rsInfo->getCampo("largura"),
                "ordem" => $rsInfo->getCampo("ordem"),
                "descricao" => $rsInfo->getCampo( "descricao" ),
                "valor" => $stValorVariavel
            );

            $rsInfo->proximo();
        }

        $this->obRCarneMata->arLayoutInfo = $arLayoutInfo;

        $obTARRVariaveisLayoutCarne = new TARRVariaveisLayoutCarne;
        $obTARRVariaveisLayoutCarne->recuperaVariaveisLayout( $rsVariaveis, $stFiltroLayout, " ORDER BY ordem, posicao_inicial ASC " ); //utilizando mesmo filtro das observações

        $arModulos[12] = array(
            1 => "TCIMAtributoNivelValor",
            2 => "TCIMAtributoLoteUrbanoValor",
            3 => "TCIMAtributoLoteRuralValor",
            4 => "TCIMAtributoImovelValor",
            5 => "TCIMAtributoTipoEdificacaoValor",
            6 => "TCIMAtributoCondominioValor",
            7 => "TCIMAtributoTrechoValor",
            8 => "TCIMAtributoFaceQuadraValor",
            9 => "TCIMAtributoConstrucaoOutrosValor"
        );

        $arModulos[14] = array (
            1 => "TCEMAtributoEmpresaFatoValor",
            2 => "TCEMAtributoEmpresaDireitoValor",
            3 => "TCEMAtributoCadEconAutonomoValor",
            5 => "TCEMAtributoElemCadEconomicoValor"
        );

        while ( !$rsVariaveis->Eof() ) {
            $obRCadastroDinamico = new RCadastroDinamico;

            if ( $rsVariaveis->getCampo( "cod_modulo" ) == 12 ) {
                include_once ( CAM_GT_CIM_MAPEAMENTO.$arModulos[$rsVariaveis->getCampo( "cod_modulo" )][$rsVariaveis->getCampo( "cod_cadastro" )].".class.php" );

//echo "comando = ".$rsVariaveis->getCampo( "cod_cadastro" )."<br>";
                switch ( $rsVariaveis->getCampo( "cod_cadastro" ) ) {
                    case 1: //imovel -> cod_lote -> cod_localizacao
                        ...
                        $obTCIMImovelLote = new TCIMImovelLote;
                        $stFiltro = " WHERE inscricao_municipal = ".$chave[0]["inscricao"];
                        $obTCIMImovelLote->recuperaTodos( $rsLote, $stFiltro, " timestamp desc " );

                        $obTCIMLoteLocalizacao = new TCIMLoteLocalizacao;
                        $stFiltro = " WHERE cod_lote = ".$rsLote->getCampo('cod_lote');
                        $obTCIMLoteLocalizacao->recuperaTodos( $rsLocal, $stFiltro );
                        $arChaveAtributo = array( "cod_localizacao" => $rsLocal->getCampo('cod_localizacao'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;

                    case 2: //imovel -> cod_lote
                    case 3:
                        include_once ( CAM_GT_CIM_MAPEAMENTO."TCIMImovelLote.class.php" );
                        ...
                        $arChaveAtributo = array( "cod_lote" => $rsLote->getCampo('cod_lote'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;

                    case 4: //imovel
                        $arChaveAtributo = array( "inscricao_municipal" => $chave[0]["inscricao"], "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;

                    case 5: //imovel -> unidade autonoma -> cod_construcao -> cod_tipo
                        include_once ( CAM_GT_CIM_MAPEAMENTO."TCIMUnidadeAutonoma.class.php" );

                        $stFiltro = " WHERE inscricao_municipal = ".$chave[0]["inscricao"];
                        $obTCIMUnidadeAutonoma = new TCIMUnidadeAutonoma;
                        $obTCIMUnidadeAutonoma->recuperaTodos( $rsUnidade, $stFiltro, " timestamp desc " );
                        $arChaveAtributo = array( "cod_construcao" => $rsUnidade->getCampo('cod_construcao'), "cod_tipo" => $rsUnidade->getCampo('cod_tipo'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;

                    case 6: //imovel_condominio
                        include_once ( CAM_GT_CIM_MAPEAMENTO."TCIMImovelCondominio.class.php" );

                        $stFiltro = " WHERE inscricao_municipal = ".$chave[0]["inscricao"];
                        $obTCIMImovelCondominio = new TCIMImovelCondominio;
                        $obTCIMImovelCondominio->recuperaTodos( $rsCond, $stFiltro );
                        $arChaveAtributo = array( "cod_condominio" => $rsCond->getCampo('cod_condominio'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;

                    case 7: //imobiliario.imovel_confrontacao->imobiliario.confrontacao_trecho->trecho
                        ...
                        $stFiltro = " WHERE inscricao_municipal = ".$chave[0]["inscricao"];
                        $obTCIMImovelConfrontacao = new TCIMImovelConfrontacao;
                        $obTCIMImovelConfrontacao->recuperaTodos( $rsConfrontacao, $stFiltro );

                        $stFiltro = " WHERE cod_confrontacao = ".$rsConfrontacao->getCampo("cod_confrontacao");
                        $obTCIMConfrontacaoTrecho = new TCIMConfrontacaoTrecho;
                        $obTCIMConfrontacaoTrecho->recuperaTodos( $rsTrecho, $stFiltro );
                        $arChaveAtributo = array( "cod_trecho" => $rsTrecho->getCampo('cod_trecho'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;
                    ...
                    case 8: //face
                        ...
                        $obTCIMFaceQuadra = new TCIMFaceQuadra;
                        $stFiltro = " WHERE cod_localizacao = ".$rsLocal->getCampo('cod_localizacao');
                        $obTCIMFaceQuadra->recuperaTodos( $rsFace, $stFiltro );
                        $arChaveAtributo = array( "cod_face" => $rsFace->getCampo('cod_face'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;
                    ...
                    case 9:
                        ...
                        $obTCIMUnidadeAutonoma = new TCIMUnidadeAutonoma;
                        $obTCIMUnidadeAutonoma->recuperaTodos( $rsUnidade, $stFiltro, " timestamp desc " );
                        $arChaveAtributo = array( "cod_construcao" => $rsUnidade->getCampo('cod_construcao'), "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                        break;
                }
            }else
                if ( $rsVariaveis->getCampo( "cod_modulo" ) == 14 ) {
                    include_once ( CAM_GT_CEM_MAPEAMENTO.$arModulos[$rsVariaveis->getCampo( "cod_modulo" ) ][$rsVariaveis->getCampo( "cod_cadastro" )].".class.php" );

                    switch ( $rsVariaveis->getCampo( "cod_cadastro" ) ) {
                        case 1:
                        case 2:
                        case 3:
                            $arChaveAtributo = array( "inscricao_economica" => $chave[0]["inscricao"], "cod_atributo" => $rsVariaveis->getCampo( "cod_atributo" ) );
                            break;
                    }
                }

            $obRCadastroDinamico->setPersistenteValores ( new $arModulos[$rsVariaveis->getCampo( "cod_modulo" )][$rsVariaveis->getCampo( "cod_cadastro" )] );
            $obRCadastroDinamico->setCodCadastro ( $rsVariaveis->getCampo( "cod_cadastro" ) );
            $obRCadastroDinamico->obRModulo->setCodModulo ( $rsVariaveis->getCampo( "cod_modulo" ) );
            ...
            if ( !$rsAtributos->Eof() ) {
                switch ( $rsAtributos->getCampo('nom_tipo') ) {
                    case "Texto":
                        $stValorVariavel = $rsAtributos->getCampo('valor');
                        break;

                    case "Numerico":
                        $stValorVariavel = number_format( $rsAtributos->getCampo('valor'), 2, ',' , '.' );
                        break;

                    case "Lista":
                        $arValorPadrao = explode( '[][][]' , $rsAtributos->getCampo('valor_padrao_desc') );
                        $inPosicao = $rsAtributos->getCampo('valor') - 1;
                        $stValorVariavel = $arValorPadrao[$inPosicao];
                        break;
        ...
        $this->obRCarneMata->arLayoutVariaveis = $arLayoutVariaveis;
        $this->obRCarneMata->arObservacaoLayout = array();

        while ( !$rsObservacoes->Eof() ) {
            if ( $rsObservacoes->getCampo("capa") == 't' )
                $this->obRCarneMata->arObservacaoLayout[0] = $rsObservacoes->getCampo("observacao");
            else
                $this->obRCarneMata->arObservacaoLayout[1] = $rsObservacoes->getCampo("observacao");
...
//echo "finito<br>";exit;
        ...
        $rsListaDetalheCreditos->ordena( "numeracao_parcela" );
        $flValorTotal = 0.00;
    ...
    //    while ( !$rsGeraCarneCabecalho->eof() ) {
        ...
        foreach ($chave as $parcela) { // impressao das parcelas selecionadas para cada codigo de lancamento
            $obTDATDividaAtiva->recuperaListaExercicioOriginalParcela( $rsExercicioDividaOrigem, $parcela['cod_parcela'] );

            if ( !$rsExercicioDividaOrigem->Eof() ) {
                $this->obRCarneMata->stNatureza = $rsGeraCarneCabecalho->getCampo( 'natureza' )." (D.A.)";
                $this->obRCarneMata->stExercicio_selecionado = "";
                while ( !$rsExercicioDividaOrigem->Eof() ) {
                    if ($this->obRCarneMata->stExercicio_selecionado) {
                        $this->obRCarneMata->stExercicio_selecionado .= ',';
                    }

                    $this->obRCarneMata->stExercicio_selecionado .= $rsExercicioDividaOrigem->getCampo( "exercicio_original" );
                    $rsExercicioDividaOrigem->proximo();
                }

                $rsExercicioDividaOrigem->setPrimeiroElemento();
            } else {
                $this->obRARRCarne->stExercicio_selecionado = $chave[0]["exercicio"];
                $this->obRCarneMata->stNatureza = $rsGeraCarneCabecalho->getCampo( 'natureza' );
            }

            if ( $inTmpParcela != -1 )
                $this->obRCarneMata->arDados[0]["parcela"] = $inTmpParcela;

            if ($boCapaPrimeiraFolha == 't') {
                if (!$inParcela) {
                    $this->inHorizontal = 7;
                    $this->inVertical = 20;
            ...
            $inTmpParcela = $this->obRCarneMata->arDados[0]["parcela"];
...
//            $this->arBarra['valor_documento'] = number_format( 112, 2, '.', '' );
            ...
            if ($boCapaPrimeiraFolha == 't') {
                if (!$inParcela) {
                    ...
                    $this->obRCarneMata->drawCarne( $this->inHorizontal, $this->inVertical-170 );
                    $this->obRCarneMata->posicionaVariaveis( $this->inHorizontal, $this->inVertical-170 );
                    $this->obRCarneMata->setPicote( $this->inHorizontal, $this->inVertical-170 );
                }
            } else {
                $this->obRCarneMata->drawCarne( $this->inHorizontal, $this->inVertical );
                $this->obRCarneMata->posicionaVariaveis( $this->inHorizontal, $this->inVertical );
