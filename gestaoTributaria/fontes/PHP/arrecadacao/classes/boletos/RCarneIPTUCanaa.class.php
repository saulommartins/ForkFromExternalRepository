...
/**
  ...
  * @package URBEM
    ...
    * $Id: RCarneIPTUCanaa.class.php 59612 2014-09-02 12:00:51Z gelson $
...
//eh aakiiiiii o carnets
        ...
        $this->Line( $x+145, $y+127, (145+$x), $y+134 );
...
//esta linha vai subir um pouco
        ...
        $this->Text   ( $x+97, $y+130, "Quantidade Edificação:" );

$stComposicao =
"                                                                                                        COMPOSIÇÂO DO CÁLCULO\n
I- Imposto Sobre a Propriedade Predial e Territorial Urbana (IPTU) => Valor do Imposto = Valor Venal x Alíquota + Taxa de Lixo\n\n Valor Venal = Valor do Terreno (Área Terreno x Vlr m2 x Fator de Correção) + Valor da Edificação\n
(Área Edificação x Vlr M2 x Fator de Correção da Estrutura x Fator da Área Construida x Pontos).\n
II- Taxa de Recolhimento de Lixo => Valor da Taxa = Testada * 0.64\n
Observações:\n
  1) O recolhimento fora do prazo enseja a cobrança dos seguintes acréscimos, calculados sobre o valor atualizado:\n
     - Multa de 10 % (dez por cento), após o vencimento.\n
     - Juros de mora de 1% (um por cento) ao mês, calculado a partir da data do vencimento.\n
  2) Os tributos poderão ser pagos em até 9 cotas, e a opção de cota única, possuí 20% de desconto.\n
  3) Necessitando alterar os dados cadastrais compareça, munido de documento comprobatório, ao Departamento de Tributos, órgão da Secretária Municipal de Finanças,\n
  sito a Rua Tancredo Neves, 100 Centro - CEP 68.537-000 / Canaã dos Carajás / PA.\n";

        $this->SetXY( $x+2, $y+136 );

        $this->setFont( 'Arial','',7 );
        $this->setLeftMargin(10);
        $this->Write( 1.3 , $stComposicao );
        $this->setLeftMargin(0);
    ...
    public function posicionaVariaveis($x, $y)
    {
        ...
        $stObs = "Instruções: Após Vencimento Cobrar: Multa de Mora 10% e Juros de 1% A.M.";
    ...
    public function posicionaVariaveisCapa($x, $y)
    {
        ...
        $this->Text   ( $x+97, $y+133, $this->stQtdEdificacao );
...
/*
    ...
    * @access Private
*/
function RCarneIPTUCanaa($arEmissao, $horizontal = 7, $vertical = 95)
{
...
function imprimirCarne($diffBaixa = FALSE)
{
    ...
    $inCountTemp = 0;
    $stDataConsolidacao = '';
    if ($this->dtVencimentoConsolidacao) {
        $arDataConsolidacao = explode('/', $this->dtVencimentoConsolidacao );
        $stDataConsolidacao = $arDataConsolidacao[2].'-'.$arDataConsolidacao[1].'-'.$arDataConsolidacao[0];
    }
    ...
    foreach ($this->arEmissao as $valor => $chave) {
        ...
        $obTARRCarne->recuperaDadosValorVenalIPTUGenerico( $rsListaCarne, $chave[0]['inscricao'], $chave[0]['exercicio'], $chave[0]['cod_parcela'] );
        ...
        $this->obRCarneMata->stQtdEdificacao = $rsListaCarne->getCampo("qtd_edificacao");
        ...
        if ( count( $chave ) > 1 ) {
            ...
            $obTARRCarne->ListaDadosPorGrupoParaCarneIPTUDataBase( $rsListaDetalheCreditos, " WHERE al.cod_lancamento = ".$valor.$stFiltro1.$stFiltro2,$stDataConsolidacao );
        } else {
            ...
            $rsListaDetalheCreditos->ordena( "ordem" );
        ...
        $this->obRCarneMata->setNomCgm            ( substr( $rsGeraCarneCabecalho->getCampo( 'nom_cgm' ) , 0, 55 ) );
        ...
        foreach ($chave as $parcela) { // impressao das parcelas selecionadas para cada codigo de lancamento
            $obTARRCarne->ListaDadosPorGrupoParaCarneIPTUDataBase( $rsListaDetalheCreditos, " WHERE al.cod_lancamento = ".$valor." AND ap.cod_parcela = ".$parcela['cod_parcela']." AND carne.numeracao = '".$parcela['numeracao']."'",$stDataConsolidacao );
            $flValorTotal = 0.00;
            $flValorTotalJuros = 0.00;
            $flValorTotalMulta = 0.00;
            $flValorTotalCorrecao = 0.00;
