...
<?php
/**
  * Carnê diversos para Monte Horebe
  ...
  * @package URBEM
    ...
    * $Id: RCarneDiversosMonteHorebe.class.php 59612 2014-09-02 12:00:51Z gelson $
...
/*
$Log$
Revision 1.4  2007/09/28 14:16:37  cercato
alteracao em label.

Revision 1.3  2007/09/27 19:27:51  cercato
*** empty log message ***

Revision 1.2  2007/09/27 19:13:13  cercato
*** empty log message ***

Revision 1.1  2007/09/26 13:45:46  cercato
*** empty log message ***
...
{
    /* Labels  */
    public $Titulo1         = 'P M MONTE HOREBE';
    public $Titulo2         = '';
...
{
    /* labels */
    public $lblTitulo1 = 'MONTE HOREBE - Secretaria de Administração e Finanças';
    ...
    public function drawCarne($x, $y)
    {
        ...
        /* returna retangulo */
        $this->Rect( $x, $y, 197, 80*$this->tamY );

        /* linha horizontais */
        $this->Line( $x, ($y+(11*$this->tamY)), ($x+197), ($y+(11*$this->tamY)) );
        $this->Line( $x, ($y+(18*$this->tamY)), (197+$x), ($y+(18*$this->tamY)) );
        $this->Line( $x, ($y+(25*$this->tamY)), (197+$x), ($y+(25*$this->tamY)) );
        $this->Line( $x+151, ($y+(32*$this->tamY)), (197+$x), ($y+(32*$this->tamY)) );
        $this->Line( $x+151, ($y+(39*$this->tamY)), ($x+197), ($y+(39*$this->tamY)) );

        // linhas adicionais no carne no lado esquerdo
        if ($this->repeticao == 2) {
            $this->Line( $x+72, ($y+(60*$this->tamY)), 72+$x, ($y+(80*$this->tamY)) ); //nova
            $this->Line( $x, ($y+(68*$this->tamY)), (72+$x), ($y+(68*$this->tamY)) ); //nova
        }else
            $this->Line( $x, ($y+(68*$this->tamY)), (197+$x), ($y+(68*$this->tamY)) ); //nova

        $this->Line( $x, ($y+(60*$this->tamY)), (197+$x), ($y+(60*$this->tamY)) );
        $this->Line( $x, ($y+(46*$this->tamY)), ($x+197), ($y+(46*$this->tamY)) );

        /* linhas verticais */
        $this->Line( ($x+18), ($y+(18*$this->tamY)), ($x+18), ($y+(25*$this->tamY)) );
        $this->Line( ($x+151), $y, ($x+151), ($y+(46*$this->tamY)) );
        ...
        $this->Text   ( ($x+40), ($y+(4*$this->tamY)) , $this->lblTitulo1 );
        $this->Text   ( ($x+65), ($y+(10*$this->tamY)), $this->lblExercicio );

        $this->setFont('Arial'  ,'B',6);
        $this->Text   ( ($x+1)  , ($y+(13.5*$this->tamY)), $this->lblNumeracao    );
        $this->Text   ( ($x+1)  , ($y+(20.5*$this->tamY)), $this->lblParcela      );
        $this->Text   ( ($x+19) , ($y+(20.5*$this->tamY)), $this->lblInscricao      );
        $this->Text   ( ($x+1)  , ($y+(27.5*$this->tamY)),  $this->lblTributo   );

        /* retangulo para vencimento */
        $this->setFillColor( 240 );
        $this->Rect   ( ($x+151), ($y+(0*$this->tamY)), 46, 11*$this->tamY, 'DF' );
        $this->setFillColor( 0 );

        $this->Text   ( ($x+154), ($y+(2.5*$this->tamY)), $this->lblVencimento   );
...
//        if ( !$this->getObsVencimento() ) {
  //          $this->setObsVencimento ( 'Receber até 31/12/2007.' );
    //    }
        ...
        $this->Text   ( ($x+154)  , ($y+(10.5*$this->tamY)), $this->getObsVencimento() );

        // contribuinte
        $this->Text   ( ($x+1)  , ($y+(48.5*$this->tamY)), $this->lblContribuinte );
        ...
        if ($this->repeticao == 0) {
            $this->Text   ( ($x+170), ($y+(56.5*$this->tamY)), 'VIA CONTRIBUINTE');
        }else
            $this->Text   ( ($x+170), ($y+(56.5*$this->tamY)), 'VIA PREFEITURA');
    ...
    public function posicionaVariaveis($x, $y)
    {
        ...
        $this->Text   ( ($x+85), ($y+(10*$this->tamY))  ,$this->stExercicio ); // ok
        ...
        $this->Text   ( ($x+166) , ($y+(8*$this->tamY)), $this->dtVencimento ); // ok
//57
        $this->Text   ( ($x+2), ($y+(52*$this->tamY)), $this->getNomCgm() ); // ok  contribuinte

        $this->Text   ( ($x+2), ($y+(66*$this->tamY)), substr($this->getRua(), 0, 46) ) ; // end . do imovel

        $this->Text   ( ($x+2) , ($y+(17*$this->tamY))  , $this->stNumeracao ); // ok
        ...
        $this->Text   ( ($x+2) , ($y+(24*$this->tamY))  , $this->stParcela   ); // ok
...
class RCarneMonteHorebe extends RProtocolo
{
    /* labels */
    public $lblTitulo1 = 'P M Monte Horebe';
    public $lblTitulo2 = 'IPTU';
    public $lblExercicio = 'EXERCÍCIO';
    public $lblInscricao = 'INSCRIÇÃO';
    public $lblCodDivida = 'CÓD. DÍVIDA';
...
class RCarneDiversosMonteHorebe
{
/*
    * @var Integer
    * @access Private
...
/*
    ...
    * @access Private
*/
function RCarneDiversosMonteHorebe($arEmissao, $horizontal = 7, $vertical = 95)
{
...
function imprimirCarne($diffBaixa = FALSE)
{
    ...
    $inSaltaPagina = "";
    $this->obRCarneMonteHorebe = new RCarneDiversosPetropolis;
    $this->obRCarneMonteHorebe->configuraProtocolo();
    ...
    foreach ($this->arEmissao as $valor => $chave) {
        $inSaltaPagina++;
        $this->obRCarneMonteHorebe->setImagem(CAM_FW_TEMAS."imagens/".$stNomeImagem );
        ...
        $this->obRCarneMonteHorebe->setObservacaoL1 ('Créditos: ');

        while ( !$rsGeraCarneCabecalho->eof() ) {
            /* montagem cabecalho (protocolo) */
            $this->obRCarneMonteHorebe->setCt                ( $rsGeraCarneCabecalho->getCampo( 'ano_aquisicao' )          );
            $this->obRCarneMonteHorebe->setCa                ( $rsGeraCarneCabecalho->getCampo( 'ca' )                     );
            $this->obRCarneMonteHorebe->setCep               ( $rsGeraCarneCabecalho->getCampo( 'cep')                     );
            $this->obRCarneMonteHorebe->setExercicio         ( $rsGeraCarneCabecalho->getCampo( 'exercicio' )              );
            $this->obRCarneMonteHorebe->setNomCgm            ( $rsGeraCarneCabecalho->getCampo( 'nom_cgm' )                );

            $this->obRCarneMonteHorebe->setRua               ( str_replace ( "Não Informado ", "", $rsGeraCarneCabecalho->getCampo( 'nom_logradouro' ) )  );
            $this->obRCarneMonteHorebe->setNumero            ( $rsGeraCarneCabecalho->getCampo( 'numero' )                 );
            $this->obRCarneMonteHorebe->setComplemento       ( $rsGeraCarneCabecalho->getCampo( 'complemento' )            );
            $this->obRCarneMonteHorebe->setCidade            ( $rsGeraCarneCabecalho->getCampo( 'nom_municipio' )          );
            $this->obRCarneMonteHorebe->setUf                ( $rsGeraCarneCabecalho->getCampo( 'sigla_uf' )               );
            $this->obRCarneMonteHorebe->setInscricao         ( str_pad($rsGeraCarneCabecalho->getCampo( 'inscricao_municipal' ),strlen( $stMascaraInscricao ), '0', STR_PAD_LEFT) );
            $this->obRCarneMonteHorebe->setCtmDci            ( $rsGeraCarneCabecalho->getCampo( 'ctm_dci' )                );
            $this->obRCarneMonteHorebe->setCodLogradouro     ( $rsGeraCarneCabecalho->getCampo( 'cod_logradouro' )         );
            $this->obRCarneMonteHorebe->setDistrito          ( $rsGeraCarneCabecalho->getCampo( 'distrito' )               );
            $this->obRCarneMonteHorebe->setProcessamento     ( $rsGeraCarneCabecalho->getCampo( 'data_processamento' )     );
            $this->obRCarneMonteHorebe->setAreaTerreno       ( $rsGeraCarneCabecalho->getCampo( 'area_real' )              );
            $this->obRCarneMonteHorebe->setAreaEdificada     ( $rsGeraCarneCabecalho->getCampo( 'area_edificada' )         );
            $this->obRCarneMonteHorebe->setUtilizacaoImovel  ( $rsGeraCarneCabecalho->getCampo( 'utilizacao' )             );
            if ( !$this->getConsolidacao() ) {
            $this->obRCarneMonteHorebe->setTributo           ( $rsGeraCarneCabecalho->getCampo( 'cod_grupo')." - ".$rsGeraCarneCabecalho->getCampo( 'descricao' )              );
            } else {
                ...
                while ( $contGrupos < 3 && $contGrupos < count ( $arGruposConsolidados ) ) {
                    ...
                    if ($contGrupos == 0) {
                        $this->obRCarneMonteHorebe->setTributoAbrev ( $stGrupo );
                    } elseif ($contGrupos == 1) {
                        $this->obRCarneMonteHorebe->setTributoAbrev2 ( $stGrupo );
                    } elseif ($contGrupos == 2) {
                        $this->obRCarneMonteHorebe->setTributoAbrev3 ( $stGrupo );
                    }
                ...
                $this->obRCarneMonteHorebe->setTributo ( $stGrupoTotal );
            }

            $this->obRCarneMonteHorebe->setValorTributoReal  ( $rsGeraCarneCabecalho->getCampo( 'valor_venal_total' )      );
            $this->obRCarneMonteHorebe->setObservacao        ( wordwrap($rsGeraCarneCabecalho->getCampo('observacao' ),40,chr(13)) );
            $this->obRCarneMonteHorebe->setNomBairro         ( $rsGeraCarneCabecalho->getCampo( 'nom_bairro' )             );
            $this->obRCarneMonteHorebe->setCodDivida         ( $rsGeraCarneCabecalho->getCampo( 'cod_grupo' )              );
            if ( preg_match( '/LIMPEZA.*/i',$rsGeraCarneCabecalho->getCampo( 'descricao_credito' ) ) ) {
                $this->obRCarneMonteHorebe->setTaxaLimpezaAnual  ( $rsGeraCarneCabecalho->getCampo( 'valor' )              );
            } else {
                $flImpostoAnualReal = $rsGeraCarneCabecalho->getCampo( 'valor' );
                $this->obRCarneMonteHorebe->setImpostoAnualReal  ( $flImpostoAnualReal                                     );
            }
            $this->obRCarneMonteHorebe->setReferencia        ( ""                                                          );
            $this->obRCarneMonteHorebe->setNumeroPlanta      ( ""                                                          );

            // capturar creditos
            $this->obRCarneMonteHorebe->setObservacaoL1 ( $this->obRCarneMonteHorebe->getObservacaoL1().$rsGeraCarneCabecalho->getCampo( 'descricao_credito').": ".$rsGeraCarneCabecalho->getCampo( 'valor' )."  ");

            $rsGeraCarneCabecalho->proximo();

        } //fim do loop de reemitirCarne
        $this->obRCarneMonteHorebe->setValorAnualReal        ( $flImpostoAnualReal + $this->obRCarneMonteHorebe->getTaxaLimpezaAnual() );
        // formatar
        $this->obRCarneMonteHorebe->setValorAnualReal    ( number_format($this->obRCarneMonteHorebe->getValorAnualReal(),2,',','.') );
        $this->obRCarneMonteHorebe->setTaxaLimpezaAnual  ( number_format($this->obRCarneMonteHorebe->getTaxaLimpezaAnual(),2,',','.') );
        $this->obRCarneMonteHorebe->setImpostoAnualReal  ( number_format($this->obRCarneMonteHorebe->getImpostoAnualReal(),2,',','.') );
        $this->obRCarneMonteHorebe->setValorTributoReal  ( number_format($this->obRCarneMonteHorebe->getValorTributoReal(),2,',','.') );
/*        $this->obRCarneMonteHorebe->drawProtocolo();
        $this->obRCarneMonteHorebe->posicionaVariaveisProtocolo();
*/
        $inParcela = $inCount = 0;
    ...
    if ( $this->getConsolidacao() ) {
        ...
        #echo '<h2>CONSOLIDACAO </h2>'; #exit;
            ...
            foreach ($chave as $parcela) {
                ...
                $this->obRCarneMonteHorebe->setImagemCarne( CAM_FW_TEMAS."imagens/".$stNomeImagem );
                $this->obRCarneMonteHorebe->setImagem("");
                ...
                //$this->obRCarneMonteHorebe->setNumeracao( (string) $rsParcela->getCampo( 'numeracao' ) );
                $this->arBarra['nosso_numero'] = (string) $this->getNumeracaoConsolidacao();
                $this->obRCarneMonteHorebe->setNumeracao( (string) $this->getNumeracaoConsolidacao() );
...
*/
            ...
            if ( $inCountTemp == (count($this->arEmissao)-1) ) {
                $this->obRCarneMonteHorebe->setObservacaoL1 ( 'Não receber após o vencimento. ' );
                $this->obRCarneMonteHorebe->setParcela ( "1/1" );
                $this->obRCarneMonteHorebe->setVencimento  ( $this->getVencimentoConsolidacao() );
                $this->obRCarneMonteHorebe->flValorJuros = ( number_format(round($nuValorJuroNormal,2),2,',',''));
                $this->obRCarneMonteHorebe->flValorMulta = ( number_format(round($nuValorMultaNormal,2),2,',',''));
                $this->obRCarneMonteHorebe->setValor       ( number_format(round($nuValorNormal,2),2,',',''));
                $this->obRCarneMonteHorebe->setValorTotal(number_format(round($nuValorTotal,2),2,',',''));

                $this->arCodigoBarra = $this->obBarra->geraFebraban( $this->arBarra );
                $this->obRCarneMonteHorebe->setBarCode( $this->arCodigoBarra['codigo_barras'] );
                $this->obRCarneMonteHorebe->setLinhaCode( $this->arCodigoBarra['linha_digitavel'] );
                $this->obRCarneMonteHorebe->drawCarne( $this->inHorizontal, $this->inVertical );
                $this->obRCarneMonteHorebe->posicionaVariaveis( $this->inHorizontal, $this->inVertical );
                $this->obRCarneMonteHorebe->setPicote( $this->inHorizontal, $this->inVertical );
                $this->inVertical += 95;
            }
            ...
            foreach ($chave as $parcela) { // impressao das parcelas selecionadas para cada codigo de lancamento
                ...
                if ($diffBaixa) {
                        ...
                        $this->obRCarneMonteHorebe->setParcelaUnica ( true );
                        $this->obRCarneMonteHorebe->lblTitulo2        = ' ';
                        $this->obRCarneMonteHorebe->lblValorCotaUnica = 'VALOR TOTAL';
                        $this->obRCarneMonteHorebe->setVencimento   ( $rsParcela->getCampo( 'vencimento' ) );
                        $this->obRCarneMonteHorebe->setValor        ( number_format($nuValorNormal,2,',','.') );
    //                    $this->obRCarneMonteHorebe->setObservacaoL1 ( 'Não receber após o vencimento. ' );
    //                    $this->obRCarneMonteHorebe->setObservacaoL2 ( ' ' );
    //                    $this->obRCarneMonteHorebe->setObservacaoL3 ( ' ' );
                        $this->obRCarneMonteHorebe->setParcela ( $rsParcela->getCampo( 'info' ) );
                } else {
                    if ( $rsParcela->getCampo( 'nr_parcela' ) == 0 ) {
                        $this->arBarra['tipo_moeda'] = 6;
                        $this->obRCarneMonteHorebe->setParcelaUnica ( true );
                        $this->obRCarneMonteHorebe->setVencimento   ( $rsParcela->getCampo( 'vencimento' ) );
                        //$this->obRCarneMonteHorebe->setValor        ( number_format($rsParcela->getCampo( 'valor' ),2,',','') );
                        $this->obRCarneMonteHorebe->setValor        ( number_format($nuValorNormal,2,',','.') );
                        $this->obRCarneMonteHorebe->flValorJuros = number_format($stJuroNormal,2,',','.');
                        $this->obRCarneMonteHorebe->flValorMulta = number_format($stMultaNormal,2,',','.');
                        ...
                        */
                        ...
                        $this->obRCarneMonteHorebe->setObservacaoL1 ( 'Cota Única com '.$rsPercentual->getCampo('valor').'% de desconto.' );                      ;
                        $this->obRCarneMonteHorebe->setObservacaoL2 ( 'Desconto não incide sobre a Taxa de Coleta de Lixo' );
                        $this->obRCarneMonteHorebe->setObservacaoL3 ( 'Não receber após o vencimento.' );
                        $this->obRCarneMonteHorebe->setParcela ( 'ÚNICA' );
                    } else {
                        ...
                        //$this->obRCarneMonteHorebe->setParcela( $inParcela.'/'.count($chave) );
                        $this->obRCarneMonteHorebe->setParcela( $rsParcela->getCampo( 'info' ));
    //                    $this->obRCarneMonteHorebe->setObservacaoL1( 'Após os vencimentos previstos nesta guia, retirar 2ª via na' );
    //                    $this->obRCarneMonteHorebe->setObservacaoL2( 'Secretaria de Fazenda' );
    //                    $this->obRCarneMonteHorebe->setObservacaoL3( ' ' );
                        $this->obRCarneMonteHorebe->setParcelaUnica( false );
                        $this->obRCarneMonteHorebe->setVencimento  ( $rsParcela->getCampo( 'vencimento' ) );
                        ...
                        if ($this->stLocal != "WEB") {
                            ...
                            if ($arTmp[1] >= $stMes) {
                                $stMes = $arTmp[1];
                                $boVenc1 = true;
                                $this->obRCarneMonteHorebe->setVencimento1 ( $arVencimentos[0] );
                                $arTmp = explode('/',$arVencimentos[1]);
                                if ($arTmp[1] >= $stMes) {
                                    $stMes = $arTmp[1];
                                    $boVenc2 = true;
                                    $this->obRCarneMonteHorebe->setVencimento2 ( $arVencimentos[1] );
                                    $arTmp = explode('/',$arVencimentos[2]);
                                    if ($arTmp[1] >= $stMes) {
                                        $boVenc3 = true;
                                        $this->obRCarneMonteHorebe->setVencimento3 ( $arVencimentos[2] );
                                    }
                                }
                            }
    ...
    //                      $stJuroNormal = $rsTmp->getCampo('valor');
        ...
        //                  $stMultaNormal = $rsTmp->getCampo('valor');
                            ...
                            if ($boVenc1 == true) {
                                ...
                                $this->obRCarneMonteHorebe->lblJuros2 = $stJuro1;
                                // % de multa
                                $obErro = $obCalculaJM->executaFuncao($rsTmp,$stParametro2.",'m'");

                                $stMulta1 = $rsTmp->getCampo('valor');
                                $this->obRCarneMonteHorebe->lblMulta2 = $stMulta1;
                            } else {
                                $this->obRCarneMonteHorebe->lblJuros2 = "";
                                $this->obRCarneMonteHorebe->lblMulta2 = "";
                            }
                            //-----------------------------------------------------------------------
                            ...
                            if ($boVenc2 == true) {
                                ...
                                $this->obRCarneMonteHorebe->lblJuros3 = $stJuro2;

                                // % de multa
                                $obErro = $obCalculaJM->executaFuncao($rsTmp3,$stParametro3.",'m'");
                                $stMulta2 = $rsTmp3->getCampo('valor');
                                $this->obRCarneMonteHorebe->lblMulta3 = $stMulta2;
                            } else {
                                $this->obRCarneMonteHorebe->lblJuros3 = "";
                                $this->obRCarneMonteHorebe->lblMulta3 = "";
                            }
                            //-----------------------------------------------------------------------
                            ...
                            if ($boVenc3 == true) {
                                ...
                                $this->obRCarneMonteHorebe->lblJuros4 = $stJuro3;
                                // % de multa
                                $obErro = $obCalculaJM->executaFuncao($rsTmp3,$stParametro4.",'m'");
                                $stMulta3 = $rsTmp3->getCampo('valor');

                                $this->obRCarneMonteHorebe->lblMulta4 = $stMulta3;
                            } else {
                                $this->obRCarneMonteHorebe->lblJuros4 = "";
                                $this->obRCarneMonteHorebe->lblMulta4 = "";
                            }
                            //-----------------------------------------------------------------------
                            ...
                            $this->obRCarneMonteHorebe->setValor       (number_format(round($nuValorNormal,2),2,',','.'));
                            if ($boVenc1 == true) {
                                $this->obRCarneMonteHorebe->setValor1      (number_format(round($nuValor1,2),2,',','.')) ;
                                if ($boVenc2 == true) {
                                    $this->obRCarneMonteHorebe->setValor2      (number_format(round($nuValor2,2),2,',','.')) ;
                                    if ($boVenc3 == true) {
                                        $this->obRCarneMonteHorebe->setValor3      (number_format(round($nuValor3,2),2,',','.')) ;
                                    }
                                }
                            }
                        } else {
                ...
                $this->obRCarneMonteHorebe->setValorTotal   ( number_format($nuValorTotal,2,',','.') );
                $this->arCodigoBarra = $this->obBarra->geraFebraban( $this->arBarra );
                $this->obRCarneMonteHorebe->setBarCode( $this->arCodigoBarra['codigo_barras'] );
                $this->obRCarneMonteHorebe->setLinhaCode( $this->arCodigoBarra['linha_digitavel'] );
                ...
                for ($inD=0; $inD<3; $inD++) {
                    ...
                    if ( ( $inParcela == 3 ) || ( $inCount == 3 ) ) {
                        $this->obRCarneMonteHorebe->novaPagina();
                        $inCount = 0;
                        $this->inVertical = 7;
        ...
        if( in_array($this->obRCarneMonteHorebe->getCodDivida(),$arGruposValidos))
            $this->obRCarneMonteHorebe->drawComplemento($this->inHorizontal, $this->inVertical);
    ...
    if ( Sessao::read( 'stParamPdf' ) )
        ...
        $stParam    = "I";
    $this->obRCarneMonteHorebe->show($stNome,$stParam); // lanca o pdf
}
