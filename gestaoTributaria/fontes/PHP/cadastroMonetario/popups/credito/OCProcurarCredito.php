...
<?php
/**
  * Página de Processamento Oculto para popup de CREDITO
  * Data de criação : 01/06/2005

    * @author Analista: Fabio Bertold Rodrigues
    * @author Programador: Lucas Teixeira Stephanou

    * $Id: OCProcurarCredito.php 59612 2014-09-02 12:00:51Z gelson $
...
/*
$Log$
Revision 1.6  2006/09/18 08:47:18  fabio
...
switch ($_GET["stCtrl"]) {
    ...
    case "buscaPopup":
        ...
        if ($_POST[$_GET['stNomCampoCod']] != '') {
            $arCodigoCredito = explode( '.', $_POST[$_GET['stNomCampoCod']] );
            $obRMONCredito->setCodCredito ( $arCodigoCredito[0] );
            $obRMONCredito->setCodEspecie ( $arCodigoCredito[1] );
            $obRMONCredito->setCodGenero  ( $arCodigoCredito[2] );
            $obRMONCredito->setCodNatureza( $arCodigoCredito[3] );
            $obRMONCredito->listarCreditos( $rsCreditos         );
            $stValor = $rsCreditos->getCampo( "descricao_credito" );
        }
        SistemaLegado::executaFrameOculto("retornaValorBscInner( '".$_GET['stNomCampoCod']."', '".$_GET['stIdCampoDesc']."', '".$_GET['stNomForm']."', '".$stValor."')");
    break;
