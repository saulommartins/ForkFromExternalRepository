...
/**
    ...
    * $Id: JSManterValor.js 62838 2015-06-26 13:02:49Z diogo.zarpelon $
...
function DefinirValores() {
    ...
    stCampoValor = document.frm.inValor;
    stCampoDtVigencia = document.frm.dtVigencia;
    if ( stCampoValor.value == "" ) {
        alertaAviso("@Campo 'Valor' inválido!",'form','erro','<?=Sessao::getId();?>', '../');
    }else
    if ( stCampoDtVigencia.value == "" ) {
        alertaAviso("@Campo 'Data de Vigência' inválido!",'form','erro','<?=Sessao::getId();?>', '../');
    }else {
        document.frm.stCtrl.value = 'DefinirValores';
        document.frm.target = "oculto";
        document.frm.action = '<?=$pgOcul;?>?<?=Sessao::getId();?>';
        document.frm.submit();
    }
...
function editarValor( inIndice1, inIndice2 ) {
    var stTarget = document.frm.target;
    var stAction = document.frm.action;

    document.frm.inValor.value = inIndice2;
    document.frm.dtVigencia.value = inIndice1;
    document.frm.stCtrl.value = 'editarValor';
    document.frm.target = "oculto";
