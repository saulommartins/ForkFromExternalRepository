...
/**
    ...
    * $Id: PRConcederLicencaGeralTipo.php 59612 2014-09-02 12:00:51Z gelson $
...
switch ($stAcao) {
    ...
    case "incGeral":
        ...
        if ($_REQUEST["inCodigoProcesso"]) {
            $arProcesso = explode( "/" , $_REQUEST["inCodigoProcesso"] );
            $obRCEMLicencaDiversa->obRProcesso->setCodigoProcesso ( $arProcesso[0] );
            $obRCEMLicencaDiversa->obRProcesso->setExercicio ( $arProcesso[1] );
        }
        ...
        $obRCEMLicencaDiversa->setObservacao($_REQUEST["stObservacao"]);
        ...
        if ($inCodigoTipoDocumento == '') {
        $inCodigoTipoDocumento = Sessao::read( 'inTipoDocumento' );
        }
        if ($inCodigoDocumento == '') {
        $inCodigoDocumento     = Sessao::read( 'inCodDocumento' );
        }
            ...
            if ( !$obErro->ocorreu() ) {
                ...
                if ( !$obErro->ocorreu() ) {
                    ...
                    #echo '<br><b>NUMERO DESTA EMISSAO: '. $inNumEmissao .'</b><br>';
...
#exit;
        if ( !$obErro->ocorreu() ) {
            ...
            if ($boEmissaoDocumento) { //boEmissaoDocumento
                ...
                $stFiltro .="AND b.cod_documento = ".$inCodigoDocumento; //$_REQUEST['stCodDocumentoTxt'];
