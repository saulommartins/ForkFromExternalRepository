...
*
* $Id: atributoTipoEdificacaoValorCobertura.sql 29199 2008-04-15 13:42:29Z fabio $
*
...
*/
   ...
   CREATE OR REPLACE FUNCTION atributoTipoEdificacaoValorCobertura( intInscricaoMunicipal INTEGER)
   RETURNS CHARACTER VARYING AS $$
   DECLARE
      varValor          VARCHAR := NULL;
      intCodModulo      INTEGER  := 12;
      intCodCadastro    INTEGER  := 5 ;
      intCodAtributo    INTEGER  := 1 ;
         ...
         SELECT atributo_tipo_edificacao_valor.valor
           ...
           FROM imobiliario.atributo_tipo_edificacao_valor
               , ( SELECT cod_modulo, cod_cadastro, cod_atributo, cod_tipo, cod_construcao, MAX(timestamp) AS timestamp
                     FROM imobiliario.atributo_tipo_edificacao_valor
                    WHERE atributo_tipo_edificacao_valor.cod_modulo   = intCodModuloz
