...
*
* $Id: consultaAtividade.plsql 59612 2014-09-02 12:00:51Z gelson $
*
...
--
CREATE OR REPLACE FUNCTION economico.fn_consulta_atividade(INTEGER,INTEGER)
RETURNS VARCHAR AS '
        ...
        stSql := ''
                  ...
                  FROM
                      ( SELECT
                           N.*
                       FROM
                           ECONOMICO.VIGENCIA_ATIVIDADE AS V,
                           ECONOMICO.NIVEL_ATIVIDADE AS N
                       WHERE
                           V.cod_vigencia = N.cod_vigencia ) AS N,
                       ECONOMICO.NIVEL_ATIVIDADE_VALOR AS LN
                  WHERE
                       N.cod_nivel     = LN.cod_nivel          AND
                       N.cod_vigencia  = LN.cod_vigencia       AND
                       LN.cod_vigencia = ''||inCodVigencia||'' AND
                       LN.cod_atividade = ''||inCodAtividade||''
                  ORDER BY
                       LN.cod_nivel
                  '';
