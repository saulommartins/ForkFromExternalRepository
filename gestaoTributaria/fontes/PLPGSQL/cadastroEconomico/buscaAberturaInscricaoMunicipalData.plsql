...
*
* $Id: buscaAberturaInscricaoMunicipalData.sql 29200 2008-04-15 13:48:27Z fabio $
*
...
*/
   ...
   CREATE OR REPLACE FUNCTION buscaAberturaInscricaoMunicipalData( intInscricaoEconomica INTEGER)
   RETURNS DATE AS $$
   DECLARE
      dDataAbertura DATE;
   BEGIN

      SELECT cadastro_economico.dt_abertura
        INTO dDataAbertura
        FROM economico.cadastro_economico
       WHERE cadastro_economico.inscricao_economica = intInscricaoEconomica
      ;

      RETURN dDataAbertura;
   END;
   $$ LANGUAGE 'plpgsql';
