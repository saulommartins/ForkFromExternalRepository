...
*
* $Id: fn_calcula_multa_lancamento.plsql 59612 2014-09-02 12:00:51Z gelson $
*
...
CREATE OR REPLACE FUNCTION arrecadacao.calcula_multa_lancamento( VARCHAR, DATE ) RETURNS VARCHAR AS '
DECLARE
...
SELECT 
        ...
        coalesce ( sum (tbl.valor_multa_credito) , 0.00 )
    INTO
        valor
    ...
    FROM
        (
         select 
            (
                case when ( aplica_multa (acar.numeracao, acar.exercicio::int, apar.cod_parcela, dtDataBase ) ) > 0 then
                    ...
                    *  
                    aplica_multa (acar.numeracao, acar.exercicio::int, apar.cod_parcela, dtDataBase )
                    * arrecadacao.calculaProporcaoParcela(apar.cod_parcela)  
                    /100
                else
                    0.00
            ...
            )::numeric(14,2) as valor_multa_credito
         FROM
