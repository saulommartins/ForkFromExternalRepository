...
*
* $Id: listaEmissaoGraficaImobiliariaEmissaoIsento.plsql 53399 2012-10-31 17:31:01Z carolina $
*
...
CREATE OR REPLACE FUNCTION arrecadacao.fn_lista_emissao_grafica_imobiliaria_emissao_isento
...
IF ( inExercicio > 0 ) THEN
    stFiltro := stFiltro||'' and aic2.exercicio = ''||inExercicio||'' '';
END IF;
...
SELECT DISTINCT
    ...
    , sem_acentos(tudo.nom_localizacao)
    , tudo.cod_lote
    , tudo.cod_construcao
    ...
    , sem_acentos(split_part(tudo.endereco,''''§'''',3)::varchar) as nom_logradouro
    ...
    , ( case when tudo.endereco_c is null then
            sem_acentos(split_part(tudo.endereco,''''§'''',3))
        else  sem_acentos(split_part(tudo.endereco_c,''''§'''',3))
        end
...
FROM
    ...
    SELECT DISTINCT
        ...
        , aic2.cod_grupo
        , aic2.descricao
        , arrecadacao.buscaCgmLancamento (al.cod_lancamento) as numcgm
        , arrecadacao.fn_proprietarios_imovel_nomes(aic.inscricao_municipal) as nom_cgm
       -- , cgm.numcgm
       -- , cgm.nom_cgm
...
INNER JOIN (
        ...
        INNER JOIN arrecadacao.lancamento_usa_desoneracao
            ...
            group by aic.inscricao_municipal
                   , ac.exercicio ''|| stGroupByOrigem ||''
