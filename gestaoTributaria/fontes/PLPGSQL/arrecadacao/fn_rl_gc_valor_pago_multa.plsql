...
*
* $Id: fn_rl_gc_valor_pago_multa.plsql 59612 2014-09-02 12:00:51Z gelson $
*
...
CREATE OR REPLACE FUNCTION arrecadacao.fn_rl_gc_valor_pago_multa ( integer, integer,  date )  RETURNS NUMERIC(14,2) AS '
DECLARE
...
BEGIN
        ...
        select 
            coalesce ( sum( APAGA.valor ), 0.00 ) as soma
        INTO
            nuResultado
        FROM
            ...
            INNER JOIN
                arrecadacao.pagamento_acrescimo as APAGA
            ON
                APAGA.numeracao = APAG.numeracao
                AND APAGA.cod_tipo = 3

        WHERE
            APAG.data_pagamento > APAR.vencimento
