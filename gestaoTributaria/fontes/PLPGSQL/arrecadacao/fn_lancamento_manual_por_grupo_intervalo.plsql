...
CREATE OR REPLACE FUNCTION arrecadacao.fn_lancamento_manual_por_grupo_intervalo( integer, integer, varchar, integer, varchar, varchar, varchar, varchar, varchar, integer ) RETURNS varchar AS '
DECLARE
    ...
    ELSE
        stSql := ''
                ...
                WHERE 
                    ...
                    AND acc.numcgm BETWEEN  ''||stIntervalo||''
                    AND COALESCE( 
                        (
                            SELECT
                                count(lancamento_calculo.cod_lancamento)
