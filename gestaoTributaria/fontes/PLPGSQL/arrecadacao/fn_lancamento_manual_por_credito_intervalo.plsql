...
CREATE OR REPLACE FUNCTION arrecadacao.fn_lancamento_manual_por_credito_intervalo( integer, integer, integer, integer, integer, varchar, integer, varchar, varchar, varchar, varchar, varchar, integer ) RETURNS varchar AS $$
DECLARE
    inCodCredito        ALIAS FOR $1;
    inCodEspecie        ALIAS FOR $2;
    inCodGenero         ALIAS FOR $3;
    inCodNatureza       ALIAS FOR $4;
    inAnoExercicio      ALIAS FOR $5;
    stIntervalo         ALIAS FOR $6;
    inQTDparcelas       ALIAS FOR $7;
    stDTvencParc        ALIAS FOR $8;
    stTipoDescParc      ALIAS FOR $9;
    stDescParc          ALIAS FOR $10;
    stDTdescParc        ALIAS FOR $11;
    stNumeroParc        ALIAS FOR $12;
    inTipoInscricao     ALIAS FOR $13; --1 = insc imob 2 = insc eco 3 = cgm
    stRetorno           VARCHAR;
    stSql               VARCHAR;
    stSql2              VARCHAR;
    stExecuta           VARCHAR;
...
BEGIN
    ...
    IF ( inTipoInscricao = 1 ) THEN
        stSql := '
                    SELECT DISTINCT 
                        imovel.inscricao_municipal AS inscricao
                    FROM 
                    imobiliario.imovel
                    ...
                    INNER JOIN (
                        SELECT 
                            MAX(baixa_imovel.timestamp) AS timestamp
                ...
                ON 
                    bi.inscricao_municipal = imovel.inscricao_municipal

                INNER JOIN 
                    arrecadacao.imovel_calculo AS AIC
                ON 
                    AIC.inscricao_municipal = imovel.inscricao_municipal

                INNER JOIN 
                    arrecadacao.calculo AS ACGC
                ON 
                    ACGC.cod_calculo = AIC.cod_calculo
                    AND ACGC.cod_credito = ' || inCodCredito || '
                    AND ACGC.cod_especie = ' || inCodEspecie || '
                    AND ACGC.cod_genero = ' || inCodGenero || '
                    AND ACGC.cod_natureza = ' || inCodNatureza || '
                    AND ACGC.exercicio = '|| quote_literal(inAnoExercicio) || '
                    AND ACGC.ativo = true

                WHERE 
                    ( ( bi.inscricao_municipal IS NULL ) OR ( (bi.inscricao_municipal IS NOT NULL) AND (bi.dt_termino IS NOT NULL) ) )
                    AND imovel.inscricao_municipal BETWEEN  '|| stIntervalo ||'
        ';
    ELSIF ( inTipoInscricao = 2 ) THEN
        stSql := '
                    ...
                    INNER JOIN arrecadacao.calculo AS ACGC
                           ...
                           AND ACGC.exercicio    = '|| quote_literal(inAnoExercicio) ||'
                           AND ACGC.ativo        = true
                         WHERE (    ( bc.inscricao_economica IS NULL )
                                 OR (     (bc.inscricao_economica IS NOT NULL)
                                      AND (bc.dt_termino IS NOT NULL)
                                    )
                               )
                           AND cadastro_economico.inscricao_economica BETWEEN '|| stIntervalo ||'
                             ;
                 ';
    ELSE
        stSql := '
            ...
            WHERE 
                acc.numcgm BETWEEN  '|| stIntervalo ||'
        ';
    ...
    FOR reRegistro IN EXECUTE stSql LOOP
        if ( inTipoInscricao = 1 ) THEN
            stSql2 := '
                ...
                WHERE
                    ...
                    AND calculo.exercicio = '|| quote_literal(inAnoExercicio) ||'
        ...
        ELSIF ( inTipoInscricao = 2 ) THEN
           stSql2 := '
                        ...
                        INNER JOIN arrecadacao.cadastro_economico_calculo
                                ON cadastro_economico_calculo.cod_calculo = calculo.cod_calculo
                         LEFT JOIN arrecadacao.lancamento_calculo
                                ON lancamento_calculo.cod_calculo = calculo.cod_calculo
                             WHERE lancamento_calculo IS NULL
                               AND calculo.ativo                                  = true
                               AND calculo.cod_credito                            = '||  inCodCredito         ||'
                               AND calculo.cod_especie                            = '||  inCodEspecie         ||'
                               AND calculo.cod_genero                             = '||  inCodGenero          ||'
        ...
        SELECT COALESCE(MAX(cod_lancamento)+1,1)
        ...
        flValorTotalGeral := 0.00;
            ...
            SELECT funcao.nom_funcao
              INTO stNomeRegraDeson
              FROM monetario.regra_desoneracao_credito
        INNER JOIN administracao.funcao
                ON funcao.cod_funcao     = regra_desoneracao_credito.cod_funcao
               AND funcao.cod_modulo     = regra_desoneracao_credito.cod_modulo
               AND funcao.cod_biblioteca = regra_desoneracao_credito.cod_biblioteca
             WHERE regra_desoneracao_credito.cod_credito  = inCodCredito
               AND regra_desoneracao_credito.cod_especie  = inCodEspecie
               AND regra_desoneracao_credito.cod_genero   = inCodGenero
               AND regra_desoneracao_credito.cod_natureza = inCodNatureza
                 ;


        FOR reRegistro2 IN EXECUTE stSql2 LOOP
            IF ( stNomeRegraDeson IS NOT NULL ) THEN
                SELECT desoneracao.cod_desoneracao
                  INTO inCodDesoneracao
                  FROM arrecadacao.desoneracao
                 WHERE desoneracao.cod_credito = reRegistro2.cod_credito
            ...
            IF ( inTipoInscricao = 1 AND stBuffer != '0' ) THEN
                    SELECT DISTINCT desonerado.numcgm
                         , CASE
                                WHEN desonerado_imovel.cod_desoneracao IS NOT NULL
                                 AND desoneracao.cod_desoneracao       IS NOT NULL  THEN
                                    CASE
                                        WHEN desoneracao.revogavel      = true  
                                         AND desonerado.data_revogacao <= now()     THEN
                                            'vazio'
                                    ELSE
                                        CASE
                                            WHEN desoneracao.expiracao >= now()
                                              OR (
                                                       desoneracao.prorrogavel      = true
                                                   AND desonerado.data_prorrogacao >= now()
                                                 )                                  THEN
                                                 (
                                                   SELECT funcao.nom_funcao
                                                     FROM administracao.funcao
                                                     JOIN arrecadacao.desoneracao
                                                       ON desoneracao.cod_funcao     = funcao.cod_funcao
                                                      AND desoneracao.cod_modulo     = funcao.cod_modulo
                                                      AND desoneracao.cod_biblioteca = funcao.cod_biblioteca
                                                    WHERE desoneracao.cod_credito          = reRegistro2.cod_credito
                                                      AND desoneracao.cod_especie          = reRegistro2.cod_especie
                                                      AND desoneracao.cod_genero           = reRegistro2.cod_genero
                                                      AND desoneracao.cod_natureza         = reRegistro2.cod_natureza
                                                      AND desoneracao.cod_desoneracao      = inCodDesoneracao
                                                      AND desoneracao.cod_tipo_desoneracao = 1
                                                      AND NOW()::DATE BETWEEN desoneracao.inicio AND desoneracao.termino
                                                 )
                                        ELSE
                                            'vazio'
                                        END
                ...
                INNER JOIN arrecadacao.desonerado_imovel
                        ON desoneracao.cod_desoneracao = desonerado_imovel.cod_desoneracao
                INNER JOIN (
                                 SELECT tmp.*
                                   FROM arrecadacao.desonerado AS tmp
                             INNER JOIN (
                                            SELECT max(desonerado.ocorrencia) AS ocorrencia
                                                 , desonerado.numcgm
                                                 , desonerado.cod_desoneracao
                                              FROM arrecadacao.desonerado
                                          GROUP BY desonerado.numcgm
                                                 , desonerado.cod_desoneracao
                                        ) AS tmp2
                                     ON tmp2.numcgm = tmp.numcgm
            ...
            ELSIF ( inTipoInscricao = 2 AND stBuffer != '0' ) THEN
                    SELECT DISTINCT desonerado.numcgm
                         , CASE
                                WHEN desonerado_cad_economico.cod_desoneracao IS NOT NULL
                                 AND desoneracao.cod_desoneracao              IS NOT NULL   THEN
                                    CASE
                                        WHEN desoneracao.revogavel      = true
                                         AND desonerado.data_revogacao <= now()             THEN
                      ...
                      INTO inNumCGMDeson
                         ...
                         , inOcorrenciaDeson
                      FROM arrecadacao.desoneracao
                INNER JOIN arrecadacao.desonerado_cad_economico
                        ON desoneracao.cod_desoneracao = desonerado_cad_economico.cod_desoneracao
                INNER JOIN (
                                 SELECT tmp.*
                                   FROM arrecadacao.desonerado AS tmp
                             INNER JOIN (
            ...
            ELSIF ( inTipoInscricao = 3 AND stBuffer != '0' ) THEN
                SELECT DISTINCT desonerado.numcgm
                     , CASE
                            WHEN desoneracao.cod_desoneracao IS NOT NULL            THEN
                                CASE
                                    WHEN desoneracao.revogavel = true
                                     AND desonerado.data_revogacao <= now()         THEN
                                        'vazio'
                                ELSE
                                    CASE
                                        WHEN desoneracao.expiracao >= now()
                                          OR (     esoneracao.prorrogavel = true
                                               AND desonerado.data_prorrogacao >= now()
                                             )                                      THEN
                                             (
                                               SELECT funcao.nom_funcao
        ...
        IF ( flValorTotalGeral >= flValorMinimoLanc ) THEN
            ...
            RAISE NOTICE 'venc: %', dtVencParcUnica;
            WHILE ( inLaco <= inQTDparcelas ) LOOP
                IF ( split_part ( stNumeroParc, ';', inLaco )::numeric != 0 ) THEN
            ...
            IF ( boUtilUnica = true ) THEN --flag indica que devo utilizar parcela unica
                inLaco := 1;
                WHILE ( inLaco <= inQTDparcelas ) LOOP
                    IF ( split_part ( stNumeroParc, ';', inLaco )::numeric = 0 ) THEN --soh entra para parcelas unicas
                            ...
                            WHILE ( inLaco2 < inTotaldeCreditos ) LOOP
                                IF ( boUtilizarDesconto[inLaco2] = true ) THEN
                                    IF ( split_part( stTipoDescParc, ';', inLaco ) = 'true' ) THEN --verificando se eh desconto percentual (true = percentual, false = valor)
                                        flTMPval := ( flValorPorCredito[inLaco2] * flTMPval ) / 100;
                                    ELSE
                                        flTMPval := flValorPorCredito[inLaco2] - flTMPval;
                                    END IF;
            ...
            WHILE ( inLaco4 <= inQTDparcelas ) LOOP
                IF ( split_part ( stNumeroParc, ';', inLaco4 )::numeric > 0 ) THEN --soh entra para parcelas diferentes de unicas
