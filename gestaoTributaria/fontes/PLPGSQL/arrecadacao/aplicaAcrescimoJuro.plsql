...
*
* $Id: aplicaAcrescimoJuro.plsql 59612 2014-09-02 12:00:51Z gelson $
*
...
CREATE OR REPLACE FUNCTION arrecadacao.aplica_acrescimo(varchar,integer,integer,date, integer , integer) returns numeric as '
declare
   ...
   '';
    ...
    FOR reRecord IN EXECUTE stSqlCreditos LOOP
        stSqlFuncoes := ''                                       
             ...
             AND    ACRE.cod_acrescimo =  ''|| inCodAcrescimo||''
             AND    ACRE.cod_tipo = ''||inCodTipo;
        -- executa
        FOR reRecordFuncoes IN EXECUTE stSqlFuncoes LOOP
            stExecuta :=  ''SELECT ''||reRecordFuncoes.funcao||''(''''''||reRecord.vencimento||'''''',''''''||dtDataBase||'''''',''||reRecord.valor||'', ''||reRecordFuncoes.cod_acrescimo||'' , ''||reRecordFuncoes.cod_tipo||'') as valor '';     
