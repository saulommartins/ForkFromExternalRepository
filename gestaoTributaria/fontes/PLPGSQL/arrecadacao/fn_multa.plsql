...
*
* $Id: fn_multa.plsql 59612 2014-09-02 12:00:51Z gelson $
*
...
CREATE OR REPLACE FUNCTION fn_multa(date,date,float,integer,integer) RETURNS numeric as '
    ...
    BEGIN
        ...
        IF dtVencimento < dtDataCalculo  THEN                
            ...
            IF ( inDiff = 0 ) THEN
                flMulta := ( flCorrigido * 5) / 100 ;
            ELSIF ( inDiff = 1 ) THEN
                flMulta := ( flCorrigido * 10) / 100 ;
            ELSIF ( inDiff = 2 ) THEN
                flMulta := ( flCorrigido * 15) / 100 ;
            ELSE 
                flMulta := ( flCorrigido * 20) / 100 ;
            END IF;
        END IF;             
