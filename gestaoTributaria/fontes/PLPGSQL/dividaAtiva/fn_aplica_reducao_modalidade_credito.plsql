...
*
* $Id: fn_aplica_reducao_modalidade_credito.plsql 59612 2014-09-02 12:00:51Z gelson $
*
...
CREATE OR REPLACE FUNCTION aplica_reducao_modalidade_credito(integer,integer,numeric,integer,integer,integer,integer,date,integer) returns numeric as '
declare
...
begin
    stSqlFuncoes := ''                                       
        ...
        LEFT JOIN
            divida.modalidade_reducao_credito
        ON
            divida.modalidade_reducao_credito.timestamp = modalidade_reducao.timestamp
            AND divida.modalidade_reducao_credito.cod_modalidade = modalidade_reducao.cod_modalidade
            AND divida.modalidade_reducao_credito.cod_modulo = modalidade_reducao.cod_modulo
            AND divida.modalidade_reducao_credito.cod_funcao = modalidade_reducao.cod_funcao
            AND divida.modalidade_reducao_credito.cod_biblioteca = modalidade_reducao.cod_biblioteca
            AND divida.modalidade_reducao_credito.valor = modalidade_reducao.valor
            AND divida.modalidade_reducao_credito.percentual = modalidade_reducao.percentual

        WHERE
            divida.modalidade.cod_modalidade = ''||inCodModalidade||''
            AND divida.modalidade_reducao_credito.cod_credito = ''||inCodCredito||''
            AND divida.modalidade_reducao_credito.cod_especie = ''||inCodEspecie||''
            AND divida.modalidade_reducao_credito.cod_natureza = ''||inCodNatureza||''
            AND divida.modalidade_reducao_credito.cod_genero = ''||inCodGenero||''
    '';
