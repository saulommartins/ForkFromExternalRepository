...
---------------------------------------------------------------------------
-- Inclusão da função recuperaExercicioConstrucaoImovel como função interna
---------------------------------------------------------------------------
   ...
   BEGIN
    ...
    --Inclusão de função interna arrecadacao/fn_acrescimo_indice.plsql
      ...
      intCodFuncao   := public.manutencao_funcao   (  12, 1, 'recuperaExercicioConstrucaoImovel', 4);
                                                 --( intCodmodulo , intCodBiblioteca , varNomeFunc , intCodTiporetorno )

      intCodVariavel := public.manutencao_variavel (  12, 1, intCodFuncao, 'dtConstrucao', 5 );
...
CREATE OR REPLACE FUNCTION incluirInterna() RETURNS VOID AS $$
DECLARE
    inMaxModelo     INTEGER;
BEGIN
    PERFORM 1
      ...
      WHERE cod_modulo = 12
        ...
        AND nom_funcao = 'recuperaExercicioConstrucaoImovel'
          ;
    IF NOT FOUND THEN
...
SELECT        incluirInterna();
DROP FUNCTION incluirInterna();
