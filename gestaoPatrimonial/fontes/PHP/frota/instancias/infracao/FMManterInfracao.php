...
$obTxtAutoInfracao->setNull     ( false              );
...
$obSelectMotivoInfracao->setNull      ( false             );
...
$obTxtPontos->setNull    ( false          );

if ($stAcao == 'alterar') {
    $obTFrotaInfracao = new TFrotaInfracao();
    $stFiltro = ' WHERE infracao.id ='.$_REQUEST['id'];
    $obTFrotaInfracao->recuperaInfracao($rsFrotaInfracao, $stFiltro);

    $obHdnId->setValue($rsFrotaInfracao->getCampo('id'));
    $obIPopUpVeiculo->obCampoCod->setValue($rsFrotaInfracao->getCampo('cod_veiculo'));
    $obIPopUpVeiculo->setValue($rsFrotaInfracao->getCampo('nom_modelo'));
    $obIPopUpMotorista->obCampoCod->setValue($rsFrotaInfracao->getCampo('numcgm'));
    $obIPopUpMotorista->setValue($rsFrotaInfracao->getCampo('nom_cgm'));

    $obTxtAutoInfracao->setValue($rsFrotaInfracao->getCampo('auto_infracao'));
    $obDtInfracao->setValue($rsFrotaInfracao->getCampo('data_infracao'));
    $obSelectMotivoInfracao->setValue($rsFrotaInfracao->getCampo('cod_infracao'));
    $obTxtBaseLegal->setValue($rsFrotaInfracao->getCampo('base_legal'));
    $obTxtGravidade->setValue($rsFrotaInfracao->getCampo('gravidade'));
    $obTxtPontos->setValue($rsFrotaInfracao->getCampo('pontos'));
}

//monta o formulário
$obFormulario = new Formulario;
$obFormulario->addTitulo    ( 'Dados da Infração' );
