...
class  IPopUpItem extends BuscaInner
{
    ...
    */
    ...
    public function IPopUpItem($obForm)
    {
        ...
        $this->obCampoCod->setId         ( "inCodItem"      );
        $this->obCampoCod->setAlign      ( "left"           );
        $this->obImagem->setId           ( "imgItem"     );

    }

    public function montaHTML()
    {
        $pgOcul = "'".CAM_GP_FRO_INSTANCIAS."processamento/OCManterItem.php?".Sessao::getId()."&".$this->obCampoCod->getName()."='+this.value+'&stNomCampoCod=".$this->obCampoCod->getId()."&stIdCampoDesc=".$this->getId()."&stTipoConsulta=".$this->getTipoConsulta()."'";
        $this->obCampoCod->obEvento->setOnChange ( "ajaxJavaScript(".$pgOcul.",'buscaPopup');" );

        $this->setFuncaoBusca("abrePopUp('" . CAM_GP_FRO_POPUPS . "item/FLManterItem.php','".$this->obForm->getName()."', '". $this->obCampoCod->stName ."','". $this->stId . "','','" . Sessao::getId() ."&stTipoConsulta=".$this->getTipoConsulta()."','800','550');");
