	...
	function selectCurrent() {
		...
		if ( options.multiple ) {
			...
			if ( words.length > 1 ) {
				...
				//jQuery.Autocompleter.Selection(input, progress + seperator, progress + seperator);
				...
				function (result){
					...
					if( !result ) {
						if (options.multiple) {
							...
							jQueryinput.val( "" );
							jQueryinput.trigger("result", null);
						}
					}
				}
			);
...
jQuery.Autocompleter.defaults = {
	...
	highlight: function(value, term) {
            if(value != undefined){
		return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term.replace("/([\^\jQuery\(\)\[\]\{\}\*\.\+\?\|\\])/gi", "\\jQuery1") + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<strong>"+term+"</strong>");
                //return value;
            } else {
                return false;
            }
...
jQuery.Autocompleter.Select = function (options, input, select, config) {
	...
	function fillList() {
		...
		for (var i=0; i < max; i++) {
			if ((!data[i]) || (data[i].value == undefined))
				continue;
