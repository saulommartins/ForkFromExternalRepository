 ...
 * $Id: FLLancamentoContabilReavaliacao.php 66372 2016-08-19 19:06:35Z michel $
...
$obSlcCompentecia->setTitle('Selecione a compentência em que ocorrerá a reavaliação.');
$obSlcCompentecia->setValue(date('n'));
$obSlcCompentecia->setNull(false);
$obSlcCompentecia->obEvento->setOnChange( "montaParametrosGET('montaBem');" );
...
$obIntExercicio->obEvento->setOnChange( "montaParametrosGET('montaBem');" );

//Define o objeto COMBO para Bem
$obCmbBem = new Select;
$obCmbBem->setName      ( "inCodBem" );
$obCmbBem->setId        ( "inCodBem" );
$obCmbBem->setRotulo    ( "Bem" );
$obCmbBem->addOption    ( "", "Selecione" );
$obCmbBem->setNull      ( true );
$obCmbBem->setTitle     ( 'Selecione um Bem' );
...
if ($stAcao == 'incluir') {
  $obFormulario->addTitulo    ( "Dados para o Lançamento Contábil de Reavaliação" );
} else {
  $obFormulario->addTitulo    ( "Dados para Estorno de Lançamento de Reavaliação" );
}

$obFormulario->addComponente( $obCmbEntidade );
$obFormulario->agrupaComponentes(array($obSlcCompentecia,$obIntExercicio));
$obFormulario->addComponente( $obCmbBem );

$obFormulario->OK();
$obFormulario->show();

$jsOnLoad = "montaParametrosGET('montaBem');";
