...
?>
    ...
    <form name="frm" action="apoliceSegurosVencer.php?<?=Sessao::getId()?>" method="POST">
    <table width="100%">
    <tr>
        <td class="alt_dados" colspan="2">Selecione as Datas para Procura</td>
    </tr>
<?php
   geraCampoData2("*Data Inicial", "datainicial", $datainicial, false, "onKeyPress=\"return(isValido(this, event, '0123456789'));\"onKeyUp=\"mascaraData(this, event);\" onBlur=\"JavaScript: if (!verificaData(this)) {alertaAviso('@Data inválida!('+this.value+')','form','erro','Sessao::getId()');};\"","Informe  data inicial","Buscar data inicial" );
   geraCampoDataHidden("*Data Final", "datafinal", $datafinal, false, "onKeyPress=\"return(isValido(this, event, '0123456789'));\"onKeyUp=\"mascaraData(this, event);\" onBlur=\"JavaScript: if (!verificaData(this)) {alertaAviso('@Data inválida!('+this.value+')','form','erro','Sessao::getId()');};\"","Informe a data final","Buscar data final" );
?>
    <tr>
...
?>
            <script type="text/javascript">
                alertaMensagem("A Data Inicial deve ser menor que a data final","unica","aviso","<?=Sessao::getId();?>");
                window.location = "apoliceSegurosVencer.php?<?=Sessao::getId();?>";
            </script>
<?php
        } else {
            ...
            Sessao::write('order',$orderby);
        }
    }

    $botoesPDF = new botoesPdfLegado;
    $sqlPDF = Sessao::read('sSQLs');
    $sqlPDF .= " ORDER BY ".Sessao::read('order')." DESC";
